...
1. [`.cursor/flows/icu-flow.mdc`](.cursor/flows/icu-flow.mdc) — ICU workflow, queue contract, stay lifecycle, bed board, cross-flow rules, and status-based actions
2. [`.cursor/flows/ipd-flow.mdc`](.cursor/flows/ipd-flow.mdc) — steps 8–9 (ward/ICU handover, nursing admission), §9 transfers (ICU escalation/step-down), §11 `In ICU` status, §15 ICU actions, §16 IPD encounter as hub
3. [`.cursor/flows/opd-flow.mdc`](.cursor/flows/opd-flow.mdc) — §3 stage `ADMITTED` as outpatient completion, §6 summary-card worklist pattern, §7 OPD-to-IPD handoff
4. [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU module boundaries vs IPD, Nursing, Billing, Theater, Emergency, Discharge
...
1. Re-read [`.cursor/flows/icu-flow.mdc`](.cursor/flows/icu-flow.mdc) — full step-by-step ICU workflow, entry paths, queue contract, stay lifecycle, bed board, monitoring, orders, transfers, discharge handoff, cross-flow integration (§17), module boundaries.
2. Re-read [`.cursor/flows/ipd-flow.mdc`](.cursor/flows/ipd-flow.mdc) — §8 care loop, §9 transfers, §10–§12 discharge, §11 `In ICU`, §14 boards, §15 status-based actions, §16 encounter hub rule.
3. Re-read [`.cursor/flows/opd-flow.mdc`](.cursor/flows/opd-flow.mdc) — §3 stage contract (`ADMITTED`), §7 OPD-to-IPD handoff, §6 UI rules (summary cards filter worklist; reuse `frontend/lib/shared/*`).
4. Re-read [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU vs Inpatient vs Nursing vs Discharge vs Billing boundaries.
...
- Do not implement full discharge clearance in ICU — link to **IPD workspace** discharge panel or **Discharge** / **Billing** modules for clearance tasks.
...
- **Role-appropriate actions:** ICU nurse (vitals, observations, alerts, MAR), intensivist/doctor (rounds, orders, discharge plan), bed manager (ICU bed assign/transfer), billing (clearance links).
...
9. **Discharge readiness handoff** — clearance links to IPD/Discharge/Billing; no fake “discharged” state.
