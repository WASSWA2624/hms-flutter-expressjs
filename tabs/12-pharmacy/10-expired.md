...
- Label: `pharmacyDeskExpiredLabel` (`Expired`)
- Icon: `Icons.event_busy_outlined`
- Count source:
  - Sibling: `stockAlertSummary.expiredRows`
...
- Stock query: `PharmacyInventoryStockQuery(expiredOnly: true)` via `applyDeskStockFilter`
- Tab gate: `PharmacyExpiredAtomPermissions.tab` = catalog browse ∩ `pharmacy:read` + `pharmacy-dispensing`
...
- Expired preset applied via section `stockQuery`; further filters edit the same inventory query model

## 3. Table

- Inventory stock rows filtered to expired-only window
...
Inventory stock filters (status / item / SKU / facility / storage / pending) with expired preset.  
...
| Atom | Gate |
| --- | --- |
| Tab | `PharmacyExpiredAtomPermissions` browse ∩ |
