...
Produce a complete, per-tab inventory of the Subscriptions workspace (`SubscriptionsWorkspacePage` at `/subscriptions`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Subscription panels (tabs):** `overview`, `catalog`, `modules`, `operations`, `billing`, `governance`, `denied` (`SubscriptionPanel`).
...
1. Inventory **every** `SubscriptionPanel` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `panel`|`section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Upgrade, Manage admins)—exact labels as shown (or l10n keys when labels are localized).
   3. Table / panel surface: row model when tabular, default columns, all column choices, row-select behavior; for non-table panels, list the primary chrome sections instead.
...
3. Include shared access-admin / billing / tenant-facility surfaces reused by Subscriptions whenever Subscriptions opens them; mark them as **reused** vs Subscriptions-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Subscriptions presentation code.
...
- [ ] Every `SubscriptionPanel` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Subscriptions vs reused shared/access-admin/billing/tenant-facility).
...
- Trace call sites from `subscriptions_workspace_page.dart` and Subscriptions widgets under `frontend/lib/features/subscriptions/presentation/`.
- Cross-check tab and action gates in `subscriptions_access.dart` and related tests under `frontend/test/features/subscriptions/`.
- Confirm deep-link panel values via `SubscriptionPanel.serverValue` / `fromServer`.
- Spot-check `subscription_upgrade_dialog.dart`, `subscription_report_admins_dialog.dart`, and table column helpers.
- Manual check (optional): open Subscriptions with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
