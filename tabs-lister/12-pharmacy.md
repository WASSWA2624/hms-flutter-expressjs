...
Produce a complete, per-tab inventory of the Pharmacy workspace (`PharmacyWorkspacePage` at `/pharmacy`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Pharmacy desk sections (tabs):** `queue`, `inProgress`, `pendingPayment`, `completed`, `cancelled`, `allOrders`, `catalog`, `suppliers`, `nearExpiry`, `expired`, `lowStock`, `outOfStock` (`PharmacyDeskSection`). Catalog sub-tabs include `drugs`, `formulary`, `inventory`, `storageLayout`, `shelves` (`PharmacyCatalogTab`).
...
1. Inventory **every** `PharmacyDeskSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Dispense, Add drug, Add supplier)—exact labels as shown (or l10n keys when labels are localized).
...
3. Include shared billing / patient / clinical order surfaces reused by Pharmacy whenever Pharmacy opens them; mark them as **reused** vs Pharmacy-owned widgets. For `catalog`, also inventory nested `PharmacyCatalogTab` surfaces.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Pharmacy presentation code.
...
- [ ] Every `PharmacyDeskSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Pharmacy vs reused shared/billing/patient).
...
- Trace call sites from `pharmacy_workspace_page.dart` and Pharmacy widgets under `frontend/lib/features/pharmacy/presentation/`.
- Cross-check tab and action gates in `pharmacy_access.dart` and related tests under `frontend/test/features/pharmacy/`.
- Confirm deep-link section values (`queue`, `in-progress`, `pending-payment`, `completed`, `cancelled`, `all`, `catalog`, `suppliers`, `near-expiry`, `expired`, `low-stock`, `out-of-stock`).
- Spot-check catalog/supplier/drug dialogs and dispense/payment-related reused billing surfaces.
- Manual check (optional): open Pharmacy with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
...
- `frontend/lib/features/pharmacy/presentation/widgets/pharmacy_catalog_dialog.dart`
