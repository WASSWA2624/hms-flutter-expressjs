...
Produce a complete, per-tab inventory of the ICU workspace (`IcuWorkspacePage` at `/icu`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**ICU workspace sections (tabs):** `active`, `critical`, `transfers`, `discharge`, `ended`, `all`, `beds`, `followUps` (`IcuWorkspaceSection`). Detail deep-link panels are defined by `IcuDetailPanel`.
...
1. Inventory **every** `IcuWorkspaceSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   ...
   6. Dialogs opened from that tab (detail, actions, pickers, mutation dialogs), including detail panels opened via deep links.
...
3. Include shared IPD / nursing / discharge / patient surfaces reused by ICU whenever ICU opens them; mark them as **reused** vs ICU-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from ICU presentation code.
...
- [ ] Every `IcuWorkspaceSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (ICU vs reused shared/IPD/nursing/discharge/patient).
...
- Trace call sites from `icu_workspace_page.dart` and ICU widgets under `frontend/lib/features/icu/presentation/`.
- Cross-check tab and action gates in `icu_access.dart` and related tests under `frontend/test/features/icu/`.
- Confirm deep-link section / panel values via `IcuWorkspaceSection` / `IcuDetailPanel` query helpers.
- Spot-check reused IPD/nursing/discharge dialogs ICU opens.
- Manual check (optional): open ICU with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
