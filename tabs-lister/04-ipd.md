...
Produce a complete, per-tab inventory of the IPD workspace (`IpdWorkspacePage` at `/ipd`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**IPD workspace sections (tabs):** `admissionQueue`, `activePatients`, `transferPending`, `dischargePlanned`, `bedBoard`, `followUps` (`IpdWorkspaceSection`). Detail deep-link panels include `beds`, `nursing`, `medication`, `discharge`, `transfer`, `rounds` (`IpdDetailPanel`).
...
1. Inventory **every** `IpdWorkspaceSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Start admission, Transfer)—exact labels as shown (or l10n keys when labels are localized).
   3. Table / board: row model (or bed-board cell model), default columns, all column choices, row-select behavior.
   ...
   6. Dialogs opened from that tab (detail, actions, pickers, mutation dialogs), including detail panels opened via `panel` deep links.
...
3. Include shared nursing / discharge / rooms-beds / patient / billing surfaces reused by IPD whenever IPD opens them; mark them as **reused** vs IPD-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from IPD presentation code.
...
- [ ] Every `IpdWorkspaceSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (IPD vs reused shared/nursing/discharge/patient/billing).
...
- Trace call sites from `ipd_workspace_page.dart` and IPD widgets under `frontend/lib/features/ipd/presentation/`.
- Cross-check tab and action gates in `ipd_access.dart` and related tests under `frontend/test/features/ipd/`.
- Confirm deep-link section / panel values via `IpdWorkspaceSection` / `IpdDetailPanel` query helpers.
- Spot-check reused nursing, discharge, and rooms-beds surfaces IPD opens.
- Manual check (optional): open IPD with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
