...
Produce a complete, per-tab inventory of the Lab workspace (`LabWorkspacePage` at `/lab`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Lab desk sections (tabs):** `collection` (Pending), `critical`, `completed`, `followUps`, `worklist` (All patients) (`LabDeskSection`). Workbench view is patient-grouped (`LabWorkbenchView.patients`).
...
1. Inventory **every** `LabDeskSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Enter results, Collect)—exact labels as shown (or l10n keys when labels are localized).
...
3. Include shared clinical / patient / billing surfaces reused by Lab whenever Lab opens them; mark them as **reused** vs Lab-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Lab presentation code.
...
- [ ] Every `LabDeskSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Lab vs reused shared/clinical/patient/billing).
...
- Trace call sites from `lab_workspace_page.dart` and Lab widgets under `frontend/lib/features/lab/presentation/`.
- Cross-check tab and action gates in `lab_access.dart` and related tests under `frontend/test/features/lab/`.
- Confirm deep-link section values via Lab desk section query helpers.
- Spot-check `lab_result_entry_dialog.dart`, desk/report preview settings dialogs, and print/report entry points.
- Manual check (optional): open Lab with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
