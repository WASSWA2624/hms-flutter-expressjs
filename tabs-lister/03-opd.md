...
Produce a complete, per-tab inventory of the OPD workspace (`OpdWorkspacePage` at `/opd`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**OPD workspace sections (tabs):** `all`, `arrivals`, `queue`, `triage`, `active`, `followUps` (`OpdWorkspaceSection`).
...
1. Inventory **every** `OpdWorkspaceSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Check-in, Start encounter)—exact labels as shown (or l10n keys when labels are localized).
...
3. Include shared reception / patient / billing / clinical surfaces reused by OPD whenever OPD opens them; mark them as **reused** vs OPD-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from OPD presentation code.
...
- [ ] Every `OpdWorkspaceSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (OPD vs reused shared/reception/patient/billing/clinical).
...
- Trace call sites from `opd_workspace_page.dart` and OPD widgets under `frontend/lib/features/opd/presentation/`.
- Cross-check tab and action gates in `opd_access.dart` and related tests under `frontend/test/features/opd/`.
- Confirm deep-link section values via `_parseOpdSection` / section query writers on the workspace page.
- Spot-check reused dialogs in `frontend/lib/shared/opd_actions/` and patient/billing dialogs OPD opens.
- Manual check (optional): open OPD with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
