...
Produce a complete, per-tab inventory of the Clinical workspace (`ClinicalWorkspacePage` at `/clinical`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Clinical workspace sections (tabs):** `all` (pending), `assignedToMe`, `urgent`, `resultsReady`, `completed`, `followUps` (`ClinicalWorkspaceSection`).
...
1. Inventory **every** `ClinicalWorkspaceSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
...
3. Include shared OPD / patient / lab / radiology / order surfaces reused by Clinical whenever Clinical opens them; mark them as **reused** vs Clinical-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Clinical presentation code.
...
- [ ] Every `ClinicalWorkspaceSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Clinical vs reused shared/OPD/patient/lab/radiology).
...
- Trace call sites from `clinical_workspace_page.dart` and Clinical widgets under `frontend/lib/features/clinical/presentation/`.
- Cross-check tab and action gates in `clinical_access.dart` and related tests under `frontend/test/features/clinical/`.
- Confirm deep-link section values via `_parseClinicalSection` / section query writers.
- Spot-check reused OPD/encounter and results dialogs Clinical opens.
- Manual check (optional): open Clinical with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
