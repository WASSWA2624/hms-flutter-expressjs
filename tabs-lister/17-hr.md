...
Produce a complete, per-tab inventory of the HR workspace (`HrWorkspacePage` at `/hr`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**HR desk sections (tabs):** `staffDirectory`, `positions`, `shiftRoster`, `leaveRequests`, `swapRequests`, `unassignedShifts`, `payroll`, `access` (`HrDeskSection`).
...
1. Inventory **every** `HrDeskSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` (and `queue` when used) query value.
   ...
   3. Table / panel: row model when tabular, default columns, all column choices, row-select behavior; for embedded panels (`staffDirectory`, `positions`, `access`), list the panel chrome and table surfaces inside.
   ...
   9. Print / label / document preview entry points tied to that tab or its dialogs, including preview template names when identifiable (staff, position, roster).
3. Include shared access-admin / settings / user-management surfaces reused by HR whenever HR opens them; mark them as **reused** vs HR-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from HR presentation code.
...
- [ ] Every `HrDeskSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (HR vs reused shared/access-admin/settings).
...
- Trace call sites from `hr_workspace_page.dart` and HR widgets under `frontend/lib/features/hr/presentation/`.
- Cross-check tab and action gates in `hr_access.dart` and related tests under `frontend/test/features/hr/`.
- Confirm deep-link section values via `HrDeskSection.routeQueryValue` / `fromQuery` / `fromQueue`.
- Spot-check `hr_workspace_dialogs.dart`, assign/compensation/access dialogs, and print helpers.
- Manual check (optional): open HR with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
