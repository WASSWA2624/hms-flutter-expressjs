...
Produce a complete, per-tab inventory of the Patients registry workspace (`PatientRegistryPage` at `/patients`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Patient registry sections (tabs):** `all`, `active`, `admitted`, `balanceDue` (`PatientRegistrySection`).
...
1. Inventory **every** `PatientRegistrySection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Register, Admit)—exact labels as shown (or l10n keys when labels are localized).
...
3. Include shared reception / OPD / billing / admission surfaces reused by Patients whenever the registry opens them; mark them as **reused** vs Patients-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Patients presentation code.
...
- [ ] Every `PatientRegistrySection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Patients vs reused shared/reception/OPD/billing).
...
- Trace call sites from `patient_registry_page.dart` and Patients widgets under `frontend/lib/features/patients/presentation/`.
- Cross-check tab and action gates in `patient_registry_access.dart` and related tests under `frontend/test/features/patients/`.
- Confirm deep-link section values via `PatientRegistrySection` query parsing / writers on the registry page.
- Spot-check reused dialogs (admission, discharge planning, billing/patient detail) that Patients opens.
- Manual check (optional): open Patients with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
