...
Produce a complete, per-tab inventory of the Billing workspace (`BillingWorkspacePage` at `/billing`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Billing desk sections (tabs via `BillingQueueType`):** `all` (`work`), `needsIssue` (`issue`), `pendingPayment` (`collect`), `claimsPending` (`claims`), `approvalRequired` (`approvals`). Treat `overdue` as a Collect due-filter (not a primary desk tab) when it appears in code.
...
1. Inventory **every** billing desk tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
2. For **each** tab, list in this order:
   1. Tab strip: label, count source, count tone, deep-link / `section` query value.
   2. Search / Filters / Settings / Export / Print and any context actions (e.g. Issue invoice, Receive payment, Quick charge)—exact labels as shown (or l10n keys when labels are localized).
   ...
   9. Print / label / document preview entry points tied to that tab or its dialogs, including preview template names when identifiable (receipt, invoice, claim, approval, price book).
3. Include shared claims / accounts / patient / price-book surfaces reused by Billing whenever Billing opens them; mark them as **reused** vs Billing-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Billing presentation code.
...
- [ ] Every billing desk tab appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Billing vs reused shared/claims/accounts/patient).
...
- Trace call sites from `billing_workspace_page.dart` and Billing widgets under `frontend/lib/features/billing/presentation/`.
- Cross-check tab and action gates in `billing_access.dart` and related tests under `frontend/test/features/billing/`.
- Confirm deep-link section values via `BillingQueueType.sectionQueryValue`.
- Spot-check form dialogs, receive-payment / quick-charge / ledger / price-book dialogs, and print option helpers.
- Manual check (optional): open Billing with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
