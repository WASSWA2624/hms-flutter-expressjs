...
Produce a complete, per-tab inventory of the Radiology workspace (`RadiologyWorkspacePage` at `/radiology`). The goal is an exhaustive, readable catalog of every visible and reachable UI atom on that screen—not a redesign and not a new inventory folder under `screens/`.

**Radiology desk sections (tabs):** `worklist`, `reporting`, `allOrders`, `followUps` (`RadiologyDeskSection`).
...
1. Inventory **every** `RadiologyDeskSection` tab the workspace can show, including tabs omitted for unauthorized users (note the permission that hides them).
...
3. Include shared clinical / patient / lab-style order surfaces reused by Radiology whenever Radiology opens them; mark them as **reused** vs Radiology-owned widgets.
...
- Do not invent tabs, dialogs, or print paths that are not reachable from Radiology presentation code.
...
- [ ] Every `RadiologyDeskSection` appears as its own section with the ordered checklist from Requirement 2.
- [ ] Every list-table toolbar action on each tab is named (Filters, Settings, Export, Print when present, plus context actions).
- [ ] Every dialog and nested/follow-on dialog reachable from each tab is named, with owner (Radiology vs reused shared/clinical/patient).
...
- Trace call sites from `radiology_workspace_page.dart` (and part files: `.print.dart`, `.configurations.dart`, `.detail_cells.dart`) under `frontend/lib/features/radiology/presentation/`.
- Cross-check tab and action gates in `radiology_access.dart` and related tests under `frontend/test/features/radiology/`.
- Confirm deep-link section values (`worklist`, `reporting`, `all`, `follow-ups`).
- Spot-check print helpers / report preview entry points in `radiology_workspace_page.print.dart`.
- Manual check (optional): open Radiology with a fully privileged user and an under-privileged user; confirm listed omissions match the UI.
