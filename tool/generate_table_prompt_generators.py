...
You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **{screen["name"]}** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
- Save as `prompts/{{NN}}-standardize-{screen["slug"]}-tables.md` using the next number.
  - Feature slug for this screen: `{screen["slug"]}`
  - Example: `prompts/28-standardize-{screen["slug"]}-tables.md`
