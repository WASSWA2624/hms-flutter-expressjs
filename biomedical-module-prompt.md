...
- [`.cursor/flows/ipd-flow.mdc`](.cursor/flows/ipd-flow.mdc) — steps 8–9 (ward/ICU handover, nursing admission), §9 transfers (ICU escalation/step-down), §11 `In ICU` status, §15 ICU actions, §16 IPD encounter as hub
- [`.cursor/flows/opd-flow.mdc`](.cursor/flows/opd-flow.mdc) — §7 OPD-to-IPD handoff when admission routes to ICU; stage `ADMITTED` as outpatient completion that hands off to IPD
- [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU module boundaries vs IPD, Nursing, Billing, Theater, Emergency
...
3. Re-read [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU vs Inpatient vs Nursing vs Discharge vs Billing boundaries.
...
| Emergency ICU path | ipd-flow §2.1 | Patients from `startEmergencyIcuFlow` appear on active ICU board |
| IPD bed manager | ipd-flow §3, §6 | ICU bed board is ICU-ward scoped view of same bed catalog |
...
| Billing | ipd-flow §4, billing prompt | Orders use `ClinicalRequestBillingPanel`; clearance stays in Billing/IPD |
...
- **No ICU bed board** — flow expects ICU bed assignment visibility; UI shows ward/bed on patient rows only, no live ICU bed occupancy board.
...
- Link to billing workspace for deposits and interim bills (see `billing-mudule-prompt.md`).
...
3. **Widget extraction + deep links** — split `icu_workspace_page.dart`; parse `/icu?id=&panel=` query params.
...
9. **Discharge readiness handoff** — clearance links to IPD/Billing; no fake “discharged” state.
