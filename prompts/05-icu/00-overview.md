# ICU overview — workspace compliance program

## Context

Bring the ICU desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/05-icu/`. Per-surface remediation prompts live beside this file under `prompts/05-icu/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/05-icu/00-overview.md` as the authoritative tab index (`IcuWorkspaceSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/05-icu/01-active.md`
   - `prompts/05-icu/02-critical.md`
   - `prompts/05-icu/03-transfers.md`
   - `prompts/05-icu/04-discharge.md`
   - `prompts/05-icu/05-ended.md`
   - `prompts/05-icu/06-all.md`
   - `prompts/05-icu/07-beds.md`
   - `prompts/05-icu/08-follow-ups.md`
...
6. After each remediation, update the matching inventory file under `tabs/05-icu/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/icu/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/05-icu/00-overview.md` has a completed remediation prompt under `prompts/05-icu/` and a matching updated inventory under `tabs/05-icu/`.
...
- Trace `frontend/lib/features/icu/presentation/pages/icu_workspace_page.dart`, `frontend/lib/features/icu/presentation/icu_access.dart`, and widgets listed in `tabs/05-icu/00-overview.md`.
- Run feature tests under `frontend/test/features/icu/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/05-icu/00-overview.md`
- `prompts/05-icu/00-shared-chrome.md`
- `prompts/05-icu/99-convention-gaps.md`
- `prompts/05-icu/01-active.md`
- `prompts/05-icu/02-critical.md`
- `prompts/05-icu/03-transfers.md`
- `prompts/05-icu/04-discharge.md`
