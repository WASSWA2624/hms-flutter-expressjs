# ICU Critical tab — rule compliance

## Context

Make the Critical desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/05-icu/02-critical.md`. Apply shared chrome fixes from `prompts/05-icu/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `icuCriticalAlertsLabel`, deep-link `critical`, and omit the tab when gate denies access (`IcuCriticalAtomPermissions.tab` = `icuWorkspaceReadRequirement`) — never show a disabled placeholder (`tabs.mdc`). Align `IcuWorkspaceSection` / query helpers with `tabs/05-icu/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.criticalCount` (`hasCriticalAlert`)). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
3. Use `AppTabCountTone.danger` as inventoried, or escalate to `danger` only when product policy requires (`tabs.mdc`).
...
8. Preserve row-select → detail/actions hub with **generic titles**; keep nested mutations in-desk via shared dialogs/forms; omit unauthorized nested actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Stay detail; Other ICU mutation dialogs; Clinical order / Rx.
...
- [x] `tabs/05-icu/02-critical.md` updated to match.
