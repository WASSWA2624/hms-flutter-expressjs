# ICU Transfers tab — rule compliance

## Context

Make the Transfers desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/05-icu/03-transfers.md`. Apply shared chrome fixes from `prompts/05-icu/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `icuTransfersLabel`, deep-link `transfers`, and omit the tab when gate denies access (`IcuTransfersAtomPermissions.tab` = `icuWorkspaceReadRequirement`) — never show a disabled placeholder (`tabs.mdc`). Align `IcuWorkspaceSection` / query helpers with `tabs/05-icu/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.transferCount` (`hasOpenTransfer`)). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [x] `tabs/05-icu/03-transfers.md` updated to match.
