# Radiology Follow Ups tab — rule compliance

## Context

Make the Follow Ups desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/11-radiology/04-follow-ups.md`. Apply shared chrome fixes from `prompts/11-radiology/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `opdFollowUpsTitle`, deep-link `follow-ups` (aliases `follow_ups`, `followups`), and omit the tab when gate denies access (`RadiologyFollowUpsAtomPermissions.tab` = ∩ `radiology:read` + `radiology-workflows`) — never show a disabled placeholder (`tabs.mdc`). Align `RadiologyDeskSection` / query helpers with `tabs/11-radiology/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`followUpTabCountProvider(const FollowUpWorklistScope())`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
3. Use `AppTabCountTone.info` as inventoried, or escalate to `danger` only when product policy requires (`tabs.mdc`).
4. Toolbar order: **Filters → Settings → (no Export/Print on table) → Request imaging** — normalize shared labels to `Filters`, `Settings`, `Export`, and `Print` when those controls apply (`tables.mdc`, `printing.mdc`).
...
8. Preserve row-select → detail/actions hub with **generic titles**; keep nested mutations in-desk via shared dialogs/forms; omit unauthorized nested actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Follow-up detail / complete / reschedule.
...
- [ ] `tabs/11-radiology/04-follow-ups.md` updated to match.
