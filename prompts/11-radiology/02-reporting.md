# Radiology Reporting tab — rule compliance

## Context

Make the Reporting desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/11-radiology/02-reporting.md`. Apply shared chrome fixes from `prompts/11-radiology/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `radiologyReportingSummaryLabel`, deep-link `reporting` (aliases `reports`, `draft`), and omit the tab when gate denies access (`RadiologyReportingAtomPermissions.tab` = ∩ `radiology:read` + `radiology-workflows`) — never show a disabled placeholder (`tabs.mdc`). Align `RadiologyDeskSection` / query helpers with `tabs/11-radiology/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (sibling `state.reportingCount`; when active, `state.orders.totalItemCount`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/11-radiology/02-reporting.md` updated to match.
