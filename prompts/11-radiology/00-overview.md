# Radiology overview — workspace compliance program

## Context

Bring the Radiology desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/11-radiology/`. Per-surface remediation prompts live beside this file under `prompts/11-radiology/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/11-radiology/00-overview.md` as the authoritative tab index (`RadiologyDeskSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/11-radiology/01-worklist.md`
   - `prompts/11-radiology/02-reporting.md`
   - `prompts/11-radiology/03-all-orders.md`
   - `prompts/11-radiology/04-follow-ups.md`
...
6. After each remediation, update the matching inventory file under `tabs/11-radiology/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/radiology/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/11-radiology/00-overview.md` has a completed remediation prompt under `prompts/11-radiology/` and a matching updated inventory under `tabs/11-radiology/`.
...
- Trace `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart`, `frontend/lib/features/radiology/presentation/radiology_access.dart`, and widgets listed in `tabs/11-radiology/00-overview.md`.
- Run feature tests under `frontend/test/features/radiology/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/11-radiology/00-overview.md`
- `prompts/11-radiology/00-shared-chrome.md`
- `prompts/11-radiology/99-convention-gaps.md`
- `prompts/11-radiology/01-worklist.md`
- `prompts/11-radiology/02-reporting.md`
- `prompts/11-radiology/03-all-orders.md`
- `prompts/11-radiology/04-follow-ups.md`
