...
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2481` |
| Kind | `custom` |
...
1. **Read contract + source**
   ...
   - Read `_PatientAdmissionQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2481` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Patient Admission Quick (patients).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_PatientAdmissionQuickDialog` opens through `AppDialog` / approved helpers only.
