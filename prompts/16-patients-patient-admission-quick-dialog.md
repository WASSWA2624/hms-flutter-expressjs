...
| Field | Value |
| --- | --- |
| Symbol | `_PatientAdmissionQuickDialog` |
| Purpose | Patient Admission Quick |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2537` |
...
| Paired opener(s) | `_openPatientAdmissionQuickDialog` |
| Primary commit | Start / request admission |
| Slices to keep in sync | admission request, patient detail, IPD intake cues |
| Sibling reuse targets | `IpdStartAdmissionDialog`, `ClinicalAdmissionActionDialog` |
...
| Mutations (region) | `mutation: requestAdmission` |
...
- `ClinicalAdmissionActionDialog — frontend/lib/shared/clinical_actions/dialogs/clinical_admission_action_dialog.dart`
...
| Title snippets | `l10n.patientsAdmissionDialogTitle`, `context.l10n.patientsWorkflowSectionTitle`, `title`, `l10n.clinicalAdmissionDetailsTitle` |
...
| Cross-stack trace files found | 6 |
| Peek region size | 3371 chars |
...
**Scope lock:** only `_PatientAdmissionQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_PatientAdmissionQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2537` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Admission Quick** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Start / request admission`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `IpdStartAdmissionDialog`, `ClinicalAdmissionActionDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch admission request, patient detail, IPD intake cues, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPatientAdmissionQuickDialog`. Update all call sites in the same change when signatures move.
...
1. `_PatientAdmissionQuickDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `IpdStartAdmissionDialog`, `ClinicalAdmissionActionDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: admission request, patient detail, IPD intake cues.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_PatientAdmissionQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 16 slug=patients-patient-admission-quick-dialog symbol=_PatientAdmissionQuickDialog shape=widget_dialog -->
