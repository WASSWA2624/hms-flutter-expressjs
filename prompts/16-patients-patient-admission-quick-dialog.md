...
**Scope:** only `_PatientAdmissionQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Purpose | Patient/encounter flow: Patient Admission Quick (patients). |
...
1. **Read contract + source**
   ...
   - Read `_PatientAdmissionQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2537` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Patient Admission Quick (patients).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_openPatientAdmissionQuickDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_PatientAdmissionQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 16 slug=patients-patient-admission-quick-dialog symbol=_PatientAdmissionQuickDialog -->
