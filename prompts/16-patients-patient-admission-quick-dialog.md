...
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2537` |
...
| Sibling reuse targets | `IpdStartAdmissionDialog`, `_OpdAdmissionHandoffDialog` |
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | approved `show*` helper |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| `CircularProgressIndicator` | not seen |
| Title snippets | `l10n.patientsAdmissionDialogTitle`, `context.l10n.patientsWorkflowSectionTitle` |
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `IpdStartAdmissionDialog`, `_OpdAdmissionHandoffDialog`.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `IpdStartAdmissionDialog`, `_OpdAdmissionHandoffDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: admission request, patient detail, IPD intake cues.
