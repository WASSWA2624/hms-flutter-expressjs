...
| Verified definition | `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:830` |
...
| Mutations (region) | `mutation: assignBed` |
...
| Title snippets | `l10n.icuAssignBedDialogTitle`, `widget.title` |
...
1. **Read contracts + source**
   ...
   - Read `_AssignBedDialog` at `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:830` and every paired opener / *Used from* site.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_TransferRequestDialog`, `IpdStartAdmissionDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: ICU bed occupancy, admission detail, ward lists.
...
<!-- generator: encounter-dialog prompt 08 slug=icu-assign-bed-dialog symbol=_AssignBedDialog shape=widget_dialog -->
