...
3. **Normalize title + icon**
   - Use a general, role-based title for **Assign Bed (icu).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_AssignBedDialog` opens through `AppDialog` / approved helpers only.
