...
**Scope:** only `_AssignBedDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: assignBed`, `mutation-ish call: transferOut`, `mutation-ish call: transferOut`
...
| Purpose | Patient/encounter flow: Assign Bed (icu). |
...
| Title snippets | `l10n.icuAssignBedDialogTitle` |
...
- Footer button order may place Cancel/secondary after primary — `prompt.md` requires left→right: secondary actions, **Cancel**, primary commit.
...
1. **Read contract + source**
   ...
   - Read `_AssignBedDialog` at `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:824` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Assign Bed (icu).** (flow/action name — never the patient display name as `AppDialog` title).
   - Pass the title through the shell so uppercase normalization applies.
   - Add/keep a meaningful `icon` if peer dialogs in `icu` already use icons.
...
8. **Preserve reachability**
   - Do not break `openIcuAssignBedDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_AssignBedDialog`'s path.
...
<!-- generator: encounter-dialog prompt 08 slug=icu-assign-bed-dialog symbol=_AssignBedDialog -->
