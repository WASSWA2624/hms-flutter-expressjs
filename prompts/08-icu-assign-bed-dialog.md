...
| Field | Value |
| --- | --- |
| Symbol | `_AssignBedDialog` |
| Purpose | Assign Bed |
| Module / surface | `icu` |
| Inventory kind | `custom` |
| Presentation shape | `confirm` |
| Defined in | `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:824` |
| Extends / uses | AppDialog / showAppDialog (typical) |
| Paired opener(s) | `openIcuAssignBedDialog` |
| Primary commit | Assign bed |
| Slices to keep in sync | ICU bed occupancy, admission detail, ward lists |
| Sibling reuse targets | `_TransferRequestDialog`, `IpdStartAdmissionDialog` |
...
| Mutations (region) | `mutation: assignBed`, `mutation: transferOut` |
...
| Raw `showDialog` / `AlertDialog` | yes — migrate |
| `CircularProgressIndicator` | not seen |
| Title snippets | `l10n.icuAssignBedDialogTitle`, `l10n.icuRoundDialogTitle`, `l10n.icuStepDownPromptTitle`, `title` |
| `AppButton` variants (order seen) | tertiary -> primary -> tertiary -> primary |
| `AppActionIcons` | seen |
| `barrierDismissible: false` | yes |
| `closeEnabled: false` | yes |
| Loading primitives | seen |
| Peek region size | 17048 chars |

### Priority gaps to close

1. Raw Material dialog API detected — migrate to `AppDialog` / `showAppDialog` (or approved workspace / action helpers).
2. Footer button order may place Cancel/secondary after primary — `prompt.md` requires L→R: secondary actions, **Cancel**, primary commit.
...
**Scope lock:** only `_AssignBedDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_AssignBedDialog` at `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:824` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Assign Bed** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Assign bed`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_TransferRequestDialog`, `IpdStartAdmissionDialog`.
...
7. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch ICU bed occupancy, admission detail, ward lists, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

8. **Preserve reachability**
   - Do not break `openIcuAssignBedDialog`. Update all call sites in the same change when signatures move.
...
1. `_AssignBedDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_TransferRequestDialog`, `IpdStartAdmissionDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: ICU bed occupancy, admission detail, ward lists.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_AssignBedDialog`'s path.
...
<!-- generator: encounter-dialog prompt 08 slug=icu-assign-bed-dialog symbol=_AssignBedDialog shape=confirm -->
