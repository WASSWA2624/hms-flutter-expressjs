...
| Defined in | `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1494` |
| Kind | `custom` |
| Paired opener(s) | `—` |
...
1. **Read contract + source**
   ...
   - Read `_openResponseDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1494` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Response (emergency).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_openResponseDialog` opens through `AppDialog` / approved helpers only.
