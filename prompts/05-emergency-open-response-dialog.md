...
**Scope:** only `_openResponseDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.

**Module / surface:** `emergency`  
**Inventory kind:** `custom`  
**Extends / uses (inventory):** showAppDialog / AppTextActionDialog  
**Action helper peek:** _none detected — adopt an approved action helper when the footer fits_  
**Controller / mutation peek:** `mutation-ish call: dispatchAmbulance`, `mutation-ish call: updateLatestDispatchStatus`, `mutation-ish call: handoff`
...
| Purpose | Patient/encounter flow: Response (emergency). |
...
1. **Read contract + source**
   ...
   - Read `_openResponseDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1501` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Response (emergency).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_openResponseDialog`'s path.
...
<!-- generator: encounter-dialog prompt 05 slug=emergency-open-response-dialog symbol=_openResponseDialog -->
