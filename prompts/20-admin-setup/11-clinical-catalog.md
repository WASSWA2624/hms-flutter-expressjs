# Admin setup Clinical Catalog tab — rule compliance

## Context

Make the Clinical Catalog desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/20-admin-setup/11-clinical-catalog.md`. Apply shared chrome fixes from `prompts/20-admin-setup/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `tenantFacilitySetupTabClinicalCatalog`, deep-link `clinical-services` (aliases `clinical-catalog`, `clinical`, `catalog`, `services`), and omit the tab when gate denies access (facility\|tenant manage) — never show a disabled placeholder (`tabs.mdc`). Align `TenantFacilitySetupDeskSection` / query helpers with `tabs/20-admin-setup/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (**none** (nested strip also without desk counts)). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
8. Preserve row-select → detail/actions hub with **generic titles**; keep nested mutations in-desk via shared dialogs/forms; omit unauthorized nested actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Radiology / lab / clinical catalog mutate & details; Configure visibility helpers.
...
- [ ] `tabs/20-admin-setup/11-clinical-catalog.md` updated to match.
