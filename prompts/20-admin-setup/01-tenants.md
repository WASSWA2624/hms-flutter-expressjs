# Admin setup Tenants tab — rule compliance

## Context

Make the Tenants desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/20-admin-setup/01-tenants.md`. Apply shared chrome fixes from `prompts/20-admin-setup/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `tenantFacilitySetupTabTenants`, deep-link `tenants` (alias `tenant`), and omit the tab when gate denies access (`canManageTenant()` (elevated ∪ `tenant:admin`) — **not** platform-only) — never show a disabled placeholder (`tabs.mdc`). Align `TenantFacilitySetupDeskSection` / query helpers with `tabs/20-admin-setup/00-overview.md`.
...
8. Preserve in-desk actions for this surface via shared dialogs/forms; omit unauthorized actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Confirm delete/restore/permanent.
...
- [ ] `tabs/20-admin-setup/01-tenants.md` updated to match.
