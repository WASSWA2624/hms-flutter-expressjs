# Admin setup Users tab — rule compliance

## Context

Make the Users desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/20-admin-setup/10-users.md`. Apply shared chrome fixes from `prompts/20-admin-setup/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `tenantFacilitySetupTabUsers`, deep-link `users`, and omit the tab when gate denies access (`canManageAccess`) — never show a disabled placeholder (`tabs.mdc`). Align `TenantFacilitySetupDeskSection` / query helpers with `tabs/20-admin-setup/00-overview.md`.
...
8. Preserve in-desk actions for this surface via shared dialogs/forms; omit unauthorized actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Create/edit user / similarity; User detail / role assignment.
...
- [ ] `tabs/20-admin-setup/10-users.md` updated to match.
