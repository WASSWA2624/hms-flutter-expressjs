# IPD Active Patients tab — rule compliance

## Context

Make the Active Patients desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/04-ipd/02-active-patients.md`. Apply shared chrome fixes from `prompts/04-ipd/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `ipdActivePatientsTabLabel`, deep-link `active` (aliases `active-patients`, …), and omit the tab when gate denies access (`IpdActivePatientsAtomPermissions.tab`) — never show a disabled placeholder (`tabs.mdc`). Align `IpdWorkspaceSection` / query helpers with `tabs/04-ipd/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.activePatientCount`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [x] `tabs/04-ipd/02-active-patients.md` updated to match.
