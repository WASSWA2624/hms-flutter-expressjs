...
| Field | Value |
| --- | --- |
| Symbol | `OpdRescheduleAppointmentDialog` |
| Purpose | Opd Reschedule Appointment |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:289` |
...
| Paired opener(s) | `_openReschedule`, `showOpdRescheduleAppointmentDialog` |
| Primary commit | Edit / save schedule |
| Slices to keep in sync | appointment schedule, reception/OPD calendars |
| Sibling reuse targets | `OpdCancelAppointmentDialog`, `PatientAppointmentQuickDialog` |
...
| Mutations (region) | `mutation: rescheduleAppointment` |
...
| Title snippets | `l10n.opdRescheduleAction`, `widget.title`, `title`, `notesLabel` |
...
**Scope lock:** only `OpdRescheduleAppointmentDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `OpdRescheduleAppointmentDialog` at `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:289` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Opd Reschedule Appointment** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Edit / save schedule`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `OpdCancelAppointmentDialog`, `PatientAppointmentQuickDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch appointment schedule, reception/OPD calendars, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openReschedule`, `showOpdRescheduleAppointmentDialog`. Update all call sites in the same change when signatures move.
...
1. `OpdRescheduleAppointmentDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdCancelAppointmentDialog`, `PatientAppointmentQuickDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: appointment schedule, reception/OPD calendars.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `OpdRescheduleAppointmentDialog`'s path.
...
<!-- generator: encounter-dialog prompt 27 slug=opd-reschedule-appointment-dialog symbol=OpdRescheduleAppointmentDialog shape=widget_dialog -->
