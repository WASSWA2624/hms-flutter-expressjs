# Prompt 007 — Implement Payment Runs

## Context

- **Sequence:** 007 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 046 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → Supplier Adjustments` (`prompts/01-accounts-and-finance/05-purchases-and-payables/006-supplier-adjustments.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → Payment Runs`
- **Canonical route:** `/accounts?section=payment-runs`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/payment-runs.md`
...
2. **Register and scope the tab.**
   - Implement the `Payment Runs` tab at `Accounts & Finance → Purchases & Payables → Payment Runs` with canonical section slug `payment-runs`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/payment-runs.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/payment-runs.md`:
     `Run No.`, `Run Date`, `Payment Date`, `Bank/Cash Account`, `Supplier Count`, `Invoice Count`, `Gross Amount`, `Discounts`, `Withholding Tax`, `Net Amount`, `Currency`, `Prepared By`, `Approval Status`, `Processing Status`, `Export Reference`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Create payment run:** gate `accounts:write` ∩ `facility-accounts`; enable when approved due invoices exist; Builds a proposed supplier batch.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Run Date`, `Payment Date`, `Bank/Cash Account`, `Gross Amount`, `Discounts`, `Withholding Tax`, `Net Amount`, `Currency`, `Export Reference`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Supplier Payments` (`prompts/01-accounts-and-finance/05-purchases-and-payables/008-supplier-payments.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=payment-runs` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/payment-runs.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/payment-runs.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_payment_runs_panel.dart`
