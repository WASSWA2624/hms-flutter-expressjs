...
2. **Register and scope the menu item.**
   - Implement the `Payment Runs` leaf menu item at `Accounts & Finance → Purchases & Payables → Payment Runs` with canonical section slug `payment-runs`.
...
- **AC2 (R2):** `/accounts?section=payment-runs` opens the correct authorized menu item, restores state, and omits it when access is denied.
