...
2. **Register and scope the menu item.**
   - Implement the `Supplier Statements` leaf menu item at `Accounts & Finance → Purchases & Payables → Supplier Statements` with canonical section slug `supplier-statements`.
...
- **AC2 (R2):** `/accounts?section=supplier-statements` opens the correct authorized menu item, restores state, and omits it when access is denied.
