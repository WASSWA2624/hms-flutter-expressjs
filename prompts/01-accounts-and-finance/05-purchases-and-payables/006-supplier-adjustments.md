...
2. **Register and scope the menu item.**
   - Implement the `Supplier Adjustments` leaf menu item at `Accounts & Finance → Purchases & Payables → Supplier Adjustments` with canonical section slug `supplier-adjustments`.
...
- **AC2 (R2):** `/accounts?section=supplier-adjustments` opens the correct authorized menu item, restores state, and omits it when access is denied.
