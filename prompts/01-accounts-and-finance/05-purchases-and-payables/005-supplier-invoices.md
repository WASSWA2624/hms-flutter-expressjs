# Prompt 005 — Implement Supplier Invoices

## Context

- **Sequence:** 005 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 044 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → Goods Received Notes` (`prompts/01-accounts-and-finance/05-purchases-and-payables/004-goods-received-notes.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → Supplier Invoices`
- **Canonical route:** `/accounts?section=supplier-invoices`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-invoices.md`
...
2. **Register and scope the tab.**
   - Implement the `Supplier Invoices` tab at `Accounts & Finance → Purchases & Payables → Supplier Invoices` with canonical section slug `supplier-invoices`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-invoices.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-invoices.md`:
     `Supplier Invoice No.`, `Internal Reference`, `Invoice Date`, `Due Date`, `Supplier`, `PO No.`, `GRN No.`, `Subtotal`, `Tax`, `Withholding Tax`, `Total`, `Paid Amount`, `Balance`, `Currency`, `Aging Days`, `Matching Status`, `Approval Status`, `Posting Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New invoice:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens invoice header and line-item dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Internal Reference`, `Invoice Date`, `Due Date`, `Supplier`, `PO No.`, `GRN No.`, `Withholding Tax`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Supplier Adjustments` (`prompts/01-accounts-and-finance/05-purchases-and-payables/006-supplier-adjustments.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=supplier-invoices` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-invoices.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-invoices.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_supplier_invoices_panel.dart`
