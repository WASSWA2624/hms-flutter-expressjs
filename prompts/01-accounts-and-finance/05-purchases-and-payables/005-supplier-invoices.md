...
2. **Register and scope the menu item.**
   - Implement the `Supplier Invoices` leaf menu item at `Accounts & Finance → Purchases & Payables → Supplier Invoices` with canonical section slug `supplier-invoices`.
...
- **AC2 (R2):** `/accounts?section=supplier-invoices` opens the correct authorized menu item, restores state, and omits it when access is denied.
