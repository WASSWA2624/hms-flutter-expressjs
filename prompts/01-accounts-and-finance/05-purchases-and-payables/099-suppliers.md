# Prompt 099 — Implement Suppliers

## Context

- **Sequence:** 099 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** Prompt 098 — Insurance Reports (`prompts/03-insurance-and-claims/098-insurance-reports.md`).
...
2. **Register and scope the menu item.**
   - Implement the `Suppliers` leaf menu item at `Accounts & Finance → Purchases & Payables → Suppliers` with canonical section slug `suppliers`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to Prompt 100 until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=suppliers` opens the correct authorized menu item, restores state, and omits it when access is denied.
