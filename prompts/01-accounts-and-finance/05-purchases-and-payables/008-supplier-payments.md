# Prompt 008 — Implement Supplier Payments

## Context

- **Sequence:** 008 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 047 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → Payment Runs` (`prompts/01-accounts-and-finance/05-purchases-and-payables/007-payment-runs.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → Supplier Payments`
- **Canonical route:** `/accounts?section=supplier-payments`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-payments.md`
...
2. **Register and scope the tab.**
   - Implement the `Supplier Payments` tab at `Accounts & Finance → Purchases & Payables → Supplier Payments` with canonical section slug `supplier-payments`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-payments.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-payments.md`:
     `Payment No.`, `Payment Date`, `Supplier`, `Payment Run No.`, `Invoice Reference(s)`, `Payment Method`, `Bank/Cash Account`, `External Reference`, `Gross Amount`, `Withholding Tax`, `Net Amount`, `Allocated Amount`, `Unallocated Amount`, `Currency`, `Approval Status`, `Payment Status`, `Processed By`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Payment Date`, `Supplier`, `Payment Run No.`, `Invoice Reference(s)`, `Payment Method`, `Bank/Cash Account`, `External Reference`, `Gross Amount`, `Withholding Tax`, `Net Amount`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Supplier Statements` (`prompts/01-accounts-and-finance/05-purchases-and-payables/009-supplier-statements.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=supplier-payments` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-payments.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/supplier-payments.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_supplier_payments_panel.dart`
