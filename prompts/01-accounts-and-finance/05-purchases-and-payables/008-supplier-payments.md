...
2. **Register and scope the menu item.**
   - Implement the `Supplier Payments` leaf menu item at `Accounts & Finance → Purchases & Payables → Supplier Payments` with canonical section slug `supplier-payments`.
...
- **AC2 (R2):** `/accounts?section=supplier-payments` opens the correct authorized menu item, restores state, and omits it when access is denied.
