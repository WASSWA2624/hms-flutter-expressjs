# Prompt 003 — Implement Purchase Orders

## Context

- **Sequence:** 003 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 042 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → Purchase Requisitions` (`prompts/01-accounts-and-finance/05-purchases-and-payables/002-purchase-requisitions.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → Purchase Orders`
- **Canonical route:** `/accounts?section=purchase-orders`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-orders.md`
...
2. **Register and scope the tab.**
   - Implement the `Purchase Orders` tab at `Accounts & Finance → Purchases & Payables → Purchase Orders` with canonical section slug `purchase-orders`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-orders.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-orders.md`:
     `PO No.`, `PO Date`, `Supplier`, `Requisition Reference(s)`, `Buyer`, `Expected Delivery Date`, `Item Count`, `Subtotal`, `Discount`, `Tax`, `Total`, `Currency`, `Received Amount`, `Invoiced Amount`, `Remaining Amount`, `Approval Status`, `PO Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New purchase order:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens PO header and line-item dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `PO Date`, `Supplier`, `Requisition Reference(s)`, `Buyer`, `Expected Delivery Date`, `Discount`, `Currency`, `Received Amount`, `Invoiced Amount`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Goods Received Notes` (`prompts/01-accounts-and-finance/05-purchases-and-payables/004-goods-received-notes.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=purchase-orders` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-orders.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-orders.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_purchase_orders_panel.dart`
