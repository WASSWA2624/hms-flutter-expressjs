...
2. **Register and scope the menu item.**
   - Implement the `Purchase Orders` leaf menu item at `Accounts & Finance → Purchases & Payables → Purchase Orders` with canonical section slug `purchase-orders`.
...
- **AC2 (R2):** `/accounts?section=purchase-orders` opens the correct authorized menu item, restores state, and omits it when access is denied.
