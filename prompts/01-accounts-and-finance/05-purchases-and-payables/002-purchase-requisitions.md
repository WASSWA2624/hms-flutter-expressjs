# Prompt 002 — Implement Purchase Requisitions

## Context

- **Sequence:** 002 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 041 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → Suppliers` (`prompts/01-accounts-and-finance/05-purchases-and-payables/001-suppliers.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → Purchase Requisitions`
- **Canonical route:** `/accounts?section=purchase-requisitions`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-requisitions.md`
...
2. **Register and scope the tab.**
   - Implement the `Purchase Requisitions` tab at `Accounts & Finance → Purchases & Payables → Purchase Requisitions` with canonical section slug `purchase-requisitions`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-requisitions.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-requisitions.md`:
     `Requisition No.`, `Request Date`, `Requested By`, `Department`, `Cost Centre`, `Required Date`, `Item Count`, `Estimated Amount`, `Currency`, `Budget Reference`, `Available Budget`, `Priority`, `Approval Level`, `Approval Status`, `Purchase Order No.`, `Requisition Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New requisition:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens requisition and line-item dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Request Date`, `Department`, `Cost Centre`, `Required Date`, `Estimated Amount`, `Currency`, `Budget Reference`, `Priority`, `Approval Level`, `Purchase Order No.`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Purchase Orders` (`prompts/01-accounts-and-finance/05-purchases-and-payables/003-purchase-orders.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=purchase-requisitions` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-requisitions.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/purchase-requisitions.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_purchase_requisitions_panel.dart`
