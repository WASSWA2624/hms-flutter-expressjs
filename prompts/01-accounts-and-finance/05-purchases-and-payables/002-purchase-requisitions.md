...
2. **Register and scope the menu item.**
   - Implement the `Purchase Requisitions` leaf menu item at `Accounts & Finance → Purchases & Payables → Purchase Requisitions` with canonical section slug `purchase-requisitions`.
...
- **AC2 (R2):** `/accounts?section=purchase-requisitions` opens the correct authorized menu item, restores state, and omits it when access is denied.
