...
2. **Register and scope the menu item.**
   - Implement the `AP Reconciliation` leaf menu item at `Accounts & Finance → Purchases & Payables → AP Reconciliation` with canonical section slug `ap-reconciliation`.
...
- **AC2 (R2):** `/accounts?section=ap-reconciliation` opens the correct authorized menu item, restores state, and omits it when access is denied.
