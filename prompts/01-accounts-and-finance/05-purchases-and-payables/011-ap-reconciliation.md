# Prompt 011 — Implement AP Reconciliation

## Context

- **Sequence:** 011 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 050 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → AP Aging` (`prompts/01-accounts-and-finance/05-purchases-and-payables/010-ap-aging.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → AP Reconciliation`
- **Canonical route:** `/accounts?section=ap-reconciliation`
- **Surface profile:** `reconciliation` table/worklist tab
- **Navigation model:** `Accounts & Finance → Purchases & Payables` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/ap-reconciliation.md`
...
2. **Register and scope the tab.**
   - Implement the `AP Reconciliation` tab at `Accounts & Finance → Purchases & Payables → AP Reconciliation` with canonical section slug `ap-reconciliation`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Purchases & Payables` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/ap-reconciliation.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/ap-reconciliation.md`:
     `Reconciliation No.`, `Reconciliation Date`, `Supplier`, `Statement Balance`, `Subledger Balance`, `GL Balance`, `Unmatched Invoice Amount`, `Unmatched Payment Amount`, `Difference`, `Currency`, `Exception Count`, `Prepared By`, `Reviewed By`, `Reconciliation Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Reconciliation Date`, `Supplier`, `Unmatched Invoice Amount`, `Unmatched Payment Amount`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Expense Entries` (`prompts/01-accounts-and-finance/06-expenses/001-expense-entries.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=ap-reconciliation` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/ap-reconciliation.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/ap-reconciliation.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_ap_reconciliation_panel.dart`
