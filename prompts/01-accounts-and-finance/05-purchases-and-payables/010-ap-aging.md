...
2. **Register and scope the menu item.**
   - Implement the `AP Aging` leaf menu item at `Accounts & Finance → Purchases & Payables → AP Aging` with canonical section slug `ap-aging`.
...
- **AC2 (R2):** `/accounts?section=ap-aging` opens the correct authorized menu item, restores state, and omits it when access is denied.
