# Prompt 004 — Implement Goods Received Notes

## Context

- **Sequence:** 004 of 011 in `prompts/01-accounts-and-finance/05-purchases-and-payables/`
- **Overall step:** 043 of 158
- **Phase:** Accounts operations — purchases and payables
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → Purchase Orders` (`prompts/01-accounts-and-finance/05-purchases-and-payables/003-purchase-orders.md`).
- **Menu path:** `Accounts & Finance → Purchases & Payables → Goods Received Notes`
- **Canonical route:** `/accounts?section=goods-received-notes`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/purchases-and-payables/goods-received-notes.md`
...
2. **Register and scope the tab.**
   - Implement the `Goods Received Notes` tab at `Accounts & Finance → Purchases & Payables → Goods Received Notes` with canonical section slug `goods-received-notes`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/purchases-and-payables/goods-received-notes.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/purchases-and-payables/goods-received-notes.md`:
     `GRN No.`, `Receipt Date`, `Supplier`, `PO No.`, `Delivery Note No.`, `Warehouse/Store`, `Item Count`, `Ordered Quantity`, `Received Quantity`, `Rejected Quantity`, `Accepted Value`, `Currency`, `Inspection Status`, `Received By`, `Posted Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Record goods receipt:** gate `accounts:write` ∩ `facility-accounts`; enable when approved PO has open quantity; Opens receipt and inspection dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Receipt Date`, `Supplier`, `PO No.`, `Delivery Note No.`, `Warehouse/Store`, `Ordered Quantity`, `Received Quantity`, `Rejected Quantity`, `Accepted Value`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Supplier Invoices` (`prompts/01-accounts-and-finance/05-purchases-and-payables/005-supplier-invoices.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=goods-received-notes` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/purchases-and-payables/goods-received-notes.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/purchases-and-payables/goods-received-notes.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_goods_received_notes_panel.dart`
