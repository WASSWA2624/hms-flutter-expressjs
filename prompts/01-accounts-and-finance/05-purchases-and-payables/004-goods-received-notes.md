...
2. **Register and scope the menu item.**
   - Implement the `Goods Received Notes` leaf menu item at `Accounts & Finance → Purchases & Payables → Goods Received Notes` with canonical section slug `goods-received-notes`.
...
- **AC2 (R2):** `/accounts?section=goods-received-notes` opens the correct authorized menu item, restores state, and omits it when access is denied.
