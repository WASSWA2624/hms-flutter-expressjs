# Prompt 005 — Implement Cash Counts

## Context

- **Sequence:** 005 of 008 in `prompts/01-accounts-and-finance/03-cash-management/`
- **Overall step:** 028 of 158
- **Phase:** Treasury foundation — cash management
- **Prerequisite:** `Accounts & Finance → Cash Management → Cash Transfers` (`prompts/01-accounts-and-finance/03-cash-management/004-cash-transfers.md`).
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Cash Management → Session Closures` (`prompts/01-accounts-and-finance/03-cash-management/006-session-closures.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-counts` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/cash-management/cash-counts.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_counts_panel.dart`
