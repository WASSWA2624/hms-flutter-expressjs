...
2. **Register and scope the menu item.**
   - Implement the `Cash Counts` leaf menu item at `Accounts & Finance → Cash Management → Cash Counts` with canonical section slug `cash-counts`.
...
- **AC2 (R2):** `/accounts?section=cash-counts` opens the correct authorized menu item, restores state, and omits it when access is denied.
