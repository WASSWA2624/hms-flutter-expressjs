...
2. **Register and scope the menu item.**
   - Implement the `Cash Transactions` leaf menu item at `Accounts & Finance → Cash Management → Cash Transactions` with canonical section slug `cash-transactions`.
...
- **AC2 (R2):** `/accounts?section=cash-transactions` opens the correct authorized menu item, restores state, and omits it when access is denied.
