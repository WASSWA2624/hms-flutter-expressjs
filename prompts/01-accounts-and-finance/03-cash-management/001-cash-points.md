...
2. **Register and scope the menu item.**
   - Implement the `Cash Points` leaf menu item at `Accounts & Finance → Cash Management → Cash Points` with canonical section slug `cash-points`.
...
- **AC2 (R2):** `/accounts?section=cash-points` opens the correct authorized menu item, restores state, and omits it when access is denied.
