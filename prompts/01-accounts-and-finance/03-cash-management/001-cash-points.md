# Prompt 001 — Implement Cash Points

## Context

- **Sequence:** 001 of 008 in `prompts/01-accounts-and-finance/03-cash-management/`
- **Overall step:** 024 of 158
- **Phase:** Treasury foundation — cash management
- **Prerequisite:** `Accounts & Finance → General Accounting → Reversing Entries` (`prompts/01-accounts-and-finance/02-general-accounting/015-reversing-entries.md`).
- **Menu path:** `Accounts & Finance → Cash Management → Cash Points`
- **Canonical route:** `/accounts?section=cash-points`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/cash-management/cash-points.md`
...
2. **Register and scope the tab.**
   - Implement the `Cash Points` tab at `Accounts & Finance → Cash Management → Cash Points` with canonical section slug `cash-points`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/cash-management/cash-points.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/cash-management/cash-points.md`:
     `Cash Point Code`, `Cash Point Name`, `Facility`, `Department`, `Location`, `Default Currency`, `GL Account`, `Custodian`, `Allowed Payment Methods`, `Opening Float Limit`, `Maximum Balance`, `Active Session Count`, `Last Reconciled At`, `Cash Point Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Cash Points create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Cash Point Code`, `Cash Point Name`, `Facility`, `Department`, `Location`, `Default Currency`, `GL Account`, `Custodian`, `Allowed Payment Methods`, `Opening Float Limit`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Cash Management → Cashier Sessions` (`prompts/01-accounts-and-finance/03-cash-management/002-cashier-sessions.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-points` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/cash-management/cash-points.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/cash-management/cash-points.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_points_panel.dart`
