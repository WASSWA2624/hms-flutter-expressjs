# Prompt 006 — Implement Session Closures

## Context

- **Sequence:** 006 of 008 in `prompts/01-accounts-and-finance/03-cash-management/`
- **Overall step:** 029 of 158
- **Phase:** Treasury foundation — cash management
- **Prerequisite:** `Accounts & Finance → Cash Management → Cash Counts` (`prompts/01-accounts-and-finance/03-cash-management/005-cash-counts.md`).
- **Menu path:** `Accounts & Finance → Cash Management → Session Closures`
- **Canonical route:** `/accounts?section=session-closures`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/cash-management/session-closures.md`
...
2. **Register and scope the tab.**
   - Implement the `Session Closures` tab at `Accounts & Finance → Cash Management → Session Closures` with canonical section slug `session-closures`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/cash-management/session-closures.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/cash-management/session-closures.md`:
     `Closure No.`, `Business Date`, `Session No.`, `Cash Point`, `Cashier`, `Expected Cash`, `Counted Cash`, `Variance`, `Currency`, `Receipt Sequence From`, `Receipt Sequence To`, `Refund Total`, `Transfer Total`, `Variance Reason`, `Closed By`, `Approved By`, `Closure Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Close session:** gate `accounts:write` ∩ `facility-accounts`; enable when session is reconcilable; Opens closure review dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Business Date`, `Session No.`, `Cash Point`, `Cashier`, `Expected Cash`, `Counted Cash`, `Currency`, `Receipt Sequence From`, `Receipt Sequence To`, `Refund Total`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Cash Management → Cash Variances` (`prompts/01-accounts-and-finance/03-cash-management/007-cash-variances.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=session-closures` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/cash-management/session-closures.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/cash-management/session-closures.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_session_closures_panel.dart`
