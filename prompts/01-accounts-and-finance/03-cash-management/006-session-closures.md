...
2. **Register and scope the menu item.**
   - Implement the `Session Closures` leaf menu item at `Accounts & Finance → Cash Management → Session Closures` with canonical section slug `session-closures`.
...
- **AC2 (R2):** `/accounts?section=session-closures` opens the correct authorized menu item, restores state, and omits it when access is denied.
