# Prompt 004 — Implement Cash Transfers

## Context

- **Sequence:** 004 of 008 in `prompts/01-accounts-and-finance/03-cash-management/`
- **Overall step:** 027 of 158
- **Phase:** Treasury foundation — cash management
- **Prerequisite:** `Accounts & Finance → Cash Management → Cash Transactions` (`prompts/01-accounts-and-finance/03-cash-management/003-cash-transactions.md`).
- **Menu path:** `Accounts & Finance → Cash Management → Cash Transfers`
- **Canonical route:** `/accounts?section=cash-transfers`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/cash-management/cash-transfers.md`
...
2. **Register and scope the tab.**
   - Implement the `Cash Transfers` tab at `Accounts & Finance → Cash Management → Cash Transfers` with canonical section slug `cash-transfers`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/cash-management/cash-transfers.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/cash-management/cash-transfers.md`:
     `Transfer No.`, `Request Date/Time`, `From Cash Point`, `From Session`, `To Cash Point/Bank`, `Transfer Amount`, `Currency`, `Reason`, `Requested By`, `Released By`, `Received By`, `Released At`, `Received At`, `Difference`, `Transfer Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Cash Transfers mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Request Date/Time`, `From Cash Point`, `From Session`, `To Cash Point/Bank`, `Transfer Amount`, `Currency`, `Reason`, `Released By`, `Released At`, `Received At`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Cash Management → Cash Counts` (`prompts/01-accounts-and-finance/03-cash-management/005-cash-counts.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-transfers` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/cash-management/cash-transfers.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/cash-management/cash-transfers.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_transfers_panel.dart`
