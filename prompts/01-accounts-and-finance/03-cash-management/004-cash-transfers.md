...
2. **Register and scope the menu item.**
   - Implement the `Cash Transfers` leaf menu item at `Accounts & Finance → Cash Management → Cash Transfers` with canonical section slug `cash-transfers`.
...
- **AC2 (R2):** `/accounts?section=cash-transfers` opens the correct authorized menu item, restores state, and omits it when access is denied.
