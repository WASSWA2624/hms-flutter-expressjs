...
2. **Register and scope the menu item.**
   - Implement the `Cashier Sessions` leaf menu item at `Accounts & Finance → Cash Management → Cashier Sessions` with canonical section slug `cashier-sessions`.
...
- **AC2 (R2):** `/accounts?section=cashier-sessions` opens the correct authorized menu item, restores state, and omits it when access is denied.
