...
2. **Register and scope the menu item.**
   - Implement the `Cash Variances` leaf menu item at `Accounts & Finance → Cash Management → Cash Variances` with canonical section slug `cash-variances`.
...
- **AC2 (R2):** `/accounts?section=cash-variances` opens the correct authorized menu item, restores state, and omits it when access is denied.
