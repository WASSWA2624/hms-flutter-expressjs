# Prompt 007 — Implement Cash Variances

## Context

- **Sequence:** 007 of 008 in `prompts/01-accounts-and-finance/03-cash-management/`
- **Overall step:** 030 of 158
- **Phase:** Treasury foundation — cash management
- **Prerequisite:** `Accounts & Finance → Cash Management → Session Closures` (`prompts/01-accounts-and-finance/03-cash-management/006-session-closures.md`).
- **Menu path:** `Accounts & Finance → Cash Management → Cash Variances`
- **Canonical route:** `/accounts?section=cash-variances`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/cash-management/cash-variances.md`
...
2. **Register and scope the tab.**
   - Implement the `Cash Variances` tab at `Accounts & Finance → Cash Management → Cash Variances` with canonical section slug `cash-variances`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/cash-management/cash-variances.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/cash-management/cash-variances.md`:
     `Variance No.`, `Business Date`, `Session No.`, `Cash Point`, `Cashier`, `Expected Amount`, `Counted Amount`, `Variance Amount`, `Currency`, `Variance Type`, `Reason Code`, `Explanation`, `Investigation Owner`, `Resolution`, `Journal Entry No.`, `Approval Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Cash Variances mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Business Date`, `Session No.`, `Cash Point`, `Cashier`, `Expected Amount`, `Counted Amount`, `Currency`, `Reason Code`, `Explanation`, `Investigation Owner`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Cash Management → Cash Reconciliation` (`prompts/01-accounts-and-finance/03-cash-management/008-cash-reconciliation.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-variances` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/cash-management/cash-variances.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/cash-management/cash-variances.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_variances_panel.dart`
