...
2. **Register and scope the menu item.**
   - Implement the `Cash Reconciliation` leaf menu item at `Accounts & Finance → Cash Management → Cash Reconciliation` with canonical section slug `cash-reconciliation`.
...
- **AC2 (R2):** `/accounts?section=cash-reconciliation` opens the correct authorized menu item, restores state, and omits it when access is denied.
