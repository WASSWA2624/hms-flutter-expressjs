# Prompt 008 — Implement Cash Reconciliation

## Context

- **Sequence:** 008 of 008 in `prompts/01-accounts-and-finance/03-cash-management/`
- **Overall step:** 031 of 158
- **Phase:** Treasury foundation — cash management
- **Prerequisite:** `Accounts & Finance → Cash Management → Cash Variances` (`prompts/01-accounts-and-finance/03-cash-management/007-cash-variances.md`).
- **Menu path:** `Accounts & Finance → Cash Management → Cash Reconciliation`
- **Canonical route:** `/accounts?section=cash-reconciliation`
- **Surface profile:** `reconciliation` table/worklist tab
- **Navigation model:** `Accounts & Finance → Cash Management` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/cash-management/cash-reconciliation.md`
...
2. **Register and scope the tab.**
   - Implement the `Cash Reconciliation` tab at `Accounts & Finance → Cash Management → Cash Reconciliation` with canonical section slug `cash-reconciliation`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Cash Management` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/cash-management/cash-reconciliation.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/cash-management/cash-reconciliation.md`:
     `Reconciliation No.`, `Business Date`, `Cash Point`, `Session Count`, `Receipt Total`, `System Cash Total`, `Counted Cash Total`, `Deposit Total`, `Outstanding Deposit`, `Difference`, `Currency`, `Exception Count`, `Prepared By`, `Reviewed By`, `Reconciliation Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Business Date`, `Cash Point`, `Receipt Total`, `System Cash Total`, `Counted Cash Total`, `Deposit Total`, `Outstanding Deposit`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Bank Accounts` (`prompts/01-accounts-and-finance/04-bank-management/001-bank-accounts.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-reconciliation` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/cash-management/cash-reconciliation.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/cash-management/cash-reconciliation.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_reconciliation_panel.dart`
