...
2. **Register and scope the menu item.**
   - Implement the `Posting History` leaf menu item at `Accounts & Finance → Audit Trail → Posting History` with canonical section slug `posting-history`.
...
- **AC2 (R2):** `/accounts?section=posting-history` opens the correct authorized menu item, restores state, and omits it when access is denied.
