# Prompt 001 — Implement Transaction History

## Context

- **Sequence:** 001 of 005 in `prompts/01-accounts-and-finance/12-audit-trail/`
- **Overall step:** 094 of 158
- **Phase:** Accounts operations — audit trail
- **Prerequisite:** `Accounts & Finance → Financial Reports → Consolidated Reports` (`prompts/01-accounts-and-finance/11-financial-reports/009-consolidated-reports.md`).
- **Menu path:** `Accounts & Finance → Audit Trail → Transaction History`
- **Canonical route:** `/accounts?section=transaction-history`
- **Surface profile:** `audit` table/worklist tab
- **Navigation model:** `Accounts & Finance → Audit Trail` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/audit-trail/transaction-history.md`
...
2. **Register and scope the tab.**
   - Implement the `Transaction History` tab at `Accounts & Finance → Audit Trail → Transaction History` with canonical section slug `transaction-history`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Audit Trail` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/audit-trail/transaction-history.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/audit-trail/transaction-history.md`:
     `Event Date/Time`, `Module`, `Entity Type`, `Entity Reference`, `Event Type`, `Before Summary`, `After Summary`, `Amount`, `Currency`, `Facility`, `Performed By`, `Role`, `Source IP/Device`, `Correlation ID`, `Outcome`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - None beyond the standard table controls required by `tables.mdc`.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Inspect event:** gate `accounts:read` ∩ `facility-accounts`; enable when always; Shows immutable before/after, actor, scope, and correlation data.
     - **Open source:** gate `accounts:read` ∩ `facility-accounts`; enable when source still visible; Opens authoritative record without duplicating it.
   - Bulk buttons:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Audit Trail → Approval History` (`prompts/01-accounts-and-finance/12-audit-trail/002-approval-history.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=transaction-history` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/audit-trail/transaction-history.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/audit-trail/transaction-history.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_transaction_history_panel.dart`
