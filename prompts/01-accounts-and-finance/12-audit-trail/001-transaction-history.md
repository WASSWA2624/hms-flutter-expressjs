...
- **Surface profile:** `audit` table/worklist opened from a nested menu item
...
2. **Register and scope the menu item.**
   - Implement the `Transaction History` leaf menu item at `Accounts & Finance → Audit Trail → Transaction History` with canonical section slug `transaction-history`.
...
- **AC2 (R2):** `/accounts?section=transaction-history` opens the correct authorized menu item, restores state, and omits it when access is denied.
