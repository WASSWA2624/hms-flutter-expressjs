...
2. **Register and scope the menu item.**
   - Implement the `Approval History` leaf menu item at `Accounts & Finance → Audit Trail → Approval History` with canonical section slug `approval-history`.
...
- **AC2 (R2):** `/accounts?section=approval-history` opens the correct authorized menu item, restores state, and omits it when access is denied.
