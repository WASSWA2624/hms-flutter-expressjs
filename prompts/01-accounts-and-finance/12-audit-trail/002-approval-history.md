# Prompt 002 — Implement Approval History

## Context

- **Sequence:** 002 of 005 in `prompts/01-accounts-and-finance/12-audit-trail/`
- **Overall step:** 095 of 158
- **Phase:** Accounts operations — audit trail
- **Prerequisite:** `Accounts & Finance → Audit Trail → Transaction History` (`prompts/01-accounts-and-finance/12-audit-trail/001-transaction-history.md`).
- **Menu path:** `Accounts & Finance → Audit Trail → Approval History`
- **Canonical route:** `/accounts?section=approval-history`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/audit-trail/approval-history.md`
...
2. **Register and scope the tab.**
   - Implement the `Approval History` tab at `Accounts & Finance → Audit Trail → Approval History` with canonical section slug `approval-history`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/audit-trail/approval-history.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/audit-trail/approval-history.md`:
     `Decision Date/Time`, `Module`, `Document Type`, `Document Reference`, `Amount`, `Currency`, `Approval Level`, `Decision`, `Decision Reason`, `Approver`, `Delegated From`, `Waiting Time`, `SLA Status`, `Correlation ID`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Audit Trail → Posting History` (`prompts/01-accounts-and-finance/12-audit-trail/003-posting-history.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=approval-history` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/audit-trail/approval-history.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/audit-trail/approval-history.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_approval_history_panel.dart`
