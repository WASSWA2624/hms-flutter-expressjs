# Prompt 154 — Implement Transaction History

## Context

- **Sequence:** 154 of 159
- **Phase:** Accounts operations — audit trail
- **Prerequisite:** Prompt 153 — Consolidated Reports (`prompts/01-finance-tabs/01-accounts-and-finance/11-financial-reports/153-consolidated-reports.md`).
- **Menu path:** `Accounts & Finance → Audit Trail → Transaction History`
- **Canonical route:** `/accounts?section=transaction-history`
- **Tab profile:** `audit` table/worklist
...
10. **Add and run verification.**
    ...
    - Do not proceed to Prompt 155 until all acceptance criteria below pass.
...
- **AC1 (R1):** `/accounts?section=transaction-history` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC2 (R2):** All API operations in `.cursor/finance/accounts-and-finance/audit-trail/transaction-history.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `frontend/lib/features/accounts/presentation/widgets/transaction_history_tab.dart`
