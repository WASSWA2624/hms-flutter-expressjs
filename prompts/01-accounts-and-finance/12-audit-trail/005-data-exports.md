# Prompt 005 — Implement Data Exports

## Context

- **Sequence:** 005 of 005 in `prompts/01-accounts-and-finance/12-audit-trail/`
- **Overall step:** 098 of 158
- **Phase:** Accounts operations — audit trail
- **Prerequisite:** `Accounts & Finance → Audit Trail → Reversals & Voids` (`prompts/01-accounts-and-finance/12-audit-trail/004-reversals-and-voids.md`).
- **Menu path:** `Accounts & Finance → Audit Trail → Data Exports`
- **Canonical route:** `/accounts?section=data-exports`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/audit-trail/data-exports.md`
...
2. **Register and scope the tab.**
   - Implement the `Data Exports` tab at `Accounts & Finance → Audit Trail → Data Exports` with canonical section slug `data-exports`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/audit-trail/data-exports.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/audit-trail/data-exports.md`:
     `Export No.`, `Requested At`, `Requested By`, `Module`, `Tab/Report`, `Filter Summary`, `Date Range`, `Facility Scope`, `Column Count`, `Row Count`, `File Format`, `File Size`, `Completed At`, `Download Count`, `Expires At`, `Export Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Overview` (`prompts/01-accounts-and-finance/13-overview/001-overview.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=data-exports` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/audit-trail/data-exports.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/audit-trail/data-exports.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_data_exports_panel.dart`
