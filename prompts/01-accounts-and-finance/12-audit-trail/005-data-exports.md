...
2. **Register and scope the menu item.**
   - Implement the `Data Exports` leaf menu item at `Accounts & Finance → Audit Trail → Data Exports` with canonical section slug `data-exports`.
...
- **AC2 (R2):** `/accounts?section=data-exports` opens the correct authorized menu item, restores state, and omits it when access is denied.
