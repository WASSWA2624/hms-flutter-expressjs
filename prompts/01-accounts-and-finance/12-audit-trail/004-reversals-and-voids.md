...
2. **Register and scope the menu item.**
   - Implement the `Reversals & Voids` leaf menu item at `Accounts & Finance → Audit Trail → Reversals & Voids` with canonical section slug `reversals-and-voids`.
...
- **AC2 (R2):** `/accounts?section=reversals-and-voids` opens the correct authorized menu item, restores state, and omits it when access is denied.
