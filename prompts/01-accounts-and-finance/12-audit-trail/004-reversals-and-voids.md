# Prompt 004 — Implement Reversals & Voids

## Context

- **Sequence:** 004 of 005 in `prompts/01-accounts-and-finance/12-audit-trail/`
- **Overall step:** 097 of 158
- **Phase:** Accounts operations — audit trail
- **Prerequisite:** `Accounts & Finance → Audit Trail → Posting History` (`prompts/01-accounts-and-finance/12-audit-trail/003-posting-history.md`).
- **Menu path:** `Accounts & Finance → Audit Trail → Reversals & Voids`
- **Canonical route:** `/accounts?section=reversals-and-voids`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/audit-trail/reversals-and-voids.md`
...
2. **Register and scope the tab.**
   - Implement the `Reversals & Voids` tab at `Accounts & Finance → Audit Trail → Reversals & Voids` with canonical section slug `reversals-and-voids`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/audit-trail/reversals-and-voids.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/audit-trail/reversals-and-voids.md`:
     `Action Date/Time`, `Module`, `Document Type`, `Original Reference`, `Action Type`, `Reason Code`, `Reason`, `Original Amount`, `Reversed/Voided Amount`, `Currency`, `Replacement Reference`, `Requested By`, `Approved By`, `Journal Entry No.`, `Action Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Audit Trail → Data Exports` (`prompts/01-accounts-and-finance/12-audit-trail/005-data-exports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=reversals-and-voids` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/audit-trail/reversals-and-voids.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/audit-trail/reversals-and-voids.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_reversals_and_voids_panel.dart`
