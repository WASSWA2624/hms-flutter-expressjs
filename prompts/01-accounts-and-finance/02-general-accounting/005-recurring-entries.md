# Prompt 005 — Implement Recurring Entries

## Context

- **Sequence:** 005 of 006 in `prompts/01-accounts-and-finance/02-general-accounting/`
- **Overall step:** 022 of 158
- **Phase:** Accounts foundation — books and recurring entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Control Accounts` (`prompts/01-accounts-and-finance/02-general-accounting/004-control-accounts.md`).
- **Menu path:** `Accounts & Finance → General Accounting → Recurring Entries`
- **Canonical route:** `/accounts?section=recurring-entries`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/general-accounting/recurring-entries.md`
...
2. **Register and scope the menu item.**
   - Implement the `Recurring Entries` leaf menu item at `Accounts & Finance → General Accounting → Recurring Entries` with canonical section slug `recurring-entries`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/general-accounting/recurring-entries.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/general-accounting/recurring-entries.md`:
     `Template No.`, `Template Name`, `Journal Type`, `Description`, `Frequency`, `Start Date`, `Next Run Date`, `End Date`, `Total Debit`, `Total Credit`, `Currency`, `Auto-post`, `Approval Required`, `Last Generated Entry`, `Last Run Status`, `Template Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Recurring Entries mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Template Name`, `Journal Type`, `Description`, `Frequency`, `Start Date`, `Next Run Date`, `End Date`, `Currency`, `Auto-post`, `Approval Required`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Reversing Entries` (`prompts/01-accounts-and-finance/02-general-accounting/006-reversing-entries.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=recurring-entries` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/general-accounting/recurring-entries.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/general-accounting/recurring-entries.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_recurring_entries_panel.dart`
