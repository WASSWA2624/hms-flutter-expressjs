...
2. **Register and scope the tab.**
   - Implement the `General Ledger` tab at `Accounts & Finance → General Accounting → General Ledger` with canonical section slug `general-ledger`.
...
- **AC2 (R2):** `/accounts?section=general-ledger` opens the correct authorized tab, restores state, and omits it when access is denied.
