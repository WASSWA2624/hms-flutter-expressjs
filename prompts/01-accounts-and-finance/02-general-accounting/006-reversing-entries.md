# Prompt 006 — Implement Reversing Entries

## Context

- **Sequence:** 006 of 006 in `prompts/01-accounts-and-finance/02-general-accounting/`
- **Overall step:** 023 of 158
- **Phase:** Accounts foundation — books and recurring entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Recurring Entries` (`prompts/01-accounts-and-finance/02-general-accounting/005-recurring-entries.md`).
- **Menu path:** `Accounts & Finance → General Accounting → Reversing Entries`
- **Canonical route:** `/accounts?section=reversing-entries`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/general-accounting/reversing-entries.md`
...
2. **Register and scope the menu item.**
   - Implement the `Reversing Entries` leaf menu item at `Accounts & Finance → General Accounting → Reversing Entries` with canonical section slug `reversing-entries`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/general-accounting/reversing-entries.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/general-accounting/reversing-entries.md`:
     `Reversal No.`, `Original Entry No.`, `Original Posting Date`, `Reversal Date`, `Fiscal Period`, `Journal Type`, `Reason Code`, `Reason`, `Debit`, `Credit`, `Currency`, `Reversal Entry No.`, `Requested By`, `Approved By`, `Reversal Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Reversing Entries mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Original Entry No.`, `Original Posting Date`, `Reversal Date`, `Fiscal Period`, `Journal Type`, `Reason Code`, `Reason`, `Debit`, `Credit`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Cash Management → Cash Points` (`prompts/01-accounts-and-finance/03-cash-management/001-cash-points.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=reversing-entries` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/general-accounting/reversing-entries.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/general-accounting/reversing-entries.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_reversing_entries_panel.dart`
