# Prompt 014 — Implement Recurring Entries

## Context

- **Sequence:** 014 of 015 in `prompts/01-accounts-and-finance/02-general-accounting/`
- **Overall step:** 022 of 158
- **Phase:** Accounts foundation — books and recurring entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Control Accounts` (`prompts/01-accounts-and-finance/02-general-accounting/013-control-accounts.md`).
...
2. **Register and scope the tab.**
   - Implement the `Recurring Entries` tab at `Accounts & Finance → General Accounting → Recurring Entries` with canonical section slug `recurring-entries`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Reversing Entries` (`prompts/01-accounts-and-finance/02-general-accounting/015-reversing-entries.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=recurring-entries` opens the correct authorized tab, restores state, and omits it when access is denied.
