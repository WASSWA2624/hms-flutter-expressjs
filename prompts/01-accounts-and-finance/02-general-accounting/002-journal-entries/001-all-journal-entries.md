# Prompt 001 — Implement All Journal Entries

## Context

- **Sequence:** 001 of 010 in `prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/`
- **Overall step:** 010 of 158
- **Phase:** Accounts foundation — journal entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Chart of Accounts` (`prompts/01-accounts-and-finance/02-general-accounting/001-chart-of-accounts.md`).
- **Menu path:** `Accounts & Finance → General Accounting → Journal Entries → All Journal Entries`
- **Canonical route:** `/accounts?section=all-journal-entries`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/all-journal-entries.md`
...
2. **Register and scope the menu item.**
   - Implement the `All Journal Entries` leaf menu item at `Accounts & Finance → General Accounting → Journal Entries → All Journal Entries` with canonical section slug `all-journal-entries`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/all-journal-entries.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/all-journal-entries.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Journal Entries → General Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/002-general-journal.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=all-journal-entries` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/all-journal-entries.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/all-journal-entries.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_all_journal_entries_panel.dart`
