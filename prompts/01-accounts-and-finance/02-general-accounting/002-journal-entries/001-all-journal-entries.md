...
2. **Register and scope the tab.**
   - Implement the `All Journal Entries` tab at `Accounts & Finance → General Accounting → Journal Entries → All Journal Entries` with canonical section slug `all-journal-entries`.
...
- **AC2 (R2):** `/accounts?section=all-journal-entries` opens the correct authorized tab, restores state, and omits it when access is denied.
