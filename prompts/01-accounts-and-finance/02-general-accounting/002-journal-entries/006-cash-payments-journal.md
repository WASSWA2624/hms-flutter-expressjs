...
2. **Register and scope the tab.**
   - Implement the `Cash Payments Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Cash Payments Journal` with canonical section slug `cash-payments-journal`.
...
- **AC2 (R2):** `/accounts?section=cash-payments-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
