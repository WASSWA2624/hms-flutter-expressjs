# Prompt 006 — Implement Cash Payments Journal

## Context

- **Sequence:** 006 of 010 in `prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/`
- **Overall step:** 015 of 158
- **Phase:** Accounts foundation — journal entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Journal Entries → Cash Receipts Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/005-cash-receipts-journal.md`).
- **Menu path:** `Accounts & Finance → General Accounting → Journal Entries → Cash Payments Journal`
- **Canonical route:** `/accounts?section=cash-payments-journal`
- **Surface profile:** `transaction` table/worklist opened from a nested menu item
- **Navigation model:** nested sidebar menu items under `Accounts & Finance`; no category or nested tab strip
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-payments-journal.md`
...
2. **Register and scope the menu item.**
   - Implement the `Cash Payments Journal` leaf menu item at `Accounts & Finance → General Accounting → Journal Entries → Cash Payments Journal` with canonical section slug `cash-payments-journal`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-payments-journal.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-payments-journal.md`:
     `Entry No.`, `Journal Type`, `Entry Date`, `Posting Date`, `Fiscal Period`, `Source Module`, `Source Document Type`, `Source Reference`, `Description`, `Total Debit`, `Total Credit`, `Difference`, `Currency`, `Department`, `Cost Centre`, `Entry Status`, `Approval Status`, `Created By`, `Approved By`, `Posted By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Record payment:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens payment dialog and returns the created row.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Journal Type`, `Entry Date`, `Posting Date`, `Fiscal Period`, `Source Module`, `Source Document Type`, `Source Reference`, `Description`, `Currency`, `Department`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Journal Entries → Adjustment Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/007-adjustment-journal.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-payments-journal` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-payments-journal.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-payments-journal.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_payments_journal_panel.dart`
