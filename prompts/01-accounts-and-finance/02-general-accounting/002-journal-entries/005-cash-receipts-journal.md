...
2. **Register and scope the tab.**
   - Implement the `Cash Receipts Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Cash Receipts Journal` with canonical section slug `cash-receipts-journal`.
...
- **AC2 (R2):** `/accounts?section=cash-receipts-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
