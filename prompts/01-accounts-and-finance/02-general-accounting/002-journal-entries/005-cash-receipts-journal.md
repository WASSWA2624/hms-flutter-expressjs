# Prompt 005 — Implement Cash Receipts Journal

## Context

- **Sequence:** 005 of 010 in `prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/`
- **Overall step:** 014 of 158
- **Phase:** Accounts foundation — journal entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Journal Entries → Purchase Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/004-purchase-journal.md`).
- **Menu path:** `Accounts & Finance → General Accounting → Journal Entries → Cash Receipts Journal`
- **Canonical route:** `/accounts?section=cash-receipts-journal`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-receipts-journal.md`
...
2. **Register and scope the menu item.**
   - Implement the `Cash Receipts Journal` leaf menu item at `Accounts & Finance → General Accounting → Journal Entries → Cash Receipts Journal` with canonical section slug `cash-receipts-journal`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-receipts-journal.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-receipts-journal.md`:
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New journal entry:** gate `accounts:write` ∩ `facility-accounts`; enable when period open; Opens balanced journal-entry dialog.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Journal Entries → Cash Payments Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/006-cash-payments-journal.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-receipts-journal` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-receipts-journal.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/cash-receipts-journal.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_receipts_journal_panel.dart`
