# Prompt 007 — Implement Adjustment Journal

## Context

- **Sequence:** 007 of 010 in `prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/`
- **Overall step:** 016 of 158
- **Phase:** Accounts foundation — journal entries
- **Prerequisite:** `Accounts & Finance → General Accounting → Journal Entries → Cash Payments Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/006-cash-payments-journal.md`).
- **Menu path:** `Accounts & Finance → General Accounting → Journal Entries → Adjustment Journal`
- **Canonical route:** `/accounts?section=adjustment-journal`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/adjustment-journal.md`
...
2. **Register and scope the menu item.**
   - Implement the `Adjustment Journal` leaf menu item at `Accounts & Finance → General Accounting → Journal Entries → Adjustment Journal` with canonical section slug `adjustment-journal`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/adjustment-journal.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/adjustment-journal.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Journal Entries → Payroll Journal` (`prompts/01-accounts-and-finance/02-general-accounting/002-journal-entries/008-payroll-journal.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=adjustment-journal` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/adjustment-journal.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/general-accounting/journal-entries/adjustment-journal.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_adjustment_journal_panel.dart`
