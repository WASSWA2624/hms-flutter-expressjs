...
2. **Register and scope the tab.**
   - Implement the `Adjustment Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Adjustment Journal` with canonical section slug `adjustment-journal`.
...
- **AC2 (R2):** `/accounts?section=adjustment-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
