...
2. **Register and scope the tab.**
   - Implement the `General Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → General Journal` with canonical section slug `general-journal`.
...
- **AC2 (R2):** `/accounts?section=general-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
