...
2. **Register and scope the tab.**
   - Implement the `Purchase Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Purchase Journal` with canonical section slug `purchase-journal`.
...
- **AC2 (R2):** `/accounts?section=purchase-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
