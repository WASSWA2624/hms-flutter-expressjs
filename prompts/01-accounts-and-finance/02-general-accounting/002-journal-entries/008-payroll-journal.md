...
2. **Register and scope the tab.**
   - Implement the `Payroll Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Payroll Journal` with canonical section slug `payroll-journal`.
...
- **AC2 (R2):** `/accounts?section=payroll-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
