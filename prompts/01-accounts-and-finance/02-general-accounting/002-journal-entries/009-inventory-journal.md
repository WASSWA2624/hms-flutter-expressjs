...
2. **Register and scope the tab.**
   - Implement the `Inventory Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Inventory Journal` with canonical section slug `inventory-journal`.
...
- **AC2 (R2):** `/accounts?section=inventory-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
