...
2. **Register and scope the tab.**
   - Implement the `Sales Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Sales Journal` with canonical section slug `sales-journal`.
...
- **AC2 (R2):** `/accounts?section=sales-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
