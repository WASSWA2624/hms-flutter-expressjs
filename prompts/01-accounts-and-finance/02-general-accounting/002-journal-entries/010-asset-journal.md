...
2. **Register and scope the tab.**
   - Implement the `Asset Journal` tab at `Accounts & Finance → General Accounting → Journal Entries → Asset Journal` with canonical section slug `asset-journal`.
...
- **AC2 (R2):** `/accounts?section=asset-journal` opens the correct authorized tab, restores state, and omits it when access is denied.
