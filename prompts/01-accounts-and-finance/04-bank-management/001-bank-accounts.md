# Prompt 001 — Implement Bank Accounts

## Context

- **Sequence:** 001 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 032 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Cash Management → Cash Reconciliation` (`prompts/01-accounts-and-finance/03-cash-management/008-cash-reconciliation.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Bank Accounts`
- **Canonical route:** `/accounts?section=bank-accounts`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/bank-accounts.md`
...
2. **Register and scope the tab.**
   - Implement the `Bank Accounts` tab at `Accounts & Finance → Bank Management → Bank Accounts` with canonical section slug `bank-accounts`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/bank-accounts.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/bank-accounts.md`:
     `Account Code`, `Bank Name`, `Branch`, `Account Name`, `Masked Account No.`, `Account Type`, `Currency`, `GL Account`, `Opening Balance`, `Book Balance`, `Statement Balance`, `Available Balance`, `Last Statement Date`, `Last Reconciled Date`, `Signatory Rule`, `Account Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Bank Accounts create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Account Code`, `Bank Name`, `Branch`, `Account Name`, `Masked Account No.`, `Account Type`, `Currency`, `GL Account`, `Book Balance`, `Statement Balance`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Bank Transactions` (`prompts/01-accounts-and-finance/04-bank-management/002-bank-transactions.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=bank-accounts` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/bank-accounts.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/bank-accounts.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_bank_accounts_panel.dart`
