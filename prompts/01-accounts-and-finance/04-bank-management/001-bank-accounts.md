...
2. **Register and scope the menu item.**
   - Implement the `Bank Accounts` leaf menu item at `Accounts & Finance → Bank Management → Bank Accounts` with canonical section slug `bank-accounts`.
...
- **AC2 (R2):** `/accounts?section=bank-accounts` opens the correct authorized menu item, restores state, and omits it when access is denied.
