# Prompt 002 — Implement Bank Transactions

## Context

- **Sequence:** 002 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 033 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Bank Accounts` (`prompts/01-accounts-and-finance/04-bank-management/001-bank-accounts.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Bank Transactions`
- **Canonical route:** `/accounts?section=bank-transactions`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/bank-transactions.md`
...
2. **Register and scope the tab.**
   - Implement the `Bank Transactions` tab at `Accounts & Finance → Bank Management → Bank Transactions` with canonical section slug `bank-transactions`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/bank-transactions.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/bank-transactions.md`:
     `Transaction No.`, `Value Date`, `Posting Date`, `Bank Account`, `Transaction Type`, `Description`, `Counterparty`, `External Reference`, `Source Document`, `Debit`, `Credit`, `Running Balance`, `Currency`, `Match Status`, `Reconciliation No.`, `Posting Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Bank Transactions mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Value Date`, `Posting Date`, `Bank Account`, `Transaction Type`, `Description`, `Counterparty`, `External Reference`, `Source Document`, `Debit`, `Credit`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Bank Deposits` (`prompts/01-accounts-and-finance/04-bank-management/003-bank-deposits.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=bank-transactions` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/bank-transactions.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/bank-transactions.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_bank_transactions_panel.dart`
