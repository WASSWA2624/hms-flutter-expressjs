...
2. **Register and scope the menu item.**
   - Implement the `Bank Transactions` leaf menu item at `Accounts & Finance → Bank Management → Bank Transactions` with canonical section slug `bank-transactions`.
...
- **AC2 (R2):** `/accounts?section=bank-transactions` opens the correct authorized menu item, restores state, and omits it when access is denied.
