...
2. **Register and scope the menu item.**
   - Implement the `Matching Workbench` leaf menu item at `Accounts & Finance → Bank Management → Matching Workbench` with canonical section slug `matching-workbench`.
...
- **AC2 (R2):** `/accounts?section=matching-workbench` opens the correct authorized menu item, restores state, and omits it when access is denied.
