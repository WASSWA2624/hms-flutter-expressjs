# Prompt 007 — Implement Matching Workbench

## Context

- **Sequence:** 007 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 038 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Statement Imports` (`prompts/01-accounts-and-finance/04-bank-management/006-statement-imports.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Matching Workbench`
- **Canonical route:** `/accounts?section=matching-workbench`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/matching-workbench.md`
...
2. **Register and scope the tab.**
   - Implement the `Matching Workbench` tab at `Accounts & Finance → Bank Management → Matching Workbench` with canonical section slug `matching-workbench`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/matching-workbench.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/matching-workbench.md`:
     `Statement Line No.`, `Value Date`, `Description`, `External Reference`, `Statement Amount`, `Currency`, `Suggested System Transaction`, `Suggested Reference`, `Suggested Amount`, `Match Score`, `Difference`, `Match Rule`, `Assigned To`, `Match Status`, `Reconciliation No.`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Value Date`, `Description`, `External Reference`, `Statement Amount`, `Currency`, `Suggested System Transaction`, `Suggested Reference`, `Suggested Amount`, `Match Score`, `Match Rule`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Bank Reconciliations` (`prompts/01-accounts-and-finance/04-bank-management/008-bank-reconciliations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=matching-workbench` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/matching-workbench.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/matching-workbench.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_matching_workbench_panel.dart`
