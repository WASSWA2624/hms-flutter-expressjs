# Prompt 005 — Implement Bank Transfers

## Context

- **Sequence:** 005 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 036 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Withdrawals & Charges` (`prompts/01-accounts-and-finance/04-bank-management/004-withdrawals-and-charges.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Bank Transfers`
- **Canonical route:** `/accounts?section=bank-transfers`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/bank-transfers.md`
...
2. **Register and scope the tab.**
   - Implement the `Bank Transfers` tab at `Accounts & Finance → Bank Management → Bank Transfers` with canonical section slug `bank-transfers`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/bank-transfers.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/bank-transfers.md`:
     `Transfer No.`, `Transfer Date`, `From Account`, `To Account`, `From Amount`, `From Currency`, `Exchange Rate`, `To Amount`, `To Currency`, `Bank Charges`, `External Reference`, `Requested By`, `Approved By`, `Processed At`, `Transfer Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Bank Transfers mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Transfer Date`, `From Account`, `To Account`, `From Amount`, `From Currency`, `Exchange Rate`, `To Amount`, `To Currency`, `Bank Charges`, `External Reference`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Statement Imports` (`prompts/01-accounts-and-finance/04-bank-management/006-statement-imports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=bank-transfers` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/bank-transfers.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/bank-transfers.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_bank_transfers_panel.dart`
