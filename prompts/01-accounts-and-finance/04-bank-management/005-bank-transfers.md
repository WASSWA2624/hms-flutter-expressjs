...
2. **Register and scope the menu item.**
   - Implement the `Bank Transfers` leaf menu item at `Accounts & Finance → Bank Management → Bank Transfers` with canonical section slug `bank-transfers`.
...
- **AC2 (R2):** `/accounts?section=bank-transfers` opens the correct authorized menu item, restores state, and omits it when access is denied.
