...
2. **Register and scope the menu item.**
   - Implement the `Bank Deposits` leaf menu item at `Accounts & Finance → Bank Management → Bank Deposits` with canonical section slug `bank-deposits`.
...
- **AC2 (R2):** `/accounts?section=bank-deposits` opens the correct authorized menu item, restores state, and omits it when access is denied.
