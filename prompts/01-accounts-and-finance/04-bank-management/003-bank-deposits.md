# Prompt 003 — Implement Bank Deposits

## Context

- **Sequence:** 003 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 034 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Bank Transactions` (`prompts/01-accounts-and-finance/04-bank-management/002-bank-transactions.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Bank Deposits`
- **Canonical route:** `/accounts?section=bank-deposits`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/bank-deposits.md`
...
2. **Register and scope the tab.**
   - Implement the `Bank Deposits` tab at `Accounts & Finance → Bank Management → Bank Deposits` with canonical section slug `bank-deposits`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/bank-deposits.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/bank-deposits.md`:
     `Deposit No.`, `Deposit Date`, `Bank Account`, `Source Cash Point(s)`, `Cashier Session(s)`, `Deposit Slip No.`, `Receipt Count`, `Cash Amount`, `Cheque Amount`, `Total Deposit`, `Currency`, `Deposited By`, `Confirmed Date`, `Bank Reference`, `Match Status`, `Deposit Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Record deposit:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Links deposit to cashier sessions.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Deposit Date`, `Bank Account`, `Source Cash Point(s)`, `Cashier Session(s)`, `Deposit Slip No.`, `Cash Amount`, `Cheque Amount`, `Currency`, `Deposited By`, `Confirmed Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Withdrawals & Charges` (`prompts/01-accounts-and-finance/04-bank-management/004-withdrawals-and-charges.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=bank-deposits` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/bank-deposits.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/bank-deposits.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_bank_deposits_panel.dart`
