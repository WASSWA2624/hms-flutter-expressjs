...
2. **Register and scope the menu item.**
   - Implement the `Statement Imports` leaf menu item at `Accounts & Finance → Bank Management → Statement Imports` with canonical section slug `statement-imports`.
...
- **AC2 (R2):** `/accounts?section=statement-imports` opens the correct authorized menu item, restores state, and omits it when access is denied.
