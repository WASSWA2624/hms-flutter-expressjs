# Prompt 006 — Implement Statement Imports

## Context

- **Sequence:** 006 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 037 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Bank Transfers` (`prompts/01-accounts-and-finance/04-bank-management/005-bank-transfers.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Statement Imports`
- **Canonical route:** `/accounts?section=statement-imports`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/statement-imports.md`
...
2. **Register and scope the tab.**
   - Implement the `Statement Imports` tab at `Accounts & Finance → Bank Management → Statement Imports` with canonical section slug `statement-imports`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/statement-imports.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/statement-imports.md`:
     `Import No.`, `Bank Account`, `Statement From`, `Statement To`, `File Name`, `File Format`, `Imported At`, `Imported By`, `Opening Balance`, `Closing Balance`, `Row Count`, `Duplicate Rows`, `Invalid Rows`, `Matched Rows`, `Unmatched Rows`, `Import Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **Import statement:** gate `accounts:write` ∩ `facility-accounts`; enable when supported bank format; Opens file validation wizard.
     - **Save view:** gate `accounts:read` ∩ `facility-accounts`; enable when filters changed; Stores a named personal view.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Matching Workbench` (`prompts/01-accounts-and-finance/04-bank-management/007-matching-workbench.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=statement-imports` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/statement-imports.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/statement-imports.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_statement_imports_panel.dart`
