# Prompt 008 — Implement Bank Reconciliations

## Context

- **Sequence:** 008 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 039 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Matching Workbench` (`prompts/01-accounts-and-finance/04-bank-management/007-matching-workbench.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Bank Reconciliations`
- **Canonical route:** `/accounts?section=bank-reconciliations`
- **Surface profile:** `reconciliation` table/worklist tab
- **Navigation model:** `Accounts & Finance → Bank Management` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/bank-reconciliations.md`
...
2. **Register and scope the tab.**
   - Implement the `Bank Reconciliations` tab at `Accounts & Finance → Bank Management → Bank Reconciliations` with canonical section slug `bank-reconciliations`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Bank Management` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/bank-reconciliations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/bank-reconciliations.md`:
     `Reconciliation No.`, `Bank Account`, `Period From`, `Period To`, `Statement Opening Balance`, `Statement Closing Balance`, `Book Balance`, `Deposits in Transit`, `Unpresented Payments`, `Bank-only Items`, `Adjusted Balance`, `Difference`, `Currency`, `Prepared By`, `Reviewed By`, `Reconciliation Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Currency`, `Amount range`, `Owner / assigned user`.
   ...
   - Implement these context-specific toolbar buttons:
     - **Start work:** gate `accounts:write` ∩ `facility-accounts`; enable when eligible period/source selected; Creates a draft work item.
     - **Auto-match:** gate `accounts:write` ∩ `facility-accounts`; enable when unmatched rows exist; Applies deterministic matching rules and shows exceptions.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Open workbench:** gate `accounts:write` ∩ `facility-accounts`; enable when record not finalized; Opens matches, exceptions, evidence, and notes.
     - **Resolve exception:** gate `accounts:write` ∩ `facility-accounts`; enable when exception selected; Captures resolution and supporting reference.
     - **Submit / approve:** gate `accounts:write` ∩ `financial:approve` ∩ `facility-accounts`; enable when all blocking exceptions resolved; Completes maker-checker review.
     - **Finalize / reopen:** gate `accounts:write` ∩ `financial:approve` ∩ `facility-accounts`; enable when transition permitted; Finalizes or creates an audited reopen request.
   - Bulk buttons:
     - **Assign selected:** gate `accounts:write` ∩ `facility-accounts`; Assigns compatible exceptions to one owner.
     - **Resolve selected:** gate `accounts:write` ∩ `facility-accounts`; Requires one resolution code and note.
     - **Export selected:** gate `accounts:read` ∩ `evidence:export` ∩ `facility-accounts`; Includes reconciliation references and evidence counts.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Bank Account`, `Period From`, `Period To`, `Deposits in Transit`, `Unpresented Payments`, `Bank-only Items`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Purchases & Payables → Suppliers` (`prompts/01-accounts-and-finance/05-purchases-and-payables/001-suppliers.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=bank-reconciliations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/bank-reconciliations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/bank-reconciliations.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_bank_reconciliations_panel.dart`
