...
- **Surface profile:** `reconciliation` table/worklist opened from a nested menu item
...
2. **Register and scope the menu item.**
   - Implement the `Bank Reconciliations` leaf menu item at `Accounts & Finance → Bank Management → Bank Reconciliations` with canonical section slug `bank-reconciliations`.
...
- **AC2 (R2):** `/accounts?section=bank-reconciliations` opens the correct authorized menu item, restores state, and omits it when access is denied.
