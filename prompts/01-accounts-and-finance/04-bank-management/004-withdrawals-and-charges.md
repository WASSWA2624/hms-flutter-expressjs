...
2. **Register and scope the menu item.**
   - Implement the `Withdrawals & Charges` leaf menu item at `Accounts & Finance → Bank Management → Withdrawals & Charges` with canonical section slug `withdrawals-and-charges`.
...
- **AC2 (R2):** `/accounts?section=withdrawals-and-charges` opens the correct authorized menu item, restores state, and omits it when access is denied.
