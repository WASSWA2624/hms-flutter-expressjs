# Prompt 004 — Implement Withdrawals & Charges

## Context

- **Sequence:** 004 of 008 in `prompts/01-accounts-and-finance/04-bank-management/`
- **Overall step:** 035 of 158
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** `Accounts & Finance → Bank Management → Bank Deposits` (`prompts/01-accounts-and-finance/04-bank-management/003-bank-deposits.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Withdrawals & Charges`
- **Canonical route:** `/accounts?section=withdrawals-and-charges`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/bank-management/withdrawals-and-charges.md`
...
2. **Register and scope the tab.**
   - Implement the `Withdrawals & Charges` tab at `Accounts & Finance → Bank Management → Withdrawals & Charges` with canonical section slug `withdrawals-and-charges`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/bank-management/withdrawals-and-charges.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/bank-management/withdrawals-and-charges.md`:
     `Transaction No.`, `Value Date`, `Bank Account`, `Type`, `Payee/Bank`, `Description`, `External Reference`, `Gross Amount`, `Tax`, `Net Amount`, `Currency`, `GL Account`, `Journal Entry No.`, `Approval Status`, `Reconciliation Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Add charge:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens charge mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Value Date`, `Bank Account`, `Type`, `Payee/Bank`, `Description`, `External Reference`, `Gross Amount`, `Net Amount`, `Currency`, `GL Account`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Bank Management → Bank Transfers` (`prompts/01-accounts-and-finance/04-bank-management/005-bank-transfers.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=withdrawals-and-charges` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/withdrawals-and-charges.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/bank-management/withdrawals-and-charges.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_withdrawals_and_charges_panel.dart`
