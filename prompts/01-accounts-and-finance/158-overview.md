# Prompt 158 — Implement Overview

## Context

- **Sequence:** 158 of 158
- **Phase:** Accounts & Finance overview and final integration
- **Prerequisite:** Prompt 157 — Data Exports (`prompts/01-accounts-and-finance/12-audit-trail/157-data-exports.md`).
- **Menu path:** `Accounts & Finance → Overview`
- **Canonical route:** `/accounts?section=overview`
- **Surface profile:** `overview` table/worklist opened from a nested menu item
...
2. **Register and scope the menu item.**
   - Implement the `Overview` leaf menu item at `Accounts & Finance → Overview` with canonical section slug `overview`.
...
- **AC2 (R2):** `/accounts?section=overview` opens the correct authorized menu item, restores state, and omits it when access is denied.
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_overview_panel.dart`
