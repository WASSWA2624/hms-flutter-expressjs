...
2. **Register and scope the menu item.**
   - Implement the `Audit Schedules` leaf menu item at `Accounts & Finance → Tax & Compliance → Audit Schedules` with canonical section slug `audit-schedules`.
...
- **AC2 (R2):** `/accounts?section=audit-schedules` opens the correct authorized menu item, restores state, and omits it when access is denied.
