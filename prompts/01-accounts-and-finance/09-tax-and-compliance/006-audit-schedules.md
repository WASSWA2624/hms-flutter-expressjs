# Prompt 006 — Implement Audit Schedules

## Context

- **Sequence:** 006 of 006 in `prompts/01-accounts-and-finance/09-tax-and-compliance/`
- **Overall step:** 078 of 158
- **Phase:** Accounts operations — tax and compliance
- **Prerequisite:** `Accounts & Finance → Tax & Compliance → Regulatory Reports` (`prompts/01-accounts-and-finance/09-tax-and-compliance/005-regulatory-reports.md`).
- **Menu path:** `Accounts & Finance → Tax & Compliance → Audit Schedules`
- **Canonical route:** `/accounts?section=audit-schedules`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/tax-and-compliance/audit-schedules.md`
...
2. **Register and scope the tab.**
   - Implement the `Audit Schedules` tab at `Accounts & Finance → Tax & Compliance → Audit Schedules` with canonical section slug `audit-schedules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/tax-and-compliance/audit-schedules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/tax-and-compliance/audit-schedules.md`:
     `Schedule No.`, `Schedule Name`, `Fiscal Period`, `Account/Area`, `Prepared Balance`, `Supporting Balance`, `Difference`, `Currency`, `Attachment Count`, `Prepared By`, `Reviewed By`, `Review Notes`, `Prepared At`, `Reviewed At`, `Schedule Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Audit Schedules mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Schedule Name`, `Fiscal Period`, `Account/Area`, `Currency`, `Review Notes`, `Prepared At`, `Reviewed At`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Period Close → Close Checklist` (`prompts/01-accounts-and-finance/10-period-close/001-close-checklist.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=audit-schedules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/tax-and-compliance/audit-schedules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/tax-and-compliance/audit-schedules.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_audit_schedules_panel.dart`
