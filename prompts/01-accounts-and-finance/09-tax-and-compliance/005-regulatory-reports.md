...
2. **Register and scope the menu item.**
   - Implement the `Regulatory Reports` leaf menu item at `Accounts & Finance → Tax & Compliance → Regulatory Reports` with canonical section slug `regulatory-reports`.
...
- **AC2 (R2):** `/accounts?section=regulatory-reports` opens the correct authorized menu item, restores state, and omits it when access is denied.
