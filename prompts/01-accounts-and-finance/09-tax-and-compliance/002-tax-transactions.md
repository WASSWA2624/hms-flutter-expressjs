...
2. **Register and scope the menu item.**
   - Implement the `Tax Transactions` leaf menu item at `Accounts & Finance → Tax & Compliance → Tax Transactions` with canonical section slug `tax-transactions`.
...
- **AC2 (R2):** `/accounts?section=tax-transactions` opens the correct authorized menu item, restores state, and omits it when access is denied.
