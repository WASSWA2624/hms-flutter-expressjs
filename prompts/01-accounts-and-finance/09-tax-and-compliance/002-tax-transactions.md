# Prompt 002 — Implement Tax Transactions

## Context

- **Sequence:** 002 of 006 in `prompts/01-accounts-and-finance/09-tax-and-compliance/`
- **Overall step:** 074 of 158
- **Phase:** Accounts operations — tax and compliance
- **Prerequisite:** `Accounts & Finance → Tax & Compliance → Tax Codes & Rates` (`prompts/01-accounts-and-finance/09-tax-and-compliance/001-tax-codes-and-rates.md`).
- **Menu path:** `Accounts & Finance → Tax & Compliance → Tax Transactions`
- **Canonical route:** `/accounts?section=tax-transactions`
- **Surface profile:** `analysis` table/worklist tab
- **Navigation model:** `Accounts & Finance → Tax & Compliance` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-transactions.md`
...
2. **Register and scope the tab.**
   - Implement the `Tax Transactions` tab at `Accounts & Finance → Tax & Compliance → Tax Transactions` with canonical section slug `tax-transactions`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Tax & Compliance` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-transactions.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-transactions.md`:
     `Tax Date`, `Tax Period`, `Tax Code`, `Transaction Type`, `Source Document`, `Document Reference`, `Party`, `Taxable Amount`, `Tax Rate`, `Input Tax`, `Output Tax`, `Withheld Tax`, `Currency`, `GL Account`, `Filing Status`, `Journal Entry No.`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Tax & Compliance → Withholding Tax` (`prompts/01-accounts-and-finance/09-tax-and-compliance/003-withholding-tax.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=tax-transactions` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-transactions.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-transactions.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_tax_transactions_panel.dart`
