# Prompt 004 — Implement Tax Returns

## Context

- **Sequence:** 004 of 006 in `prompts/01-accounts-and-finance/09-tax-and-compliance/`
- **Overall step:** 076 of 158
- **Phase:** Accounts operations — tax and compliance
- **Prerequisite:** `Accounts & Finance → Tax & Compliance → Withholding Tax` (`prompts/01-accounts-and-finance/09-tax-and-compliance/003-withholding-tax.md`).
- **Menu path:** `Accounts & Finance → Tax & Compliance → Tax Returns`
- **Canonical route:** `/accounts?section=tax-returns`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-returns.md`
...
2. **Register and scope the tab.**
   - Implement the `Tax Returns` tab at `Accounts & Finance → Tax & Compliance → Tax Returns` with canonical section slug `tax-returns`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-returns.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-returns.md`:
     `Return No.`, `Tax Type`, `Filing Period`, `Due Date`, `Taxable Sales/Purchases`, `Input Tax`, `Output Tax`, `Tax Withheld`, `Adjustments`, `Tax Payable/Refundable`, `Currency`, `Submitted Date`, `Submission Reference`, `Payment Reference`, `Prepared By`, `Approved By`, `Return Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Prepare return:** gate `accounts:write` ∩ `facility-accounts`; enable when tax period open; Builds a return from posted tax transactions.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Tax Type`, `Filing Period`, `Due Date`, `Taxable Sales/Purchases`, `Input Tax`, `Output Tax`, `Tax Withheld`, `Adjustments`, `Tax Payable/Refundable`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Tax & Compliance → Regulatory Reports` (`prompts/01-accounts-and-finance/09-tax-and-compliance/005-regulatory-reports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=tax-returns` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-returns.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-returns.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_tax_returns_panel.dart`
