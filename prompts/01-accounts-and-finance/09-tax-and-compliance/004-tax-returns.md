...
2. **Register and scope the menu item.**
   - Implement the `Tax Returns` leaf menu item at `Accounts & Finance → Tax & Compliance → Tax Returns` with canonical section slug `tax-returns`.
...
- **AC2 (R2):** `/accounts?section=tax-returns` opens the correct authorized menu item, restores state, and omits it when access is denied.
