# Prompt 003 — Implement Withholding Tax

## Context

- **Sequence:** 003 of 006 in `prompts/01-accounts-and-finance/09-tax-and-compliance/`
- **Overall step:** 075 of 158
- **Phase:** Accounts operations — tax and compliance
- **Prerequisite:** `Accounts & Finance → Tax & Compliance → Tax Transactions` (`prompts/01-accounts-and-finance/09-tax-and-compliance/002-tax-transactions.md`).
- **Menu path:** `Accounts & Finance → Tax & Compliance → Withholding Tax`
- **Canonical route:** `/accounts?section=withholding-tax`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/tax-and-compliance/withholding-tax.md`
...
2. **Register and scope the tab.**
   - Implement the `Withholding Tax` tab at `Accounts & Finance → Tax & Compliance → Withholding Tax` with canonical section slug `withholding-tax`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/tax-and-compliance/withholding-tax.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/tax-and-compliance/withholding-tax.md`:
     `Certificate No.`, `Transaction Date`, `Supplier/Payee`, `Tax ID`, `Source Invoice/Payment`, `Withholding Code`, `Gross Amount`, `Rate`, `Withheld Amount`, `Net Paid`, `Currency`, `Certificate Date`, `Filing Period`, `Remittance Reference`, `Filing Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Withholding Tax mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Transaction Date`, `Supplier/Payee`, `Tax ID`, `Source Invoice/Payment`, `Withholding Code`, `Gross Amount`, `Rate`, `Withheld Amount`, `Net Paid`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Tax & Compliance → Tax Returns` (`prompts/01-accounts-and-finance/09-tax-and-compliance/004-tax-returns.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=withholding-tax` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/tax-and-compliance/withholding-tax.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/tax-and-compliance/withholding-tax.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_withholding_tax_panel.dart`
