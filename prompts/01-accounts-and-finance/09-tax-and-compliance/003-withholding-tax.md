...
2. **Register and scope the menu item.**
   - Implement the `Withholding Tax` leaf menu item at `Accounts & Finance → Tax & Compliance → Withholding Tax` with canonical section slug `withholding-tax`.
...
- **AC2 (R2):** `/accounts?section=withholding-tax` opens the correct authorized menu item, restores state, and omits it when access is denied.
