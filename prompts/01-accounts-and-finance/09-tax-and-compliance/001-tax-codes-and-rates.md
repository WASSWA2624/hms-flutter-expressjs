...
2. **Register and scope the menu item.**
   - Implement the `Tax Codes & Rates` leaf menu item at `Accounts & Finance → Tax & Compliance → Tax Codes & Rates` with canonical section slug `tax-codes-and-rates`.
...
- **AC2 (R2):** `/accounts?section=tax-codes-and-rates` opens the correct authorized menu item, restores state, and omits it when access is denied.
