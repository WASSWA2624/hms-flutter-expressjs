# Prompt 001 — Implement Tax Codes & Rates

## Context

- **Sequence:** 001 of 006 in `prompts/01-accounts-and-finance/09-tax-and-compliance/`
- **Overall step:** 073 of 158
- **Phase:** Accounts operations — tax and compliance
- **Prerequisite:** `Accounts & Finance → Budgets & Cost Control → Variance Analysis` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/007-variance-analysis.md`).
- **Menu path:** `Accounts & Finance → Tax & Compliance → Tax Codes & Rates`
- **Canonical route:** `/accounts?section=tax-codes-and-rates`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-codes-and-rates.md`
...
2. **Register and scope the tab.**
   - Implement the `Tax Codes & Rates` tab at `Accounts & Finance → Tax & Compliance → Tax Codes & Rates` with canonical section slug `tax-codes-and-rates`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-codes-and-rates.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-codes-and-rates.md`:
     `Tax Code`, `Tax Name`, `Tax Type`, `Jurisdiction`, `Rate`, `Inclusive/Exclusive`, `Recoverable Percentage`, `Input Tax Account`, `Output Tax Account`, `Payable Account`, `Effective From`, `Effective To`, `Filing Category`, `Tax Authority`, `Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Tax Codes & Rates create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Tax Code`, `Tax Name`, `Tax Type`, `Jurisdiction`, `Rate`, `Inclusive/Exclusive`, `Recoverable Percentage`, `Input Tax Account`, `Output Tax Account`, `Payable Account`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Tax & Compliance → Tax Transactions` (`prompts/01-accounts-and-finance/09-tax-and-compliance/002-tax-transactions.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=tax-codes-and-rates` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-codes-and-rates.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/tax-and-compliance/tax-codes-and-rates.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_tax_codes_and_rates_panel.dart`
