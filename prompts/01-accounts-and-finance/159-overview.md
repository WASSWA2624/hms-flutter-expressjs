# Prompt 159 — Implement Overview

## Context

- **Sequence:** 159 of 159
- **Phase:** Accounts & Finance overview and final integration
- **Prerequisite:** Prompt 158 — Data Exports (`prompts/01-finance-tabs/01-accounts-and-finance/12-audit-trail/158-data-exports.md`).
...
5. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Open queue:** gate `accounts:read` ∩ `facility-accounts`; enable when metric has a destination; Opens the matching filtered tab.
   - Bulk buttons:
...
- **AC1 (R1):** `/accounts?section=overview` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC2 (R2):** All API operations in `.cursor/finance/accounts-and-finance/overview.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `frontend/lib/features/accounts/presentation/widgets/overview_tab.dart`
