...
2. **Register and scope the menu item.**
   - Implement the `Opening Balances` leaf menu item at `Accounts & Finance → Setup & Controls → Opening Balances` with canonical section slug `opening-balances`.
...
- **AC2 (R2):** `/accounts?section=opening-balances` opens the correct authorized menu item, restores state, and omits it when access is denied.
