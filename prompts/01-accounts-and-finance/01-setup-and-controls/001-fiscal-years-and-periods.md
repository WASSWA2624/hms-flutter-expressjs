...
2. **Register and scope the menu item.**
   - Implement the `Fiscal Years & Periods` leaf menu item at `Accounts & Finance → Setup & Controls → Fiscal Years & Periods` with canonical section slug `fiscal-years-and-periods`.
...
- **AC2 (R2):** `/accounts?section=fiscal-years-and-periods` opens the correct authorized menu item, restores state, and omits it when access is denied.
