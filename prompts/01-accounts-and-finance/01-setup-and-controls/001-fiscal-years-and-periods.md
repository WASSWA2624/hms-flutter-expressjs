# Prompt 001 — Implement Fiscal Years & Periods

## Context

- **Sequence:** 001 of 008 in `prompts/01-accounts-and-finance/01-setup-and-controls/`
- **Overall step:** 001 of 158
- **Phase:** Accounts foundation — setup and controls
- **Prerequisite:** None. This is the first finance implementation prompt.
- **Menu path:** `Accounts & Finance → Setup & Controls → Fiscal Years & Periods`
- **Canonical route:** `/accounts?section=fiscal-years-and-periods`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/setup-and-controls/fiscal-years-and-periods.md`
...
2. **Register and scope the tab.**
   - Implement the `Fiscal Years & Periods` tab at `Accounts & Finance → Setup & Controls → Fiscal Years & Periods` with canonical section slug `fiscal-years-and-periods`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/setup-and-controls/fiscal-years-and-periods.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/setup-and-controls/fiscal-years-and-periods.md`:
     `Fiscal Year`, `Period No.`, `Period Name`, `Start Date`, `End Date`, `Entity/Facility`, `Module`, `Open Date`, `Soft-close Date`, `Close Date`, `Lock Date`, `Reopened At`, `Reopened By`, `Period Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Fiscal Years & Periods create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Fiscal Year`, `Period No.`, `Period Name`, `Start Date`, `End Date`, `Entity/Facility`, `Module`, `Open Date`, `Soft-close Date`, `Close Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Setup & Controls → Departments & Cost Centres` (`prompts/01-accounts-and-finance/01-setup-and-controls/002-departments-and-cost-centres.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=fiscal-years-and-periods` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/setup-and-controls/fiscal-years-and-periods.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_fiscal_years_and_periods_panel.dart`
