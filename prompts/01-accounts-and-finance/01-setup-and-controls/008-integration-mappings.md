# Prompt 008 — Implement Integration Mappings

## Context

- **Sequence:** 008 of 008 in `prompts/01-accounts-and-finance/01-setup-and-controls/`
- **Overall step:** 008 of 158
- **Phase:** Accounts foundation — setup and controls
- **Prerequisite:** `Accounts & Finance → Setup & Controls → Opening Balances` (`prompts/01-accounts-and-finance/01-setup-and-controls/007-opening-balances.md`).
- **Menu path:** `Accounts & Finance → Setup & Controls → Integration Mappings`
- **Canonical route:** `/accounts?section=integration-mappings`
- **Surface profile:** `master` table/worklist tab
- **Navigation model:** `Accounts & Finance → Setup & Controls` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/setup-and-controls/integration-mappings.md`
...
2. **Register and scope the tab.**
   - Implement the `Integration Mappings` tab at `Accounts & Finance → Setup & Controls → Integration Mappings` with canonical section slug `integration-mappings`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/setup-and-controls/integration-mappings.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/setup-and-controls/integration-mappings.md`:
     `Mapping Code`, `Integration/System`, `Source Entity`, `Source Event`, `Source Value`, `Target Entity`, `Target Value/Account`, `Facility Scope`, `Direction`, `Transformation Rule`, `Last Synced At`, `Error Count`, `Version`, `Mapping Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Integration Mappings create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Mapping Code`, `Integration/System`, `Source Entity`, `Source Event`, `Source Value`, `Target Entity`, `Target Value/Account`, `Facility Scope`, `Direction`, `Transformation Rule`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → General Accounting → Chart of Accounts` (`prompts/01-accounts-and-finance/02-general-accounting/001-chart-of-accounts.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=integration-mappings` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/setup-and-controls/integration-mappings.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/setup-and-controls/integration-mappings.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_integration_mappings_panel.dart`
