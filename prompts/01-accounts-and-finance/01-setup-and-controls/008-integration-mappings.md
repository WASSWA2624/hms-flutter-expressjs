...
2. **Register and scope the menu item.**
   - Implement the `Integration Mappings` leaf menu item at `Accounts & Finance → Setup & Controls → Integration Mappings` with canonical section slug `integration-mappings`.
...
- **AC2 (R2):** `/accounts?section=integration-mappings` opens the correct authorized menu item, restores state, and omits it when access is denied.
