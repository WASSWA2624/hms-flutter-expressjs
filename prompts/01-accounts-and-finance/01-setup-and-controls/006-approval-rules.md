# Prompt 006 — Implement Approval Rules

## Context

- **Sequence:** 006 of 008 in `prompts/01-accounts-and-finance/01-setup-and-controls/`
- **Overall step:** 006 of 158
- **Phase:** Accounts foundation — setup and controls
- **Prerequisite:** `Accounts & Finance → Setup & Controls → Posting Rules` (`prompts/01-accounts-and-finance/01-setup-and-controls/005-posting-rules.md`).
- **Menu path:** `Accounts & Finance → Setup & Controls → Approval Rules`
- **Canonical route:** `/accounts?section=approval-rules`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/setup-and-controls/approval-rules.md`
...
2. **Register and scope the tab.**
   - Implement the `Approval Rules` tab at `Accounts & Finance → Setup & Controls → Approval Rules` with canonical section slug `approval-rules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/setup-and-controls/approval-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/setup-and-controls/approval-rules.md`:
     `Rule Code`, `Rule Name`, `Module`, `Document Type`, `Facility/Department Scope`, `Minimum Amount`, `Maximum Amount`, `Currency`, `Approval Levels`, `Approver Role(s)`, `Escalation Time`, `Segregation Rule`, `Effective From`, `Effective To`, `Rule Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Facility`, `Department / cost centre`, `Currency`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Approval Rules create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Rule Code`, `Rule Name`, `Module`, `Document Type`, `Facility/Department Scope`, `Minimum Amount`, `Maximum Amount`, `Currency`, `Approval Levels`, `Approver Role(s)`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Setup & Controls → Opening Balances` (`prompts/01-accounts-and-finance/01-setup-and-controls/007-opening-balances.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=approval-rules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/setup-and-controls/approval-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/setup-and-controls/approval-rules.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_approval_rules_panel.dart`
