...
2. **Register and scope the menu item.**
   - Implement the `Approval Rules` leaf menu item at `Accounts & Finance → Setup & Controls → Approval Rules` with canonical section slug `approval-rules`.
...
- **AC2 (R2):** `/accounts?section=approval-rules` opens the correct authorized menu item, restores state, and omits it when access is denied.
