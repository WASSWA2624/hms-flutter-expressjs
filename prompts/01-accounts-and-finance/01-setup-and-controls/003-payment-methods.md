...
2. **Register and scope the menu item.**
   - Implement the `Payment Methods` leaf menu item at `Accounts & Finance → Setup & Controls → Payment Methods` with canonical section slug `payment-methods`.
...
- **AC2 (R2):** `/accounts?section=payment-methods` opens the correct authorized menu item, restores state, and omits it when access is denied.
