...
2. **Register and scope the menu item.**
   - Implement the `Document Numbering` leaf menu item at `Accounts & Finance → Setup & Controls → Document Numbering` with canonical section slug `document-numbering`.
...
- **AC2 (R2):** `/accounts?section=document-numbering` opens the correct authorized menu item, restores state, and omits it when access is denied.
