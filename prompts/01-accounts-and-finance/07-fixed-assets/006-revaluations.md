# Prompt 006 — Implement Revaluations

## Context

- **Sequence:** 006 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 063 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Asset Transfers` (`prompts/01-accounts-and-finance/07-fixed-assets/005-asset-transfers.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Revaluations`
- **Canonical route:** `/accounts?section=revaluations`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/revaluations.md`
...
2. **Register and scope the tab.**
   - Implement the `Revaluations` tab at `Accounts & Finance → Fixed Assets → Revaluations` with canonical section slug `revaluations`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/revaluations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/revaluations.md`:
     `Revaluation No.`, `Revaluation Date`, `Asset No.`, `Asset Name`, `Previous Cost`, `Previous Net Book Value`, `Revalued Amount`, `Increase/Decrease`, `Currency`, `Valuer`, `Valuation Reference`, `Reason`, `Approval Status`, `Journal Entry No.`, `Revaluation Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Revaluations mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Revaluation Date`, `Asset No.`, `Asset Name`, `Previous Cost`, `Revalued Amount`, `Increase/Decrease`, `Currency`, `Valuer`, `Valuation Reference`, `Reason`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Disposals` (`prompts/01-accounts-and-finance/07-fixed-assets/007-disposals.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=revaluations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/revaluations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/revaluations.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_revaluations_panel.dart`
