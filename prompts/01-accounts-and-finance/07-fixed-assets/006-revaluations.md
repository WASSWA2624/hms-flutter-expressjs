...
2. **Register and scope the menu item.**
   - Implement the `Revaluations` leaf menu item at `Accounts & Finance → Fixed Assets → Revaluations` with canonical section slug `revaluations`.
...
- **AC2 (R2):** `/accounts?section=revaluations` opens the correct authorized menu item, restores state, and omits it when access is denied.
