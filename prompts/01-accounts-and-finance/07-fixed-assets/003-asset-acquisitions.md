# Prompt 003 — Implement Asset Acquisitions

## Context

- **Sequence:** 003 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 060 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Asset Categories` (`prompts/01-accounts-and-finance/07-fixed-assets/002-asset-categories.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Asset Acquisitions`
- **Canonical route:** `/accounts?section=asset-acquisitions`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/asset-acquisitions.md`
...
2. **Register and scope the tab.**
   - Implement the `Asset Acquisitions` tab at `Accounts & Finance → Fixed Assets → Asset Acquisitions` with canonical section slug `asset-acquisitions`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/asset-acquisitions.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/asset-acquisitions.md`:
     `Acquisition No.`, `Acquisition Date`, `Asset No./Count`, `Supplier`, `PO No.`, `Supplier Invoice No.`, `Department`, `Location`, `Gross Cost`, `Tax`, `Capitalized Cost`, `Currency`, `In-service Date`, `Approval Status`, `Journal Entry No.`, `Acquisition Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Asset Acquisitions mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Acquisition Date`, `Supplier`, `PO No.`, `Supplier Invoice No.`, `Department`, `Location`, `Gross Cost`, `Capitalized Cost`, `Currency`, `In-service Date`.
   - Detail sections: `Asset summary`, `Valuation history`, `Transfers & documents`, `Posting & audit`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Depreciation Runs` (`prompts/01-accounts-and-finance/07-fixed-assets/004-depreciation-runs.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=asset-acquisitions` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/asset-acquisitions.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/asset-acquisitions.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_asset_acquisitions_panel.dart`
