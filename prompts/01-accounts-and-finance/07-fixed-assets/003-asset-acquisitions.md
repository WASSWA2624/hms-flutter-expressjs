...
2. **Register and scope the menu item.**
   - Implement the `Asset Acquisitions` leaf menu item at `Accounts & Finance → Fixed Assets → Asset Acquisitions` with canonical section slug `asset-acquisitions`.
...
- **AC2 (R2):** `/accounts?section=asset-acquisitions` opens the correct authorized menu item, restores state, and omits it when access is denied.
