# Prompt 005 — Implement Asset Transfers

## Context

- **Sequence:** 005 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 062 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Depreciation Runs` (`prompts/01-accounts-and-finance/07-fixed-assets/004-depreciation-runs.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Asset Transfers`
- **Canonical route:** `/accounts?section=asset-transfers`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/asset-transfers.md`
...
2. **Register and scope the tab.**
   - Implement the `Asset Transfers` tab at `Accounts & Finance → Fixed Assets → Asset Transfers` with canonical section slug `asset-transfers`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/asset-transfers.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/asset-transfers.md`:
     `Transfer No.`, `Transfer Date`, `Asset No.`, `Asset Name`, `From Facility`, `From Location`, `From Department/Custodian`, `To Facility`, `To Location`, `To Department/Custodian`, `Reason`, `Requested By`, `Approved By`, `Received By`, `Transfer Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Facility`, `Department / cost centre`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Asset Transfers mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Transfer Date`, `Asset No.`, `Asset Name`, `From Facility`, `From Location`, `From Department/Custodian`, `To Facility`, `To Location`, `To Department/Custodian`, `Reason`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Revaluations` (`prompts/01-accounts-and-finance/07-fixed-assets/006-revaluations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=asset-transfers` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/asset-transfers.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/asset-transfers.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_asset_transfers_panel.dart`
