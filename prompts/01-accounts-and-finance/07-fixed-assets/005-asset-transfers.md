...
2. **Register and scope the menu item.**
   - Implement the `Asset Transfers` leaf menu item at `Accounts & Finance → Fixed Assets → Asset Transfers` with canonical section slug `asset-transfers`.
...
- **AC2 (R2):** `/accounts?section=asset-transfers` opens the correct authorized menu item, restores state, and omits it when access is denied.
