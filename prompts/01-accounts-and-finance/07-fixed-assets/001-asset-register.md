...
2. **Register and scope the menu item.**
   - Implement the `Asset Register` leaf menu item at `Accounts & Finance → Fixed Assets → Asset Register` with canonical section slug `asset-register`.
...
- **AC2 (R2):** `/accounts?section=asset-register` opens the correct authorized menu item, restores state, and omits it when access is denied.
