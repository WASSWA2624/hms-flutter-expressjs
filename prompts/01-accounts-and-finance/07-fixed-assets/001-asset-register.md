# Prompt 001 — Implement Asset Register

## Context

- **Sequence:** 001 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 058 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Expenses → Expense Analysis` (`prompts/01-accounts-and-finance/06-expenses/007-expense-analysis.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Asset Register`
- **Canonical route:** `/accounts?section=asset-register`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/asset-register.md`
...
2. **Register and scope the tab.**
   - Implement the `Asset Register` tab at `Accounts & Finance → Fixed Assets → Asset Register` with canonical section slug `asset-register`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/asset-register.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/asset-register.md`:
     `Asset No.`, `Asset Name`, `Asset Category`, `Serial No.`, `Tag No.`, `Acquisition Date`, `In-service Date`, `Supplier`, `Location`, `Department`, `Custodian`, `Acquisition Cost`, `Accumulated Depreciation`, `Net Book Value`, `Currency`, `Useful Life`, `Depreciation Method`, `Asset Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Asset Register create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Asset No.`, `Asset Name`, `Asset Category`, `Serial No.`, `Tag No.`, `Acquisition Date`, `In-service Date`, `Supplier`, `Location`, `Department`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Asset Categories` (`prompts/01-accounts-and-finance/07-fixed-assets/002-asset-categories.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=asset-register` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/asset-register.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/asset-register.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_asset_register_panel.dart`
