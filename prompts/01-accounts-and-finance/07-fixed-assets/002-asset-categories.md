# Prompt 002 — Implement Asset Categories

## Context

- **Sequence:** 002 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 059 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Asset Register` (`prompts/01-accounts-and-finance/07-fixed-assets/001-asset-register.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Asset Categories`
- **Canonical route:** `/accounts?section=asset-categories`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/asset-categories.md`
...
2. **Register and scope the tab.**
   - Implement the `Asset Categories` tab at `Accounts & Finance → Fixed Assets → Asset Categories` with canonical section slug `asset-categories`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/asset-categories.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/asset-categories.md`:
     `Category Code`, `Category Name`, `Parent Category`, `Asset GL Account`, `Accumulated Depreciation Account`, `Depreciation Expense Account`, `Disposal Gain/Loss Account`, `Default Useful Life`, `Depreciation Method`, `Residual Value Rule`, `Capitalization Threshold`, `Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens the Asset Categories create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Category Code`, `Category Name`, `Parent Category`, `Asset GL Account`, `Depreciation Expense Account`, `Disposal Gain/Loss Account`, `Default Useful Life`, `Depreciation Method`, `Residual Value Rule`, `Capitalization Threshold`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Asset Acquisitions` (`prompts/01-accounts-and-finance/07-fixed-assets/003-asset-acquisitions.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=asset-categories` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/asset-categories.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/asset-categories.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_asset_categories_panel.dart`
