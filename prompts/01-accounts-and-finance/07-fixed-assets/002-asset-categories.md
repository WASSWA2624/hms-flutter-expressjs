...
2. **Register and scope the menu item.**
   - Implement the `Asset Categories` leaf menu item at `Accounts & Finance → Fixed Assets → Asset Categories` with canonical section slug `asset-categories`.
...
- **AC2 (R2):** `/accounts?section=asset-categories` opens the correct authorized menu item, restores state, and omits it when access is denied.
