...
2. **Register and scope the menu item.**
   - Implement the `Depreciation Runs` leaf menu item at `Accounts & Finance → Fixed Assets → Depreciation Runs` with canonical section slug `depreciation-runs`.
...
- **AC2 (R2):** `/accounts?section=depreciation-runs` opens the correct authorized menu item, restores state, and omits it when access is denied.
