# Prompt 004 — Implement Depreciation Runs

## Context

- **Sequence:** 004 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 061 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Asset Acquisitions` (`prompts/01-accounts-and-finance/07-fixed-assets/003-asset-acquisitions.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Depreciation Runs`
- **Canonical route:** `/accounts?section=depreciation-runs`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/depreciation-runs.md`
...
2. **Register and scope the tab.**
   - Implement the `Depreciation Runs` tab at `Accounts & Finance → Fixed Assets → Depreciation Runs` with canonical section slug `depreciation-runs`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/depreciation-runs.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/depreciation-runs.md`:
     `Run No.`, `Fiscal Period`, `Run Date`, `Asset Category`, `Asset Count`, `Opening Cost`, `Depreciable Base`, `Period Depreciation`, `Accumulated Depreciation`, `Net Book Value`, `Currency`, `Exception Count`, `Calculated By`, `Approved By`, `Journal Entry No.`, `Run Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Calculate depreciation:** gate `accounts:write` ∩ `facility-accounts`; enable when period open; Creates a preview run without posting.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Fiscal Period`, `Run Date`, `Asset Category`, `Opening Cost`, `Depreciable Base`, `Period Depreciation`, `Currency`, `Calculated By`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Asset Transfers` (`prompts/01-accounts-and-finance/07-fixed-assets/005-asset-transfers.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=depreciation-runs` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/depreciation-runs.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/depreciation-runs.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_depreciation_runs_panel.dart`
