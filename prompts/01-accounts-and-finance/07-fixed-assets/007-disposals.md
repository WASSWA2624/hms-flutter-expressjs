...
2. **Register and scope the menu item.**
   - Implement the `Disposals` leaf menu item at `Accounts & Finance → Fixed Assets → Disposals` with canonical section slug `disposals`.
...
- **AC2 (R2):** `/accounts?section=disposals` opens the correct authorized menu item, restores state, and omits it when access is denied.
