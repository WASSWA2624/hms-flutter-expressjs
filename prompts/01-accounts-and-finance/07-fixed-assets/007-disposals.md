# Prompt 007 — Implement Disposals

## Context

- **Sequence:** 007 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 064 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Revaluations` (`prompts/01-accounts-and-finance/07-fixed-assets/006-revaluations.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Disposals`
- **Canonical route:** `/accounts?section=disposals`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/disposals.md`
...
2. **Register and scope the tab.**
   - Implement the `Disposals` tab at `Accounts & Finance → Fixed Assets → Disposals` with canonical section slug `disposals`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/disposals.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/disposals.md`:
     `Disposal No.`, `Disposal Date`, `Asset No.`, `Asset Name`, `Disposal Method`, `Buyer/Recipient`, `Proceeds`, `Net Book Value`, `Gain/Loss`, `Currency`, `Reason`, `Approval Status`, `Receipt Reference`, `Journal Entry No.`, `Disposal Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Disposals mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Disposal Date`, `Asset No.`, `Asset Name`, `Disposal Method`, `Buyer/Recipient`, `Proceeds`, `Gain/Loss`, `Currency`, `Reason`, `Receipt Reference`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Asset Ledger` (`prompts/01-accounts-and-finance/07-fixed-assets/008-asset-ledger.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=disposals` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/disposals.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/disposals.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_disposals_panel.dart`
