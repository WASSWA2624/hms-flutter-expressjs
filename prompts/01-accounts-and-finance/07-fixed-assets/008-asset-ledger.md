# Prompt 008 — Implement Asset Ledger

## Context

- **Sequence:** 008 of 008 in `prompts/01-accounts-and-finance/07-fixed-assets/`
- **Overall step:** 065 of 158
- **Phase:** Accounts operations — fixed assets
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Disposals` (`prompts/01-accounts-and-finance/07-fixed-assets/007-disposals.md`).
- **Menu path:** `Accounts & Finance → Fixed Assets → Asset Ledger`
- **Canonical route:** `/accounts?section=asset-ledger`
- **Surface profile:** `analysis` table/worklist tab
- **Navigation model:** `Accounts & Finance → Fixed Assets` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/fixed-assets/asset-ledger.md`
...
2. **Register and scope the tab.**
   - Implement the `Asset Ledger` tab at `Accounts & Finance → Fixed Assets → Asset Ledger` with canonical section slug `asset-ledger`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Fixed Assets` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/fixed-assets/asset-ledger.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/fixed-assets/asset-ledger.md`:
     `Posting Date`, `Fiscal Period`, `Asset No.`, `Asset Name`, `Transaction Type`, `Document Reference`, `Description`, `Cost Debit`, `Cost Credit`, `Depreciation Debit`, `Depreciation Credit`, `Net Book Value`, `Currency`, `Journal Entry No.`, `Posting Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Budgets & Cost Control → Annual Budgets` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/001-annual-budgets.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=asset-ledger` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/fixed-assets/asset-ledger.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/fixed-assets/asset-ledger.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_asset_ledger_panel.dart`
