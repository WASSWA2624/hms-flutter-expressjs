...
2. **Register and scope the menu item.**
   - Implement the `Asset Ledger` leaf menu item at `Accounts & Finance → Fixed Assets → Asset Ledger` with canonical section slug `asset-ledger`.
...
- **AC2 (R2):** `/accounts?section=asset-ledger` opens the correct authorized menu item, restores state, and omits it when access is denied.
