...
- **Navigation model:** `Accounts & Finance → Fixed Assets` are the two sidebar menu levels; this surface is a tab in that menu item's flat tab strip
...
2. **Register and scope the tab.**
   ...
   - Add this tab to the flat `AppTabStrip` of the `Fixed Assets` sidebar menu item. The menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.
