...
2. **Register and scope the menu item.**
   - Implement the `Cashier & Bank Reports` leaf menu item at `Accounts & Finance → Financial Reports → Cashier & Bank Reports` with canonical section slug `cashier-and-bank-reports`.
...
- **AC2 (R2):** `/accounts?section=cashier-and-bank-reports` opens the correct authorized menu item, restores state, and omits it when access is denied.
