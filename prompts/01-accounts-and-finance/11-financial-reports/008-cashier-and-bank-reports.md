# Prompt 008 — Implement Cashier & Bank Reports

## Context

- **Sequence:** 008 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 092 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → Department Performance` (`prompts/01-accounts-and-finance/11-financial-reports/007-department-performance.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Cashier & Bank Reports`
- **Canonical route:** `/accounts?section=cashier-and-bank-reports`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/cashier-and-bank-reports.md`
...
2. **Register and scope the tab.**
   - Implement the `Cashier & Bank Reports` tab at `Accounts & Finance → Financial Reports → Cashier & Bank Reports` with canonical section slug `cashier-and-bank-reports`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/cashier-and-bank-reports.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/cashier-and-bank-reports.md`:
     `Report Date`, `Facility`, `Cash Point/Bank Account`, `Session/Statement Count`, `Opening Balance`, `Receipts`, `Payments`, `Refunds`, `Transfers`, `Deposits`, `Closing Balance`, `Variance`, `Unreconciled Amount`, `Currency`, `Report Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Facility`, `Currency`, `Amount range`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Consolidated Reports` (`prompts/01-accounts-and-finance/11-financial-reports/009-consolidated-reports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cashier-and-bank-reports` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/cashier-and-bank-reports.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/cashier-and-bank-reports.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cashier_and_bank_reports_panel.dart`
