...
2. **Register and scope the menu item.**
   - Implement the `Trial Balance` leaf menu item at `Accounts & Finance → Financial Reports → Trial Balance` with canonical section slug `trial-balance`.
...
- **AC2 (R2):** `/accounts?section=trial-balance` opens the correct authorized menu item, restores state, and omits it when access is denied.
