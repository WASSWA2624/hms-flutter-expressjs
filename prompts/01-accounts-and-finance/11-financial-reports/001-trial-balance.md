# Prompt 001 — Implement Trial Balance

## Context

- **Sequence:** 001 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 085 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Period Close → Year-end Close Runs` (`prompts/01-accounts-and-finance/10-period-close/006-year-end-close-runs.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Trial Balance`
- **Canonical route:** `/accounts?section=trial-balance`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/trial-balance.md`
...
2. **Register and scope the tab.**
   - Implement the `Trial Balance` tab at `Accounts & Finance → Financial Reports → Trial Balance` with canonical section slug `trial-balance`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/trial-balance.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/trial-balance.md`:
     `Account Code`, `Account Name`, `Account Type`, `Opening Debit`, `Opening Credit`, `Period Debit`, `Period Credit`, `Closing Debit`, `Closing Credit`, `Currency`, `Comparative Closing Balance`, `Difference`.
...
8. **Implement workflow, validation, permissions, and audit.**
   ...
   - Apply these tab-specific validation requirements:
     ...
     - Journal debits and credits must balance in every currency before approval or posting.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Income Statement` (`prompts/01-accounts-and-finance/11-financial-reports/002-income-statement.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=trial-balance` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/trial-balance.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/trial-balance.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_trial_balance_panel.dart`
