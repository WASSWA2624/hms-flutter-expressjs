...
2. **Register and scope the menu item.**
   - Implement the `Balance Sheet` leaf menu item at `Accounts & Finance → Financial Reports → Balance Sheet` with canonical section slug `balance-sheet`.
...
- **AC2 (R2):** `/accounts?section=balance-sheet` opens the correct authorized menu item, restores state, and omits it when access is denied.
