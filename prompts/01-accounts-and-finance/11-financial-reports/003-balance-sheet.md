# Prompt 003 — Implement Balance Sheet

## Context

- **Sequence:** 003 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 087 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → Income Statement` (`prompts/01-accounts-and-finance/11-financial-reports/002-income-statement.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Balance Sheet`
- **Canonical route:** `/accounts?section=balance-sheet`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/balance-sheet.md`
...
2. **Register and scope the tab.**
   - Implement the `Balance Sheet` tab at `Accounts & Finance → Financial Reports → Balance Sheet` with canonical section slug `balance-sheet`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/balance-sheet.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/balance-sheet.md`:
     `Section`, `Account Group`, `Account Code`, `Account Name`, `Current Balance`, `Prior-period Balance`, `Prior-year Balance`, `Movement`, `Currency`, `Supporting Schedule`, `Reconciliation Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Cash Flow Statement` (`prompts/01-accounts-and-finance/11-financial-reports/004-cash-flow-statement.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=balance-sheet` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/balance-sheet.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/balance-sheet.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_balance_sheet_panel.dart`
