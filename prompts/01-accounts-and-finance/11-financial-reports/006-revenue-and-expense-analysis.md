...
- **Surface profile:** `report` table/worklist opened from a nested menu item
...
2. **Register and scope the menu item.**
   - Implement the `Revenue & Expense Analysis` leaf menu item at `Accounts & Finance → Financial Reports → Revenue & Expense Analysis` with canonical section slug `revenue-and-expense-analysis`.
...
- **AC2 (R2):** `/accounts?section=revenue-and-expense-analysis` opens the correct authorized menu item, restores state, and omits it when access is denied.
