# Prompt 006 — Implement Revenue & Expense Analysis

## Context

- **Sequence:** 006 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 090 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → General Ledger Report` (`prompts/01-accounts-and-finance/11-financial-reports/005-general-ledger-report.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Revenue & Expense Analysis`
- **Canonical route:** `/accounts?section=revenue-and-expense-analysis`
- **Surface profile:** `report` table/worklist tab
- **Navigation model:** `Accounts & Finance → Financial Reports` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/revenue-and-expense-analysis.md`
...
2. **Register and scope the tab.**
   - Implement the `Revenue & Expense Analysis` tab at `Accounts & Finance → Financial Reports → Revenue & Expense Analysis` with canonical section slug `revenue-and-expense-analysis`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Financial Reports` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/revenue-and-expense-analysis.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/revenue-and-expense-analysis.md`:
     `Fiscal Period`, `Account Type`, `Account Group`, `Account`, `Facility`, `Department`, `Cost Centre`, `Service/Expense Category`, `Actual Amount`, `Budget Amount`, `Variance`, `Prior-period Amount`, `Growth Percentage`, `Currency`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Facility`, `Department / cost centre`, `Currency`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New expense:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens expense request/entry dialog.
     - **Run report:** gate `accounts:read` ∩ `facility-accounts`; enable when filters valid; Refreshes report rows and totals.
     - **Schedule:** gate `accounts:write` ∩ `facility-accounts`; enable when scheduling supported; Opens report schedule dialog.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Drill down:** gate `accounts:read` ∩ `facility-accounts`; enable when source dimension available; Opens the authoritative filtered source tab.
     - **Export row/detail:** gate `accounts:read` ∩ `evidence:export` ∩ `facility-accounts`; enable when row visible; Exports the selected breakdown.
   - Bulk buttons:
     - **Export selected:** gate `accounts:read` ∩ `evidence:export` ∩ `facility-accounts`; Exports only rows in the user's current ABAC scope.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Department Performance` (`prompts/01-accounts-and-finance/11-financial-reports/007-department-performance.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=revenue-and-expense-analysis` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/revenue-and-expense-analysis.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/revenue-and-expense-analysis.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_revenue_and_expense_analysis_panel.dart`
