...
2. **Register and scope the menu item.**
   - Implement the `Consolidated Reports` leaf menu item at `Accounts & Finance → Financial Reports → Consolidated Reports` with canonical section slug `consolidated-reports`.
...
- **AC2 (R2):** `/accounts?section=consolidated-reports` opens the correct authorized menu item, restores state, and omits it when access is denied.
