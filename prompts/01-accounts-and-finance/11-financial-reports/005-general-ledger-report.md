# Prompt 005 — Implement General Ledger Report

## Context

- **Sequence:** 005 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 089 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → Cash Flow Statement` (`prompts/01-accounts-and-finance/11-financial-reports/004-cash-flow-statement.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → General Ledger Report`
- **Canonical route:** `/accounts?section=general-ledger-report`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/general-ledger-report.md`
...
2. **Register and scope the tab.**
   - Implement the `General Ledger Report` tab at `Accounts & Finance → Financial Reports → General Ledger Report` with canonical section slug `general-ledger-report`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/general-ledger-report.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/general-ledger-report.md`:
     `Posting Date`, `Fiscal Period`, `Account Code`, `Account Name`, `Journal Entry No.`, `Source Module`, `Document Reference`, `Description`, `Party`, `Department`, `Cost Centre`, `Debit`, `Credit`, `Running Balance`, `Currency`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Department / cost centre`, `Currency`, `Amount range`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   ...
   - Detail sections: `Entry/account summary`, `Journal lines`, `Source documents`, `Approval & posting history`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Revenue & Expense Analysis` (`prompts/01-accounts-and-finance/11-financial-reports/006-revenue-and-expense-analysis.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=general-ledger-report` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/general-ledger-report.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/general-ledger-report.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_general_ledger_report_panel.dart`
