...
2. **Register and scope the menu item.**
   - Implement the `General Ledger Report` leaf menu item at `Accounts & Finance → Financial Reports → General Ledger Report` with canonical section slug `general-ledger-report`.
...
- **AC2 (R2):** `/accounts?section=general-ledger-report` opens the correct authorized menu item, restores state, and omits it when access is denied.
