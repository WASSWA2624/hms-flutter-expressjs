# Prompt 004 — Implement Cash Flow Statement

## Context

- **Sequence:** 004 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 088 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → Balance Sheet` (`prompts/01-accounts-and-finance/11-financial-reports/003-balance-sheet.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Cash Flow Statement`
- **Canonical route:** `/accounts?section=cash-flow-statement`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/cash-flow-statement.md`
...
2. **Register and scope the tab.**
   - Implement the `Cash Flow Statement` tab at `Accounts & Finance → Financial Reports → Cash Flow Statement` with canonical section slug `cash-flow-statement`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/cash-flow-statement.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/cash-flow-statement.md`:
     `Cash Flow Section`, `Line Item`, `Source Account(s)`, `Current-period Inflow`, `Current-period Outflow`, `Net Cash Flow`, `Year-to-date Amount`, `Prior-period Amount`, `Budget Amount`, `Variance`, `Currency`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Currency`, `Amount range`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → General Ledger Report` (`prompts/01-accounts-and-finance/11-financial-reports/005-general-ledger-report.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=cash-flow-statement` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/cash-flow-statement.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/cash-flow-statement.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_cash_flow_statement_panel.dart`
