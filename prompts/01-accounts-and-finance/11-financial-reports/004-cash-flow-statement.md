...
2. **Register and scope the menu item.**
   - Implement the `Cash Flow Statement` leaf menu item at `Accounts & Finance → Financial Reports → Cash Flow Statement` with canonical section slug `cash-flow-statement`.
...
- **AC2 (R2):** `/accounts?section=cash-flow-statement` opens the correct authorized menu item, restores state, and omits it when access is denied.
