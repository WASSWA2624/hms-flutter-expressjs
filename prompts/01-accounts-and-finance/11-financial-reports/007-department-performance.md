...
2. **Register and scope the menu item.**
   - Implement the `Department Performance` leaf menu item at `Accounts & Finance → Financial Reports → Department Performance` with canonical section slug `department-performance`.
...
- **AC2 (R2):** `/accounts?section=department-performance` opens the correct authorized menu item, restores state, and omits it when access is denied.
