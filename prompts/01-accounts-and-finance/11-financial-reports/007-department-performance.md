# Prompt 007 — Implement Department Performance

## Context

- **Sequence:** 007 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 091 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → Revenue & Expense Analysis` (`prompts/01-accounts-and-finance/11-financial-reports/006-revenue-and-expense-analysis.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Department Performance`
- **Canonical route:** `/accounts?section=department-performance`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/department-performance.md`
...
2. **Register and scope the tab.**
   - Implement the `Department Performance` tab at `Accounts & Finance → Financial Reports → Department Performance` with canonical section slug `department-performance`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/department-performance.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/department-performance.md`:
     `Fiscal Period`, `Facility`, `Department`, `Revenue`, `Direct Costs`, `Allocated Costs`, `Gross Margin`, `Operating Margin`, `Budget`, `Variance`, `Patient/Activity Volume`, `Revenue per Activity`, `Cost per Activity`, `Currency`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Facility`, `Department / cost centre`, `Party`, `Currency`, `Amount range`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Cashier & Bank Reports` (`prompts/01-accounts-and-finance/11-financial-reports/008-cashier-and-bank-reports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=department-performance` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/department-performance.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/department-performance.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_department_performance_panel.dart`
