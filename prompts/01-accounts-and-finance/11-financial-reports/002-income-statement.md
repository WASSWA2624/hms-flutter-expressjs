...
2. **Register and scope the menu item.**
   - Implement the `Income Statement` leaf menu item at `Accounts & Finance → Financial Reports → Income Statement` with canonical section slug `income-statement`.
...
- **AC2 (R2):** `/accounts?section=income-statement` opens the correct authorized menu item, restores state, and omits it when access is denied.
