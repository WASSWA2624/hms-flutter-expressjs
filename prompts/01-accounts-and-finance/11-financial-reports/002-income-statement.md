# Prompt 002 — Implement Income Statement

## Context

- **Sequence:** 002 of 009 in `prompts/01-accounts-and-finance/11-financial-reports/`
- **Overall step:** 086 of 158
- **Phase:** Accounts operations — financial reports
- **Prerequisite:** `Accounts & Finance → Financial Reports → Trial Balance` (`prompts/01-accounts-and-finance/11-financial-reports/001-trial-balance.md`).
- **Menu path:** `Accounts & Finance → Financial Reports → Income Statement`
- **Canonical route:** `/accounts?section=income-statement`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/financial-reports/income-statement.md`
...
2. **Register and scope the tab.**
   - Implement the `Income Statement` tab at `Accounts & Finance → Financial Reports → Income Statement` with canonical section slug `income-statement`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/financial-reports/income-statement.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/financial-reports/income-statement.md`:
     `Section`, `Account Group`, `Account Code`, `Account Name`, `Current Period`, `Year to Date`, `Prior Period`, `Prior Year`, `Budget`, `Variance Amount`, `Variance Percentage`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Financial Reports → Balance Sheet` (`prompts/01-accounts-and-finance/11-financial-reports/003-balance-sheet.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=income-statement` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/financial-reports/income-statement.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/financial-reports/income-statement.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_income_statement_panel.dart`
