...
2. **Register and scope the menu item.**
   - Implement the `Recurring Expenses` leaf menu item at `Accounts & Finance → Expenses → Recurring Expenses` with canonical section slug `recurring-expenses`.
...
- **AC2 (R2):** `/accounts?section=recurring-expenses` opens the correct authorized menu item, restores state, and omits it when access is denied.
