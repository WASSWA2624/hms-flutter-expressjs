# Prompt 004 — Implement Recurring Expenses

## Context

- **Sequence:** 004 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 054 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Expenses → Expense Approvals` (`prompts/01-accounts-and-finance/06-expenses/003-expense-approvals.md`).
- **Menu path:** `Accounts & Finance → Expenses → Recurring Expenses`
- **Canonical route:** `/accounts?section=recurring-expenses`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/recurring-expenses.md`
...
2. **Register and scope the tab.**
   - Implement the `Recurring Expenses` tab at `Accounts & Finance → Expenses → Recurring Expenses` with canonical section slug `recurring-expenses`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/recurring-expenses.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/recurring-expenses.md`:
     `Schedule No.`, `Expense Name`, `Supplier/Payee`, `Expense Category`, `GL Account`, `Department`, `Frequency`, `Next Due Date`, `End Date`, `Expected Amount`, `Currency`, `Auto-create`, `Approval Required`, `Last Expense No.`, `Last Run Status`, `Schedule Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Expense Name`, `Supplier/Payee`, `Expense Category`, `GL Account`, `Department`, `Frequency`, `Next Due Date`, `End Date`, `Expected Amount`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Staff Reimbursements` (`prompts/01-accounts-and-finance/06-expenses/005-staff-reimbursements.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=recurring-expenses` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/recurring-expenses.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/recurring-expenses.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_recurring_expenses_panel.dart`
