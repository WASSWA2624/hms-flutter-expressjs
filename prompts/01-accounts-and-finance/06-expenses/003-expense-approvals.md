...
2. **Register and scope the menu item.**
   - Implement the `Expense Approvals` leaf menu item at `Accounts & Finance → Expenses → Expense Approvals` with canonical section slug `expense-approvals`.
...
- **AC2 (R2):** `/accounts?section=expense-approvals` opens the correct authorized menu item, restores state, and omits it when access is denied.
