# Prompt 003 — Implement Expense Approvals

## Context

- **Sequence:** 003 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 053 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Expenses → Expense Requests` (`prompts/01-accounts-and-finance/06-expenses/002-expense-requests.md`).
- **Menu path:** `Accounts & Finance → Expenses → Expense Approvals`
- **Canonical route:** `/accounts?section=expense-approvals`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/expense-approvals.md`
...
2. **Register and scope the tab.**
   - Implement the `Expense Approvals` tab at `Accounts & Finance → Expenses → Expense Approvals` with canonical section slug `expense-approvals`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/expense-approvals.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/expense-approvals.md`:
     `Request No.`, `Request Date`, `Requester`, `Department`, `Expense Category`, `Purpose`, `Requested Amount`, `Currency`, `Approval Level`, `Current Approver`, `Submitted At`, `Waiting Time`, `Budget Check`, `Policy Exceptions`, `Decision Due`, `Approval Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Request Date`, `Requester`, `Department`, `Expense Category`, `Purpose`, `Requested Amount`, `Currency`, `Approval Level`, `Current Approver`, `Submitted At`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Recurring Expenses` (`prompts/01-accounts-and-finance/06-expenses/004-recurring-expenses.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=expense-approvals` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/expense-approvals.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/expense-approvals.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_expense_approvals_panel.dart`
