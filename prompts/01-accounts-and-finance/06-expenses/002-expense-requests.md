...
2. **Register and scope the menu item.**
   - Implement the `Expense Requests` leaf menu item at `Accounts & Finance → Expenses → Expense Requests` with canonical section slug `expense-requests`.
...
- **AC2 (R2):** `/accounts?section=expense-requests` opens the correct authorized menu item, restores state, and omits it when access is denied.
