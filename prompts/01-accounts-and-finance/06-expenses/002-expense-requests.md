# Prompt 002 — Implement Expense Requests

## Context

- **Sequence:** 002 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 052 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Expenses → Expense Entries` (`prompts/01-accounts-and-finance/06-expenses/001-expense-entries.md`).
- **Menu path:** `Accounts & Finance → Expenses → Expense Requests`
- **Canonical route:** `/accounts?section=expense-requests`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/expense-requests.md`
...
2. **Register and scope the tab.**
   - Implement the `Expense Requests` tab at `Accounts & Finance → Expenses → Expense Requests` with canonical section slug `expense-requests`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/expense-requests.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/expense-requests.md`:
     `Request No.`, `Request Date`, `Requested By`, `Payee`, `Expense Category`, `Department`, `Cost Centre`, `Purpose`, `Required Date`, `Requested Amount`, `Currency`, `Available Budget`, `Priority`, `Attachment Count`, `Current Approver`, `Request Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Request Date`, `Payee`, `Expense Category`, `Department`, `Cost Centre`, `Purpose`, `Required Date`, `Requested Amount`, `Currency`, `Priority`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Expense Approvals` (`prompts/01-accounts-and-finance/06-expenses/003-expense-approvals.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=expense-requests` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/expense-requests.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/expense-requests.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_expense_requests_panel.dart`
