# Prompt 001 — Implement Expense Entries

## Context

- **Sequence:** 001 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 051 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Purchases & Payables → AP Reconciliation` (`prompts/01-accounts-and-finance/05-purchases-and-payables/011-ap-reconciliation.md`).
- **Menu path:** `Accounts & Finance → Expenses → Expense Entries`
- **Canonical route:** `/accounts?section=expense-entries`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/expense-entries.md`
...
2. **Register and scope the tab.**
   - Implement the `Expense Entries` tab at `Accounts & Finance → Expenses → Expense Entries` with canonical section slug `expense-entries`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/expense-entries.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/expense-entries.md`:
     `Expense No.`, `Expense Date`, `Expense Category`, `GL Account`, `Supplier/Payee`, `Department`, `Cost Centre`, `Project`, `Description`, `Net Amount`, `Tax`, `Gross Amount`, `Currency`, `Payment Method`, `Payment Reference`, `Receipt Attached`, `Approval Status`, `Posting Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Expense Date`, `Expense Category`, `GL Account`, `Supplier/Payee`, `Department`, `Cost Centre`, `Project`, `Description`, `Net Amount`, `Gross Amount`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Expense Requests` (`prompts/01-accounts-and-finance/06-expenses/002-expense-requests.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=expense-entries` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/expense-entries.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/expense-entries.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_expense_entries_panel.dart`
