...
2. **Register and scope the menu item.**
   - Implement the `Expense Entries` leaf menu item at `Accounts & Finance → Expenses → Expense Entries` with canonical section slug `expense-entries`.
...
- **AC2 (R2):** `/accounts?section=expense-entries` opens the correct authorized menu item, restores state, and omits it when access is denied.
