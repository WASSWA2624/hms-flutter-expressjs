# Prompt 005 — Implement Staff Reimbursements

## Context

- **Sequence:** 005 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 055 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Expenses → Recurring Expenses` (`prompts/01-accounts-and-finance/06-expenses/004-recurring-expenses.md`).
- **Menu path:** `Accounts & Finance → Expenses → Staff Reimbursements`
- **Canonical route:** `/accounts?section=staff-reimbursements`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/staff-reimbursements.md`
...
2. **Register and scope the tab.**
   - Implement the `Staff Reimbursements` tab at `Accounts & Finance → Expenses → Staff Reimbursements` with canonical section slug `staff-reimbursements`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/staff-reimbursements.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/staff-reimbursements.md`:
     `Claim No.`, `Claim Date`, `Employee No.`, `Employee Name`, `Department`, `Expense Category`, `Travel/Activity Reference`, `Claimed Amount`, `Approved Amount`, `Paid Amount`, `Currency`, `Receipt Count`, `Policy Exceptions`, `Approval Status`, `Payment No.`, `Reimbursement Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Staff Reimbursements mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Claim Date`, `Employee No.`, `Employee Name`, `Department`, `Expense Category`, `Travel/Activity Reference`, `Claimed Amount`, `Approved Amount`, `Currency`, `Policy Exceptions`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Expense Categories` (`prompts/01-accounts-and-finance/06-expenses/006-expense-categories.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=staff-reimbursements` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/staff-reimbursements.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/staff-reimbursements.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_staff_reimbursements_panel.dart`
