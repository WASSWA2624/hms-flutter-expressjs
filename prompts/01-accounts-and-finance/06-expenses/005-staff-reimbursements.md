...
2. **Register and scope the menu item.**
   - Implement the `Staff Reimbursements` leaf menu item at `Accounts & Finance → Expenses → Staff Reimbursements` with canonical section slug `staff-reimbursements`.
...
- **AC2 (R2):** `/accounts?section=staff-reimbursements` opens the correct authorized menu item, restores state, and omits it when access is denied.
