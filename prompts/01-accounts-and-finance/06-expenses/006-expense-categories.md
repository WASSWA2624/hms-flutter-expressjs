# Prompt 006 — Implement Expense Categories

## Context

- **Sequence:** 006 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 056 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Expenses → Staff Reimbursements` (`prompts/01-accounts-and-finance/06-expenses/005-staff-reimbursements.md`).
- **Menu path:** `Accounts & Finance → Expenses → Expense Categories`
- **Canonical route:** `/accounts?section=expense-categories`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/expense-categories.md`
...
2. **Register and scope the tab.**
   - Implement the `Expense Categories` tab at `Accounts & Finance → Expenses → Expense Categories` with canonical section slug `expense-categories`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/expense-categories.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/expense-categories.md`:
     `Category Code`, `Category Name`, `Parent Category`, `Default GL Account`, `Tax Code`, `Budget Required`, `Receipt Required`, `Approval Rule`, `Spending Limit`, `Allowed Payment Methods`, `Effective From`, `Effective To`, `Category Status`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Category Code`, `Category Name`, `Parent Category`, `Default GL Account`, `Tax Code`, `Budget Required`, `Receipt Required`, `Approval Rule`, `Spending Limit`, `Allowed Payment Methods`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Expenses → Expense Analysis` (`prompts/01-accounts-and-finance/06-expenses/007-expense-analysis.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=expense-categories` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/expense-categories.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/expense-categories.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_expense_categories_panel.dart`
