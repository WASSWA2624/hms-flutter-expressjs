...
2. **Register and scope the menu item.**
   - Implement the `Expense Categories` leaf menu item at `Accounts & Finance → Expenses → Expense Categories` with canonical section slug `expense-categories`.
...
- **AC2 (R2):** `/accounts?section=expense-categories` opens the correct authorized menu item, restores state, and omits it when access is denied.
