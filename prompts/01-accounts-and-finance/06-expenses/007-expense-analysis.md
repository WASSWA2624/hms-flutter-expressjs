...
2. **Register and scope the menu item.**
   - Implement the `Expense Analysis` leaf menu item at `Accounts & Finance → Expenses → Expense Analysis` with canonical section slug `expense-analysis`.
...
- **AC2 (R2):** `/accounts?section=expense-analysis` opens the correct authorized menu item, restores state, and omits it when access is denied.
