# Prompt 007 — Implement Expense Analysis

## Context

- **Sequence:** 007 of 007 in `prompts/01-accounts-and-finance/06-expenses/`
- **Overall step:** 057 of 158
- **Phase:** Accounts operations — expenses
- **Prerequisite:** `Accounts & Finance → Expenses → Expense Categories` (`prompts/01-accounts-and-finance/06-expenses/006-expense-categories.md`).
- **Menu path:** `Accounts & Finance → Expenses → Expense Analysis`
- **Canonical route:** `/accounts?section=expense-analysis`
- **Surface profile:** `analysis` table/worklist tab
- **Navigation model:** `Accounts & Finance → Expenses` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/expenses/expense-analysis.md`
...
2. **Register and scope the tab.**
   - Implement the `Expense Analysis` tab at `Accounts & Finance → Expenses → Expense Analysis` with canonical section slug `expense-analysis`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Expenses` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/expenses/expense-analysis.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/expenses/expense-analysis.md`:
     `Period`, `Expense Category`, `GL Account`, `Department`, `Cost Centre`, `Project`, `Supplier/Payee`, `Transaction Count`, `Actual Amount`, `Budget Amount`, `Committed Amount`, `Variance Amount`, `Variance Percentage`, `Currency`, `Prior-period Amount`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Department / cost centre`, `Party`, `Currency`, `Amount range`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Fixed Assets → Asset Register` (`prompts/01-accounts-and-finance/07-fixed-assets/001-asset-register.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=expense-analysis` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/expenses/expense-analysis.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/expenses/expense-analysis.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_expense_analysis_panel.dart`
