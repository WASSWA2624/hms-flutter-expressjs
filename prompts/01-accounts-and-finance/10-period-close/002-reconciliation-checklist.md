...
2. **Register and scope the menu item.**
   - Implement the `Reconciliation Checklist` leaf menu item at `Accounts & Finance → Period Close → Reconciliation Checklist` with canonical section slug `reconciliation-checklist`.
...
- **AC2 (R2):** `/accounts?section=reconciliation-checklist` opens the correct authorized menu item, restores state, and omits it when access is denied.
