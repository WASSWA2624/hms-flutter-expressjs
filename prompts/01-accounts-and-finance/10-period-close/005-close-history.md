# Prompt 005 — Implement Close History

## Context

- **Sequence:** 005 of 006 in `prompts/01-accounts-and-finance/10-period-close/`
- **Overall step:** 083 of 158
- **Phase:** Accounts operations — period close
- **Prerequisite:** `Accounts & Finance → Period Close → Reopen Requests` (`prompts/01-accounts-and-finance/10-period-close/004-reopen-requests.md`).
- **Menu path:** `Accounts & Finance → Period Close → Close History`
- **Canonical route:** `/accounts?section=close-history`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/period-close/close-history.md`
...
2. **Register and scope the tab.**
   - Implement the `Close History` tab at `Accounts & Finance → Period Close → Close History` with canonical section slug `close-history`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/period-close/close-history.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/period-close/close-history.md`:
     `Fiscal Period`, `Entity/Facility`, `Close Type`, `Closed At`, `Closed By`, `Reopened Count`, `Last Reopened At`, `Reclosed At`, `Journal Count`, `Adjustment Count`, `Exception Count`, `Lock Status`, `Audit Reference`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Period Close → Year-end Close Runs` (`prompts/01-accounts-and-finance/10-period-close/006-year-end-close-runs.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=close-history` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/period-close/close-history.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/period-close/close-history.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_close_history_panel.dart`
