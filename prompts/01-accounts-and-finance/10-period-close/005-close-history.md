...
2. **Register and scope the menu item.**
   - Implement the `Close History` leaf menu item at `Accounts & Finance → Period Close → Close History` with canonical section slug `close-history`.
...
- **AC2 (R2):** `/accounts?section=close-history` opens the correct authorized menu item, restores state, and omits it when access is denied.
