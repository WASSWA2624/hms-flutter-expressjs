...
2. **Register and scope the menu item.**
   - Implement the `Close Checklist` leaf menu item at `Accounts & Finance → Period Close → Close Checklist` with canonical section slug `close-checklist`.
...
- **AC2 (R2):** `/accounts?section=close-checklist` opens the correct authorized menu item, restores state, and omits it when access is denied.
