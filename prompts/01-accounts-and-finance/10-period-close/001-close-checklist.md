# Prompt 001 — Implement Close Checklist

## Context

- **Sequence:** 001 of 006 in `prompts/01-accounts-and-finance/10-period-close/`
- **Overall step:** 079 of 158
- **Phase:** Accounts operations — period close
- **Prerequisite:** `Accounts & Finance → Tax & Compliance → Audit Schedules` (`prompts/01-accounts-and-finance/09-tax-and-compliance/006-audit-schedules.md`).
- **Menu path:** `Accounts & Finance → Period Close → Close Checklist`
- **Canonical route:** `/accounts?section=close-checklist`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/period-close/close-checklist.md`
...
2. **Register and scope the tab.**
   - Implement the `Close Checklist` tab at `Accounts & Finance → Period Close → Close Checklist` with canonical section slug `close-checklist`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/period-close/close-checklist.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/period-close/close-checklist.md`:
     `Fiscal Period`, `Checklist Item`, `Category`, `Description`, `Dependency`, `Responsible Role/User`, `Due Date`, `Completion Date`, `Evidence Required`, `Evidence Attached`, `Exception Count`, `Reviewed By`, `Review Date`, `Checklist Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Fiscal Period`, `Checklist Item`, `Category`, `Description`, `Dependency`, `Responsible Role/User`, `Due Date`, `Completion Date`, `Evidence Required`, `Evidence Attached`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Period Close → Reconciliation Checklist` (`prompts/01-accounts-and-finance/10-period-close/002-reconciliation-checklist.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=close-checklist` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/period-close/close-checklist.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/period-close/close-checklist.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_close_checklist_panel.dart`
