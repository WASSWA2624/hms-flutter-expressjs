...
2. **Register and scope the menu item.**
   - Implement the `Year-end Close Runs` leaf menu item at `Accounts & Finance → Period Close → Year-end Close Runs` with canonical section slug `year-end-close-runs`.
...
- **AC2 (R2):** `/accounts?section=year-end-close-runs` opens the correct authorized menu item, restores state, and omits it when access is denied.
