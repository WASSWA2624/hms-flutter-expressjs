# Prompt 003 — Implement Period Adjustments

## Context

- **Sequence:** 003 of 006 in `prompts/01-accounts-and-finance/10-period-close/`
- **Overall step:** 081 of 158
- **Phase:** Accounts operations — period close
- **Prerequisite:** `Accounts & Finance → Period Close → Reconciliation Checklist` (`prompts/01-accounts-and-finance/10-period-close/002-reconciliation-checklist.md`).
- **Menu path:** `Accounts & Finance → Period Close → Period Adjustments`
- **Canonical route:** `/accounts?section=period-adjustments`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/period-close/period-adjustments.md`
...
2. **Register and scope the tab.**
   - Implement the `Period Adjustments` tab at `Accounts & Finance → Period Close → Period Adjustments` with canonical section slug `period-adjustments`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/period-close/period-adjustments.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/period-close/period-adjustments.md`:
     `Adjustment No.`, `Fiscal Period`, `Entry Date`, `Adjustment Type`, `Description`, `Total Debit`, `Total Credit`, `Currency`, `Supporting Reference`, `Requested By`, `Approved By`, `Journal Entry No.`, `Posting Date`, `Adjustment Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted and source prerequisites met; Opens the Period Adjustments mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Fiscal Period`, `Entry Date`, `Adjustment Type`, `Description`, `Currency`, `Supporting Reference`, `Posting Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Period Close → Reopen Requests` (`prompts/01-accounts-and-finance/10-period-close/004-reopen-requests.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=period-adjustments` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/period-close/period-adjustments.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/period-close/period-adjustments.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_period_adjustments_panel.dart`
