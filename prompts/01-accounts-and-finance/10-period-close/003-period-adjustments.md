...
2. **Register and scope the menu item.**
   - Implement the `Period Adjustments` leaf menu item at `Accounts & Finance → Period Close → Period Adjustments` with canonical section slug `period-adjustments`.
...
- **AC2 (R2):** `/accounts?section=period-adjustments` opens the correct authorized menu item, restores state, and omits it when access is denied.
