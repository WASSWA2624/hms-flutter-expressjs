...
2. **Register and scope the menu item.**
   - Implement the `Reopen Requests` leaf menu item at `Accounts & Finance → Period Close → Reopen Requests` with canonical section slug `reopen-requests`.
...
- **AC2 (R2):** `/accounts?section=reopen-requests` opens the correct authorized menu item, restores state, and omits it when access is denied.
