...
2. **Register and scope the tab.**
   - Implement the `Budget vs Actual` tab at `Accounts & Finance → Budgets & Cost Control → Budget vs Actual` with canonical section slug `budget-vs-actual`.
...
- **AC2 (R2):** `/accounts?section=budget-vs-actual` opens the correct authorized tab, restores state, and omits it when access is denied.
