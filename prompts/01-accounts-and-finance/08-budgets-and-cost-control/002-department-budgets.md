...
2. **Register and scope the tab.**
   - Implement the `Department Budgets` tab at `Accounts & Finance → Budgets & Cost Control → Department Budgets` with canonical section slug `department-budgets`.
...
- **AC2 (R2):** `/accounts?section=department-budgets` opens the correct authorized tab, restores state, and omits it when access is denied.
