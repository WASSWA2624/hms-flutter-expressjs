# Prompt 002 — Implement Department Budgets

## Context

- **Sequence:** 002 of 007 in `prompts/01-accounts-and-finance/08-budgets-and-cost-control/`
- **Overall step:** 067 of 158
- **Phase:** Accounts operations — budgets and cost control
- **Prerequisite:** `Accounts & Finance → Budgets & Cost Control → Annual Budgets` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/001-annual-budgets.md`).
- **Menu path:** `Accounts & Finance → Budgets & Cost Control → Department Budgets`
- **Canonical route:** `/accounts?section=department-budgets`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/budgets-and-cost-control/department-budgets.md`
...
2. **Register and scope the menu item.**
   - Implement the `Department Budgets` leaf menu item at `Accounts & Finance → Budgets & Cost Control → Department Budgets` with canonical section slug `department-budgets`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/department-budgets.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/department-budgets.md`:
     `Budget No.`, `Fiscal Year`, `Department`, `Cost Centre`, `Budget Owner`, `Currency`, `Original Budget`, `Revised Budget`, `Commitments`, `Actual Amount`, `Available Amount`, `Utilization Percentage`, `Forecast Amount`, `Variance`, `Approval Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Department / cost centre`, `Currency`, `Amount range`, `Owner / assigned user`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New budget record:** gate `accounts:write` ∩ `facility-accounts`; enable when write permitted; Opens budget header and lines editor.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Fiscal Year`, `Department`, `Cost Centre`, `Budget Owner`, `Currency`, `Original Budget`, `Revised Budget`, `Commitments`, `Actual Amount`, `Utilization Percentage`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Budgets & Cost Control → Project Budgets` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/003-project-budgets.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=department-budgets` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/department-budgets.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/budgets-and-cost-control/department-budgets.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_department_budgets_panel.dart`
