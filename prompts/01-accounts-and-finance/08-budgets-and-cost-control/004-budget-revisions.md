# Prompt 004 — Implement Budget Revisions

## Context

- **Sequence:** 004 of 007 in `prompts/01-accounts-and-finance/08-budgets-and-cost-control/`
- **Overall step:** 069 of 158
- **Phase:** Accounts operations — budgets and cost control
- **Prerequisite:** `Accounts & Finance → Budgets & Cost Control → Project Budgets` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/003-project-budgets.md`).
- **Menu path:** `Accounts & Finance → Budgets & Cost Control → Budget Revisions`
- **Canonical route:** `/accounts?section=budget-revisions`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/budgets-and-cost-control/budget-revisions.md`
...
2. **Register and scope the menu item.**
   - Implement the `Budget Revisions` leaf menu item at `Accounts & Finance → Budgets & Cost Control → Budget Revisions` with canonical section slug `budget-revisions`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/budget-revisions.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/budget-revisions.md`:
     `Revision No.`, `Revision Date`, `Budget No.`, `Fiscal Year`, `Department/Project`, `Revision Type`, `Source Line`, `Destination Line`, `Increase`, `Decrease`, `Net Change`, `Currency`, `Reason`, `Requested By`, `Approved By`, `Approval Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Department / cost centre`, `Currency`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Revision Date`, `Budget No.`, `Fiscal Year`, `Department/Project`, `Revision Type`, `Source Line`, `Destination Line`, `Increase`, `Decrease`, `Net Change`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Budgets & Cost Control → Commitments` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/005-commitments.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=budget-revisions` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/budget-revisions.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/budgets-and-cost-control/budget-revisions.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_budget_revisions_panel.dart`
