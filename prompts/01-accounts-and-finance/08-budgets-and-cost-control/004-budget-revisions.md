...
2. **Register and scope the tab.**
   - Implement the `Budget Revisions` tab at `Accounts & Finance → Budgets & Cost Control → Budget Revisions` with canonical section slug `budget-revisions`.
...
- **AC2 (R2):** `/accounts?section=budget-revisions` opens the correct authorized tab, restores state, and omits it when access is denied.
