...
2. **Register and scope the tab.**
   - Implement the `Annual Budgets` tab at `Accounts & Finance → Budgets & Cost Control → Annual Budgets` with canonical section slug `annual-budgets`.
...
- **AC2 (R2):** `/accounts?section=annual-budgets` opens the correct authorized tab, restores state, and omits it when access is denied.
