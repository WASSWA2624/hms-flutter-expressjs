# Prompt 001 — Implement Annual Budgets

## Context

- **Sequence:** 001 of 007 in `prompts/01-accounts-and-finance/08-budgets-and-cost-control/`
- **Overall step:** 066 of 158
- **Phase:** Accounts operations — budgets and cost control
- **Prerequisite:** `Accounts & Finance → Fixed Assets → Asset Ledger` (`prompts/01-accounts-and-finance/07-fixed-assets/008-asset-ledger.md`).
- **Menu path:** `Accounts & Finance → Budgets & Cost Control → Annual Budgets`
- **Canonical route:** `/accounts?section=annual-budgets`
...
- **Authoritative tab specification:** `.cursor/finance/accounts-and-finance/budgets-and-cost-control/annual-budgets.md`
...
2. **Register and scope the menu item.**
   - Implement the `Annual Budgets` leaf menu item at `Accounts & Finance → Budgets & Cost Control → Annual Budgets` with canonical section slug `annual-budgets`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/annual-budgets.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/annual-budgets.md`:
     `Budget No.`, `Fiscal Year`, `Budget Version`, `Entity/Facility`, `Currency`, `Department Count`, `Line Count`, `Original Budget`, `Approved Revisions`, `Current Budget`, `Commitments`, `Actual Amount`, `Available Amount`, `Utilization Percentage`, `Prepared By`, `Approved By`, `Budget Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Facility`, `Department / cost centre`, `Currency`, `Amount range`, `Owner / assigned user`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Fiscal Year`, `Budget Version`, `Entity/Facility`, `Currency`, `Original Budget`, `Approved Revisions`, `Current Budget`, `Commitments`, `Actual Amount`, `Utilization Percentage`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Budgets & Cost Control → Department Budgets` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/002-department-budgets.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=annual-budgets` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/annual-budgets.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/budgets-and-cost-control/annual-budgets.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_annual_budgets_panel.dart`
