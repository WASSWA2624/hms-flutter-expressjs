# Prompt 007 — Implement Variance Analysis

## Context

- **Sequence:** 007 of 007 in `prompts/01-accounts-and-finance/08-budgets-and-cost-control/`
- **Overall step:** 072 of 158
- **Phase:** Accounts operations — budgets and cost control
- **Prerequisite:** `Accounts & Finance → Budgets & Cost Control → Budget vs Actual` (`prompts/01-accounts-and-finance/08-budgets-and-cost-control/006-budget-vs-actual.md`).
...
2. **Register and scope the menu item.**
   - Implement the `Variance Analysis` leaf menu item at `Accounts & Finance → Budgets & Cost Control → Variance Analysis` with canonical section slug `variance-analysis`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/variance-analysis.md`:
     `Fiscal Period`, `Department`, `Cost Centre`, `Project`, `Account`, `Budget Amount`, `Actual Amount`, `Variance Amount`, `Variance Percentage`, `Favorable/Unfavorable`, `Threshold`, `Explanation`, `Corrective Action`, `Responsible Owner`, `Review Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Department / cost centre`, `Amount range`, `Owner / assigned user`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Accounts & Finance → Tax & Compliance → Tax Codes & Rates` (`prompts/01-accounts-and-finance/09-tax-and-compliance/001-tax-codes-and-rates.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/accounts?section=variance-analysis` opens the correct authorized menu item, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/accounts-and-finance/budgets-and-cost-control/variance-analysis.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/accounts-and-finance/budgets-and-cost-control/variance-analysis.md`
...
- `frontend/lib/features/accounts/presentation/widgets/accounts_variance_analysis_panel.dart`
