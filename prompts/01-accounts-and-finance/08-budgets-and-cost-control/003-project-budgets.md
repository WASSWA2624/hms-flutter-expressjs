...
2. **Register and scope the tab.**
   - Implement the `Project Budgets` tab at `Accounts & Finance → Budgets & Cost Control → Project Budgets` with canonical section slug `project-budgets`.
...
- **AC2 (R2):** `/accounts?section=project-budgets` opens the correct authorized tab, restores state, and omits it when access is denied.
