...
| Field | Value |
| --- | --- |
| Symbol | `CorrectStageDialog` |
| Purpose | Correct Stage |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1449` |
...
| Primary commit | Correct stage |
| Slices to keep in sync | encounter stage, queue position, stage badges |
| Sibling reuse targets | `FlowActionsDialog`, `AssignDoctorDialog` |
...
| Mutations (region) | `mutation: correctStage` |
...
- `backend/src/modules/triage/controllers/triage.controller.js`
...
| Title snippets | `l10n.opdCorrectStageAction`, `widget.title`, `showTitle ? l10n.opdEncounterContextTitle : null` |
...
| Cross-stack trace files found | 13 |
| Peek region size | 4925 chars |
...
**Scope lock:** only `CorrectStageDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `CorrectStageDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1449` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Correct Stage** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Correct stage`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `FlowActionsDialog`, `AssignDoctorDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch encounter stage, queue position, stage badges, then apply the smallest targeted reconciliation.
...
1. `CorrectStageDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FlowActionsDialog`, `AssignDoctorDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: encounter stage, queue position, stage badges.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `CorrectStageDialog`'s path.
...
<!-- generator: encounter-dialog prompt 32 slug=correct-stage-dialog symbol=CorrectStageDialog shape=widget_dialog -->
