...
| Field | Value |
| --- | --- |
| Symbol | `OpdAppointmentActionsDialog` |
| Purpose | Opd Appointment Actions |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `action_hub` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:45` |
...
| Paired opener(s) | `showOpdAppointmentActionsDialog` |
| Primary commit | Execute selected action / Cancel |
| Slices to keep in sync | appointment row, linked encounter entry points |
| Sibling reuse targets | `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog`, `PatientAppointmentQuickDialog` |
...
| Mutations (region) | `mutation: assignAppointmentToQueue`, `mutation: markAppointmentInProgress` |
...
| Title snippets | `l10n.receptionAppointmentActionsAction`, `widget.title`, `title`, `notesLabel` |
| `AppButton` variants (order seen) | secondary -> secondary -> secondary -> secondary -> primary |
| `AppActionIcons` | seen |
...
**Scope lock:** only `OpdAppointmentActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `OpdAppointmentActionsDialog` at `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:45` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Opd Appointment Actions** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Execute selected action / Cancel`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog`, `PatientAppointmentQuickDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch appointment row, linked encounter entry points, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showOpdAppointmentActionsDialog`. Update all call sites in the same change when signatures move.
...
1. `OpdAppointmentActionsDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog`, `PatientAppointmentQuickDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: appointment row, linked encounter entry points.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `OpdAppointmentActionsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 26 slug=opd-appointment-actions-dialog symbol=OpdAppointmentActionsDialog shape=action_hub -->
