...
| Title snippets | `widget.appointment.displayTitle` |
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Opd Appointment Actions (shared/opd_actions).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `OpdAppointmentActionsDialog` opens through `AppDialog` / approved helpers only.
