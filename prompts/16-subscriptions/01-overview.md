# Subscriptions Overview tab — rule compliance

## Context

Make the Overview desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/16-subscriptions/01-overview.md`. Apply shared chrome fixes from `prompts/16-subscriptions/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `Overview`, deep-link `section` per inventory, and omit the tab when gate denies access (`SubscriptionsOverviewAtomPermissions.tab` = ∩ `subscriptions:read` + module) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/16-subscriptions/00-overview.md`.
...
4. Toolbar order: **Filters → Settings → Create\*** — normalize shared labels to `Filters`, `Settings`, `Export`, and `Print` when those controls apply (`tables.mdc`, `printing.mdc`).
...
8. Preserve in-desk actions for this surface via shared dialogs/forms; omit unauthorized actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Cohort accounts list; New / Edit subscription.
...
- [ ] `tabs/16-subscriptions/01-overview.md` updated to match.
