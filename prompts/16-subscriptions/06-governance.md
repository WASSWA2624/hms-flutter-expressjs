# Subscriptions Governance tab — rule compliance

## Context

Make the Governance desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/16-subscriptions/06-governance.md`. Apply shared chrome fixes from `prompts/16-subscriptions/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `Licenses`, deep-link `section` per inventory, and omit the tab when gate denies access (`SubscriptionsLicensesAtomPermissions.tab` = read ∩) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/16-subscriptions/00-overview.md`.
...
4. Target trailing order Filters → Settings → Export → Print → context actions. Inventory today: **Filters → Settings → Add license**. Normalize shared labels to `Filters`, `Settings`, `Export`, and `Print` when those controls apply; keep inventoried context actions after Print (`tables.mdc`, `printing.mdc`).
...
8. Preserve row-select → detail/actions hub with **generic titles**; keep nested mutations in-desk via shared dialogs/forms; omit unauthorized nested actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: License detail; Add / Update license; Revoke confirm.
...
- [ ] `tabs/16-subscriptions/06-governance.md` updated to match.
