# Subscriptions overview — workspace compliance program

## Context

Bring the Subscriptions desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/16-subscriptions/`. Per-surface remediation prompts live beside this file under `prompts/16-subscriptions/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/16-subscriptions/00-overview.md` as the authoritative tab index (`desk section enum` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/16-subscriptions/01-overview.md`
   - `prompts/16-subscriptions/02-catalog.md`
   - `prompts/16-subscriptions/03-modules.md`
   - `prompts/16-subscriptions/04-operations.md`
   - `prompts/16-subscriptions/05-billing.md`
   - `prompts/16-subscriptions/06-governance.md`
   - `prompts/16-subscriptions/07-denied.md`
...
6. After each remediation, update the matching inventory file under `tabs/16-subscriptions/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/subscriptions/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/16-subscriptions/00-overview.md` has a completed remediation prompt under `prompts/16-subscriptions/` and a matching updated inventory under `tabs/16-subscriptions/`.
...
- Trace `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart`, `frontend/lib/features/subscriptions/presentation/subscriptions_access.dart`, and widgets listed in `tabs/16-subscriptions/00-overview.md`.
- Run feature tests under `frontend/test/features/subscriptions/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/16-subscriptions/00-overview.md`
- `prompts/16-subscriptions/00-shared-chrome.md`
- `prompts/16-subscriptions/99-convention-gaps.md`
- `prompts/16-subscriptions/01-overview.md`
- `prompts/16-subscriptions/02-catalog.md`
- `prompts/16-subscriptions/03-modules.md`
- `prompts/16-subscriptions/04-operations.md`
