# Subscriptions Billing tab — rule compliance

## Context

Make the Billing desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/16-subscriptions/05-billing.md`. Apply shared chrome fixes from `prompts/16-subscriptions/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `Invoices`, deep-link `section` per inventory, and omit the tab when gate denies access (`SubscriptionsInvoicesAtomPermissions.tab` = read ∩) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/16-subscriptions/00-overview.md`.
...
8. Preserve row-select → detail/actions hub with **generic titles**; keep nested mutations in-desk via shared dialogs/forms; omit unauthorized nested actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Invoice detail; Collect / Retry.
...
- [ ] `tabs/16-subscriptions/05-billing.md` updated to match.
