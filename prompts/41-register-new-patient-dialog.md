...
1. **Read contracts + source**
   ...
   - Read `RegisterNewPatientDialog` at `frontend/lib/shared/patient_actions/register_new_patient_dialog.dart:463` and every paired opener / *Used from* site.
...
8. **Preserve reachability**
   - Do not break `_openRegisterPatientDialog`. Update all call sites in the same change when signatures move.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_ReceptionPatientPickerDialog`, `OpdEncounterDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: patient registry lists/details, reception picker results.
