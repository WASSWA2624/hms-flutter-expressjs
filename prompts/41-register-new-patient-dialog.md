...
**Scope:** only `RegisterNewPatientDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Defined in | `frontend/lib/shared/patient_actions/register_new_patient_dialog.dart:463` |
| Kind | `shared` |
| Paired opener(s) | `_openRegisterPatientDialog` |
...
| Title snippets | `l10n.patientsRegisterNewPatientTitle` |
...
1. **Read contract + source**
   ...
   - Read `RegisterNewPatientDialog` at `frontend/lib/shared/patient_actions/register_new_patient_dialog.dart:463` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Register New Patient** (flow/action name — never the patient display name as `AppDialog` title).
   - Pass the title through the shell so uppercase normalization applies.
   - Add/keep a meaningful `icon` if peer dialogs in `shared/patient_actions` already use icons.
...
8. **Preserve reachability**
   - Do not break `_openRegisterPatientDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `RegisterNewPatientDialog`'s path.
...
<!-- generator: encounter-dialog prompt 41 slug=register-new-patient-dialog symbol=RegisterNewPatientDialog -->
