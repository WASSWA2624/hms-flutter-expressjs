...
3. **Normalize title + icon**
   - Use a general, role-based title for **Register New Patient** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `RegisterNewPatientDialog` opens through `AppDialog` / approved helpers only.
