...
| Field | Value |
| --- | --- |
| Symbol | `OpdDispositionDialog` |
| Purpose | Opd Disposition |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1988` |
...
| Paired opener(s) | `_openDisposition`, `showOpdDispositionDialog` |
| Primary commit | Save disposition |
| Slices to keep in sync | disposition, encounter completion cues, follow-up eligibility |
| Sibling reuse targets | `RoutingDecisionDialog`, `FollowUpDialog`, `ReferralDialog` |
...
| Mutations (region) | `mutation: completeDisposition` |
...
| Title snippets | `actionLabel`, `widget.title`, `showTitle ? l10n.opdEncounterContextTitle : null` |
...
**Scope lock:** only `OpdDispositionDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `OpdDispositionDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1988` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Opd Disposition** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save disposition`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `RoutingDecisionDialog`, `FollowUpDialog`, `ReferralDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch disposition, encounter completion cues, follow-up eligibility, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openDisposition`, `showOpdDispositionDialog`. Update all call sites in the same change when signatures move.
...
1. `OpdDispositionDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `RoutingDecisionDialog`, `FollowUpDialog`, `ReferralDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: disposition, encounter completion cues, follow-up eligibility.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `OpdDispositionDialog`'s path.
...
<!-- generator: encounter-dialog prompt 35 slug=opd-disposition-dialog symbol=OpdDispositionDialog shape=widget_dialog -->
