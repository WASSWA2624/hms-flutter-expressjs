...
**Scope:** only `OpdDispositionDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Purpose | Patient/encounter flow: Opd Disposition (shared/opd_actions). |
...
1. **Read contract + source**
   ...
   - Read `OpdDispositionDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1742` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Opd Disposition (shared/opd_actions).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `OpdDispositionDialog`'s path.
...
<!-- generator: encounter-dialog prompt 35 slug=opd-disposition-dialog symbol=OpdDispositionDialog -->
