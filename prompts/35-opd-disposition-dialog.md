...
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1742` |
...
1. **Read contracts + source**
   ...
   - Read `OpdDispositionDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1742` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `RoutingDecisionDialog`, `FollowUpDialog`, `ReferralDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: disposition, encounter completion cues, follow-up eligibility.
