...
2. Badge count must be the authoritative total for this tab’s scope (client combined `_tableItems` length (`allItems.length`)). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/03-opd/01-all.md` updated to match.
