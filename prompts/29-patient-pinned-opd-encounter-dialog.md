...
| Title snippets | `l10n.opdCheckInAction` |
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Patient Pinned Opd Encounter (shared/opd_actions).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `PatientPinnedOpdEncounterDialog` opens through `AppDialog` / approved helpers only.
