...
**Scope:** only `PatientPinnedOpdEncounterDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `opdRepositoryProvider`, `mutation-ish call: cancelEncounter`, `mutation-ish call: closeEncounter`, `mutation-ish call: submitOpdEncounter`, `mutation-ish call: cancelOpdEncounter`, `mutation-ish call: closeOpdEncounter`
...
| Purpose | Patient/encounter flow: Patient Pinned Opd Encounter (shared/opd_actions). |
...
| Title snippets | `l10n.opdWalkInDialogTitle` |
...
1. **Read contract + source**
   ...
   - Read `PatientPinnedOpdEncounterDialog` at `frontend/lib/shared/opd_actions/opd_encounter_flow.dart:75` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Patient Pinned Opd Encounter (shared/opd_actions).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `showPatientPinnedOpdEncounterDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `PatientPinnedOpdEncounterDialog`'s path.
...
<!-- generator: encounter-dialog prompt 29 slug=patient-pinned-opd-encounter-dialog symbol=PatientPinnedOpdEncounterDialog -->
