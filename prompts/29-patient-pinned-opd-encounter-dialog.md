...
| Verified definition | `frontend/lib/shared/opd_actions/opd_encounter_flow.dart:102` |
...
| Paired opener(s) | `openPatientOpdEncounterFlow`, `showPatientPinnedOpdEncounterDialog` |
...
| Mutations (region) | `mutation: cancelEncounter`, `mutation: closeEncounter` |
...
- `OpdEncounterDialog — frontend/lib/shared/components/opd_encounter_dialog.dart`
...
| Title snippets | `l10n.opdWalkInDialogTitle`, `l10n.opdLoadingTitle`, `widget.title`, `l10n.opdPatientSectionTitle` |
| `AppButton` variants (order seen) | secondary -> primary -> secondary -> secondary -> secondary -> secondary -> secondary -> primary |
| `AppActionIcons` | seen |
...
| Cross-stack trace files found | 7 |
| Peek region size | 4419 chars |
...
1. **Read contracts + source**
   ...
   - Read `PatientPinnedOpdEncounterDialog` at `frontend/lib/shared/opd_actions/opd_encounter_flow.dart:102` and every paired opener / *Used from* site.
...
9. **Preserve reachability**
   - Do not break `openPatientOpdEncounterFlow`, `showPatientPinnedOpdEncounterDialog`. Update all call sites in the same change when signatures move.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdEncounterDialog`, `showOpdEncounterDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: pinned encounter view, underlying encounter slices.
...
<!-- generator: encounter-dialog prompt 29 slug=patient-pinned-opd-encounter-dialog symbol=PatientPinnedOpdEncounterDialog shape=confirm -->
