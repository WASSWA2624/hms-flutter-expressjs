# Prompt 052 — Implement Bank Reconciliations

## Context

- **Sequence:** 052 of 159
- **Phase:** Treasury foundation — bank management
- **Prerequisite:** Prompt 051 — Matching Workbench (`prompts/01-finance-tabs/01-accounts-and-finance/04-bank-management/051-matching-workbench.md`).
- **Menu path:** `Accounts & Finance → Bank Management → Bank Reconciliations`
- **Canonical route:** `/accounts?section=bank-reconciliations`
- **Tab profile:** `reconciliation` table/worklist
...
10. **Add and run verification.**
    ...
    - Do not proceed to Prompt 053 until all acceptance criteria below pass.
...
- **AC1 (R1):** `/accounts?section=bank-reconciliations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC2 (R2):** All API operations in `.cursor/finance/accounts-and-finance/bank-management/bank-reconciliations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `frontend/lib/features/accounts/presentation/widgets/bank_reconciliations_tab.dart`
