# Prompt 098 — Implement Overview

## Context

- **Sequence:** 098 of 159
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** Prompt 097 — Disputes (`prompts/01-finance-tabs/05-insurance-and-claims/03-claims-and-settlement/097-disputes.md`).
- **Menu path:** `Insurance & Claims → Overview`
- **Canonical route:** `/claims?section=overview`
- **Tab profile:** `overview` table/worklist
...
10. **Add and run verification.**
    ...
    - Do not proceed to Prompt 099 until all acceptance criteria below pass.
...
- **AC1 (R1):** `/claims?section=overview` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC2 (R2):** All API operations in `.cursor/finance/insurance-and-claims/overview.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `frontend/lib/features/claims/presentation/widgets/overview_tab.dart`
