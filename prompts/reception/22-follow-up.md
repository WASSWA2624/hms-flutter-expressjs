...
Add Reception call worklist with schedule, complete, and reschedule. Follow `prompts/.cursor/prompt.mdc`.

## Context

Flow Actions **Follow up** and `POST /follow-ups` exist, but Reception lacks a call worklist. Follow-ups are optional at disposition or earlier.

**Follow-up:** encounter-linked `SCHEDULED` callback (`scheduled_at`, optional notes).
**Complete:** mark successful (`COMPLETED`).
**Reschedule:** create another `SCHEDULED` follow-up for ongoing calls.
...
3. On save, reuse patient contact; if missing, require capture.
...
6. Reuse on OPD, Emergency, Inpatient, ICU when already exposed; omit Billing/Lab.

## Constraints

- Reuse follow-up API, `ClinicalFollowUpActionDialog`, auth, localization, design-system; extend contracts only for list contact.
- Do not invent clinical stages or force follow-up for every patient.
...
- R5–R6: States clear; unauthorized UI absent; reuse excludes Billing/Lab.
- Update follow-up and reception tests; run analysis.
