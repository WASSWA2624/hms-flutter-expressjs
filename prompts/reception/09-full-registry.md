...
Rename Reception’s ambiguous registry action while preserving direct patient-registry navigation. Follow `prompts/.cursor/prompt.mdc`.
...
2. Keep the action secondary and preserve its placement, icon, responsive behavior, and availability across Reception tabs.
3. Navigate directly through the existing patient route (`AppRoutes.patients` / `/patients`). Do not open a dialog, preselect a patient, mutate data, or add an intermediate step.
4. Preserve route authorization. Render the action for authorized users; omit it when access is unauthorized or the module is inactive.
5. Update localization sources, generated output, semantics, tooltips, and tests derived from the old wording. Remove unused old wording.
...
- Preserve loading, error, theme, accessibility, and responsive behavior.
...
- Test label, route, authorization, tabs, semantics, and viewports; run localization generation and Flutter analysis.
