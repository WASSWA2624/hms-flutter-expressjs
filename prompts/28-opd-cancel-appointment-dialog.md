...
| Defined in | `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:392` |
...
| Title snippets | `l10n.opdCancelAction` |
...
1. **Read contracts + source**
   ...
   - Read `OpdCancelAppointmentDialog` at `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:392` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdRescheduleAppointmentDialog`, `_CancelEncounterDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: appointment status, queue links.
