...
| Field | Value |
| --- | --- |
| Symbol | `OpdCancelAppointmentDialog` |
| Purpose | Opd Cancel Appointment |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `confirm` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:488` |
...
| Paired opener(s) | `_openCancel`, `showOpdCancelAppointmentDialog` |
| Primary commit | Cancel appointment |
| Slices to keep in sync | appointment status, queue links |
| Sibling reuse targets | `OpdRescheduleAppointmentDialog`, `_CancelEncounterDialog` |
...
| Mutations (region) | `mutation: cancelAppointment` |
...
| Title snippets | `l10n.opdCancelAction`, `widget.title` |
...
**Scope lock:** only `OpdCancelAppointmentDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `confirm`
...
1. **Read contracts + source**
   ...
   - Read `OpdCancelAppointmentDialog` at `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:488` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Opd Cancel Appointment** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Cancel appointment`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `OpdRescheduleAppointmentDialog`, `_CancelEncounterDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch appointment status, queue links, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openCancel`, `showOpdCancelAppointmentDialog`. Update all call sites in the same change when signatures move.
...
1. `OpdCancelAppointmentDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
2. Confirmation has exactly Cancel + one domain verb/Confirm, with destructive styling when applicable.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdRescheduleAppointmentDialog`, `_CancelEncounterDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: appointment status, queue links.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `OpdCancelAppointmentDialog`'s path.
...
<!-- generator: encounter-dialog prompt 28 slug=opd-cancel-appointment-dialog symbol=OpdCancelAppointmentDialog shape=confirm -->
