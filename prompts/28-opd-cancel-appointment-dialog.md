...
**Scope:** only `OpdCancelAppointmentDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: cancelAppointment`
...
| Purpose | Patient/encounter flow: Opd Cancel Appointment (shared/opd_actions). |
...
1. **Read contract + source**
   ...
   - Read `OpdCancelAppointmentDialog` at `frontend/lib/shared/opd_actions/opd_appointment_actions_dialog.dart:392` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Opd Cancel Appointment (shared/opd_actions).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `OpdCancelAppointmentDialog`'s path.
...
<!-- generator: encounter-dialog prompt 28 slug=opd-cancel-appointment-dialog symbol=OpdCancelAppointmentDialog -->
