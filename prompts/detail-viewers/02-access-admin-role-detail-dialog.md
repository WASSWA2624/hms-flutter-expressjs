...
| Field | Value |
| --- | --- |
| Symbol | `_AccessAdminRoleDetailDialog` |
| Purpose | Access Admin Role Detail |
| Module / surface | `access_admin` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/access_admin/presentation/widgets/access_admin_management_dialogs.dart:1242` |
...
| Paired opener(s) | `_openCreateRoleDialog`, `_openRoleDetail` |
| Primary commit | Cancel (dismiss) — Edit only if already present |
| Slices to keep in sync | access_admin role detail |
| Sibling reuse targets | `_AccessAdminUserDetailDialog`, `showHrAccessRoleDetailDialog`, `_openDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch access_admin role detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.accessAdminCreateRoleDetailsSectionTitle`, `l10n.accessAdminRolePermissionsLabel`, `widget.title`, `title` |
...
**Scope lock:** only `_AccessAdminRoleDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_AccessAdminRoleDetailDialog` at `frontend/lib/features/access_admin/presentation/widgets/access_admin_management_dialogs.dart:1242` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Access Admin Role Detail** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Cancel (dismiss) — Edit only if already present`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_AccessAdminUserDetailDialog`, `showHrAccessRoleDetailDialog`, `_openDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch access_admin role detail only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openCreateRoleDialog`, `_openRoleDetail`. Update all call sites in the same change when signatures move.
...
1. `_AccessAdminRoleDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_AccessAdminUserDetailDialog`, `showHrAccessRoleDetailDialog`, `_openDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_AccessAdminRoleDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 02 slug=access-admin-role-detail-dialog symbol=_AccessAdminRoleDetailDialog shape=detail_viewer -->
