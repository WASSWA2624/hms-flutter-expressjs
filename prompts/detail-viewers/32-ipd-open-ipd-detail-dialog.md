...
| Module flow | [`.cursor/flows/ipd-flow.mdc`](../../.cursor/flows/ipd-flow.mdc) | Domain workflow states, transitions, and handoffs |

## Target

| Field | Value |
| --- | --- |
| Symbol | `_openIpdDetailDialog` |
| Purpose | Ipd Detail |
| Module / surface | `ipd` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:914` |
...
| Paired opener(s) | `_openIpdDetailDialogById` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | IPD workspace selection/detail |
| Sibling reuse targets | `openIcuDetailDialog`, `_openBedDetailDialog`, `NursingPatientDetailDialog` |
| Controllers (region) | `ipdWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `ipd`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch IPD workspace selection/detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.ipdAdmissionDetailTitle`, `widget.title` |
...
**Scope lock:** only `_openIpdDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openIpdDetailDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:914` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Ipd Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `ipd`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `openIcuDetailDialog`, `_openBedDetailDialog`, `NursingPatientDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch IPD workspace selection/detail only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openIpdDetailDialogById`. Update all call sites in the same change when signatures move.
...
1. `_openIpdDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `openIcuDetailDialog`, `_openBedDetailDialog`, `NursingPatientDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openIpdDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 32 slug=ipd-open-ipd-detail-dialog symbol=_openIpdDetailDialog shape=detail_viewer -->
