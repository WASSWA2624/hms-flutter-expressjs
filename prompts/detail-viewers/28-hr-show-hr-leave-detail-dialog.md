...
| Field | Value |
| --- | --- |
| Symbol | `showHrLeaveDetailDialog` |
| Purpose | Hr Leave Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_leave_detail_dialog.dart:7` |
...
| Title snippets | `l10n.hrLeaveDetailDialogTitle`, `widget.title` |
...
- Standardize `showHrLeaveDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `showHrLeaveDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrLeaveDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_leave_detail_dialog.dart:7` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Leave Detail** — never personal display name.
...
1. `showHrLeaveDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrLeaveDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 28 slug=hr-show-hr-leave-detail-dialog symbol=showHrLeaveDetailDialog shape=detail_viewer -->
