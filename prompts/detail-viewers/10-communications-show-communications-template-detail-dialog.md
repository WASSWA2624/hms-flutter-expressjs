...
| Field | Value |
| --- | --- |
| Symbol | `showCommunicationsTemplateDetailDialog` |
| Purpose | Communications Template Detail |
| Module / surface | `communications` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/communications/presentation/widgets/communications_detail_dialogs.dart:82` |
...
| Slices to keep in sync | communications template detail |
| Sibling reuse targets | `showCommunicationsNotificationDetailDialog`, `showCommunicationsDeliveryDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch communications template detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.communicationsTemplateDetailTitle`, `widget.title` |
...
**Scope lock:** only `showCommunicationsTemplateDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showCommunicationsTemplateDetailDialog` at `frontend/lib/features/communications/presentation/widgets/communications_detail_dialogs.dart:82` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Communications Template Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showCommunicationsNotificationDetailDialog`, `showCommunicationsDeliveryDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch communications template detail only after success.
...
1. `showCommunicationsTemplateDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showCommunicationsNotificationDetailDialog`, `showCommunicationsDeliveryDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showCommunicationsTemplateDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 10 slug=communications-show-communications-template-detail-dialog symbol=showCommunicationsTemplateDetailDialog shape=detail_viewer -->
