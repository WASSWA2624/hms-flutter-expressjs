...
| Field | Value |
| --- | --- |
| Symbol | `showHrWorkQueueDialog` |
| Purpose | Hr Work Queue |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_directory` |
| Verified definition | `frontend/lib/features/hr/presentation/pages/hr_workspace_dialog_actions.dart:4` |
...
| Slices to keep in sync | HR work-queue selection |
| Sibling reuse targets | `showHrStaffDirectoryDialog`, `_showWorkItemDialog`, `_showActivityDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR work-queue selection only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.hrWorkQueuesTitle`, `widget.title` |
...
**Scope lock:** only `showHrWorkQueueDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrWorkQueueDialog` at `frontend/lib/features/hr/presentation/pages/hr_workspace_dialog_actions.dart:4` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Work Queue** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrStaffDirectoryDialog`, `_showWorkItemDialog`, `_showActivityDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR work-queue selection only after success.
...
1. `showHrWorkQueueDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrStaffDirectoryDialog`, `_showWorkItemDialog`, `_showActivityDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrWorkQueueDialog`'s path.
...
<!-- generator: detail-viewer prompt 15 slug=hr-work-queue-dialog symbol=showHrWorkQueueDialog shape=detail_directory -->
