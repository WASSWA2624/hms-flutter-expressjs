...
| Field | Value |
| --- | --- |
| Symbol | `PatientDetailDialog` |
| Purpose | Patient Detail |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/patients/presentation/widgets/patient_detail_dialog_body.dart:29` |
...
| Paired opener(s) | `showPatientDetailDialog` |
| Primary commit | Cancel (dismiss) — secondary Edit/actions only when already present |
| Slices to keep in sync | patient registry selection/detail slices |
| Sibling reuse targets | `AppPatientDetailDialog`, `NursingPatientDetailDialog`, `_PatientReportPrintPreviewDialog` |
| Controllers (region) | `appAccessPolicyProvider`, `patientRegistryControllerProvider.notifier` |
| Mutations (region) | `mutation: updatePatient`, `mutation: deletePatient`, `mutation: createRelatedRecord`, `mutation: updateRelatedRecord`, `mutation: deleteRelatedRecord` |
...
- `PatientDetailActiveWorkPanel — frontend/lib/features/patients/presentation/widgets/patient_detail_active_work.dart`
- `PatientPharmacyContextPanel — frontend/lib/features/patients/presentation/widgets/patient_pharmacy_context_panel.dart`
- `PatientBillingContextPanel — frontend/lib/features/patients/presentation/widgets/patient_billing_context_panel.dart`
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `patients`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch patient registry selection/detail slices only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `dialogTitle`, `l10n.patientsDetailLoadingTitle`, `l10n.patientsDetailTitle`, `l10n.patientsNoSelectionTitle` |
| `AppButton` variants (order seen) | secondary -> tertiary -> secondary -> secondary |
...
| Cross-stack trace files found | 9 |
| Peek region size | 19423 chars |
...
2. No obvious shared loading primitive near the symbol — add `AppLoadingIndicator` / `AppLoadingSurface` for async open/load.
3. `AppButton` seen without `AppActionIcons` — every action needs a leading icon; use `AppActionIcons` for shared verbs.
4. One-off `Icons.*` detected — prefer `AppActionIcons` (or sibling domain icon conventions) when a shared mapping exists.
5. Title may include patient name — must be role/flow based
6. No `AppInfoSheet*` / `AppSectionPanel` delegation detected — replace bespoke key/value and section chrome with shared detail primitives.
...
**Scope lock:** only `PatientDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `PatientDetailDialog` at `frontend/lib/features/patients/presentation/widgets/patient_detail_dialog_body.dart:29` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `patients`.

4. **Normalize loading + footer (Req 3)**
   - Shared loading primitives only; rebuild actions with `AppButton` + `AppActionIcons` + l10n.
   - Order: secondary → **Cancel** (`Cancel (dismiss) — secondary Edit/actions only when already present`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `AppPatientDetailDialog`, `NursingPatientDetailDialog`, `_PatientReportPrintPreviewDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch patient registry selection/detail slices only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showPatientDetailDialog`. Update all call sites in the same change when signatures move.
...
1. `PatientDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `AppPatientDetailDialog`, `NursingPatientDetailDialog`, `_PatientReportPrintPreviewDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `PatientDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 39 slug=patient-detail-dialog symbol=PatientDetailDialog shape=detail_viewer -->
