...
| Field | Value |
| --- | --- |
| Symbol | `_openIntegrationDetailDialog` |
| Purpose | Integration Detail |
| Module / surface | `integrations` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart:429` |
...
| Slices to keep in sync | integrations detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | _scan inventory peers in the same module and shared detail surfaces_ |
| Controllers (region) | `integrationsWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `integrations`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch integrations detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `_detailTitle(context`, `widget.title` |
...
3. Possible Close label — chrome abort must be Cancel
...
- Standardize `_openIntegrationDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openIntegrationDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openIntegrationDetailDialog` at `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart:429` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Integration Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `integrations`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: _scan inventory peers in the same module and shared detail surfaces_.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch integrations detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openIntegrationDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: _scan inventory peers in the same module and shared detail surfaces_).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openIntegrationDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 31 slug=integrations-open-integration-detail-dialog symbol=_openIntegrationDetailDialog shape=detail_viewer -->
