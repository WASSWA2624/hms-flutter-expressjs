...
| Field | Value |
| --- | --- |
| Symbol | `_openClaimsDetailDialog` |
| Purpose | Claims Detail |
| Module / surface | `claims` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart:1318` |
...
| Slices to keep in sync | claims detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | _scan inventory peers in the same module and shared detail surfaces_ |
| Controllers (region) | `claimsWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `claims`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch claims detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.claimsDetailTitle`, `title`, `widget.title` |
...
- Standardize `_openClaimsDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openClaimsDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openClaimsDetailDialog` at `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart:1318` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Claims Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `claims`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch claims detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openClaimsDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openClaimsDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 07 slug=claims-open-claims-detail-dialog symbol=_openClaimsDetailDialog shape=detail_viewer -->
