...
| Field | Value |
| --- | --- |
| Symbol | `showHrScheduleTemplateDetailDialog` |
| Purpose | Hr Schedule Template Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_enhanced_dialogs.dart:615` |
...
| Paired opener(s) | `showHrManageScheduleTemplatesDialog` |
| Primary commit | Cancel (dismiss) — optional secondary Edit/Print only if already present |
| Slices to keep in sync | hr detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showActivityDialog`, `_showWorkItemDialog` |
| Controllers (region) | `hrWorkspaceControllerProvider.notifier` |
| Mutations (region) | `mutation: deleteShiftTemplate` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch hr detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `template.label`, `l10n.hrManageScheduleTemplatesTitle`, `widget.title` |
...
4. No `AppInfoSheet*` / `AppSectionPanel` delegation detected — replace bespoke key/value and section chrome with shared detail primitives.

### Dialog-specific focus

- Standardize `showHrScheduleTemplateDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `showHrScheduleTemplateDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrScheduleTemplateDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_enhanced_dialogs.dart:615` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Schedule Template Detail** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Cancel (dismiss) — optional secondary Edit/Print only if already present`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showActivityDialog`, `_showWorkItemDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch hr detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showHrManageScheduleTemplatesDialog`. Update all call sites in the same change when signatures move.
...
1. `showHrScheduleTemplateDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showActivityDialog`, `_showWorkItemDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrScheduleTemplateDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 27 slug=hr-show-hr-schedule-template-detail-dialog symbol=showHrScheduleTemplateDetailDialog shape=detail_viewer -->
