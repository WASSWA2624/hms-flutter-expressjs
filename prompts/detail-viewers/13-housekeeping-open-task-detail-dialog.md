...
| Field | Value |
| --- | --- |
| Symbol | `_openTaskDetailDialog` |
| Purpose | Task Detail |
| Module / surface | `housekeeping` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:941` |
...
| Slices to keep in sync | housekeeping detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | `_showReportPreviewDialog` |
| Controllers (region) | `housekeepingWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `housekeeping`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch housekeeping detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `dialogContext.l10n.housekeepingDetailTitle`, `widget.title` |
...
- Standardize `_openTaskDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openTaskDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openTaskDetailDialog` at `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:941` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Task Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `housekeeping`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_showReportPreviewDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch housekeeping detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openTaskDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_showReportPreviewDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openTaskDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 13 slug=housekeeping-open-task-detail-dialog symbol=_openTaskDetailDialog shape=detail_viewer -->
