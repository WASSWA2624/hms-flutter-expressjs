...
| Field | Value |
| --- | --- |
| Symbol | `showCommunicationsDeliveryDetailDialog` |
| Purpose | Communications Delivery Detail |
| Module / surface | `communications` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/communications/presentation/widgets/communications_detail_dialogs.dart:53` |
...
| Slices to keep in sync | communications delivery detail |
| Sibling reuse targets | `showCommunicationsNotificationDetailDialog`, `showCommunicationsTemplateDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch communications delivery detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.communicationsDeliveryDetailTitle`, `widget.title` |
...
**Scope lock:** only `showCommunicationsDeliveryDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showCommunicationsDeliveryDetailDialog` at `frontend/lib/features/communications/presentation/widgets/communications_detail_dialogs.dart:53` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Communications Delivery Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showCommunicationsNotificationDetailDialog`, `showCommunicationsTemplateDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch communications delivery detail only after success.
...
1. `showCommunicationsDeliveryDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showCommunicationsNotificationDetailDialog`, `showCommunicationsTemplateDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showCommunicationsDeliveryDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 09 slug=communications-show-communications-delivery-detail-dialog symbol=showCommunicationsDeliveryDetailDialog shape=detail_viewer -->
