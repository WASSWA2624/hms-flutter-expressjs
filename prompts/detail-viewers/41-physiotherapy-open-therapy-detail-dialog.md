...
| Field | Value |
| --- | --- |
| Symbol | `_openTherapyDetailDialog` |
| Purpose | Therapy Detail |
| Module / surface | `physiotherapy` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart:751` |
...
| Slices to keep in sync | physiotherapy detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `physiotherapy`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch physiotherapy detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `item.displayTitle`, `dialogContext.l10n.physiotherapyDetailLoadingTitle`, `widget.title` |
...
- Standardize `_openTherapyDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openTherapyDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openTherapyDetailDialog` at `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart:751` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Therapy Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `physiotherapy`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch physiotherapy detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openTherapyDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openTherapyDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 41 slug=physiotherapy-open-therapy-detail-dialog symbol=_openTherapyDetailDialog shape=detail_viewer -->
