...
| Field | Value |
| --- | --- |
| Symbol | `showHrStaffDirectoryDialog` |
| Purpose | Hr Staff Directory |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_directory` |
| Verified definition | `frontend/lib/features/hr/presentation/pages/hr_workspace_dialog_actions.dart:78` |
...
| Primary commit | Cancel (dismiss) — row open may hand off to staff detail |
| Slices to keep in sync | HR directory selection |
| Sibling reuse targets | `showHrStaffDetailDialog`, `showHrWorkQueueDialog`, `_HrAssignmentDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR directory selection only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.hrStaffDirectoryTitle`, `widget.title` |
...
**Scope lock:** only `showHrStaffDirectoryDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrStaffDirectoryDialog` at `frontend/lib/features/hr/presentation/pages/hr_workspace_dialog_actions.dart:78` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Staff Directory** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Cancel (dismiss) — row open may hand off to staff detail`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrStaffDetailDialog`, `showHrWorkQueueDialog`, `_HrAssignmentDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR directory selection only after success.
...
1. `showHrStaffDirectoryDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrStaffDetailDialog`, `showHrWorkQueueDialog`, `_HrAssignmentDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrStaffDirectoryDialog`'s path.
...
<!-- generator: detail-viewer prompt 16 slug=hr-staff-directory-dialog symbol=showHrStaffDirectoryDialog shape=detail_directory -->
