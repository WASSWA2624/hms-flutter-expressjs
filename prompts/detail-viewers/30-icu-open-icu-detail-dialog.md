...
| Field | Value |
| --- | --- |
| Symbol | `openIcuDetailDialog` |
| Purpose | ICU patient detail viewer |
| Module / surface | `icu` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:22` |
...
| Slices to keep in sync | ICU board selection/detail |
| Sibling reuse targets | `openEmergencyDetailDialog`, `_openIpdDetailDialog`, `NursingPatientDetailDialog` |
| Controllers (region) | `icuWorkspaceControllerProvider.notifier` |
...
- `IcuStayDetailPanel — frontend/lib/features/icu/presentation/widgets/icu_detail_panel.dart`
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `icu`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch ICU board selection/detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.icuStayDialogTitle`, `widget.title`, `l10n.icuDetailLoadingTitle`, `l10n.icuDetailEmptyTitle` |
...
**Scope lock:** only `openIcuDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `openIcuDetailDialog` at `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:22` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **ICU patient detail viewer** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `openEmergencyDetailDialog`, `_openIpdDetailDialog`, `NursingPatientDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch ICU board selection/detail only after success.
...
1. `openIcuDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `openEmergencyDetailDialog`, `_openIpdDetailDialog`, `NursingPatientDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `openIcuDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 30 slug=icu-open-icu-detail-dialog symbol=openIcuDetailDialog shape=detail_viewer -->
