...
| Field | Value |
| --- | --- |
| Symbol | `_SetupDetailDialog` |
| Purpose | Setup Detail |
| Module / surface | `tenant_facility` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/tenant_facility/presentation/pages/tenant_facility_setup_page.dart:175` |
...
| Paired opener(s) | `_openBedsModal`, `_openBranchesModal`, `_openDepartmentsModal`, `_openRoomsModal`, `_openUnitsModal`, `_openWardsModal` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | tenant_facility setup detail |
| Sibling reuse targets | `_TenantDetailsDialog`, `_FacilityDetailsDialog` |
| Controllers (region) | `appAccessPolicyProvider`, `tenantFacilitySetupControllerProvider`, `tenantFacilitySetupControllerProvider.notifier` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch tenant_facility setup detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `title`, `l10n.tenantFacilitySetupLoadingTitle`, `l10n.tenantFacilityBedsLabel`, `l10n.tenantFacilityBranchesSectionTitle` |
...
**Scope lock:** only `_SetupDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_SetupDetailDialog` at `frontend/lib/features/tenant_facility/presentation/pages/tenant_facility_setup_page.dart:175` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Setup Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_TenantDetailsDialog`, `_FacilityDetailsDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch tenant_facility setup detail only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openBedsModal`, `_openBranchesModal`, `_openDepartmentsModal`, `_openRoomsModal`, `_openUnitsModal`, `_openWardsModal`. Update all call sites in the same change when signatures move.
...
1. `_SetupDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_TenantDetailsDialog`, `_FacilityDetailsDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_SetupDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 49 slug=tenant-facility-setup-detail-dialog symbol=_SetupDetailDialog shape=detail_viewer -->
