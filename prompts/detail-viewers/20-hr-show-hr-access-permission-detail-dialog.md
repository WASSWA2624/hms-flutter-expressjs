...
| Field | Value |
| --- | --- |
| Symbol | `showHrAccessPermissionDetailDialog` |
| Purpose | Hr Access Permission Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_access_dialogs.dart:914` |
...
| Slices to keep in sync | HR access permission detail |
| Sibling reuse targets | `showHrAccessRoleDetailDialog`, `_HrAccessUserDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR access permission detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `permission.name ?? permission.effectiveId`, `widget.title` |
| `AppButton` variants (order seen) | primary |
...
**Scope lock:** only `showHrAccessPermissionDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrAccessPermissionDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_access_dialogs.dart:914` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Access Permission Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrAccessRoleDetailDialog`, `_HrAccessUserDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR access permission detail only after success.
...
1. `showHrAccessPermissionDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrAccessRoleDetailDialog`, `_HrAccessUserDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrAccessPermissionDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 20 slug=hr-show-hr-access-permission-detail-dialog symbol=showHrAccessPermissionDetailDialog shape=detail_viewer -->
