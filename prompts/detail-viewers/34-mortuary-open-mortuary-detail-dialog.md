...
| Field | Value |
| --- | --- |
| Symbol | `_openMortuaryDetailDialog` |
| Purpose | Mortuary case detail viewer |
| Module / surface | `mortuary` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/mortuary/presentation/pages/mortuary_workspace_page.dart:487` |
...
| Slices to keep in sync | mortuary detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | _scan inventory peers in the same module and shared detail surfaces_ |
| Controllers (region) | `mortuaryWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `mortuary`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch mortuary detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `dialogContext.l10n.mortuaryDetailTitle`, `widget.title` |
...
- Standardize `_openMortuaryDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openMortuaryDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openMortuaryDetailDialog` at `frontend/lib/features/mortuary/presentation/pages/mortuary_workspace_page.dart:487` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Mortuary case detail viewer** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `mortuary`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch mortuary detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openMortuaryDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openMortuaryDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 34 slug=mortuary-open-mortuary-detail-dialog symbol=_openMortuaryDetailDialog shape=detail_viewer -->
