...
| Field | Value |
| --- | --- |
| Symbol | `_RequestDetailsEditDialog` |
| Purpose | Request Details Edit |
| Module / surface | `radiology` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart:1488` |
...
| Paired opener(s) | `_showEditRequestDetailsDialog` |
| Primary commit | Edit (not Update) when persisting; otherwise Cancel |
| Slices to keep in sync | radiology request detail/edit slices |
| Sibling reuse targets | `_openRadiologyDetailDialog` |
| Controllers (region) | _not detected in peek — trace widget → workspace controller → repository → backend route_ |
| Mutations (region) | `mutation: updateOrderRequestDetails` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch radiology request detail/edit slices only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.radiologyEditRequestDetailsDialogTitle`, `widget.title` |
...
**Scope lock:** only `_RequestDetailsEditDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_RequestDetailsEditDialog` at `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart:1488` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Request Details Edit** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Edit (not Update) when persisting; otherwise Cancel`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_openRadiologyDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch radiology request detail/edit slices only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_showEditRequestDetailsDialog`. Update all call sites in the same change when signatures move.
...
1. `_RequestDetailsEditDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_openRadiologyDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_RequestDetailsEditDialog`'s path.
...
<!-- generator: detail-viewer prompt 43 slug=radiology-request-details-edit-dialog symbol=_RequestDetailsEditDialog shape=detail_viewer -->
