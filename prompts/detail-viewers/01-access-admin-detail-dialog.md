...
| Field | Value |
| --- | --- |
| Symbol | `_openDetailDialog` |
| Purpose | Detail |
| Module / surface | `access_admin` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart:262` |
...
| Slices to keep in sync | access_admin detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | `_AccessAdminRoleDetailDialog`, `_AccessAdminUserDetailDialog` |
| Controllers (region) | `accessAdminWorkspaceControllerProvider.notifier`, `accessAdminRepositoryProvider` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch access_admin detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `selected.title`, `widget.title` |
| `AppButton` variants (order seen) | secondary -> secondary -> secondary |
...
4. Widget reads a repository provider directly — move load ownership to a controller and keep server-backed UI state in Riverpod.
5. Possible Close label — chrome abort must be Cancel
...
- Standardize `_openDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openDetailDialog` at `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart:262` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_AccessAdminRoleDetailDialog`, `_AccessAdminUserDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch access_admin detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_AccessAdminRoleDetailDialog`, `_AccessAdminUserDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 01 slug=access-admin-detail-dialog symbol=_openDetailDialog shape=detail_viewer -->
