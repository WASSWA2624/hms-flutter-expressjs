...
| Field | Value |
| --- | --- |
| Symbol | `openReportDetailDialog` |
| Purpose | Report run/detail viewer |
| Module / surface | `reports` |
| Inventory kind | `custom` |
| Presentation shape | `detail_preview` |
| Verified definition | `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart:1330` |
...
| Slices to keep in sync | reports workspace selection/detail |
| Sibling reuse targets | `openComplianceDetailDialog`, `_showReportPreviewDialog`, `SubscriptionReportAdminsDialog` |
| Controllers (region) | `reportsWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `reports`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch reports workspace selection/detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.reportsPreviewTitle`, `widget.title` |
...
**Scope lock:** only `openReportDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `openReportDetailDialog` at `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart:1330` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Report run/detail viewer** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `reports`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `openComplianceDetailDialog`, `_showReportPreviewDialog`, `SubscriptionReportAdminsDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch reports workspace selection/detail only after success.
...
1. `openReportDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `openComplianceDetailDialog`, `_showReportPreviewDialog`, `SubscriptionReportAdminsDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `openReportDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 44 slug=reports-report-detail-dialog symbol=openReportDetailDialog shape=detail_preview -->
