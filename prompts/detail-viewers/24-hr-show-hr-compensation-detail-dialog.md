...
| Field | Value |
| --- | --- |
| Symbol | `showHrCompensationDetailDialog` |
| Purpose | Hr Compensation Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_compensation_dialog.dart:61` |
...
| Title snippets | `l10n.hrCompensationDetailDialogTitle`, `widget.title` |
...
- Standardize `showHrCompensationDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `showHrCompensationDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrCompensationDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_compensation_dialog.dart:61` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Compensation Detail** — never personal display name.
...
1. `showHrCompensationDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrCompensationDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 24 slug=hr-show-hr-compensation-detail-dialog symbol=showHrCompensationDetailDialog shape=detail_viewer -->
