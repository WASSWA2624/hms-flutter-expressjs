...
| Field | Value |
| --- | --- |
| Symbol | `NursingPrintSummaryDialog` |
| Purpose | Nursing Print Summary |
| Module / surface | `nursing` |
| Inventory kind | `custom` |
| Presentation shape | `detail_preview` |
| Verified definition | `frontend/lib/features/nursing/presentation/widgets/nursing_print_summary_dialog.dart:11` |
...
| Paired opener(s) | `_openPrintSummaryDialog` |
| Primary commit | Print (with Copy/secondary as applicable) then Cancel |
| Slices to keep in sync | no server mutation unless print audit exists — verify before patching |
| Sibling reuse targets | `NursingPatientDetailDialog`, `_PatientReportPrintPreviewDialog`, `_LabReportPreviewDialog`, `_showReportPreviewDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch no server mutation unless print audit exists — verify before patching only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.nursingActionPrintSummary`, `l10n.nursingReportTitle`, `widget.title` |
| `AppButton` variants (order seen) | tertiary |
...
**Scope lock:** only `NursingPrintSummaryDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `NursingPrintSummaryDialog` at `frontend/lib/features/nursing/presentation/widgets/nursing_print_summary_dialog.dart:11` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Nursing Print Summary** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Print (with Copy/secondary as applicable) then Cancel`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `NursingPatientDetailDialog`, `_PatientReportPrintPreviewDialog`, `_LabReportPreviewDialog`, `_showReportPreviewDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch no server mutation unless print audit exists — verify before patching only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPrintSummaryDialog`. Update all call sites in the same change when signatures move.
...
1. `NursingPrintSummaryDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `NursingPatientDetailDialog`, `_PatientReportPrintPreviewDialog`, `_LabReportPreviewDialog`, `_showReportPreviewDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `NursingPrintSummaryDialog`'s path.
...
<!-- generator: detail-viewer prompt 36 slug=nursing-print-summary-dialog symbol=NursingPrintSummaryDialog shape=detail_preview -->
