...
| Field | Value |
| --- | --- |
| Symbol | `_openAssetDetailDialog` |
| Purpose | Asset Detail |
| Module / surface | `biomedical` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart:819` |
...
| Slices to keep in sync | biomedical detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | _scan inventory peers in the same module and shared detail surfaces_ |
| Controllers (region) | `biomedicalWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `biomedical`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch biomedical detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `dialogContext.l10n.biomedicalDetailTitle`, `widget.title` |
...
- Standardize `_openAssetDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openAssetDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openAssetDetailDialog` at `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart:819` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Asset Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `biomedical`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch biomedical detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openAssetDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openAssetDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 06 slug=biomedical-open-asset-detail-dialog symbol=_openAssetDetailDialog shape=detail_viewer -->
