...
| Field | Value |
| --- | --- |
| Symbol | `showHrAccessRoleDetailDialog` |
| Purpose | Hr Access Role Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_access_dialogs.dart:954` |
...
| Slices to keep in sync | HR access role detail |
| Sibling reuse targets | `showHrAccessPermissionDetailDialog`, `_HrAccessUserDetailDialog`, `_AccessAdminRoleDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR access role detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `roleLabel`, `widget.title` |
...
**Scope lock:** only `showHrAccessRoleDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrAccessRoleDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_access_dialogs.dart:954` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Access Role Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrAccessPermissionDetailDialog`, `_HrAccessUserDetailDialog`, `_AccessAdminRoleDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR access role detail only after success.
...
1. `showHrAccessRoleDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrAccessPermissionDetailDialog`, `_HrAccessUserDetailDialog`, `_AccessAdminRoleDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrAccessRoleDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 21 slug=hr-show-hr-access-role-detail-dialog symbol=showHrAccessRoleDetailDialog shape=detail_viewer -->
