...
| Field | Value |
| --- | --- |
| Symbol | `_showActivityDialog` |
| Purpose | Activity |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart:468` |
...
| Sibling reuse targets | `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showWorkItemDialog`, `showHrAccessPermissionDetailDialog` |
...
| Title snippets | `l10n.hrActivityTitle`, `widget.title` |
...
- Standardize `_showActivityDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_showActivityDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_showActivityDialog` at `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart:468` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Activity** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showWorkItemDialog`, `showHrAccessPermissionDetailDialog`.
...
1. `_showActivityDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showWorkItemDialog`, `showHrAccessPermissionDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_showActivityDialog`'s path.
...
<!-- generator: detail-viewer prompt 18 slug=hr-show-activity-dialog symbol=_showActivityDialog shape=detail_viewer -->
