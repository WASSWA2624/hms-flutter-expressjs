...
| Field | Value |
| --- | --- |
| Symbol | `_showReportPreviewDialog` |
| Purpose | Report Preview |
| Module / surface | `housekeeping` |
| Inventory kind | `custom` |
| Presentation shape | `detail_preview` |
| Verified definition | `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:1988` |
...
| Primary commit | Print/Cancel as applicable — no invented mutation |
| Slices to keep in sync | housekeeping report preview only |
| Sibling reuse targets | `NursingPrintSummaryDialog`, `_PatientReportPrintPreviewDialog`, `openReportDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch housekeeping report preview only only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.housekeepingReportSummaryTitle`, `l10n.housekeepingReportPreviewTitle` |
...
**Scope lock:** only `_showReportPreviewDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_showReportPreviewDialog` at `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:1988` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Report Preview** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Print/Cancel as applicable — no invented mutation`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `NursingPrintSummaryDialog`, `_PatientReportPrintPreviewDialog`, `openReportDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch housekeeping report preview only only after success.
...
1. `_showReportPreviewDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `NursingPrintSummaryDialog`, `_PatientReportPrintPreviewDialog`, `openReportDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_showReportPreviewDialog`'s path.
...
<!-- generator: detail-viewer prompt 14 slug=housekeeping-report-preview-dialog symbol=_showReportPreviewDialog shape=detail_preview -->
