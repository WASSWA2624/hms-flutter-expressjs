...
| Field | Value |
| --- | --- |
| Symbol | `openComplianceDetailDialog` |
| Purpose | Compliance item detail viewer |
| Module / surface | `reports` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart:1360` |
...
| Slices to keep in sync | reports/compliance selection/detail |
| Sibling reuse targets | `openReportDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch reports/compliance selection/detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.reportsComplianceDetailTitle`, `widget.title` |
...
**Scope lock:** only `openComplianceDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `openComplianceDetailDialog` at `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart:1360` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Compliance item detail viewer** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `openReportDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch reports/compliance selection/detail only after success.
...
1. `openComplianceDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `openReportDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `openComplianceDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 45 slug=reports-compliance-detail-dialog symbol=openComplianceDetailDialog shape=detail_viewer -->
