...
| Field | Value |
| --- | --- |
| Symbol | `_showWorkItemDialog` |
| Purpose | Work Item |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart:1596` |
...
| Sibling reuse targets | `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showActivityDialog`, `showHrAccessPermissionDetailDialog` |
...
| Title snippets | `_workItemTitle(context`, `widget.title` |
...
| Peek region size | 383 chars |

### Priority gaps to close

1. No obvious shared loading primitive near the symbol — add `AppLoadingIndicator` / `AppLoadingSurface` for async open/load.
2. No `AppInfoSheet*` / `AppSectionPanel` delegation detected — replace bespoke key/value and section chrome with shared detail primitives.

### Dialog-specific focus

- Standardize `_showWorkItemDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_showWorkItemDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_showWorkItemDialog` at `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart:1596` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Work Item** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showActivityDialog`, `showHrAccessPermissionDetailDialog`.
...
1. `_showWorkItemDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrWorkQueueDialog`, `showHrStaffDirectoryDialog`, `showHrStaffDetailDialog`, `_showActivityDialog`, `showHrAccessPermissionDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_showWorkItemDialog`'s path.
...
<!-- generator: detail-viewer prompt 19 slug=hr-show-work-item-dialog symbol=_showWorkItemDialog shape=detail_viewer -->
