...
| Field | Value |
| --- | --- |
| Symbol | `showHrPreviewRosterDialog` |
| Purpose | Hr Preview Roster |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_preview` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_enhanced_dialogs.dart:510` |
...
| Slices to keep in sync | HR roster preview only |
| Sibling reuse targets | `showHrPreviewPayrollDialog`, `showHrScheduleTemplateDetailDialog`, `showHrShiftDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR roster preview only only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.hrPreviewRosterDialogTitle`, `widget.title` |
...
**Scope lock:** only `showHrPreviewRosterDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrPreviewRosterDialog` at `frontend/lib/features/hr/presentation/widgets/hr_enhanced_dialogs.dart:510` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Preview Roster** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrPreviewPayrollDialog`, `showHrScheduleTemplateDetailDialog`, `showHrShiftDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR roster preview only only after success.
...
1. `showHrPreviewRosterDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrPreviewPayrollDialog`, `showHrScheduleTemplateDetailDialog`, `showHrShiftDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrPreviewRosterDialog`'s path.
...
<!-- generator: detail-viewer prompt 26 slug=hr-preview-roster-dialog symbol=showHrPreviewRosterDialog shape=detail_preview -->
