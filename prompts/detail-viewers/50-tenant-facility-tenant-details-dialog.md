...
| Field | Value |
| --- | --- |
| Symbol | `_TenantDetailsDialog` |
| Purpose | Tenant Details |
| Module / surface | `tenant_facility` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/tenant_facility/presentation/widgets/tenant_facility_management_dialogs.dart:801` |
...
| Paired opener(s) | `showTenantDetailsDialog` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | tenant_facility tenant detail |
| Sibling reuse targets | `_FacilityDetailsDialog`, `_SetupDetailDialog`, `_openSubscriptionDetailDialog` |
| Controllers (region) | `appAccessPolicyProvider` |
| Mutations (region) | `mutation: deleteTenant`, `mutation: deleteFacility` |
...
- `backend/src/tests/modules/tenant/services/tenant.service.test.js`
- `backend/src/tests/modules/tenant/controllers/tenant.controller.test.js`
- `backend/src/modules/tenant/services/tenant.service.js`
- `backend/src/modules/tenant/routes/tenant.routes.js`
- `backend/src/modules/tenant/controllers/tenant.controller.js`
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch tenant_facility tenant detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.tenantFacilityDeleteConfirmationTitle`, `l10n.tenantFacilityTenantDetailsTitle`, `widget.title` |
...
| Peek region size | 11800 chars |

### Priority gaps to close

1. Possible Close label — chrome abort must be Cancel
...
**Scope lock:** only `_TenantDetailsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_TenantDetailsDialog` at `frontend/lib/features/tenant_facility/presentation/widgets/tenant_facility_management_dialogs.dart:801` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Tenant Details** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_FacilityDetailsDialog`, `_SetupDetailDialog`, `_openSubscriptionDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch tenant_facility tenant detail only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showTenantDetailsDialog`. Update all call sites in the same change when signatures move.
...
1. `_TenantDetailsDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_FacilityDetailsDialog`, `_SetupDetailDialog`, `_openSubscriptionDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_TenantDetailsDialog`'s path.
...
<!-- generator: detail-viewer prompt 50 slug=tenant-facility-tenant-details-dialog symbol=_TenantDetailsDialog shape=detail_viewer -->
