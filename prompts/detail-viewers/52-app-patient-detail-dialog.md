...
| Field | Value |
| --- | --- |
| Symbol | `AppPatientDetailDialog` |
| Purpose | Shared patient detail content shell |
| Module / surface | `shared/components` |
| Inventory kind | `shared` |
| Presentation shape | `shared_detail_shell` |
| Verified definition | `frontend/lib/shared/components/app_patient_detail_dialog.dart:5` |
...
| Primary commit | Cancel (dismiss) — optional secondary actions injected by callers |
| Slices to keep in sync | none by itself — callers own Riverpod slices |
| Sibling reuse targets | `PatientDetailDialog`, `NursingPatientDetailDialog`, `openEmergencyDetailDialog`, `openIcuDetailDialog`, `_openIpdDetailDialog` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `shared/components`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch none by itself — callers own Riverpod slices only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `title`, `widget.title` |
...
**Scope lock:** only `AppPatientDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `shared_detail_shell`
...
1. **Read contracts + source**
   ...
   - Read `AppPatientDetailDialog` at `frontend/lib/shared/components/app_patient_detail_dialog.dart:5` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Shared patient detail content shell** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Cancel (dismiss) — optional secondary actions injected by callers`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `PatientDetailDialog`, `NursingPatientDetailDialog`, `openEmergencyDetailDialog`, `openIcuDetailDialog`, `_openIpdDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch none by itself — callers own Riverpod slices only after success.
...
1. `AppPatientDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
2. Shell preserves caller-injected actions + Cancel/close slot; remains domain-neutral and reusable.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `PatientDetailDialog`, `NursingPatientDetailDialog`, `openEmergencyDetailDialog`, `openIcuDetailDialog`, `_openIpdDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `AppPatientDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 52 slug=app-patient-detail-dialog symbol=AppPatientDetailDialog shape=shared_detail_shell -->
