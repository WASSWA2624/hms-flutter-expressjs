...
| Module flow | [`.cursor/flows/radiology-flow.mdc`](../../.cursor/flows/radiology-flow.mdc) | Domain workflow states, transitions, and handoffs |

## Target

| Field | Value |
| --- | --- |
| Symbol | `_openRadiologyDetailDialog` |
| Purpose | Radiology Detail |
| Module / surface | `radiology` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart:834` |
...
| Slices to keep in sync | radiology detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | `_RequestDetailsEditDialog` |
| Controllers (region) | `radiologyWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `radiology`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch radiology detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.radiologyDetailTitle`, `widget.title` |
...
- Standardize `_openRadiologyDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openRadiologyDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openRadiologyDetailDialog` at `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart:834` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Radiology Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `radiology`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_RequestDetailsEditDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch radiology detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openRadiologyDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_RequestDetailsEditDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openRadiologyDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 42 slug=radiology-open-radiology-detail-dialog symbol=_openRadiologyDetailDialog shape=detail_viewer -->
