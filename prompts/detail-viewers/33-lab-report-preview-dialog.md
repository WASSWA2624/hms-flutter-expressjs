...
| Module flow | [`.cursor/flows/lab-flow.mdc`](../../.cursor/flows/lab-flow.mdc) | Domain workflow states, transitions, and handoffs |

## Target

| Field | Value |
| --- | --- |
| Symbol | `_LabReportPreviewDialog` |
| Purpose | Lab Report Preview |
| Module / surface | `lab` |
| Inventory kind | `custom` |
| Presentation shape | `detail_preview` |
| Verified definition | `frontend/lib/features/lab/presentation/pages/lab_result_entry_dialog.dart:2541` |
...
| Paired opener(s) | `_openPrintPreview`, `_openRejectDialog` |
| Primary commit | Cancel / Print as already present — no invented mutation |
| Slices to keep in sync | lab result preview only unless audit API proven |
| Sibling reuse targets | `NursingPrintSummaryDialog`, `_PatientReportPrintPreviewDialog`, `AppClinicalResultsPreview` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `lab`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch lab result preview only unless audit API proven only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.labReportPreviewTitle`, `l10n.labReportTitle`, `widget.title` |
...
**Scope lock:** only `_LabReportPreviewDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_LabReportPreviewDialog` at `frontend/lib/features/lab/presentation/pages/lab_result_entry_dialog.dart:2541` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Lab Report Preview** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `lab`.

4. **Normalize loading + footer (Req 3)**
   - Shared loading primitives only; rebuild actions with `AppButton` + `AppActionIcons` + l10n.
   - Order: secondary → **Cancel** (`Cancel / Print as already present — no invented mutation`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `NursingPrintSummaryDialog`, `_PatientReportPrintPreviewDialog`, `AppClinicalResultsPreview`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch lab result preview only unless audit API proven only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPrintPreview`, `_openRejectDialog`. Update all call sites in the same change when signatures move.
...
1. `_LabReportPreviewDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `NursingPrintSummaryDialog`, `_PatientReportPrintPreviewDialog`, `AppClinicalResultsPreview`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_LabReportPreviewDialog`'s path.
...
<!-- generator: detail-viewer prompt 33 slug=lab-report-preview-dialog symbol=_LabReportPreviewDialog shape=detail_preview -->
