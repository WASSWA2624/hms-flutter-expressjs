...
| Module flow | [`.cursor/flows/pharmacy-flow.mdc`](../../.cursor/flows/pharmacy-flow.mdc) | Domain workflow states, transitions, and handoffs |

## Target

| Field | Value |
| --- | --- |
| Symbol | `_openPharmacyDetailDialog` |
| Purpose | Pharmacy Detail |
| Module / surface | `pharmacy` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart:794` |
...
| Slices to keep in sync | pharmacy detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | _scan inventory peers in the same module and shared detail surfaces_ |
| Controllers (region) | `pharmacyWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `pharmacy`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch pharmacy detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.pharmacyPrescriptionDetailTitle`, `widget.title` |
...
- Standardize `_openPharmacyDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openPharmacyDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openPharmacyDetailDialog` at `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart:794` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Pharmacy Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `pharmacy`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch pharmacy detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openPharmacyDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openPharmacyDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 40 slug=pharmacy-open-pharmacy-detail-dialog symbol=_openPharmacyDetailDialog shape=detail_viewer -->
