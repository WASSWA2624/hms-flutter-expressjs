...
| Module flow | [`.cursor/flows/nursing-flow.mdc`](../../.cursor/flows/nursing-flow.mdc) | Domain workflow states, transitions, and handoffs |

## Target

| Field | Value |
| --- | --- |
| Symbol | `NursingPatientDetailDialog` |
| Purpose | Nursing Patient Detail |
| Module / surface | `nursing` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/nursing/presentation/widgets/nursing_patient_detail_dialog.dart:34` |
...
| Paired opener(s) | `openNursingPatientDetailDialog` |
| Primary commit | Cancel (dismiss) — nursing secondary actions remain permission-gated |
| Slices to keep in sync | nursing selection/detail, related clinical badges |
| Sibling reuse targets | `AppPatientDetailDialog`, `PatientDetailDialog`, `NursingPrintSummaryDialog`, `openIcuDetailDialog` |
| Controllers (region) | `nursingWorkspaceControllerProvider`, `nursingWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `nursing`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch nursing selection/detail, related clinical badges only after HTTP success, then apply the smallest targeted reconciliation.
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | `AppDialog`, approved `show*` helper, `AppPatientDetailDialog` |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| Raw Material progress indicator | not seen |
| Title snippets | `summary?.displayTitle ?? l10n.nursingPatientContextLabel`, `l10n.nursingNoSelectionTitle`, `title` |
...
| Peek region size | 1593 chars |

### Priority gaps to close

1. No obvious shared loading primitive near the symbol — add `AppLoadingIndicator` / `AppLoadingSurface` for async open/load.
2. `AppButton` seen without `AppActionIcons` — every action needs a leading icon; use `AppActionIcons` for shared verbs.
3. One-off `Icons.*` detected — prefer `AppActionIcons` (or sibling domain icon conventions) when a shared mapping exists.
4. Possible Close label — chrome abort must be Cancel
5. No `AppInfoSheet*` / `AppSectionPanel` delegation detected — replace bespoke key/value and section chrome with shared detail primitives.
...
**Scope lock:** only `NursingPatientDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `NursingPatientDetailDialog` at `frontend/lib/features/nursing/presentation/widgets/nursing_patient_detail_dialog.dart:34` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Nursing Patient Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `nursing`.

4. **Normalize loading + footer (Req 3)**
   - Shared loading primitives only; rebuild actions with `AppButton` + `AppActionIcons` + l10n.
   - Order: secondary → **Cancel** (`Cancel (dismiss) — nursing secondary actions remain permission-gated`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `AppPatientDetailDialog`, `PatientDetailDialog`, `NursingPrintSummaryDialog`, `openIcuDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch nursing selection/detail, related clinical badges only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `openNursingPatientDetailDialog`. Update all call sites in the same change when signatures move.
...
1. `NursingPatientDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `AppPatientDetailDialog`, `PatientDetailDialog`, `NursingPrintSummaryDialog`, `openIcuDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `NursingPatientDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 35 slug=nursing-patient-detail-dialog symbol=NursingPatientDetailDialog shape=detail_viewer -->
