...
| Field | Value |
| --- | --- |
| Symbol | `_openBedDetailDialog` |
| Purpose | Bed Detail |
| Module / surface | `rooms_beds` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart:608` |
...
| Slices to keep in sync | rooms/beds selection/detail |
| Sibling reuse targets | `_openIpdDetailDialog`, `openIcuDetailDialog` |
| Controllers (region) | `roomsBedsWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `rooms_beds`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch rooms/beds selection/detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.roomsBedsDetailTitle`, `widget.title` |
...
**Scope lock:** only `_openBedDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openBedDetailDialog` at `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart:608` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Bed Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_openIpdDetailDialog`, `openIcuDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch rooms/beds selection/detail only after success.
...
1. `_openBedDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_openIpdDetailDialog`, `openIcuDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openBedDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 46 slug=rooms-beds-open-bed-detail-dialog symbol=_openBedDetailDialog shape=detail_viewer -->
