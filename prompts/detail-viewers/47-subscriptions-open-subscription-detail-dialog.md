...
| Field | Value |
| --- | --- |
| Symbol | `_openSubscriptionDetailDialog` |
| Purpose | Subscription Detail |
| Module / surface | `subscriptions` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart:3340` |
...
| Slices to keep in sync | subscriptions detail/workspace selection slices (realtime reconcile only unless a mutation is proven) |
| Sibling reuse targets | `SubscriptionReportAdminsDialog` |
| Controllers (region) | `subscriptionsWorkspaceControllerProvider.notifier`, `subscriptionsWorkspaceControllerProvider` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `subscriptions`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch subscriptions detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `dialogTitle`, `_SubscriptionsText.loadingTitle`, `widget.title` |
| `AppButton` variants (order seen) | secondary -> secondary |
...
- Standardize `_openSubscriptionDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_openSubscriptionDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_openSubscriptionDetailDialog` at `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart:3340` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Subscription Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `subscriptions`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `SubscriptionReportAdminsDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch subscriptions detail/workspace selection slices (realtime reconcile only unless a mutation is proven) only after success.
...
1. `_openSubscriptionDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `SubscriptionReportAdminsDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openSubscriptionDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 47 slug=subscriptions-open-subscription-detail-dialog symbol=_openSubscriptionDetailDialog shape=detail_viewer -->
