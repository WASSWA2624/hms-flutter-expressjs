...
| Field | Value |
| --- | --- |
| Symbol | `_BillingLedgerDialog` |
| Purpose | Billing Ledger |
| Module / surface | `billing` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/billing/presentation/widgets/billing_ledger_dialog.dart:32` |
...
| Paired opener(s) | `showBillingLedgerDialog` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | billing ledger/detail slices (realtime reconcile only) |
| Sibling reuse targets | `_showBillingDetailDialog`, `PatientDetailDialog` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `billing`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch billing ledger/detail slices (realtime reconcile only) only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.billingLedgerTitle`, `widget.title` |
...
**Scope lock:** only `_BillingLedgerDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_BillingLedgerDialog` at `frontend/lib/features/billing/presentation/widgets/billing_ledger_dialog.dart:32` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Billing Ledger** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `billing`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_showBillingDetailDialog`, `PatientDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch billing ledger/detail slices (realtime reconcile only) only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showBillingLedgerDialog`. Update all call sites in the same change when signatures move.
...
1. `_BillingLedgerDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_showBillingDetailDialog`, `PatientDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_BillingLedgerDialog`'s path.
...
<!-- generator: detail-viewer prompt 05 slug=billing-ledger-dialog symbol=_BillingLedgerDialog shape=detail_viewer -->
