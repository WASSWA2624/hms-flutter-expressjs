...
| Field | Value |
| --- | --- |
| Symbol | `_showBillingDetailDialog` |
| Purpose | Billing Detail |
| Module / surface | `billing` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart:644` |
...
| Primary commit | Cancel (dismiss) — secondary Edit only if already present |
| Slices to keep in sync | billing workspace selection/detail |
| Sibling reuse targets | `_BillingLedgerDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch billing workspace selection/detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `billingDetailTitle(context`, `widget.title` |
...
**Scope lock:** only `_showBillingDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_showBillingDetailDialog` at `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart:644` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Billing Detail** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Cancel (dismiss) — secondary Edit only if already present`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_BillingLedgerDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch billing workspace selection/detail only after success.
...
1. `_showBillingDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_BillingLedgerDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_showBillingDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 04 slug=billing-show-billing-detail-dialog symbol=_showBillingDetailDialog shape=detail_viewer -->
