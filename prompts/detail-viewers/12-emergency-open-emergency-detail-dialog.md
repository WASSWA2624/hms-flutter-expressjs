...
| Field | Value |
| --- | --- |
| Symbol | `openEmergencyDetailDialog` |
| Purpose | Emergency encounter detail viewer |
| Module / surface | `emergency` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1921` |
...
| Primary commit | Cancel (dismiss) — secondary actions stay permission-gated |
| Slices to keep in sync | emergency case detail/queue selection |
| Sibling reuse targets | `openIcuDetailDialog`, `_openIpdDetailDialog`, `PatientDetailDialog`, `AppPatientDetailDialog` |
| Controllers (region) | `emergencyWorkspaceControllerProvider.notifier` |
...
- `EmergencyDetailPanel — frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart`
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `emergency`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch emergency case detail/queue selection only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.emergencyCaseDialogTitle`, `widget.title`, `'Loading emergency case'`, `'No case selected'` |
...
**Scope lock:** only `openEmergencyDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `openEmergencyDetailDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1921` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Emergency encounter detail viewer** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Cancel (dismiss) — secondary actions stay permission-gated`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `openIcuDetailDialog`, `_openIpdDetailDialog`, `PatientDetailDialog`, `AppPatientDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch emergency case detail/queue selection only after success.
...
1. `openEmergencyDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `openIcuDetailDialog`, `_openIpdDetailDialog`, `PatientDetailDialog`, `AppPatientDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `openEmergencyDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 12 slug=emergency-open-emergency-detail-dialog symbol=openEmergencyDetailDialog shape=detail_viewer -->
