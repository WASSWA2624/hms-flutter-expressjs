...
| Field | Value |
| --- | --- |
| Symbol | `_HrAccessUserDetailDialog` |
| Purpose | Hr Access User Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_access_dialogs.dart:1097` |
...
| Paired opener(s) | `showHrAccessUserDetailDialog` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | HR access user detail |
| Sibling reuse targets | `showHrAccessRoleDetailDialog`, `showHrAccessPermissionDetailDialog`, `_AccessAdminUserDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR access user detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.accessAdminUserAccessRemoveRoleConfirmTitle`, `title`, `l10n.hrAccessUserDetailTitle`, `widget.title` |
| `AppButton` variants (order seen) | primary -> primary -> secondary -> secondary -> primary |
| `AppActionIcons` | seen |
...
**Scope lock:** only `_HrAccessUserDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_HrAccessUserDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_access_dialogs.dart:1097` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Access User Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrAccessRoleDetailDialog`, `showHrAccessPermissionDetailDialog`, `_AccessAdminUserDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR access user detail only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showHrAccessUserDetailDialog`. Update all call sites in the same change when signatures move.
...
1. `_HrAccessUserDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrAccessRoleDetailDialog`, `showHrAccessPermissionDetailDialog`, `_AccessAdminUserDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_HrAccessUserDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 22 slug=hr-access-user-detail-dialog symbol=_HrAccessUserDetailDialog shape=detail_viewer -->
