...
| Field | Value |
| --- | --- |
| Symbol | `SubscriptionReportAdminsDialog` |
| Purpose | Subscription Report Admins |
| Module / surface | `subscriptions` |
| Inventory kind | `custom` |
| Presentation shape | `detail_directory` |
| Verified definition | `frontend/lib/features/subscriptions/presentation/widgets/subscription_report_admins_dialog.dart:35` |
...
| Paired opener(s) | `showSubscriptionReportAdminsDialog` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | subscriptions report-admins presentation |
| Sibling reuse targets | `_openSubscriptionDetailDialog`, `showHrStaffDirectoryDialog` |
| Controllers (region) | `sessionStateProvider`, `sessionStateProvider.notifier` |
...
- `frontend/lib/features/subscriptions/presentation/widgets/subscription_expired_prompt.dart`

### Delegated/shared implementation evidence

- `AppDialog — frontend/lib/shared/components/app_dialog.dart`
- `AppMessagePanel — frontend/lib/shared/components/app_content_panel.dart`
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch subscriptions report-admins presentation only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `expired`, `l10n.subscriptionReportFacilityAdminsLabel`, `l10n.subscriptionReportTenantAdminsLabel`, `widget.title` |
...
**Scope lock:** only `SubscriptionReportAdminsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `detail_directory`
...
1. **Read contracts + source**
   ...
   - Read `SubscriptionReportAdminsDialog` at `frontend/lib/features/subscriptions/presentation/widgets/subscription_report_admins_dialog.dart:35` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Subscription Report Admins** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `_openSubscriptionDetailDialog`, `showHrStaffDirectoryDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch subscriptions report-admins presentation only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showSubscriptionReportAdminsDialog`. Update all call sites in the same change when signatures move.
...
1. `SubscriptionReportAdminsDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
2. Directory supports Cancel plus row-open handoff; it does not invent a generic primary mutation commit.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `_openSubscriptionDetailDialog`, `showHrStaffDirectoryDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `SubscriptionReportAdminsDialog`'s path.
...
<!-- generator: detail-viewer prompt 48 slug=subscription-report-admins-dialog symbol=SubscriptionReportAdminsDialog shape=detail_directory -->
