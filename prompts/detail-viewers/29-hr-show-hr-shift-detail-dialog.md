...
| Field | Value |
| --- | --- |
| Symbol | `showHrShiftDetailDialog` |
| Purpose | Hr Shift Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_shift_detail_dialog.dart:8` |
...
| Title snippets | `l10n.hrShiftDetailDialogTitle`, `widget.title` |
...
- Standardize `showHrShiftDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `showHrShiftDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrShiftDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_shift_detail_dialog.dart:8` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Shift Detail** — never personal display name.
...
1. `showHrShiftDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrShiftDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 29 slug=hr-show-hr-shift-detail-dialog symbol=showHrShiftDetailDialog shape=detail_viewer -->
