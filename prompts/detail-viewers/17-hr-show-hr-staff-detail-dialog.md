...
| Field | Value |
| --- | --- |
| Symbol | `showHrStaffDetailDialog` |
| Purpose | Hr Staff Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/pages/hr_workspace_dialog_actions.dart:137` |
...
| Paired opener(s) | `_openStaffDetailFromDialog` |
| Primary commit | Cancel (dismiss) |
| Slices to keep in sync | HR staff detail selection |
| Sibling reuse targets | `showHrStaffDirectoryDialog`, `_HrAssignmentDetailDialog`, `_HrAccessUserDetailDialog` |
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch HR staff detail selection only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `l10n.hrStaffDetailTitle`, `widget.title` |
...
**Scope lock:** only `showHrStaffDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showHrStaffDetailDialog` at `frontend/lib/features/hr/presentation/pages/hr_workspace_dialog_actions.dart:137` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Staff Detail** — never personal display name.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showHrStaffDirectoryDialog`, `_HrAssignmentDetailDialog`, `_HrAccessUserDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch HR staff detail selection only after success.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openStaffDetailFromDialog`. Update all call sites in the same change when signatures move.
...
1. `showHrStaffDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showHrStaffDirectoryDialog`, `_HrAssignmentDetailDialog`, `_HrAccessUserDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showHrStaffDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 17 slug=hr-show-hr-staff-detail-dialog symbol=showHrStaffDetailDialog shape=detail_viewer -->
