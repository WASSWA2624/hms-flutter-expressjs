...
| Field | Value |
| --- | --- |
| Symbol | `showCommunicationsNotificationDetailDialog` |
| Purpose | Communications Notification Detail |
| Module / surface | `communications` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/communications/presentation/widgets/communications_detail_dialogs.dart:13` |
...
| Slices to keep in sync | communications notification detail |
| Sibling reuse targets | `showCommunicationsDeliveryDetailDialog`, `showCommunicationsTemplateDetailDialog` |
| Controllers (region) | `communicationsWorkspaceControllerProvider.notifier` |
...
- [ ] Every `AppButton` has a leading icon and localized label. Use `AppActionIcons` for shared verbs; match sibling detail viewers in `communications`.
...
- [ ] Detail viewers usually do not mutate. If a secondary action persists: patch communications notification detail only after HTTP success, then apply the smallest targeted reconciliation.
...
| Title snippets | `context.l10n.communicationsNotificationDetailTitle`, `widget.title` |
...
**Scope lock:** only `showCommunicationsNotificationDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `showCommunicationsNotificationDetailDialog` at `frontend/lib/features/communications/presentation/widgets/communications_detail_dialogs.dart:13` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Communications Notification Detail** — never personal display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `communications`.
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `showCommunicationsDeliveryDetailDialog`, `showCommunicationsTemplateDetailDialog`.
...
8. **Backend + sync (Req 5)**
   ...
   - Do not invent mutations. If a secondary action already persists → patch communications notification detail only after success.
...
1. `showCommunicationsNotificationDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `showCommunicationsDeliveryDetailDialog`, `showCommunicationsTemplateDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `showCommunicationsNotificationDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 08 slug=communications-show-communications-notification-detail-dialog symbol=showCommunicationsNotificationDetailDialog shape=detail_viewer -->
