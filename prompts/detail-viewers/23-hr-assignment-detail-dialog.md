...
| Field | Value |
| --- | --- |
| Symbol | `_HrAssignmentDetailDialog` |
| Purpose | Hr Assignment Detail |
| Module / surface | `hr` |
| Inventory kind | `custom` |
| Presentation shape | `detail_viewer` |
| Verified definition | `frontend/lib/features/hr/presentation/widgets/hr_assignment_detail_dialog.dart:30` |
...
| Paired opener(s) | `showHrAssignmentDetailDialog` |
...
| Title snippets | `l10n.hrAssignmentDetailDialogTitle`, `widget.title` |
...
- Standardize `_HrAssignmentDetailDialog` onto shared detail layout primitives under `frontend/lib/shared/`.
...
**Scope lock:** only `_HrAssignmentDetailDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_HrAssignmentDetailDialog` at `frontend/lib/features/hr/presentation/widgets/hr_assignment_detail_dialog.dart:30` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Hr Assignment Detail** — never personal display name.
...
9. **Preserve reachability**
   - Do not break `showHrAssignmentDetailDialog`. Update all call sites in the same change when signatures move.
...
1. `_HrAssignmentDetailDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_HrAssignmentDetailDialog`'s path.
...
<!-- generator: detail-viewer prompt 23 slug=hr-assignment-detail-dialog symbol=_HrAssignmentDetailDialog shape=detail_viewer -->
