...
| Field | Value |
| --- | --- |
| Symbol | `_PatientReportPrintPreviewDialog` |
| Purpose | Patient Report Print Preview |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `detail_preview` |
| Verified definition | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2244` |
...
| Paired opener(s) | `_openPatientQuickAction` |
| Primary commit | Print (secondary Copy if present) then Cancel |
| Slices to keep in sync | no server mutation unless print audit exists — verify before patching |
| Sibling reuse targets | `NursingPrintSummaryDialog`, `_LabReportPreviewDialog`, `PatientDetailDialog` |
| Controllers (region) | _not detected in peek — trace widget → workspace controller → repository → backend route_ |
| Mutations (region) | `mutation: recordPatientReportPrintEvent` |
...
| Title snippets | `l10n.patientsReportPreviewDialogTitle`, `document.title`, `document.periodLabel`, `widget.title` |
...
| Cross-stack trace files found | 3 |
| Peek region size | 6929 chars |

### Priority gaps to close

1. `AppButton` seen without `AppActionIcons` — every action needs a leading icon; use `AppActionIcons` for shared verbs.
...
4. No shared report/clinical preview delegation detected — migrate onto `AppReportPreviewPanel` / `AppClinicalResultsPreview` (or extract one).
...
**Scope lock:** only `_PatientReportPrintPreviewDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_PatientReportPrintPreviewDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2244` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Report Print Preview** — never personal display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** (`Print (secondary Copy if present) then Cancel`).
...
5. **Reuse shared detail primitives (Req 2 — hard)**
   ...
   - Cross-check sibling reuse targets: `NursingPrintSummaryDialog`, `_LabReportPreviewDialog`, `PatientDetailDialog`.
...
9. **Preserve reachability**
   - Do not break `_openPatientQuickAction`. Update all call sites in the same change when signatures move.
...
1. `_PatientReportPrintPreviewDialog` opens only through `AppDialog` / `AppPatientDetailDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections reuse canonical shared detail primitives; no unjustified local forks (siblings considered: `NursingPrintSummaryDialog`, `_LabReportPreviewDialog`, `PatientDetailDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_PatientReportPrintPreviewDialog`'s path.
...
<!-- generator: detail-viewer prompt 38 slug=patient-report-print-preview-dialog symbol=_PatientReportPrintPreviewDialog shape=detail_preview -->
