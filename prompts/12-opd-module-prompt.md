...
3. Re-read executing-department flows triggered from OPD: [lab-flow.mdc](../.cursor/flows/lab-flow.mdc), [radiology-flow.mdc](../.cursor/flows/radiology-flow.mdc), [pharmacy-flow.mdc](../.cursor/flows/pharmacy-flow.mdc); upstream [emergency-flow.mdc](../.cursor/flows/emergency-flow.mdc).
4. Re-read [app-write-up.mdc](../.cursor/app-write-up.mdc) — OPD flow owns arrival, queues, routing, completion; Clinical owns consultation content; Billing owns cashier workflows.
...
| §7 OPD→IPD handoff | Doctor disposition `ADMIT` sets stage `ADMITTED`; open IPD workspace via shell route with deep-link pre-selection — admission workflow stays in IPD modals |
