...
**Scope:** only `_PatientFlowQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `effectiveDefaultCurrencyProvider`, `mutation-ish call: submitOpdEncounter`
...
| Purpose | Patient/encounter flow: Patient Flow Quick (patients). |
...
| Title snippets | `l10n.patientsBillingDialogTitle` |
...
1. **Read contract + source**
   ...
   - Read `_PatientFlowQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2733` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Patient Flow Quick (patients).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_openPatientFlowQuickDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_PatientFlowQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 17 slug=patients-patient-flow-quick-dialog symbol=_PatientFlowQuickDialog -->
