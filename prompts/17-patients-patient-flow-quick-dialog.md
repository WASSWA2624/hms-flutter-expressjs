...
| Field | Value |
| --- | --- |
| Symbol | `_PatientFlowQuickDialog` |
| Purpose | Patient Billing Quick |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2663` |
...
| Paired opener(s) | `_openPatientFlowQuickDialog` |
| Primary commit | Create billing / start OPD flow |
| Slices to keep in sync | OPD encounter, consultation invoice/payment, patient registry detail |
| Sibling reuse targets | `ConsultationPaymentDialog` |
| Action helper peek | `clinicalActionDialogActions` |
| Controllers (region) | `effectiveDefaultCurrencyProvider` |
| Mutations (region) | `mutation: submitOpdEncounter` |
...
| Title snippets | `l10n.patientsBillingDialogTitle`, `l10n.patientsWorkflowSectionTitle`, `l10n.patientsBillingSectionTitle`, `l10n.patientsMarkPaymentReceivedLabel` |
...
**Scope lock:** only `_PatientFlowQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_PatientFlowQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2663` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Billing Quick** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Create billing / start OPD flow`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `ConsultationPaymentDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch OPD encounter, consultation invoice/payment, patient registry detail, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPatientFlowQuickDialog`. Update all call sites in the same change when signatures move.
...
1. `_PatientFlowQuickDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `ConsultationPaymentDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: OPD encounter, consultation invoice/payment, patient registry detail.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_PatientFlowQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 17 slug=patients-patient-flow-quick-dialog symbol=_PatientFlowQuickDialog shape=widget_dialog -->
