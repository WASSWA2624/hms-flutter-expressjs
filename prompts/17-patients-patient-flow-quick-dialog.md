...
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2680` |
| Kind | `custom` |
| Paired opener(s) | `—` |
...
| Title snippets | `_dialogTitle(l10n` |
...
1. **Read contract + source**
   ...
   - Read `_PatientFlowQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2680` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Patient Flow Quick (patients).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_PatientFlowQuickDialog` opens through `AppDialog` / approved helpers only.
