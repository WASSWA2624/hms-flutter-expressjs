# Standardize `_PatientFlowQuickDialog` — Patient Flow Quick

## Mission

Open patient flow/stage actions from registry without duplicating `FlowActionsDialog` chrome.
...
| Purpose | Patient Flow Quick |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `action_hub` |
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2733` |
| Extends / uses | AppDialog / showAppDialog (typical) |
| Paired opener(s) | `_openPatientFlowQuickDialog` |
| Primary commit | Apply flow action / Cancel |
| Slices to keep in sync | encounter stage, patient flow badges, OPD queue cues |
| Sibling reuse targets | `FlowActionsDialog`, `QueueActionsDialog` |
...
1. **Read contracts + source**
   ...
   - Read `_PatientFlowQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2733` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Flow Quick** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Apply flow action / Cancel`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `FlowActionsDialog`, `QueueActionsDialog`.
...
7. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch encounter stage, patient flow badges, OPD queue cues, then apply the smallest targeted reconciliation.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FlowActionsDialog`, `QueueActionsDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: encounter stage, patient flow badges, OPD queue cues.
...
<!-- generator: encounter-dialog prompt 17 slug=patients-patient-flow-quick-dialog symbol=_PatientFlowQuickDialog shape=action_hub -->
