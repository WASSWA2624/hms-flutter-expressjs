...
Implement the **Need approval** desk section (`?section=approvals`) on `/billing` per `billing.md`. This queue lists refunds, voids, and adjustments awaiting approval. Source of truth: root `billing.md` §§2–8, 9–11.
...
10. Gate tab with (`billing:read` ∪ `billing:write`) ∩ `billing-payments`. Omit unauthorized Approve / Reject; no disabled “no access” chrome.
...
- Reuse Billing workspace page, controller, approval access requirement, Detail shell, and Approve / Reject dialogs.
...
- Manual check: empty/loading/error, optional Type/By/Reason columns, viewports, themes.
