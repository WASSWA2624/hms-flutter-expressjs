...
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1826` |
...
| Title snippets | `l10n.opdReferAction` |
...
1. **Read contracts + source**
   ...
   - Read `ReferralDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1826` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FollowUpDialog`, `OpdDispositionDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: referral record, encounter routing cues.
