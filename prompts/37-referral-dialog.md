...
| Field | Value |
| --- | --- |
| Symbol | `ReferralDialog` |
| Purpose | Referral |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:2222` |
...
| Paired opener(s) | `showReferralDialog` |
| Primary commit | Save referral |
| Slices to keep in sync | referral record, encounter routing cues |
| Sibling reuse targets | `FollowUpDialog`, `OpdDispositionDialog` |
...
| Mutations (region) | `mutation: createReferral` |
...
- `ClinicalReferralActionDialog — frontend/lib/shared/clinical_actions/dialogs/clinical_referral_action_dialog.dart`
...
- `backend/src/modules/referral/services/referral.service.js`
- `backend/src/modules/referral/routes/referral.routes.js`
- `backend/src/modules/referral/controllers/referral.controller.js`
- `backend/src/tests/modules/referral/controllers/referral.controller.test.js`
- `backend/src/tests/modules/referral/services/referral.service.test.js`
...
| Title snippets | `l10n.opdReferAction`, `title`, `l10n.clinicalReferralDetailsTitle`, `l10n.clinicalReferralNotesTitle` |
...
**Scope lock:** only `ReferralDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `ReferralDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:2222` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Referral** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save referral`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `FollowUpDialog`, `OpdDispositionDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch referral record, encounter routing cues, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showReferralDialog`. Update all call sites in the same change when signatures move.
...
1. `ReferralDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FollowUpDialog`, `OpdDispositionDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: referral record, encounter routing cues.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `ReferralDialog`'s path.
...
<!-- generator: encounter-dialog prompt 37 slug=referral-dialog symbol=ReferralDialog shape=widget_dialog -->
