# Billing & Sections Scan — Theater workspace / Follow-ups (`/theater?…=follow-ups`)
...
- Screen inventory: `screens/theater.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Follow-ups** (`follow-ups`).
- Feature code: `frontend/lib/features/theater/`
- Module entitlement: `theatre-anesthesia`
...
- Financial focus for this tab: Theatre/procedure packages, consumables, implants, and pre-op billing gates must create Billing lines; recovery completion must not skip unsettled required charges without documented deferral. Tab role: No Schedule case primary.
...
9. Add/update tests under `frontend/test/features/theater/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
