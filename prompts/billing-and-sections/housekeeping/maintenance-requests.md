# Billing & Sections Scan — Housekeeping workspace / Maintenance requests (`/housekeeping?…=maintenance`)
...
- Screen inventory: `screens/housekeeping.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Maintenance requests** (`maintenance`).
- Feature code: `frontend/lib/features/housekeeping/`
...
- Financial focus for this tab: Patient-billable room turnover or private-room cleaning surcharges (if configured) must hit Billing; staff-only tasks must remain NOT_BILLED. Tab role: Request maintenance primary.
...
9. Add/update tests under `frontend/test/features/housekeeping/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
