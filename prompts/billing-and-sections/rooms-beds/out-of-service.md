# Billing & Sections Scan — Rooms & beds / Out of service (`/rooms-beds?…=out-of-service`)
...
- Screen inventory: `screens/rooms-beds.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Out of service** (`out-of-service`).
- Feature code: `frontend/lib/features/rooms_beds/`
...
- Financial focus for this tab: Bed assignment/turnover that incurs room or bed charges must update Billing line items; out-of-service must not silently drop billable occupancy charges already accrued. Tab role: Mark in/out of service needs manage/write.
...
9. Add/update tests under `frontend/test/features/rooms_beds/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
...
- `backend/src/modules/rooms-beds/`
