# Billing & Sections Scan — OPD workspace / Arrivals (`/opd?…=arrivals`)
...
- Financial focus for this tab: Primary focus: arrival registration fees and consult payment gate entry into Billing. Consultation fees, triage-linked services, and payment gates (WAITING_CONSULTATION_PAYMENT) must bill via clinical-request / OPD billing sync. Completing consult must not skip unpaid required charges unless policy explicitly defers and records the deferral in Billing. Tab role: Check-in / arrival processing.
...
9. Add/update tests under `frontend/test/features/opd/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
