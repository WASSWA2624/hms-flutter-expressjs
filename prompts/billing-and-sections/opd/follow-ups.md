# Billing & Sections Scan — OPD workspace / Follow-ups (`/opd?…=follow-ups`)
...
- Financial focus for this tab: Consultation fees, triage-linked services, and payment gates (WAITING_CONSULTATION_PAYMENT) must bill via clinical-request / OPD billing sync. Completing consult must not skip unpaid required charges unless policy explicitly defers and records the deferral in Billing. Tab role: Shared follow-up worklist; no Start OPD primary.
