# Billing & Sections Scan — Patients registry / Admitted (`/patients?…=admitted`)
...
- Financial focus for this tab: Patient registration itself is free, but nested quick actions that request consults, labs, imaging, pharmacy, procedures, admission, insurance enrollment, deposits, or collect balances must create or update Billing records. Balance-due surfaces must reflect live outstanding balances from Billing. Tab role: Inpatient admissions. Row/detail may surface admit/discharge actions requiring clinical:write (and billing:read for financial status) in addition to patient:read.
