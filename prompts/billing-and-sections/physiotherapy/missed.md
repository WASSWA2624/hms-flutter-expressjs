# Billing & Sections Scan — Physiotherapy workspace / Missed (`/physiotherapy?…=missed`)
...
- Financial focus for this tab: Therapy session packs, individual sessions, missed-session fees (if policy), and referral acceptance that starts a paid plan must create Billing records and settle before or per package rules. Tab role: Missed sessions; reschedule needs write.
