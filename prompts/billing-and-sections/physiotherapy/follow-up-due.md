# Billing & Sections Scan — Physiotherapy workspace / Follow-up due (`/physiotherapy?…=follow-up`)
...
- Screen inventory: `screens/physiotherapy.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Follow-up due** (`follow-up`).
...
- Financial focus for this tab: Therapy session packs, individual sessions, missed-session fees (if policy), and referral acceptance that starts a paid plan must create Billing records and settle before or per package rules. Tab role: Due therapy follow-ups.
...
9. Add/update tests under `frontend/test/features/physiotherapy/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
