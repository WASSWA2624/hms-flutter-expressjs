# Billing & Sections Scan — Physiotherapy workspace / Today (`/physiotherapy?…=today`)
...
- Financial focus for this tab: Therapy session packs, individual sessions, missed-session fees (if policy), and referral acceptance that starts a paid plan must create Billing records and settle before or per package rules. Tab role: Today's sessions.
