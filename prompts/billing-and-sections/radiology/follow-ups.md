# Billing & Sections Scan — Radiology workspace / Follow-ups (`/radiology?…=follow-ups`)
...
- Screen inventory: `screens/radiology.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Follow-ups** (`follow-ups`).
- Feature code: `frontend/lib/features/radiology/`
- Module entitlement: `radiology-workflows`
...
- Financial focus for this tab: Imaging orders must bill at request (or documented exception); reporting/release must keep payment status consistent with Billing. Tab role: No Request imaging primary.
...
9. Add/update tests under `frontend/test/features/radiology/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
