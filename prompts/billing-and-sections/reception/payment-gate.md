# Billing & Sections Scan — Reception desk / Payment gate (`/reception?…=payment-gate`)
...
- Screen inventory: `screens/reception.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Payment gate** (`payment-gate`).
- Feature code: `frontend/lib/features/reception/`
- Module entitlement: `patient-registry + scheduling-queue`
...
- Financial focus for this tab: Primary focus: collect or clear payment holds so clinical progress can continue; every clearance must settle or defer via Billing. Desk flows often gate clinical progress on payment. Appointments, queue, visits, and payment-gate actions that collect fees, deposits, or clear holds must settle through Billing—not local-only flags. Tab role: Billing hold / payment clearance before clinical progress.
...
9. Add/update tests under `frontend/test/features/reception/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
