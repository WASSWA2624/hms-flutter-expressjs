# Billing & Sections Scan — Reception desk / High priority (`/reception?…=high-priority`)
...
- Target tab: **High priority** (`high-priority`).
...
- Financial focus for this tab: Desk flows often gate clinical progress on payment. Appointments, queue, visits, and payment-gate actions that collect fees, deposits, or clear holds must settle through Billing—not local-only flags. Tab role: Escalated desk items; emergency handoff may need emergency:read.
