# Billing & Sections Scan — Reception desk / Appointments (`/reception?…=appointments`)
...
- Target tab: **Appointments** (`appointments`).
...
- Financial focus for this tab: Desk flows often gate clinical progress on payment. Appointments, queue, visits, and payment-gate actions that collect fees, deposits, or clear holds must settle through Billing—not local-only flags. Tab role: Schedule, check-in, reschedule, cancel appointments.
