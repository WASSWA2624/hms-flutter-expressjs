# Billing & Sections Scan — Communications workspace / Deliveries (`/communications?…=deliveries`)
...
- Target tab: **Deliveries** (`deliveries`).
...
- Financial focus for this tab: Paid notification/SMS packages are tenant commercial charges if present; patient clinical Billing must not be bypassed by message send actions that imply paid care delivery. Tab role: Delivery logs; prefer read.
