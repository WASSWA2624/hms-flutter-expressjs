# Billing & Sections Scan — Communications workspace / Templates (`/communications?…=templates`)
...
- Screen inventory: `screens/communications.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Templates** (`templates`).
- Feature code: `frontend/lib/features/communications/`
- Module entitlement: `notifications-communications`
...
- Financial focus for this tab: Paid notification/SMS packages are tenant commercial charges if present; patient clinical Billing must not be bypassed by message send actions that imply paid care delivery. Tab role: Template CRUD for writers; delete gated.
...
9. Add/update tests under `frontend/test/features/communications/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
...
- `frontend/test/features/communications/`
