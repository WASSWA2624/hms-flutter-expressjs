# Billing & Sections Scan — Operations workspace / In progress (`/operations?…=in-progress`)
...
- Screen inventory: `screens/operations.md` (read-only reference for reachable controls; do not modify).
- Target tab: **In progress** (`in-progress`).
- Feature code: `frontend/lib/features/operations/`
- Module entitlement: `facilities-maintenance`
...
- Financial focus for this tab: Chargeable facility/service requests and billable asset services must post to Billing when they have a patient/encounter payer context; internal-only work must be explicitly NOT_BILLED. Tab role: In-progress work.
...
9. Add/update tests under `frontend/test/features/operations/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
...
- `frontend/test/features/operations/`
