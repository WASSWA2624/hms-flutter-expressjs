# Billing & Sections Scan — Pharmacy workspace / Pending payment (`/pharmacy?…=pending-payment`)
...
- Screen inventory: `screens/pharmacy.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Pending payment** (`pending-payment`).
- Feature code: `frontend/lib/features/pharmacy/`
- Module entitlement: `pharmacy-dispensing`
...
- Financial focus for this tab: Primary focus: block dispense until Billing shows required lines paid or explicitly waived/deferred with audit. Dispense, partial fill, and pending-payment gates must collect or confirm settlement through Billing before stock leaves; returns/refunds must reverse or credit Billing lines without duplicate charges. Tab role: Payment gate before dispense.
...
9. Add/update tests under `frontend/test/features/pharmacy/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
