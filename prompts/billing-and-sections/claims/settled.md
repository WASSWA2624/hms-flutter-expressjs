# Billing & Sections Scan — Claims workspace / Settled (`/claims?…=settled`)
...
- Screen inventory: `screens/claims.md` (read-only reference for reachable controls; do not modify).
- Target tab: **Settled** (`settled`).
...
- Financial focus for this tab: Primary focus: remittance settlement must close or adjust patient responsibility lines in Billing without duplicate receipts. Authorizations, claim prep/submit/amend, settlement, co-pay, and insurance catalog pricing must keep patient invoices, coverage splits, and remittances synchronized with Billing. Tab role: Read-heavy settled claims; exports may need evidence:export or reports:read.
...
9. Add/update tests under `frontend/test/features/claims/` and `backend/src/tests/` proving (a) billable action posts a Billing record, (b) no bypass, (c) payment status parity with Billing, (d) idempotent replay, (e) unauthorized users cannot collect/adjust, (f) no section-in-section nesting on authorized UI. Cover integration, reuse, authorization, sync, UI states, one mobile + one desktop viewport, light + dark.
