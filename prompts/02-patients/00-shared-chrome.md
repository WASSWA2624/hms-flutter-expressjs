# Patients registry shared chrome — rule compliance

## Context

Remediate cross-tab Patients registry chrome so it fully complies with `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`. Inventory baseline: `tabs/02-patients/00-shared-chrome.md`. Gaps detail: `tabs/02-patients/99-convention-gaps.md`.
...
4. Keep table trailing action order exactly: **Filters → Settings → Export → Register patient** (normalize to Filters → Settings → Export → Print → context when Print applies). Use shared labels that resolve to `Filters`, `Settings`, `Export`, and `Print` (`tables.mdc`, `printing.mdc`). Prefer `commonFiltersActionLabel` / `commonPrintActionLabel` (or equivalent keys that resolve to those strings).
