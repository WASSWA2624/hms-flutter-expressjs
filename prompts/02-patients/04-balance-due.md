# Patients registry Balance Due tab — rule compliance

## Context

Make the Balance Due desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/02-patients/04-balance-due.md`. Apply shared chrome fixes from `prompts/02-patients/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `patientsTabBalanceDue`, deep-link `balance-due` (aliases `balance_due`, `balancedue`), and omit the tab when gate denies access (`PatientBalanceDueAtomPermissions.tab` = ∩ `patient:read` + `billing:read`) — never show a disabled placeholder (`tabs.mdc`). Align `PatientRegistrySection` / query helpers with `tabs/02-patients/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.overview.unpaidInvoices`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [x] `tabs/02-patients/04-balance-due.md` updated to match.
