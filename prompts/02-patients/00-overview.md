# Patients registry overview — workspace compliance program

## Context

Bring the Patients registry desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/02-patients/`. Per-surface remediation prompts live beside this file under `prompts/02-patients/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/02-patients/00-overview.md` as the authoritative tab index (`PatientRegistrySection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/02-patients/01-all.md`
   - `prompts/02-patients/02-active.md`
   - `prompts/02-patients/03-admitted.md`
   - `prompts/02-patients/04-balance-due.md`
...
6. After each remediation, update the matching inventory file under `tabs/02-patients/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/patients/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/02-patients/00-overview.md` has a completed remediation prompt under `prompts/02-patients/` and a matching updated inventory under `tabs/02-patients/`.
...
- Trace `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart`, `frontend/lib/features/patients/presentation/patient_registry_access.dart`, and widgets listed in `tabs/02-patients/00-overview.md`.
- Run feature tests under `frontend/test/features/patients/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/02-patients/00-overview.md`
- `prompts/02-patients/00-shared-chrome.md`
- `prompts/02-patients/99-convention-gaps.md`
- `prompts/02-patients/01-all.md`
- `prompts/02-patients/02-active.md`
- `prompts/02-patients/03-admitted.md`
- `prompts/02-patients/04-balance-due.md`
