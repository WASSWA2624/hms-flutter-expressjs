# Pharmacy overview — workspace compliance program

## Context

Bring the Pharmacy desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/12-pharmacy/`. Per-surface remediation prompts live beside this file under `prompts/12-pharmacy/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/12-pharmacy/00-overview.md` as the authoritative tab index (`PharmacyDeskSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/12-pharmacy/01-queue.md`
   - `prompts/12-pharmacy/02-in-progress.md`
   - `prompts/12-pharmacy/03-pending-payment.md`
   - `prompts/12-pharmacy/04-completed.md`
   - `prompts/12-pharmacy/05-cancelled.md`
   - `prompts/12-pharmacy/06-all-orders.md`
   - `prompts/12-pharmacy/07-catalog.md`
   - `prompts/12-pharmacy/08-suppliers.md`
   - `prompts/12-pharmacy/09-near-expiry.md`
   - `prompts/12-pharmacy/10-expired.md`
   - `prompts/12-pharmacy/11-low-stock.md`
   - `prompts/12-pharmacy/12-out-of-stock.md`
...
6. After each remediation, update the matching inventory file under `tabs/12-pharmacy/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/pharmacy/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/12-pharmacy/00-overview.md` has a completed remediation prompt under `prompts/12-pharmacy/` and a matching updated inventory under `tabs/12-pharmacy/`.
...
- Trace `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart`, `frontend/lib/features/pharmacy/presentation/pharmacy_access.dart`, and widgets listed in `tabs/12-pharmacy/00-overview.md`.
- Run feature tests under `frontend/test/features/pharmacy/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/12-pharmacy/00-overview.md`
- `prompts/12-pharmacy/00-shared-chrome.md`
- `prompts/12-pharmacy/99-convention-gaps.md`
- `prompts/12-pharmacy/01-queue.md`
- `prompts/12-pharmacy/02-in-progress.md`
- `prompts/12-pharmacy/03-pending-payment.md`
- `prompts/12-pharmacy/04-completed.md`
