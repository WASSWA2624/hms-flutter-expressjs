...
**Scope:** only `_openReleaseBedDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.

**Module / surface:** `ipd`  
**Inventory kind:** `custom`  
**Extends / uses (inventory):** `AppConfirmActionDialog` / `showAppDialog`  
**Action helper peek:** _none detected — adopt an approved action helper when the footer fits_  
**Controller / mutation peek:** `mutation-ish call: releaseBed`
...
| Purpose | Patient/encounter flow: Release Bed (ipd). |
...
1. **Read contract + source**
   ...
   - Read `_openReleaseBedDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:1353` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Release Bed (ipd).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_openReleaseBedDialog`'s path.
...
<!-- generator: encounter-dialog prompt 09 slug=ipd-release-bed-dialog symbol=_openReleaseBedDialog -->
