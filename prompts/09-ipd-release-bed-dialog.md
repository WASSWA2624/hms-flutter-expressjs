...
| Field | Value |
| --- | --- |
| Symbol | `_openReleaseBedDialog` |
| Purpose | Release Bed |
| Module / surface | `ipd` |
| Inventory kind | `custom` |
| Presentation shape | `inline_confirm` |
| Verified definition | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:1352` |
| Inventory location note | Inventory and verified declaration agree at generation time. |
| Extends / uses | `AppConfirmActionDialog` / `showAppDialog` |
| Paired opener(s) | _none listed in inventory_ |
| Primary commit | Release bed |
| Slices to keep in sync | IPD admission, bed occupancy, rooms/beds workspace |
| Sibling reuse targets | `TransferUpdateDialog`, `_showTransferUpdateDialog` |
...
| Mutations (region) | `mutation: releaseBed` |
...
| Title snippets | `l10n.ipdReleaseBedAction`, `widget.title` |
...
**Scope lock:** only `_openReleaseBedDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `inline_confirm`
...
1. **Read contracts + source**
   ...
   - Read `_openReleaseBedDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:1352` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Release Bed** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Release bed`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `TransferUpdateDialog`, `_showTransferUpdateDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch IPD admission, bed occupancy, rooms/beds workspace, then apply the smallest targeted reconciliation.
...
1. `_openReleaseBedDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `TransferUpdateDialog`, `_showTransferUpdateDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: IPD admission, bed occupancy, rooms/beds workspace.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openReleaseBedDialog`'s path.
...
<!-- generator: encounter-dialog prompt 09 slug=ipd-release-bed-dialog symbol=_openReleaseBedDialog shape=inline_confirm -->
