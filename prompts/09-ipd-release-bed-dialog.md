...
| Defined in | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:1353` |
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | approved `show*` helper, `AppConfirmActionDialog` |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| `CircularProgressIndicator` | not seen |
| Title snippets | `l10n.ipdReleaseBedAction` |
...
1. **Read contracts + source**
   ...
   - Read `_openReleaseBedDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:1353` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `TransferUpdateDialog`, `_showTransferUpdateDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: IPD admission, bed occupancy, rooms/beds workspace.
