...
| Field | Value |
| --- | --- |
| Symbol | `ReleaseBedDialog` |
| Purpose | Patient/encounter flow: Release Bed (ipd). |
| Defined in | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2019` |
| Kind | `custom` |
| Paired opener(s) | `_openReleaseBedDialog` |
...
1. **Read contract + source**
   ...
   - Read `ReleaseBedDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2019` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Release Bed (ipd).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
9. **Preserve reachability**
   - Do not break ``_openReleaseBedDialog`` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
1. `ReleaseBedDialog` opens through `AppDialog` / approved helpers only.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `ReleaseBedDialog`'s path.
...
<!-- generator: encounter-dialog prompt 09 slug=ipd-release-bed-dialog symbol=ReleaseBedDialog -->
