# Lab Follow Ups tab — rule compliance

## Context

Make the Follow Ups desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/10-lab/04-follow-ups.md`. Apply shared chrome fixes from `prompts/10-lab/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `opdFollowUpsTitle`, deep-link `follow-ups` (aliases `follow_ups`, `followups`), and omit the tab when gate denies access (`LabFollowUpsAtomPermissions.tab` = ∩ `lab:read` + `lab-workflows`) — never show a disabled placeholder (`tabs.mdc`). Align `LabDeskSection` / query helpers with `tabs/10-lab/00-overview.md`.
...
8. Preserve in-desk actions for this surface via shared dialogs/forms; omit unauthorized actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Create order path; Desk settings.
...
- [ ] `tabs/10-lab/04-follow-ups.md` updated to match.
