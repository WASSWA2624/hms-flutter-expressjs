# Lab overview — workspace compliance program

## Context

Bring the Lab desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/10-lab/`. Per-surface remediation prompts live beside this file under `prompts/10-lab/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/10-lab/00-overview.md` as the authoritative tab index (`LabDeskSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/10-lab/01-collection.md`
   - `prompts/10-lab/02-critical.md`
   - `prompts/10-lab/03-completed.md`
   - `prompts/10-lab/04-follow-ups.md`
   - `prompts/10-lab/05-worklist.md`
...
6. After each remediation, update the matching inventory file under `tabs/10-lab/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/lab/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/10-lab/00-overview.md` has a completed remediation prompt under `prompts/10-lab/` and a matching updated inventory under `tabs/10-lab/`.
...
- Trace `frontend/lib/features/lab/presentation/pages/lab_workspace_page.dart`, `frontend/lib/features/lab/presentation/lab_access.dart`, and widgets listed in `tabs/10-lab/00-overview.md`.
- Run feature tests under `frontend/test/features/lab/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/10-lab/00-overview.md`
- `prompts/10-lab/00-shared-chrome.md`
- `prompts/10-lab/99-convention-gaps.md`
- `prompts/10-lab/01-collection.md`
- `prompts/10-lab/02-critical.md`
- `prompts/10-lab/03-completed.md`
- `prompts/10-lab/04-follow-ups.md`
