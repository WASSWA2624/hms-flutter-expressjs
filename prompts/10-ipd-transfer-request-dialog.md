...
**Scope:** only `TransferRequestDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Purpose | Patient/encounter flow: Transfer Request (ipd). |
...
1. **Read contract + source**
   ...
   - Read `TransferRequestDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2188` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Transfer Request (ipd).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_openTransferRequestDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `TransferRequestDialog`'s path.
...
<!-- generator: encounter-dialog prompt 10 slug=ipd-transfer-request-dialog symbol=TransferRequestDialog -->
