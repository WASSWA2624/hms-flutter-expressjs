...
3. **Normalize title + icon**
   - Use a general, role-based title for **Transfer Request (ipd).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `TransferRequestDialog` opens through `AppDialog` / approved helpers only.
