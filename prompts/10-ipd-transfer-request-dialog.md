...
| Defined in | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2188` |
...
| Title snippets | `l10n.ipdRequestTransferAction` |
...
1. **Read contracts + source**
   ...
   - Read `TransferRequestDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2188` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_TransferRequestDialog`, `TransferUpdateDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: IPD transfer queue, admission detail, destination unit.
