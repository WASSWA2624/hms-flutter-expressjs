...
| Field | Value |
| --- | --- |
| Symbol | `TransferRequestDialog` |
| Purpose | Transfer Request |
| Module / surface | `ipd` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2132` |
...
| Paired opener(s) | `_openTransferRequestDialog` |
| Primary commit | Request transfer |
| Slices to keep in sync | IPD transfer queue, admission detail, destination unit |
| Sibling reuse targets | `_TransferRequestDialog`, `TransferUpdateDialog` |
...
| Title snippets | `l10n.ipdRequestTransferAction`, `widget.title` |
...
**Scope lock:** only `TransferRequestDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `TransferRequestDialog` at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart:2132` and every paired opener / *Used from* site.
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_TransferRequestDialog`, `TransferUpdateDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch IPD transfer queue, admission detail, destination unit, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openTransferRequestDialog`. Update all call sites in the same change when signatures move.
...
1. `TransferRequestDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_TransferRequestDialog`, `TransferUpdateDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: IPD transfer queue, admission detail, destination unit.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `TransferRequestDialog`'s path.
...
<!-- generator: encounter-dialog prompt 10 slug=ipd-transfer-request-dialog symbol=TransferRequestDialog shape=widget_dialog -->
