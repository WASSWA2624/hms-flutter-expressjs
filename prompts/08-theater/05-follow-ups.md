# Theater Follow Ups tab — rule compliance

## Context

Make the Follow Ups desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/08-theater/05-follow-ups.md`. Apply shared chrome fixes from `prompts/08-theater/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `opdFollowUpsTitle`, deep-link `follow-ups` (aliases `follow_ups`, `followups`), and omit the tab when gate denies access (`TheaterFollowUpsAtomPermissions.tab`) — never show a disabled placeholder (`tabs.mdc`). Align `TheaterSection` / query helpers with `tabs/08-theater/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`followUpTabCountProvider(FollowUpWorklistScope(encounterType: 'THEATRE'))`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/08-theater/05-follow-ups.md` updated to match.
