# Theater overview — workspace compliance program

## Context

Bring the Theater desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/08-theater/`. Per-surface remediation prompts live beside this file under `prompts/08-theater/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/08-theater/00-overview.md` as the authoritative tab index (`TheaterSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/08-theater/01-scheduled.md`
   - `prompts/08-theater/02-in-theater.md`
   - `prompts/08-theater/03-recovery.md`
   - `prompts/08-theater/04-all.md`
   - `prompts/08-theater/05-follow-ups.md`
...
6. After each remediation, update the matching inventory file under `tabs/08-theater/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/theater/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/08-theater/00-overview.md` has a completed remediation prompt under `prompts/08-theater/` and a matching updated inventory under `tabs/08-theater/`.
...
- Trace `frontend/lib/features/theater/presentation/pages/theater_workspace_page.dart`, `frontend/lib/features/theater/presentation/theater_access.dart`, and widgets listed in `tabs/08-theater/00-overview.md`.
- Run feature tests under `frontend/test/features/theater/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/08-theater/00-overview.md`
- `prompts/08-theater/00-shared-chrome.md`
- `prompts/08-theater/99-convention-gaps.md`
- `prompts/08-theater/01-scheduled.md`
- `prompts/08-theater/02-in-theater.md`
- `prompts/08-theater/03-recovery.md`
- `prompts/08-theater/04-all.md`
