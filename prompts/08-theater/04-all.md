# Theater All tab — rule compliance

## Context

Make the All desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/08-theater/04-all.md`. Apply shared chrome fixes from `prompts/08-theater/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `theaterAllCasesSummaryLabel`, deep-link `all` (URL **omits** `section` when this tab is active; unknown query defaults here), and omit the tab when gate denies access (`TheaterAllAtomPermissions.tab`) — never show a disabled placeholder (`tabs.mdc`). Align `TheaterSection` / query helpers with `tabs/08-theater/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`page.totalItemCount ?? items.length`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/08-theater/04-all.md` updated to match.
