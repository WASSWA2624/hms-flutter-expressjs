...
| Field | Value |
| --- | --- |
| Symbol | `AssignDoctorDialog` |
| Purpose | Assign Doctor |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1622` |
...
| Paired opener(s) | `_openAssignDoctorDialog`, `showAssignDoctorDialog` |
| Primary commit | Assign doctor |
| Slices to keep in sync | doctor assignment, queue row, encounter header |
| Sibling reuse targets | `FlowActionsDialog`, `CorrectStageDialog` |
...
| Mutations (region) | `mutation: assignDoctor` |
...
**Scope lock:** only `AssignDoctorDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `AssignDoctorDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1622` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Assign Doctor** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Assign doctor`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `FlowActionsDialog`, `CorrectStageDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch doctor assignment, queue row, encounter header, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openAssignDoctorDialog`, `showAssignDoctorDialog`. Update all call sites in the same change when signatures move.
...
1. `AssignDoctorDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FlowActionsDialog`, `CorrectStageDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: doctor assignment, queue row, encounter header.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `AssignDoctorDialog`'s path.
...
<!-- generator: encounter-dialog prompt 33 slug=assign-doctor-dialog symbol=AssignDoctorDialog shape=widget_dialog -->
