...
**Scope:** only `AssignDoctorDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `opdRepositoryProvider`, `mutation-ish call: assignDoctor`
...
| Purpose | Patient/encounter flow: Assign Doctor (shared/opd_actions). |
...
1. **Read contract + source**
   ...
   - Read `AssignDoctorDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1504` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Assign Doctor (shared/opd_actions).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_openAssignDoctorDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `AssignDoctorDialog`'s path.
...
<!-- generator: encounter-dialog prompt 33 slug=assign-doctor-dialog symbol=AssignDoctorDialog -->
