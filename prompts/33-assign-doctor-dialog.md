...
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1504` |
| Extends / uses | AppDialog / showAppDialog (typical) |
| Paired opener(s) | `_openAssignDoctorDialog` |
...
1. **Read contracts + source**
   ...
   - Read `AssignDoctorDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1504` and every paired opener / *Used from* site.
...
8. **Preserve reachability**
   - Do not break `_openAssignDoctorDialog`. Update all call sites in the same change when signatures move.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FlowActionsDialog`, `CorrectStageDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: doctor assignment, queue row, encounter header.
