# Nursing overview — workspace compliance program

## Context

Bring the Nursing desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/06-nursing/`. Per-surface remediation prompts live beside this file under `prompts/06-nursing/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/06-nursing/00-overview.md` as the authoritative tab index (`desk section enum` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/06-nursing/01-all.md`
   - `prompts/06-nursing/02-assigned-ward.md`
   - `prompts/06-nursing/03-urgent.md`
   - `prompts/06-nursing/04-medication-due.md`
   - `prompts/06-nursing/05-handover-pending.md`
   - `prompts/06-nursing/06-transfer-pending.md`
   - `prompts/06-nursing/07-discharge-pending.md`
...
6. After each remediation, update the matching inventory file under `tabs/06-nursing/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/nursing/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- Trace `frontend/lib/features/nursing/presentation/pages/nursing_workspace_page.dart`, `frontend/lib/features/nursing/presentation/nursing_access.dart`, and widgets listed in `tabs/06-nursing/00-overview.md`.
- Run feature tests under `frontend/test/features/nursing/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/06-nursing/00-overview.md`
- `prompts/06-nursing/00-shared-chrome.md`
- `prompts/06-nursing/99-convention-gaps.md`
- `prompts/06-nursing/01-all.md`
- `prompts/06-nursing/02-assigned-ward.md`
- `prompts/06-nursing/03-urgent.md`
- `prompts/06-nursing/04-medication-due.md`
