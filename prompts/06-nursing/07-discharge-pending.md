# Nursing Discharge Pending tab — rule compliance

## Context

Make the Discharge Pending desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/06-nursing/07-discharge-pending.md`. Apply shared chrome fixes from `prompts/06-nursing/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `nursingScopeDischargePendingLabel`, deep-link `section` per inventory, and omit the tab when gate denies access (`NursingDischargePendingAtomPermissions.tab` = `nursingWorkspaceReadRequirement`) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/06-nursing/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.dischargePendingCount` (null when 0)). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [x] `tabs/06-nursing/07-discharge-pending.md` updated to match.
