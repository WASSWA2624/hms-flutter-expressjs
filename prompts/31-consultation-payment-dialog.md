...
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1109` |
...
| Title snippets | `l10n.opdManageConsultationBillingAction` |
...
1. **Read contracts + source**
   ...
   - Read `ConsultationPaymentDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1109` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FlowActionsDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: payment status, OPD stage gates, billing cues.
