...
| Field | Value |
| --- | --- |
| Symbol | `ConsultationPaymentDialog` |
| Purpose | Consultation Payment |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1188` |
...
| Paired opener(s) | `showConsultationPaymentDialog` |
| Primary commit | Record payment |
| Slices to keep in sync | payment status, OPD stage gates, billing cues |
| Sibling reuse targets | `FlowActionsDialog` |
| Action helper peek | `clinicalActionDialogActions` |
| Controllers (region) | `effectiveDefaultCurrencyProvider` |
| Mutations (region) | `mutation: payConsultation` |
...
- `OpdConsultationBillingBreakdownPanel — frontend/lib/shared/opd_actions/opd_consultation_billing_breakdown.dart`
...
| Title snippets | `l10n.opdManageConsultationBillingAction`, `widget.title`, `showTitle ? l10n.opdEncounterContextTitle : null` |
...
**Scope lock:** only `ConsultationPaymentDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `ConsultationPaymentDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1188` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Consultation Payment** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Record payment`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `FlowActionsDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch payment status, OPD stage gates, billing cues, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showConsultationPaymentDialog`. Update all call sites in the same change when signatures move.
...
1. `ConsultationPaymentDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `FlowActionsDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: payment status, OPD stage gates, billing cues.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `ConsultationPaymentDialog`'s path.
...
<!-- generator: encounter-dialog prompt 31 slug=consultation-payment-dialog symbol=ConsultationPaymentDialog shape=widget_dialog -->
