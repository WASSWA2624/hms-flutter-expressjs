...
| Purpose | Patient/encounter flow: Transfer Update (rooms_beds). |
...
| Title snippets | `context.l10n.roomsBedsDetailTitle` |
| `AppButton` variants (order seen) | tertiary -> secondary -> secondary -> secondary -> secondary -> secondary -> tertiary -> tertiary |
...
1. **Read contract + source**
   ...
   - Read `_showTransferUpdateDialog` at `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart:1376` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Transfer Update (rooms_beds).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
   - Wire title through the shell so uppercase normalization applies.
   - Add/keep a meaningful `icon` if peer dialogs in `rooms_beds` already use icons.
...
1. `_showTransferUpdateDialog` opens through `AppDialog` / approved helpers only.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_showTransferUpdateDialog`'s path.
...
<!-- generator: encounter-dialog prompt 20 slug=rooms-beds-show-transfer-update-dialog symbol=_showTransferUpdateDialog -->
