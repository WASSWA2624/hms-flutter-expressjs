...
| Field | Value |
| --- | --- |
| Symbol | `_showTransferUpdateDialog` |
| Purpose | Transfer Update |
| Module / surface | `rooms_beds` |
| Inventory kind | `custom` |
| Presentation shape | `inline_opener` |
| Verified definition | `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart:1395` |
...
| Primary commit | Edit transfer / confirm |
| Slices to keep in sync | rooms/beds occupancy, transfer row, IPD transfer views |
| Sibling reuse targets | `TransferUpdateDialog`, `TransferRequestDialog` |
...
**Scope lock:** only `_showTransferUpdateDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_showTransferUpdateDialog` at `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart:1395` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Transfer Update** — never patient display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `rooms_beds`.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Edit transfer / confirm`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `TransferUpdateDialog`, `TransferRequestDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch rooms/beds occupancy, transfer row, IPD transfer views, then apply the smallest targeted reconciliation.
...
1. `_showTransferUpdateDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `TransferUpdateDialog`, `TransferRequestDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: rooms/beds occupancy, transfer row, IPD transfer views.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_showTransferUpdateDialog`'s path.
...
<!-- generator: encounter-dialog prompt 20 slug=rooms-beds-show-transfer-update-dialog symbol=_showTransferUpdateDialog shape=inline_opener -->
