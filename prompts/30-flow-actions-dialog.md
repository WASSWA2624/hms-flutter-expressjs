...
| Field | Value |
| --- | --- |
| Symbol | `FlowActionsDialog` |
| Purpose | OPD queue/flow stage actions hub |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `action_hub` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:110` |
...
| Paired opener(s) | `showFlowActionsDialog` |
| Primary commit | Stage action / nested dialog commits |
| Slices to keep in sync | encounter stage, queue, payments, disposition, referrals, vitals |
| Sibling reuse targets | `ConsultationPaymentDialog`, `CorrectStageDialog`, `AssignDoctorDialog`, `RoutingDecisionDialog`, `OpdDispositionDialog`, `_OpdAdmissionHandoffDialog`, `ReferralDialog`, `FollowUpDialog`, `PrintOpdSummaryDialog`, `RecordVitalsDialog`, `QueueActionsDialog` |
| Action helper peek | _none detected — adopt an approved action helper when the footer fits_ |
| Controllers (region) | `opdWorkspaceControllerProvider.notifier`, `clinicalRepositoryProvider`, `sessionStateProvider` |
| Mutations (region) | `mutation: updateLabOrder` |
...
- `OpdEncounterClinicalServicesPanel — frontend/lib/shared/opd_actions/opd_encounter_clinical_services.dart`
...
- `backend/src/tests/modules/lab-order/services/lab-order.service.test.js`
- `backend/src/modules/lab-order/services/lab-order.service.js`
- `backend/src/modules/lab-order/routes/lab-order.routes.js`
- `backend/src/modules/lab-order/controllers/lab-order.controller.js`
...
| Title snippets | `l10n.opdFlowActionsTitle`, `l10n.opdLoadingTitle`, `l10n.opdActionsColumnLabel`, `l10n.opdPhysiotherapyHandoffTitle` |
| `AppButton` variants (order seen) | secondary -> secondary -> primary |
| `AppActionIcons` | seen |
...
| Cross-stack trace files found | 12 |
| Peek region size | 36616 chars |
...
3. Widget reads a repository provider directly — move load/mutation ownership to a controller and keep server-backed UI state in Riverpod.
...
**Scope lock:** only `FlowActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `action_hub`
...
1. **Read contracts + source**
   ...
   - Read `FlowActionsDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:110` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **OPD queue/flow stage actions hub** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Stage action / nested dialog commits`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `ConsultationPaymentDialog`, `CorrectStageDialog`, `AssignDoctorDialog`, `RoutingDecisionDialog`, `OpdDispositionDialog`, `_OpdAdmissionHandoffDialog`, `ReferralDialog`, `FollowUpDialog`, `PrintOpdSummaryDialog`, `RecordVitalsDialog`, `QueueActionsDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch encounter stage, queue, payments, disposition, referrals, vitals, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showFlowActionsDialog`. Update all call sites in the same change when signatures move.
...
1. `FlowActionsDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `ConsultationPaymentDialog`, `CorrectStageDialog`, `AssignDoctorDialog`, `RoutingDecisionDialog`, `OpdDispositionDialog`, `_OpdAdmissionHandoffDialog`, `ReferralDialog`, `FollowUpDialog`, `PrintOpdSummaryDialog`, `RecordVitalsDialog`, `QueueActionsDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: encounter stage, queue, payments, disposition, referrals, vitals.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `FlowActionsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 30 slug=flow-actions-dialog symbol=FlowActionsDialog shape=action_hub -->
