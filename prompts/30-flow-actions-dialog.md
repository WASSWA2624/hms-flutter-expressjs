...
| Title snippets | `context.l10n.opdEncounterDialogTitle`, `l10n.opdPhysiotherapyHandoffTitle` |
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Flow Actions** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `FlowActionsDialog` opens through `AppDialog` / approved helpers only.
