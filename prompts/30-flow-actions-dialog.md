...
**Scope:** only `FlowActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `opdWorkspaceControllerProvider.notifier`, `clinicalRepositoryProvider`, `sessionStateProvider`, `mutation-ish call: updateLabOrder`
...
| Title snippets | `l10n.opdFlowActionsTitle`, `l10n.opdPhysiotherapyHandoffTitle` |
...
1. **Read contract + source**
   ...
   - Read `FlowActionsDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:110` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Flow Actions** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `showFlowActionsDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `FlowActionsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 30 slug=flow-actions-dialog symbol=FlowActionsDialog -->
