...
| Title snippets | `l10n.receptionScheduleAppointmentAction`, `patient.effectiveDisplayName`, `patient.publicId ?? patient.id` |
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Reception Patient Picker (reception).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_ReceptionPatientPickerDialog` opens through `AppDialog` / approved helpers only.
