...
| Field | Value |
| --- | --- |
| Symbol | `_ReceptionPatientPickerDialog` |
| Purpose | Reception Patient Picker |
| Module / surface | `reception` |
| Inventory kind | `custom` |
| Presentation shape | `read_only_picker` |
| Verified definition | `frontend/lib/features/reception/presentation/widgets/reception_patient_actions.dart:114` |
...
| Paired opener(s) | `openReceptionScheduleAppointment` |
| Primary commit | Select patient / Cancel |
| Slices to keep in sync | reception selection context only (no fake patient create) |
| Sibling reuse targets | `RegisterNewPatientDialog`, `OpdEncounterDialog` |
| Action helper peek | _none detected — adopt an approved action helper when the footer fits_ |
| Controllers (region) | `patientRegistryControllerProvider,` |
...
| Title snippets | `l10n.receptionPatientPickerTitle`, `patient.effectiveDisplayName`, `patient.publicId ?? patient.id`, `widget.title` |
...
**Scope lock:** only `_ReceptionPatientPickerDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `read_only_picker`
...
1. **Read contracts + source**
   ...
   - Read `_ReceptionPatientPickerDialog` at `frontend/lib/features/reception/presentation/widgets/reception_patient_actions.dart:114` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Reception Patient Picker** — never patient display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `reception`.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Select patient / Cancel`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `RegisterNewPatientDialog`, `OpdEncounterDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch reception selection context only (no fake patient create), then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `openReceptionScheduleAppointment`. Update all call sites in the same change when signatures move.
...
1. `_ReceptionPatientPickerDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
2. Picker supports Cancel plus selection/return semantics; it performs no server mutation.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `RegisterNewPatientDialog`, `OpdEncounterDialog`).
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_ReceptionPatientPickerDialog`'s path.
...
<!-- generator: encounter-dialog prompt 18 slug=reception-patient-picker-dialog symbol=_ReceptionPatientPickerDialog shape=read_only_picker -->
