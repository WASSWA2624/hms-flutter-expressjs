...
| Defined in | `frontend/lib/features/reception/presentation/widgets/reception_patient_actions.dart:117` |
...
| Mutations (region) | `mutation: cancel` |
...
| Title snippets | `l10n.receptionPatientPickerTitle`, `patient.effectiveDisplayName`, `patient.publicId ?? patient.id` |
...
3. Title may include patient name — must be role/flow based
...
1. **Read contracts + source**
   ...
   - Read `_ReceptionPatientPickerDialog` at `frontend/lib/features/reception/presentation/widgets/reception_patient_actions.dart:117` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `RegisterNewPatientDialog`, `OpdEncounterDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: reception selection context only (no fake patient create).
...
<!-- generator: encounter-dialog prompt 18 slug=reception-patient-picker-dialog symbol=_ReceptionPatientPickerDialog shape=widget_dialog -->
