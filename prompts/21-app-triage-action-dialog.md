...
| Field | Value |
| --- | --- |
| Symbol | `AppTriageActionDialog` |
| Purpose | Shared triage action dialog |
| Module / surface | `shared/components` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/components/app_triage_components.dart:146` |
| Inventory location note | Inventory and verified declaration agree at generation time. |
| Extends / uses | AppDialog / `showAppTriageActionDialog` |
| Paired opener(s) | `_openPatientTriageQuickDialog`, `_openTriageDialog` |
| Primary commit | Save triage |
| Slices to keep in sync | triage records for the calling module's encounter/patient |
| Sibling reuse targets | `_PatientTriageQuickDialog`, `EmergencyActionPanel._openTriageDialog` |
| Action helper peek | `AppTriageActionDialog` |
| Controllers (region) | _not detected in peek — trace widget → workspace controller → repository → backend route_ |
| Mutations (region) | `mutation: recordTriage` |
...
- `frontend/lib/features/emergency/domain/repositories/emergency_repository.dart`
- `frontend/lib/features/emergency/data/repositories/emergency_repository_impl.dart`
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | `AppDialog`, approved `show*` helper, `AppTriageActionDialog` |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| Raw Material progress indicator | not seen |
| Title snippets | `widget.title`, `widget.prioritySectionTitle`, `widget.notesSectionTitle`, `widget.vitalsSectionTitle` |
...
| Peek region size | 14008 chars |

### Priority gaps to close

1. Confirm mutating openers set `barrierDismissible: false` while the dialog can mutate.
2. Loading path exists — ensure `closeEnabled: false` and disabled Cancel/competing actions while mutation/load is in flight.
3. Footer may be hand-rolled — prefer `clinicalActionDialogActions`, `buildAppDialogFormActions`, or `buildAppDialogWizardActions` when they fit.
...
**Scope lock:** only `AppTriageActionDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `AppTriageActionDialog` at `frontend/lib/shared/components/app_triage_components.dart:146` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Shared triage action dialog** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save triage`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_PatientTriageQuickDialog`, `EmergencyActionPanel._openTriageDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch triage records for the calling module's encounter/patient, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPatientTriageQuickDialog`, `_openTriageDialog`. Update all call sites in the same change when signatures move.
...
1. `AppTriageActionDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_PatientTriageQuickDialog`, `EmergencyActionPanel._openTriageDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: triage records for the calling module's encounter/patient.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `AppTriageActionDialog`'s path.
...
<!-- generator: encounter-dialog prompt 21 slug=app-triage-action-dialog symbol=AppTriageActionDialog shape=widget_dialog -->
