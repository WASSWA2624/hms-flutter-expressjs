...
| Defined in | `frontend/lib/shared/components/app_triage_components.dart:146` |
| Extends / uses | AppDialog / `showAppTriageActionDialog` |
| Paired opener(s) | `_openTriageDialog`, `_openPatientTriageQuickDialog` |
| Primary commit | Save triage |
| Slices to keep in sync | triage records for the calling module's encounter/patient |
| Sibling reuse targets | `_PatientTriageQuickDialog`, `_showTriageDialog` |
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_PatientTriageQuickDialog`, `_showTriageDialog`.
...
8. **Preserve reachability**
   - Do not break `_openTriageDialog`, `_openPatientTriageQuickDialog`. Update all call sites in the same change when signatures move.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_PatientTriageQuickDialog`, `_showTriageDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: triage records for the calling module's encounter/patient.
