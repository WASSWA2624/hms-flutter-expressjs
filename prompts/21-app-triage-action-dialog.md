...
| Defined in | `frontend/lib/shared/components/app_triage_components.dart:131` |
| Kind | `shared` |
| Paired opener(s) | `_openTriageDialog` |
...
1. **Read contract + source**
   ...
   - Read `AppTriageActionDialog` at `frontend/lib/shared/components/app_triage_components.dart:131` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **App Triage Action** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
9. **Preserve reachability**
   - Do not break ``_openTriageDialog`` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
1. `AppTriageActionDialog` opens through `AppDialog` / approved helpers only.
