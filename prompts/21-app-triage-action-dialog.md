...
**Scope:** only `AppTriageActionDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.

**Module / surface:** `shared/components`  
**Inventory kind:** `shared`  
**Extends / uses (inventory):** AppDialog / `showAppTriageActionDialog`  
**Action helper peek:** _none detected — adopt an approved action helper when the footer fits_  
**Controller / mutation peek:** `opdWorkspaceControllerProvider.notifier,`, `mutation-ish call: recordTriage`, `mutation-ish call: dispatchAmbulance`, `mutation-ish call: updateLatestDispatchStatus`, `mutation-ish call: handoff`, `mutation-ish call: submitOpdEncounter`
...
| Title snippets | `widget.title` |
| `AppButton` variants (order seen) | secondary -> primary -> primary |
...
- Loading path exists — ensure `closeEnabled: false` and disabled Cancel/competing actions while mutation/load is in flight.
- Footer may be hand-rolled — prefer `clinicalActionDialogActions`, `buildAppDialogFormActions`, or `buildAppDialogWizardActions` when they fit.
...
1. **Read contract + source**
   ...
   - Read `AppTriageActionDialog` at `frontend/lib/shared/components/app_triage_components.dart:146` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **App Triage Action** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_openTriageDialog`, `_openPatientTriageQuickDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `AppTriageActionDialog`'s path.
...
<!-- generator: encounter-dialog prompt 21 slug=app-triage-action-dialog symbol=AppTriageActionDialog -->
