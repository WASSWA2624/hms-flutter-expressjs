...
| Title snippets | `dialogContext.l10n.housekeepingDetailTitle` |
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Triage (housekeeping).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_showTriageDialog` opens through `AppDialog` / approved helpers only.
