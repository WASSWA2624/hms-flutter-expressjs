...
| Field | Value |
| --- | --- |
| Symbol | `_showTriageDialog` |
| Purpose | Triage |
| Module / surface | `housekeeping` |
| Inventory kind | `custom` |
| Presentation shape | `inline_opener` |
| Verified definition | `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:2057` |
...
| Primary commit | Triage maintenance request |
| Slices to keep in sync | housekeeping maintenance worklist and selected request detail |
| Sibling reuse targets | `showAppWorkspaceMutationDialog` |
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | approved `show*` helper, `showAppWorkspaceMutationDialog` |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| Raw Material progress indicator | not seen |
| Title snippets | `l10n.housekeepingTriageDialogTitle` |
...
| Peek region size | 1329 chars |

### Priority gaps to close

1. Confirm mutating openers set `barrierDismissible: false` while the dialog can mutate.
2. No obvious loading primitive near the symbol — add shared loading UX for async open/submit.
...
**Scope lock:** only `_showTriageDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `inline_opener`
...
1. **Read contracts + source**
   ...
   - Read `_showTriageDialog` at `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:2057` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Triage** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Triage maintenance request`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `showAppWorkspaceMutationDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch housekeeping maintenance worklist and selected request detail, then apply the smallest targeted reconciliation.
...
1. `_showTriageDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `showAppWorkspaceMutationDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: housekeeping maintenance worklist and selected request detail.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_showTriageDialog`'s path.
...
<!-- generator: encounter-dialog prompt 06 slug=housekeeping-show-triage-dialog symbol=_showTriageDialog shape=inline_opener -->
