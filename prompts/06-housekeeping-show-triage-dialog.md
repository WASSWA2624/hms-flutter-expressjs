...
**Scope:** only `_showTriageDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Purpose | Patient/encounter flow: Triage (housekeeping). |
...
| Title snippets | `l10n.housekeepingTriageDialogTitle`, `title`, `l10n.housekeepingReportSummaryTitle` |
...
1. **Read contract + source**
   ...
   - Read `_showTriageDialog` at `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:2033` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Triage (housekeeping).** (flow/action name — never the patient display name as `AppDialog` title).
   - Pass the title through the shell so uppercase normalization applies.
   - Add/keep a meaningful `icon` if peer dialogs in `housekeeping` already use icons.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_showTriageDialog`'s path.
...
<!-- generator: encounter-dialog prompt 06 slug=housekeeping-show-triage-dialog symbol=_showTriageDialog -->
