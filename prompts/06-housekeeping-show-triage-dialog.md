...
Open housekeeping triage for the selected item using the shared triage action surface rather than a local fork.
...
| Defined in | `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:2033` |
| Extends / uses | showAppDialog / showAppWorkspace* (inline) |
| Paired opener(s) | _none listed in inventory_ |
| Primary commit | Save triage |
| Slices to keep in sync | housekeeping triage state, related bed/room cues |
| Sibling reuse targets | `AppTriageActionDialog`, `_PatientTriageQuickDialog` |
...
1. **Read contracts + source**
   ...
   - Read `_showTriageDialog` at `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart:2033` and every paired opener / *Used from* site.
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `AppTriageActionDialog`, `_PatientTriageQuickDialog`.
...
7. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch housekeeping triage state, related bed/room cues, then apply the smallest targeted reconciliation.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `AppTriageActionDialog`, `_PatientTriageQuickDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: housekeeping triage state, related bed/room cues.
