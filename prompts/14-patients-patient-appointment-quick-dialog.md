...
**Scope:** only `PatientAppointmentQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: createAppointment`
...
| Purpose | Patient/encounter flow: Patient Appointment Quick (patients). |
...
1. **Read contract + source**
   ...
   - Read `PatientAppointmentQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:1923` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Patient Appointment Quick (patients).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `showPatientAppointmentQuickDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `PatientAppointmentQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 14 slug=patients-patient-appointment-quick-dialog symbol=PatientAppointmentQuickDialog -->
