...
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:1903` |
| Kind | `custom` |
| Paired opener(s) | `—` |
...
1. **Read contract + source**
   ...
   - Read `PatientAppointmentQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:1903` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Patient Appointment Quick (patients).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `PatientAppointmentQuickDialog` opens through `AppDialog` / approved helpers only.
