...
| Field | Value |
| --- | --- |
| Symbol | `PatientAppointmentQuickDialog` |
| Purpose | Patient Appointment Quick |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:1923` |
...
| Paired opener(s) | `_openPatientQuickAction`, `showPatientAppointmentQuickDialog` |
| Primary commit | Save appointment |
| Slices to keep in sync | appointments list, patient detail, reception schedule cues |
| Sibling reuse targets | `OpdAppointmentActionsDialog`, `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog` |
...
| Mutations (region) | `mutation: createAppointment` |
...
- `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart`
- `backend/src/modules/appointment/services/appointment.service.js`
- `backend/src/tests/modules/appointment/services/appointment.service.test.js`
- `backend/src/modules/appointment/routes/appointment.routes.js`
- `backend/src/modules/appointment/controllers/appointment.controller.js`
- `backend/src/tests/modules/appointment/controllers/appointment.controller.test.js`
...
| Title snippets | `l10n.patientsAppointmentDialogTitle`, `widget.title` |
...
**Scope lock:** only `PatientAppointmentQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `PatientAppointmentQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:1923` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Appointment Quick** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save appointment`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `OpdAppointmentActionsDialog`, `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch appointments list, patient detail, reception schedule cues, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPatientQuickAction`, `showPatientAppointmentQuickDialog`. Update all call sites in the same change when signatures move.
...
1. `PatientAppointmentQuickDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdAppointmentActionsDialog`, `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: appointments list, patient detail, reception schedule cues.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `PatientAppointmentQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 14 slug=patients-patient-appointment-quick-dialog symbol=PatientAppointmentQuickDialog shape=widget_dialog -->
