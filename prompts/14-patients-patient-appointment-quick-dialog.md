...
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:1923` |
| Extends / uses | AppDialog / showAppDialog (typical) |
| Paired opener(s) | `showPatientAppointmentQuickDialog` |
...
| Title snippets | `l10n.patientsAppointmentDialogTitle` |
...
8. **Preserve reachability**
   - Do not break `showPatientAppointmentQuickDialog`. Update all call sites in the same change when signatures move.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdAppointmentActionsDialog`, `OpdRescheduleAppointmentDialog`, `OpdCancelAppointmentDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: appointments list, patient detail, reception schedule cues.
