...
| Defined in | `frontend/lib/shared/components/opd_encounter_dialog.dart:164` |
| Kind | `shared` |
| Paired opener(s) | `—` |
...
1. **Read contract + source**
   ...
   - Read `OpdEncounterDialog` at `frontend/lib/shared/components/opd_encounter_dialog.dart:164` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Opd Encounter (shared/components).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `OpdEncounterDialog` opens through `AppDialog` / approved helpers only.
