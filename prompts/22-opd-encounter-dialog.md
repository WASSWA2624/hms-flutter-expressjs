...
**Scope:** only `OpdEncounterDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Purpose | Patient/encounter flow: Opd Encounter (shared/components). |
| Defined in | `frontend/lib/shared/components/opd_encounter_dialog.dart:166` |
| Kind | `shared` |
...
1. **Read contract + source**
   ...
   - Read `OpdEncounterDialog` at `frontend/lib/shared/components/opd_encounter_dialog.dart:166` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Opd Encounter (shared/components).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `showOpdEncounterDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `OpdEncounterDialog`'s path.
...
<!-- generator: encounter-dialog prompt 22 slug=opd-encounter-dialog symbol=OpdEncounterDialog -->
