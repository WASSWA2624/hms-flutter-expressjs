...
| Module flow | [`.cursor/flows/emergency-flow.mdc`](../.cursor/flows/emergency-flow.mdc) | Domain workflow states, transitions, and handoffs |

## Target

| Field | Value |
| --- | --- |
| Symbol | `HandoffDialog` |
| Purpose | Handoff |
| Module / surface | `emergency` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:362` |
...
| Paired opener(s) | `EmergencyHandoffActionCell._openHandoff`, `_openHandoffDialog` |
| Primary commit | Handoff |
| Slices to keep in sync | emergency case, destination queue/workspace, encounter |
| Sibling reuse targets | `DispatchDialog`, `QuickArrivalDialog`, `FlowActionsDialog` |
...
| Mutations (region) | `mutation: handoff` |
...
- `frontend/lib/shared/workflow_actions/workflow_action_executor.dart`
...
- `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart`
- `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart`
...
- `frontend/lib/features/emergency/domain/entities/emergency_entities.dart`
- `frontend/lib/features/emergency/data/repositories/emergency_repository_impl.dart`
- `frontend/lib/features/emergency/data/dtos/emergency_dtos.dart`
- `backend/src/modules/emergency-case/services/emergency-case.service.js`
- `backend/src/modules/emergency-case/schemas/emergency-case.schema.js`
...
| Title snippets | `EmergencyText.handoff`, `EmergencyText.closeEmergencyCase`, `EmergencyText.closeEmergencyCaseSubtitle`, `widget.title` |
...
**Scope lock:** only `HandoffDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `HandoffDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:362` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Handoff** — never patient display name.
   - Pass title through the shell for uppercase normalization.
   - Match sibling icon conventions in `emergency`.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Handoff`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `DispatchDialog`, `QuickArrivalDialog`, `FlowActionsDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch emergency case, destination queue/workspace, encounter, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `EmergencyHandoffActionCell._openHandoff`, `_openHandoffDialog`. Update all call sites in the same change when signatures move.
...
1. `HandoffDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `DispatchDialog`, `QuickArrivalDialog`, `FlowActionsDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency case, destination queue/workspace, encounter.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `HandoffDialog`'s path.
...
<!-- generator: encounter-dialog prompt 03 slug=emergency-handoff-dialog symbol=HandoffDialog shape=widget_dialog -->
