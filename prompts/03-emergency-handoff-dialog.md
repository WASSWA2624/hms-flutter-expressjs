...
| Defined in | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:353` |
| Extends / uses | AppDialog / showAppDialog (typical) |
| Paired opener(s) | `_openHandoffDialog`, `EmergencyHandoffActionCell._openHandoff` |
...
| Title snippets | `EmergencyText.handoff`, `EmergencyText.closeEmergencyCase`, `EmergencyText.closeEmergencyCaseSubtitle` |
...
1. **Read contracts + source**
   ...
   - Read `HandoffDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:353` and every paired opener / *Used from* site.
...
8. **Preserve reachability**
   - Do not break `_openHandoffDialog`, `EmergencyHandoffActionCell._openHandoff`. Update all call sites in the same change when signatures move.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `DispatchDialog`, `QuickArrivalDialog`, `FlowActionsDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency case, destination queue/workspace, encounter.
