...
| Title snippets | `EmergencyText.recordHandoff` |
...
1. **Read contract + source**
   ...
   - Read `HandoffDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:276` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Handoff (emergency).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
9. **Preserve reachability**
   - Do not break ``_openHandoffDialog`` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
1. `HandoffDialog` opens through `AppDialog` / approved helpers only.
