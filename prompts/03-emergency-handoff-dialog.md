...
**Scope:** only `HandoffDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: handoff`
...
| Purpose | Patient/encounter flow: Handoff (emergency). |
...
| Title snippets | `EmergencyText.handoff` |
...
1. **Read contract + source**
   ...
   - Read `HandoffDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:353` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Handoff (emergency).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_openHandoffDialog`, `EmergencyHandoffActionCell._openHandoff` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `HandoffDialog`'s path.
...
<!-- generator: encounter-dialog prompt 03 slug=emergency-handoff-dialog symbol=HandoffDialog -->
