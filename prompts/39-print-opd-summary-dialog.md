...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Print Opd Summary** — never patient display name.
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `OpdEncounterDialog`, `FlowActionsDialog`.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `PrintOpdSummaryDialog`'s path.
