...
3. **Normalize title + icon**
   - Use a general, role-based title for **Print Opd Summary (shared/opd_actions).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `PrintOpdSummaryDialog` opens through `AppDialog` / approved helpers only.
