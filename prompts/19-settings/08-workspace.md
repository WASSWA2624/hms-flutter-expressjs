# Settings Workspace tab — rule compliance

## Context

Make the Workspace desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/19-settings/08-workspace.md`. Apply shared chrome fixes from `prompts/19-settings/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `settingsWorkspaceSectionTitle`, deep-link `section` per inventory, and omit the tab when its inventoried gate denies access — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/19-settings/00-overview.md`.
...
- [ ] `tabs/19-settings/08-workspace.md` updated to match.
...
- `tabs/19-settings/08-workspace.md`
