...
| Field | Value |
| --- | --- |
| Symbol | `RoutingDecisionDialog` |
| Purpose | Routing Decision |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1819` |
...
| Paired opener(s) | `showRoutingDecisionDialog` |
| Primary commit | Save routing decision |
| Slices to keep in sync | routing/disposition cues, next-queue eligibility |
| Sibling reuse targets | `OpdDispositionDialog`, `FlowActionsDialog` |
...
| Mutations (region) | `mutation: disposeFlow` |
...
| Title snippets | `l10n.opdRouteDecisionLabel`, `widget.title`, `showTitle ? l10n.opdEncounterContextTitle : null` |
...
**Scope lock:** only `RoutingDecisionDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `RoutingDecisionDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1819` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Routing Decision** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save routing decision`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `OpdDispositionDialog`, `FlowActionsDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch routing/disposition cues, next-queue eligibility, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showRoutingDecisionDialog`. Update all call sites in the same change when signatures move.
...
1. `RoutingDecisionDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdDispositionDialog`, `FlowActionsDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: routing/disposition cues, next-queue eligibility.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `RoutingDecisionDialog`'s path.
...
<!-- generator: encounter-dialog prompt 34 slug=routing-decision-dialog symbol=RoutingDecisionDialog shape=widget_dialog -->
