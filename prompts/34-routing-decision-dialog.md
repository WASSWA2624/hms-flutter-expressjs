...
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1648` |
...
1. **Read contracts + source**
   ...
   - Read `RoutingDecisionDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1648` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `OpdDispositionDialog`, `FlowActionsDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: routing/disposition cues, next-queue eligibility.
