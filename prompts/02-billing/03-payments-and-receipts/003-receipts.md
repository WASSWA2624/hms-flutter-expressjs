# Prompt 003 — Implement Receipts

## Context

- **Sequence:** 003 of 006 in `prompts/02-billing/03-payments-and-receipts/`
- **Overall step:** 114 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Payment Allocations` (`prompts/02-billing/03-payments-and-receipts/002-payment-allocations.md`).
- **Menu path:** `Billing → Payments & Receipts → Receipts`
- **Canonical route:** `/billing?section=receipts`
- **Surface profile:** `transaction` table/worklist tab
- **Navigation model:** `Billing → Payments & Receipts` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/billing/payments-and-receipts/receipts.md`
...
2. **Register and scope the tab.**
   - Implement the `Receipts` tab at `Billing → Receipts` with canonical section slug `receipts`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Payments & Receipts` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/payments-and-receipts/receipts.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/payments-and-receipts/receipts.md`:
     `Receipt No.`, `Receipt Date/Time`, `Payment No.`, `Invoice Reference(s)`, `Patient/Customer`, `Payer`, `Payment Method`, `External Reference`, `Amount`, `Currency`, `Cash Point`, `Cashier`, `Remaining Balance`, `Print Count`, `Delivery Method`, `Receipt Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted and source prerequisites met; Opens the Receipts mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Receipt Date/Time`, `Payment No.`, `Invoice Reference(s)`, `Patient/Customer`, `Payer`, `Payment Method`, `External Reference`, `Amount`, `Currency`, `Cash Point`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Advances & Deposits` (`prompts/02-billing/03-payments-and-receipts/004-advances-and-deposits.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=receipts` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/payments-and-receipts/receipts.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/payments-and-receipts/receipts.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_receipts_panel.dart`
