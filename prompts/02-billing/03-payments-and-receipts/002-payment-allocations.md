# Prompt 002 — Implement Payment Allocations

## Context

- **Sequence:** 002 of 006 in `prompts/02-billing/03-payments-and-receipts/`
- **Overall step:** 113 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Payments` (`prompts/02-billing/03-payments-and-receipts/001-payments.md`).
- **Menu path:** `Billing → Payments & Receipts → Payment Allocations`
- **Canonical route:** `/billing?section=payment-allocations`
...
- **Authoritative tab specification:** `.cursor/finance/billing/payments-and-receipts/payment-allocations.md`
...
2. **Register and scope the tab.**
   - Implement the `Payment Allocations` tab at `Billing → Payment Allocations` with canonical section slug `payment-allocations`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/payments-and-receipts/payment-allocations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/payments-and-receipts/payment-allocations.md`:
     `Allocation No.`, `Allocation Date`, `Payment No.`, `Invoice No.`, `Patient/Customer`, `Payer`, `Invoice Total`, `Balance Before`, `Allocated Amount`, `Balance After`, `Currency`, `Allocation Status`, `Allocated By`, `Reversal Reference`, `Reversal Reason`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Allocate:** gate `billing:write` ∩ `billing-payments`; enable when unallocated value exists; Opens allocation workbench.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Allocation Date`, `Payment No.`, `Invoice No.`, `Patient/Customer`, `Payer`, `Invoice Total`, `Currency`, `Allocated By`, `Reversal Reference`, `Reversal Reason`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Receipts` (`prompts/02-billing/03-payments-and-receipts/003-receipts.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=payment-allocations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/payments-and-receipts/payment-allocations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/payments-and-receipts/payment-allocations.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_payment_allocations_panel.dart`
