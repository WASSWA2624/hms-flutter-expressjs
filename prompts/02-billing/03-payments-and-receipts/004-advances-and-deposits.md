# Prompt 004 — Implement Advances & Deposits

## Context

- **Sequence:** 004 of 006 in `prompts/02-billing/03-payments-and-receipts/`
- **Overall step:** 115 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Receipts` (`prompts/02-billing/03-payments-and-receipts/003-receipts.md`).
- **Menu path:** `Billing → Payments & Receipts → Advances & Deposits`
- **Canonical route:** `/billing?section=advances-and-deposits`
...
- **Authoritative tab specification:** `.cursor/finance/billing/payments-and-receipts/advances-and-deposits.md`
...
2. **Register and scope the tab.**
   - Implement the `Advances & Deposits` tab at `Billing → Advances & Deposits` with canonical section slug `advances-and-deposits`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/payments-and-receipts/advances-and-deposits.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/payments-and-receipts/advances-and-deposits.md`:
     `Advance No.`, `Receipt Date`, `Patient/Customer`, `Payer`, `Payment No.`, `Original Amount`, `Allocated Amount`, `Refunded Amount`, `Available Balance`, `Currency`, `Expiry Date`, `Last Allocation Date`, `Advance Status`, `Received By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted and source prerequisites met; Opens the Advances & Deposits mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Receipt Date`, `Patient/Customer`, `Payer`, `Payment No.`, `Original Amount`, `Refunded Amount`, `Currency`, `Expiry Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Refunds` (`prompts/02-billing/03-payments-and-receipts/005-refunds.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=advances-and-deposits` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/payments-and-receipts/advances-and-deposits.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/payments-and-receipts/advances-and-deposits.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_advances_and_deposits_panel.dart`
