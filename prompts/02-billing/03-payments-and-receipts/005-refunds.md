# Prompt 005 — Implement Refunds

## Context

- **Sequence:** 005 of 006 in `prompts/02-billing/03-payments-and-receipts/`
- **Overall step:** 116 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Advances & Deposits` (`prompts/02-billing/03-payments-and-receipts/004-advances-and-deposits.md`).
- **Menu path:** `Billing → Payments & Receipts → Refunds`
- **Canonical route:** `/billing?section=refunds`
...
- **Authoritative tab specification:** `.cursor/finance/billing/payments-and-receipts/refunds.md`
...
2. **Register and scope the tab.**
   - Implement the `Refunds` tab at `Billing → Refunds` with canonical section slug `refunds`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/payments-and-receipts/refunds.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/payments-and-receipts/refunds.md`:
     `Refund No.`, `Request Date`, `Original Payment No.`, `Receipt No.`, `Patient/Customer`, `Payer`, `Reason Code`, `Requested Amount`, `Approved Amount`, `Refunded Amount`, `Currency`, `Refund Method`, `External Reference`, `Refund Status`, `Requested By`, `Approved By`, `Processed By`, `Journal Entry No.`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Request refund:** gate `billing:write` ∩ `billing-payments`; enable when refundable balance exists; Creates a maker-checker refund request.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Request Date`, `Original Payment No.`, `Receipt No.`, `Patient/Customer`, `Payer`, `Reason Code`, `Requested Amount`, `Approved Amount`, `Refunded Amount`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Write-offs` (`prompts/02-billing/03-payments-and-receipts/006-write-offs.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=refunds` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/payments-and-receipts/refunds.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/payments-and-receipts/refunds.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_refunds_panel.dart`
