# Prompt 001 — Implement Payments

## Context

- **Sequence:** 001 of 006 in `prompts/02-billing/03-payments-and-receipts/`
- **Overall step:** 112 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Debit Notes` (`prompts/02-billing/02-charges-and-invoicing/006-debit-notes.md`).
- **Menu path:** `Billing → Payments & Receipts → Payments`
- **Canonical route:** `/billing?section=payments`
...
- **Authoritative tab specification:** `.cursor/finance/billing/payments-and-receipts/payments.md`
...
2. **Register and scope the tab.**
   - Implement the `Payments` tab at `Billing → Payments` with canonical section slug `payments`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/payments-and-receipts/payments.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/payments-and-receipts/payments.md`:
     `Payment No.`, `Payment Date/Time`, `Patient/Customer`, `Payer`, `Payment Method`, `Provider/Bank`, `External Reference`, `Amount`, `Allocated Amount`, `Unallocated Amount`, `Currency`, `Cash Point/Bank Account`, `Receipt No.`, `Reversal Status`, `Payment Status`, `Received By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Record payment:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens payment dialog and returns the created row.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Payment Date/Time`, `Patient/Customer`, `Payer`, `Payment Method`, `Provider/Bank`, `External Reference`, `Amount`, `Currency`, `Cash Point/Bank Account`, `Receipt No.`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Payment Allocations` (`prompts/02-billing/03-payments-and-receipts/002-payment-allocations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=payments` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/payments-and-receipts/payments.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/payments-and-receipts/payments.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_payments_panel.dart`
