# Prompt 015 — Implement Receipts

## Context

- **Sequence:** 015 of 024 in `prompts/02-billing/`
- **Overall step:** 114 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Payment Allocations` (`prompts/02-billing/014-payment-allocations.md`).
- **Menu path:** `Billing → Receipts`
...
- **Authoritative tab specification:** `.cursor/finance/billing/receipts.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/receipts.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/receipts.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Advances & Deposits` (`prompts/02-billing/016-advances-and-deposits.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/receipts.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/receipts.md`
