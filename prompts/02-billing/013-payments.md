# Prompt 013 — Implement Payments

## Context

- **Sequence:** 013 of 024 in `prompts/02-billing/`
- **Overall step:** 112 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Debit Notes` (`prompts/02-billing/012-debit-notes.md`).
- **Menu path:** `Billing → Payments`
...
- **Authoritative tab specification:** `.cursor/finance/billing/payments.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/payments.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/payments.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Payment Allocations` (`prompts/02-billing/014-payment-allocations.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/payments.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/payments.md`
