# Prompt 004 — Implement Collection Follow-up

## Context

- **Sequence:** 004 of 004 in `prompts/02-billing/04-accounts-and-collections/`
- **Overall step:** 121 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Receivables` (`prompts/02-billing/04-accounts-and-collections/003-receivables.md`).
- **Menu path:** `Billing → Accounts & Collections → Collection Follow-up`
- **Canonical route:** `/billing?section=collection-follow-up`
...
- **Authoritative tab specification:** `.cursor/finance/billing/accounts-and-collections/collection-follow-up.md`
...
2. **Register and scope the tab.**
   - Implement the `Collection Follow-up` tab at `Billing → Collection Follow-up` with canonical section slug `collection-follow-up`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/accounts-and-collections/collection-follow-up.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/accounts-and-collections/collection-follow-up.md`:
     `Collection Case No.`, `Debtor`, `Debtor Type`, `Outstanding Amount`, `Currency`, `Oldest Invoice No.`, `Maximum Days Overdue`, `Priority`, `Last Contact Date`, `Last Contact Method`, `Promise Date`, `Promise Amount`, `Next Follow-up Date`, `Assigned Collector`, `Follow-up Status`, `Notes`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted and source prerequisites met; Opens the Collection Follow-up mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Debtor`, `Debtor Type`, `Outstanding Amount`, `Currency`, `Oldest Invoice No.`, `Maximum Days Overdue`, `Priority`, `Promise Date`, `Promise Amount`, `Next Follow-up Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Overview` (`prompts/02-billing/05-overview/001-overview.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=collection-follow-up` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/accounts-and-collections/collection-follow-up.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/accounts-and-collections/collection-follow-up.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_collection_follow_up_panel.dart`
