# Prompt 001 — Implement Patient Accounts

## Context

- **Sequence:** 001 of 004 in `prompts/02-billing/04-accounts-and-collections/`
- **Overall step:** 118 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Write-offs` (`prompts/02-billing/03-payments-and-receipts/006-write-offs.md`).
- **Menu path:** `Billing → Accounts & Collections → Patient Accounts`
- **Canonical route:** `/billing?section=patient-accounts`
...
- **Authoritative tab specification:** `.cursor/finance/billing/accounts-and-collections/patient-accounts.md`
...
2. **Register and scope the tab.**
   - Implement the `Patient Accounts` tab at `Billing → Patient Accounts` with canonical section slug `patient-accounts`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/accounts-and-collections/patient-accounts.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/accounts-and-collections/patient-accounts.md`:
     `Account No.`, `Patient No.`, `Patient Name`, `Primary Payer`, `Last Encounter No.`, `Last Encounter Date`, `Total Charges`, `Adjustments`, `Payments`, `Credits`, `Current Balance`, `Current`, `1–30 Days`, `31–60 Days`, `61–90 Days`, `Over 90 Days`, `Last Activity`, `Account Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Party`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **Open statement:** gate `billing:write` ∩ `billing-payments`; enable when account selected; Opens the filtered Patient Statements tab.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Patient Statements` (`prompts/02-billing/04-accounts-and-collections/002-patient-statements.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=patient-accounts` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/accounts-and-collections/patient-accounts.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/accounts-and-collections/patient-accounts.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_patient_accounts_panel.dart`
