# Prompt 002 — Implement Patient Statements

## Context

- **Sequence:** 002 of 004 in `prompts/02-billing/04-accounts-and-collections/`
- **Overall step:** 119 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Patient Accounts` (`prompts/02-billing/04-accounts-and-collections/001-patient-accounts.md`).
- **Menu path:** `Billing → Accounts & Collections → Patient Statements`
- **Canonical route:** `/billing?section=patient-statements`
...
- **Authoritative tab specification:** `.cursor/finance/billing/accounts-and-collections/patient-statements.md`
...
2. **Register and scope the tab.**
   - Implement the `Patient Statements` tab at `Billing → Patient Statements` with canonical section slug `patient-statements`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/accounts-and-collections/patient-statements.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/accounts-and-collections/patient-statements.md`:
     `Transaction Date/Time`, `Account No.`, `Patient No.`, `Patient Name`, `Encounter No.`, `Transaction Type`, `Document Reference`, `Description`, `Payer`, `Debit`, `Credit`, `Running Balance`, `Currency`, `Posting Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Receivables` (`prompts/02-billing/04-accounts-and-collections/003-receivables.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=patient-statements` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/accounts-and-collections/patient-statements.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/accounts-and-collections/patient-statements.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_patient_statements_panel.dart`
