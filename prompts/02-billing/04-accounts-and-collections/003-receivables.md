# Prompt 003 — Implement Receivables

## Context

- **Sequence:** 003 of 004 in `prompts/02-billing/04-accounts-and-collections/`
- **Overall step:** 120 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Patient Statements` (`prompts/02-billing/04-accounts-and-collections/002-patient-statements.md`).
- **Menu path:** `Billing → Accounts & Collections → Receivables`
- **Canonical route:** `/billing?section=receivables`
- **Surface profile:** `analysis` table/worklist tab
- **Navigation model:** `Billing → Accounts & Collections` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/billing/accounts-and-collections/receivables.md`
...
2. **Register and scope the tab.**
   - Implement the `Receivables` tab at `Billing → Receivables` with canonical section slug `receivables`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Accounts & Collections` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/accounts-and-collections/receivables.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/accounts-and-collections/receivables.md`:
     `Debtor Account No.`, `Patient/Customer/Payer`, `Debtor Type`, `Open Invoice Count`, `Current Amount`, `1–30 Days`, `31–60 Days`, `61–90 Days`, `Over 90 Days`, `Total Outstanding`, `Currency`, `Oldest Due Date`, `Days Overdue`, `Credit Limit`, `Collection Status`, `Assigned Collector`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Assign collector:** gate `billing:write` ∩ `billing-payments`; enable when open balances selected; Assigns collection ownership without changing source invoices.
     - **Save view:** gate `billing:read` ∩ `billing-payments`; enable when filters changed; Stores a named personal view.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Drill down:** gate `billing:read` ∩ `billing-payments`; enable when source dimension available; Opens the authoritative filtered source tab.
     - **Export row/detail:** gate `billing:read` ∩ `evidence:export` ∩ `billing-payments`; enable when row visible; Exports the selected breakdown.
   - Bulk buttons:
     - **Export selected:** gate `billing:read` ∩ `evidence:export` ∩ `billing-payments`; Exports only rows in the user's current ABAC scope.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Collection Follow-up` (`prompts/02-billing/04-accounts-and-collections/004-collection-follow-up.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=receivables` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/accounts-and-collections/receivables.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/accounts-and-collections/receivables.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_receivables_panel.dart`
