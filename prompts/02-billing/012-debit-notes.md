# Prompt 012 — Implement Debit Notes

## Context

- **Sequence:** 012 of 024 in `prompts/02-billing/`
- **Overall step:** 111 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Credit Notes` (`prompts/02-billing/011-credit-notes.md`).
- **Menu path:** `Billing → Debit Notes`
...
- **Authoritative tab specification:** `.cursor/finance/billing/debit-notes.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/debit-notes.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/debit-notes.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Payments` (`prompts/02-billing/013-payments.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/debit-notes.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/debit-notes.md`
