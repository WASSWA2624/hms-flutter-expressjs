...
- **Sequence:** 001 of 001 in `prompts/02-billing/05-overview/`
- **Overall step:** 122 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Collection Follow-up` (`prompts/02-billing/04-accounts-and-collections/004-collection-follow-up.md`).
- **Menu path:** `Billing → Overview → Overview`
- **Canonical route:** `/billing?section=overview`
- **Surface profile:** `overview` table/worklist tab
- **Navigation model:** `Billing → Overview` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/billing/overview/overview.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/overview/overview.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/overview/overview.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Billing Reports` (`prompts/02-billing/06-billing-reports/001-billing-reports.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/overview/overview.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/overview/overview.md`
