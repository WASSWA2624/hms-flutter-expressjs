# Prompt 001 — Implement Charges

## Context

- **Sequence:** 001 of 006 in `prompts/02-billing/02-charges-and-invoicing/`
- **Overall step:** 106 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Document Templates` (`prompts/02-billing/01-setup-and-pricing/006-document-templates.md`).
- **Menu path:** `Billing → Charges & Invoicing → Charges`
- **Canonical route:** `/billing?section=charges`
...
- **Authoritative tab specification:** `.cursor/finance/billing/charges-and-invoicing/charges.md`
...
2. **Register and scope the tab.**
   - Implement the `Charges` tab at `Billing → Charges` with canonical section slug `charges`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/charges-and-invoicing/charges.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/charges-and-invoicing/charges.md`:
     `Charge No.`, `Charge Date/Time`, `Patient No.`, `Patient Name`, `Encounter No.`, `Visit Type`, `Department`, `Service/Item Code`, `Service/Item Description`, `Quantity`, `Unit Price`, `Discount`, `Tax`, `Net Amount`, `Payer Type`, `Payer`, `Invoice No.`, `Charge Status`, `Charged By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Add charge:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens charge mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Charge Date/Time`, `Patient No.`, `Patient Name`, `Encounter No.`, `Visit Type`, `Department`, `Service/Item Code`, `Service/Item Description`, `Quantity`, `Unit Price`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Unbilled Charges` (`prompts/02-billing/02-charges-and-invoicing/002-unbilled-charges.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=charges` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/charges-and-invoicing/charges.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/charges-and-invoicing/charges.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_charges_panel.dart`
