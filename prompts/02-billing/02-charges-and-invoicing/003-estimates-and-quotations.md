# Prompt 003 — Implement Estimates & Quotations

## Context

- **Sequence:** 003 of 006 in `prompts/02-billing/02-charges-and-invoicing/`
- **Overall step:** 108 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Unbilled Charges` (`prompts/02-billing/02-charges-and-invoicing/002-unbilled-charges.md`).
- **Menu path:** `Billing → Charges & Invoicing → Estimates & Quotations`
- **Canonical route:** `/billing?section=estimates-and-quotations`
...
- **Authoritative tab specification:** `.cursor/finance/billing/charges-and-invoicing/estimates-and-quotations.md`
...
2. **Register and scope the tab.**
   - Implement the `Estimates & Quotations` tab at `Billing → Estimates & Quotations` with canonical section slug `estimates-and-quotations`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/charges-and-invoicing/estimates-and-quotations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/charges-and-invoicing/estimates-and-quotations.md`:
     `Estimate No.`, `Estimate Date`, `Valid Until`, `Patient/Customer`, `Encounter No.`, `Payer`, `Item Count`, `Subtotal`, `Discount`, `Tax`, `Total`, `Currency`, `Approval Status`, `Conversion Status`, `Invoice No.`, `Prepared By`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Party`, `Currency`, `Owner / assigned user`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New estimate:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Creates a draft estimate.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Estimate Date`, `Valid Until`, `Patient/Customer`, `Encounter No.`, `Payer`, `Discount`, `Currency`, `Invoice No.`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Invoices` (`prompts/02-billing/02-charges-and-invoicing/004-invoices.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=estimates-and-quotations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/charges-and-invoicing/estimates-and-quotations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/charges-and-invoicing/estimates-and-quotations.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_estimates_and_quotations_panel.dart`
