# Prompt 002 — Implement Unbilled Charges

## Context

- **Sequence:** 002 of 006 in `prompts/02-billing/02-charges-and-invoicing/`
- **Overall step:** 107 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Charges` (`prompts/02-billing/02-charges-and-invoicing/001-charges.md`).
- **Menu path:** `Billing → Charges & Invoicing → Unbilled Charges`
- **Canonical route:** `/billing?section=unbilled-charges`
- **Surface profile:** `transaction` table/worklist tab
- **Navigation model:** `Billing → Charges & Invoicing` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/billing/charges-and-invoicing/unbilled-charges.md`
...
2. **Register and scope the tab.**
   - Implement the `Unbilled Charges` tab at `Billing → Unbilled Charges` with canonical section slug `unbilled-charges`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Charges & Invoicing` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/charges-and-invoicing/unbilled-charges.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/charges-and-invoicing/unbilled-charges.md`:
     `Charge No.`, `Charge Date/Time`, `Patient No.`, `Patient Name`, `Encounter No.`, `Department`, `Service/Item`, `Quantity`, `Net Amount`, `Payer`, `Eligibility Status`, `Billing Hold Reason`, `Age`, `Responsible User`, `Selected for Billing`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Department / cost centre`, `Party`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **Generate invoices:** gate `billing:write` ∩ `billing-payments`; enable when selected eligible rows; Creates draft invoices and refreshes Billing counts.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Charge Date/Time`, `Patient No.`, `Patient Name`, `Encounter No.`, `Department`, `Service/Item`, `Quantity`, `Net Amount`, `Payer`, `Billing Hold Reason`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Estimates & Quotations` (`prompts/02-billing/02-charges-and-invoicing/003-estimates-and-quotations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=unbilled-charges` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/charges-and-invoicing/unbilled-charges.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/charges-and-invoicing/unbilled-charges.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_unbilled_charges_panel.dart`
