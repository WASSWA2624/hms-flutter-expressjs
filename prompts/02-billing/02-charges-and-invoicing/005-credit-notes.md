# Prompt 005 — Implement Credit Notes

## Context

- **Sequence:** 005 of 006 in `prompts/02-billing/02-charges-and-invoicing/`
- **Overall step:** 110 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Invoices` (`prompts/02-billing/02-charges-and-invoicing/004-invoices.md`).
- **Menu path:** `Billing → Charges & Invoicing → Credit Notes`
- **Canonical route:** `/billing?section=credit-notes`
...
- **Authoritative tab specification:** `.cursor/finance/billing/charges-and-invoicing/credit-notes.md`
...
2. **Register and scope the tab.**
   - Implement the `Credit Notes` tab at `Billing → Credit Notes` with canonical section slug `credit-notes`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/charges-and-invoicing/credit-notes.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/charges-and-invoicing/credit-notes.md`:
     `Credit Note No.`, `Note Date`, `Original Invoice No.`, `Patient/Customer`, `Payer`, `Reason Code`, `Reason`, `Subtotal`, `Tax`, `Total Credit`, `Applied Amount`, `Remaining Credit`, `Currency`, `Approval Status`, `Journal Entry No.`, `Approved By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted and source prerequisites met; Opens the Credit Notes mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Note Date`, `Original Invoice No.`, `Patient/Customer`, `Payer`, `Reason Code`, `Reason`, `Applied Amount`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Debit Notes` (`prompts/02-billing/02-charges-and-invoicing/006-debit-notes.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=credit-notes` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/charges-and-invoicing/credit-notes.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/charges-and-invoicing/credit-notes.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_credit_notes_panel.dart`
