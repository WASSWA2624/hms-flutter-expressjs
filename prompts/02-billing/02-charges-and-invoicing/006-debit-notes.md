# Prompt 006 — Implement Debit Notes

## Context

- **Sequence:** 006 of 006 in `prompts/02-billing/02-charges-and-invoicing/`
- **Overall step:** 111 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Credit Notes` (`prompts/02-billing/02-charges-and-invoicing/005-credit-notes.md`).
- **Menu path:** `Billing → Charges & Invoicing → Debit Notes`
- **Canonical route:** `/billing?section=debit-notes`
...
- **Authoritative tab specification:** `.cursor/finance/billing/charges-and-invoicing/debit-notes.md`
...
2. **Register and scope the tab.**
   - Implement the `Debit Notes` tab at `Billing → Debit Notes` with canonical section slug `debit-notes`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/charges-and-invoicing/debit-notes.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/charges-and-invoicing/debit-notes.md`:
     `Debit Note No.`, `Note Date`, `Original Invoice No.`, `Patient/Customer`, `Payer`, `Reason Code`, `Reason`, `Subtotal`, `Tax`, `Total Debit`, `Applied Amount`, `Remaining Debit`, `Currency`, `Approval Status`, `Journal Entry No.`, `Approved By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted and source prerequisites met; Opens the Debit Notes mutation dialog.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Payments` (`prompts/02-billing/03-payments-and-receipts/001-payments.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=debit-notes` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/charges-and-invoicing/debit-notes.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/charges-and-invoicing/debit-notes.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_debit_notes_panel.dart`
