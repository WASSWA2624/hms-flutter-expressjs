# Prompt 004 — Implement Invoices

## Context

- **Sequence:** 004 of 006 in `prompts/02-billing/02-charges-and-invoicing/`
- **Overall step:** 109 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Estimates & Quotations` (`prompts/02-billing/02-charges-and-invoicing/003-estimates-and-quotations.md`).
- **Menu path:** `Billing → Charges & Invoicing → Invoices`
- **Canonical route:** `/billing?section=invoices`
...
- **Authoritative tab specification:** `.cursor/finance/billing/charges-and-invoicing/invoices.md`
...
2. **Register and scope the tab.**
   - Implement the `Invoices` tab at `Billing → Invoices` with canonical section slug `invoices`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/charges-and-invoicing/invoices.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/charges-and-invoicing/invoices.md`:
     `Invoice No.`, `Invoice Date`, `Due Date`, `Patient/Customer`, `Patient No.`, `Encounter No.`, `Payer Type`, `Payer`, `Subtotal`, `Discount`, `Tax`, `Total`, `Paid Amount`, `Allocated Amount`, `Balance`, `Currency`, `Aging Days`, `Invoice Status`, `Issued By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New invoice:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens invoice header and line-item dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Invoice Date`, `Due Date`, `Patient/Customer`, `Patient No.`, `Encounter No.`, `Payer Type`, `Payer`, `Discount`, `Currency`.
   - Detail sections: `Summary`, `Line items`, `Payments & allocations`, `Adjustments`, `Documents & audit`.
...
8. **Implement workflow, validation, permissions, and audit.**
   - Statuses represented by this tab: `Draft`, `Issued`, `Partially paid`, `Paid`, `Overdue`, `Cancelled / Written off`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Credit Notes` (`prompts/02-billing/02-charges-and-invoicing/005-credit-notes.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=invoices` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/charges-and-invoicing/invoices.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/charges-and-invoicing/invoices.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_invoices_panel.dart`
