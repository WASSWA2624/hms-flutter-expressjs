# Prompt 009 — Implement Estimates & Quotations

## Context

- **Sequence:** 009 of 024 in `prompts/02-billing/`
- **Overall step:** 108 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Unbilled Charges` (`prompts/02-billing/008-unbilled-charges.md`).
- **Menu path:** `Billing → Estimates & Quotations`
...
- **Authoritative tab specification:** `.cursor/finance/billing/estimates-and-quotations.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/estimates-and-quotations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/estimates-and-quotations.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Invoices` (`prompts/02-billing/010-invoices.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/estimates-and-quotations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/estimates-and-quotations.md`
