# Prompt 001 — Implement Price Lists & Tariffs

## Context

- **Sequence:** 001 of 006 in `prompts/02-billing/01-setup-and-pricing/`
- **Overall step:** 100 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Accounts & Finance → Overview` (`prompts/01-accounts-and-finance/13-overview/001-overview.md`).
- **Menu path:** `Billing → Setup & Pricing → Price Lists & Tariffs`
- **Canonical route:** `/billing?section=price-lists-and-tariffs`
- **Surface profile:** `master` table/worklist tab
- **Navigation model:** `Billing → Setup & Pricing` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/billing/setup-and-pricing/price-lists-and-tariffs.md`
...
1. **Audit the existing implementation before writing code.**
   ...
   - Reuse the existing Billing workspace, controller, entities, DTOs, and repositories under `frontend/lib/features/billing/` and `backend/src/modules/billing/`.
...
2. **Register and scope the tab.**
   - Implement the `Price Lists & Tariffs` tab at `Billing → Price Lists & Tariffs` with canonical section slug `price-lists-and-tariffs`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Setup & Pricing` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/setup-and-pricing/price-lists-and-tariffs.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/setup-and-pricing/price-lists-and-tariffs.md`:
     `Price List Code`, `Price List Name`, `Payer/Customer Group`, `Service/Item Code`, `Service/Item Name`, `Department`, `Standard Price`, `Contract Price`, `Discount Ceiling`, `Tax Code`, `Currency`, `Effective From`, `Effective To`, `Version`, `Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens the Price Lists & Tariffs create dialog.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Edit:** gate `billing:write` ∩ `billing-payments`; enable when record active/draft and version current; Opens prefilled edit dialog.
     - **Clone:** gate `billing:write` ∩ `billing-payments`; enable when record clonable; Creates an unsaved copy with a new reference.
     - **Activate / deactivate:** gate `billing:write` ∩ `billing-payments`; enable when transition allowed; Requires confirmation and refreshes dependants.
     - **Archive / restore:** gate `billing:write` ∩ `billing-payments`; enable when not referenced by a blocking workflow; Soft archive only; never hard-delete referenced data.
   - Bulk buttons:
     - **Activate / deactivate selected:** gate `billing:write` ∩ `billing-payments`; Only compatible, unblocked records are changed.
     - **Archive selected:** gate `billing:write` ∩ `billing-payments`; Soft archive; partial failures return per-row reasons.
     - **Export selected:** gate `billing:read` ∩ `evidence:export` ∩ `billing-payments`; Exports selected rows in visible-column order.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Price List Code`, `Price List Name`, `Payer/Customer Group`, `Service/Item Code`, `Service/Item Name`, `Department`, `Standard Price`, `Contract Price`, `Discount Ceiling`, `Tax Code`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Packages & Bundles` (`prompts/02-billing/01-setup-and-pricing/002-packages-and-bundles.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=price-lists-and-tariffs` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/setup-and-pricing/price-lists-and-tariffs.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/setup-and-pricing/price-lists-and-tariffs.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_price_lists_and_tariffs_panel.dart`
