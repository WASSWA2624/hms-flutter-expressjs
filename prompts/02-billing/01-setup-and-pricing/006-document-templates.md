# Prompt 006 — Implement Document Templates

## Context

- **Sequence:** 006 of 006 in `prompts/02-billing/01-setup-and-pricing/`
- **Overall step:** 105 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Tax Rules` (`prompts/02-billing/01-setup-and-pricing/005-tax-rules.md`).
- **Menu path:** `Billing → Setup & Pricing → Document Templates`
- **Canonical route:** `/billing?section=document-templates`
...
- **Authoritative tab specification:** `.cursor/finance/billing/setup-and-pricing/document-templates.md`
...
2. **Register and scope the tab.**
   - Implement the `Document Templates` tab at `Billing → Document Templates` with canonical section slug `document-templates`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/setup-and-pricing/document-templates.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/setup-and-pricing/document-templates.md`:
     `Template Code`, `Template Name`, `Document Type`, `Facility`, `Language`, `Page Size`, `Header/Footer Version`, `Numbering Sequence`, `Default Template`, `Effective From`, `Effective To`, `Last Published At`, `Published By`, `Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Facility`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens the Document Templates create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Template Code`, `Template Name`, `Document Type`, `Facility`, `Language`, `Page Size`, `Header/Footer Version`, `Numbering Sequence`, `Default Template`, `Effective From`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Charges` (`prompts/02-billing/02-charges-and-invoicing/001-charges.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=document-templates` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/setup-and-pricing/document-templates.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/setup-and-pricing/document-templates.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_document_templates_panel.dart`
