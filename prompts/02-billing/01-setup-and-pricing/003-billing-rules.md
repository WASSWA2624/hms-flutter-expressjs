# Prompt 003 — Implement Billing Rules

## Context

- **Sequence:** 003 of 006 in `prompts/02-billing/01-setup-and-pricing/`
- **Overall step:** 102 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Packages & Bundles` (`prompts/02-billing/01-setup-and-pricing/002-packages-and-bundles.md`).
- **Menu path:** `Billing → Setup & Pricing → Billing Rules`
- **Canonical route:** `/billing?section=billing-rules`
...
- **Authoritative tab specification:** `.cursor/finance/billing/setup-and-pricing/billing-rules.md`
...
2. **Register and scope the tab.**
   - Implement the `Billing Rules` tab at `Billing → Billing Rules` with canonical section slug `billing-rules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/setup-and-pricing/billing-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/setup-and-pricing/billing-rules.md`:
     `Rule Code`, `Rule Name`, `Rule Type`, `Facility Scope`, `Payer Scope`, `Service Scope`, `Trigger Event`, `Condition Summary`, `Action Summary`, `Priority`, `Effective From`, `Effective To`, `Version`, `Status`, `Last Changed By`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Facility`, `Party`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens the Billing Rules create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Rule Code`, `Rule Name`, `Rule Type`, `Facility Scope`, `Payer Scope`, `Service Scope`, `Trigger Event`, `Condition Summary`, `Action Summary`, `Priority`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Discount Rules` (`prompts/02-billing/01-setup-and-pricing/004-discount-rules.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=billing-rules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/setup-and-pricing/billing-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/setup-and-pricing/billing-rules.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_billing_rules_panel.dart`
