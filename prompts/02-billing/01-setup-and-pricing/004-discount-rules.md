# Prompt 004 — Implement Discount Rules

## Context

- **Sequence:** 004 of 006 in `prompts/02-billing/01-setup-and-pricing/`
- **Overall step:** 103 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Billing Rules` (`prompts/02-billing/01-setup-and-pricing/003-billing-rules.md`).
- **Menu path:** `Billing → Setup & Pricing → Discount Rules`
- **Canonical route:** `/billing?section=discount-rules`
...
- **Authoritative tab specification:** `.cursor/finance/billing/setup-and-pricing/discount-rules.md`
...
2. **Register and scope the tab.**
   - Implement the `Discount Rules` tab at `Billing → Discount Rules` with canonical section slug `discount-rules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/setup-and-pricing/discount-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/setup-and-pricing/discount-rules.md`:
     `Rule Code`, `Rule Name`, `Discount Type`, `Percentage/Fixed Value`, `Maximum Amount`, `Patient/Payer Scope`, `Service/Department Scope`, `Minimum Amount`, `Approval Required`, `Approver Role`, `Effective From`, `Effective To`, `Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Department / cost centre`, `Party`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens the Discount Rules create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Rule Code`, `Rule Name`, `Discount Type`, `Percentage/Fixed Value`, `Maximum Amount`, `Patient/Payer Scope`, `Service/Department Scope`, `Minimum Amount`, `Approval Required`, `Approver Role`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Tax Rules` (`prompts/02-billing/01-setup-and-pricing/005-tax-rules.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=discount-rules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/setup-and-pricing/discount-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/setup-and-pricing/discount-rules.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_discount_rules_panel.dart`
