# Prompt 002 — Implement Packages & Bundles

## Context

- **Sequence:** 002 of 006 in `prompts/02-billing/01-setup-and-pricing/`
- **Overall step:** 101 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Price Lists & Tariffs` (`prompts/02-billing/01-setup-and-pricing/001-price-lists-and-tariffs.md`).
- **Menu path:** `Billing → Setup & Pricing → Packages & Bundles`
- **Canonical route:** `/billing?section=packages-and-bundles`
...
- **Authoritative tab specification:** `.cursor/finance/billing/setup-and-pricing/packages-and-bundles.md`
...
2. **Register and scope the tab.**
   - Implement the `Packages & Bundles` tab at `Billing → Packages & Bundles` with canonical section slug `packages-and-bundles`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/setup-and-pricing/packages-and-bundles.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/setup-and-pricing/packages-and-bundles.md`:
     `Package Code`, `Package Name`, `Category`, `Included Services`, `Included Quantity`, `Standard Price`, `Package Price`, `Discount Value`, `Payer/Price List`, `Effective From`, `Effective To`, `Usage Limit`, `Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Party`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New package:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens package editor with included-services table.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Package Code`, `Package Name`, `Category`, `Included Services`, `Included Quantity`, `Standard Price`, `Package Price`, `Discount Value`, `Payer/Price List`, `Effective From`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Billing Rules` (`prompts/02-billing/01-setup-and-pricing/003-billing-rules.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=packages-and-bundles` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/setup-and-pricing/packages-and-bundles.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/setup-and-pricing/packages-and-bundles.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_packages_and_bundles_panel.dart`
