# Prompt 005 — Implement Tax Rules

## Context

- **Sequence:** 005 of 006 in `prompts/02-billing/01-setup-and-pricing/`
- **Overall step:** 104 of 158
- **Phase:** Billing foundation — configuration and revenue documents
- **Prerequisite:** `Billing → Discount Rules` (`prompts/02-billing/01-setup-and-pricing/004-discount-rules.md`).
- **Menu path:** `Billing → Setup & Pricing → Tax Rules`
- **Canonical route:** `/billing?section=tax-rules`
...
- **Authoritative tab specification:** `.cursor/finance/billing/setup-and-pricing/tax-rules.md`
...
2. **Register and scope the tab.**
   - Implement the `Tax Rules` tab at `Billing → Tax Rules` with canonical section slug `tax-rules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/setup-and-pricing/tax-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/setup-and-pricing/tax-rules.md`:
     `Tax Code`, `Tax Name`, `Tax Type`, `Rate`, `Inclusive/Exclusive`, `Recoverable`, `Service/Item Scope`, `Payer Exemption`, `GL Account`, `Effective From`, `Effective To`, `Jurisdiction`, `Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `billing-payments`; enable when write permitted; Opens the Tax Rules create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Tax Code`, `Tax Name`, `Tax Type`, `Rate`, `Inclusive/Exclusive`, `Recoverable`, `Service/Item Scope`, `Payer Exemption`, `GL Account`, `Effective From`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Document Templates` (`prompts/02-billing/01-setup-and-pricing/006-document-templates.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=tax-rules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/setup-and-pricing/tax-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/setup-and-pricing/tax-rules.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_tax_rules_panel.dart`
