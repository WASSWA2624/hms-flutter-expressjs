# Prompt 022 — Implement Collection Follow-up

## Context

- **Sequence:** 022 of 024 in `prompts/02-billing/`
- **Overall step:** 121 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Receivables` (`prompts/02-billing/021-receivables.md`).
- **Menu path:** `Billing → Collection Follow-up`
...
- **Authoritative tab specification:** `.cursor/finance/billing/collection-follow-up.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/collection-follow-up.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/collection-follow-up.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Overview` (`prompts/02-billing/023-overview.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/collection-follow-up.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/collection-follow-up.md`
