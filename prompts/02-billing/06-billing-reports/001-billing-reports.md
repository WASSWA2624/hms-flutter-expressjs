# Prompt 001 — Implement Billing Reports

## Context

- **Sequence:** 001 of 001 in `prompts/02-billing/06-billing-reports/`
- **Overall step:** 123 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Overview` (`prompts/02-billing/05-overview/001-overview.md`).
- **Menu path:** `Billing → Billing Reports → Billing Reports`
- **Canonical route:** `/billing?section=billing-reports`
- **Surface profile:** `report` table/worklist tab
- **Navigation model:** `Billing → Billing Reports` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/billing/billing-reports/billing-reports.md`
...
2. **Register and scope the tab.**
   - Implement the `Billing Reports` tab at `Billing → Billing Reports` with canonical section slug `billing-reports`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Billing Reports` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/billing-reports/billing-reports.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/billing-reports/billing-reports.md`:
     `Report Name`, `Report Category`, `Description`, `Default Period`, `Available Filters`, `Default Grouping`, `Last Generated At`, `Generated By`, `Output Format`, `Row Count`, `Schedule`, `Delivery Recipients`, `Report Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Run report:** gate `billing:read` ∩ `billing-payments`; enable when filters valid; Refreshes report rows and totals.
     - **Schedule:** gate `billing:write` ∩ `billing-payments`; enable when scheduling supported; Opens report schedule dialog.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Insurance Providers` (`prompts/03-insurance-and-claims/01-payers-and-contracts/001-insurance-providers.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=billing-reports` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/billing-reports/billing-reports.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/billing-reports/billing-reports.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_billing_reports_panel.dart`
