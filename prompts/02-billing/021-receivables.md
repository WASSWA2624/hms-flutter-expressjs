# Prompt 021 — Implement Receivables

## Context

- **Sequence:** 021 of 024 in `prompts/02-billing/`
- **Overall step:** 120 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Patient Statements` (`prompts/02-billing/020-patient-statements.md`).
...
- **Authoritative tab specification:** `.cursor/finance/billing/receivables.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/receivables.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/receivables.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Collection Follow-up` (`prompts/02-billing/022-collection-follow-up.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/billing/receivables.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/receivables.md`
