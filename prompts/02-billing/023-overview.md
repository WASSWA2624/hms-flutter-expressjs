# Prompt 023 — Implement Overview

## Context

- **Sequence:** 023 of 024 in `prompts/02-billing/`
- **Overall step:** 122 of 158
- **Phase:** Billing collections and receivables
- **Prerequisite:** `Billing → Collection Follow-up` (`prompts/02-billing/022-collection-follow-up.md`).
- **Menu path:** `Billing → Overview`
- **Canonical route:** `/billing?section=overview`
...
- **Authoritative tab specification:** `.cursor/finance/billing/overview.md`
...
2. **Register and scope the tab.**
   - Implement the `Overview` tab at `Billing → Overview` with canonical section slug `overview`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/billing/overview.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/billing/overview.md`:
     `Queue/Metric`, `Current Count`, `Total Value`, `Currency`, `Today`, `Month to Date`, `Overdue Count`, `Oldest Item Age`, `Assigned Team`, `SLA Status`, `Trend`, `Last Refreshed`.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Open queue:** gate `billing:read` ∩ `billing-payments`; enable when metric has a destination; Opens the matching filtered tab.
   - Bulk buttons:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Billing → Billing Reports` (`prompts/02-billing/024-billing-reports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/billing?section=overview` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/billing/overview.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/billing/overview.md`
...
- `frontend/lib/features/billing/presentation/widgets/billing_overview_panel.dart`
