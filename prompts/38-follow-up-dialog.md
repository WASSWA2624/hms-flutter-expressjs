...
| Field | Value |
| --- | --- |
| Symbol | `FollowUpDialog` |
| Purpose | Follow Up |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:2255` |
...
| Primary commit | Save follow-up |
| Slices to keep in sync | follow-up appointment/task, encounter disposition cues |
| Sibling reuse targets | `ReferralDialog`, `PatientAppointmentQuickDialog` |
...
| Mutations (region) | `mutation: createFollowUp` |
...
- `ClinicalFollowUpActionDialog — frontend/lib/shared/clinical_actions/dialogs/clinical_follow_up_action_dialog.dart`
...
- `backend/src/tests/modules/follow-up/services/follow-up.service.test.js`
- `backend/src/tests/modules/follow-up/controllers/follow-up.controller.test.js`
- `backend/src/modules/follow-up/services/follow-up.service.js`
- `backend/src/modules/follow-up/routes/follow-up.routes.js`
- `backend/src/modules/follow-up/controllers/follow-up.controller.js`
...
| Title snippets | `l10n.opdFollowUpAction`, `title`, `l10n.clinicalFollowUpDetailsTitle`, `l10n.clinicalFollowUpNotesTitle` |
...
**Scope lock:** only `FollowUpDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `FollowUpDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:2255` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Follow Up** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save follow-up`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `ReferralDialog`, `PatientAppointmentQuickDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch follow-up appointment/task, encounter disposition cues, then apply the smallest targeted reconciliation.
...
1. `FollowUpDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `ReferralDialog`, `PatientAppointmentQuickDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: follow-up appointment/task, encounter disposition cues.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `FollowUpDialog`'s path.
...
<!-- generator: encounter-dialog prompt 38 slug=follow-up-dialog symbol=FollowUpDialog shape=widget_dialog -->
