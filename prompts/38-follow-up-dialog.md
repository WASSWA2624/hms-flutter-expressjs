...
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1856` |
...
| Title snippets | `l10n.opdFollowUpAction` |
...
2. Confirm mutating openers set `barrierDismissible: false` while the dialog can mutate.
3. No obvious loading primitive near the symbol — add shared loading UX for async open/submit.
...
1. **Read contracts + source**
   ...
   - Read `FollowUpDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1856` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `ReferralDialog`, `PatientAppointmentQuickDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: follow-up appointment/task, encounter disposition cues.
