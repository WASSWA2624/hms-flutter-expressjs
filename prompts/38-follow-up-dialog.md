...
**Scope:** only `FollowUpDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: createFollowUp`
...
| Purpose | Patient/encounter flow: Follow Up (shared/opd_actions). |
...
- No clear `AppDialog` / `showAppDialog` / workspace-helper usage found near the symbol — verify the shell and migrate if needed.
...
1. **Read contract + source**
   ...
   - Read `FollowUpDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1856` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Follow Up (shared/opd_actions).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `FollowUpDialog`'s path.
...
<!-- generator: encounter-dialog prompt 38 slug=follow-up-dialog symbol=FollowUpDialog -->
