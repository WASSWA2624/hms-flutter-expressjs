# Clinical Results Ready tab — rule compliance

## Context

Make the Results Ready desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/07-clinical/04-results-ready.md`. Apply shared chrome fixes from `prompts/07-clinical/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `clinicalSectionResultsReadyLabel`, deep-link `results-ready` (aliases `results_ready`, `resultsready`, `results`), and omit the tab when gate denies access (`ClinicalResultsReadyAtomPermissions.tab` = `clinicalWorkspaceReadRequirement`) — never show a disabled placeholder (`tabs.mdc`). Align `ClinicalWorkspaceSection` / query helpers with `tabs/07-clinical/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.resultsReadyCount`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/07-clinical/04-results-ready.md` updated to match.
