# Clinical overview — workspace compliance program

## Context

Bring the Clinical desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/07-clinical/`. Per-surface remediation prompts live beside this file under `prompts/07-clinical/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/07-clinical/00-overview.md` as the authoritative tab index (`ClinicalWorkspaceSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/07-clinical/01-pending.md`
   - `prompts/07-clinical/02-assigned-to-me.md`
   - `prompts/07-clinical/03-urgent.md`
   - `prompts/07-clinical/04-results-ready.md`
   - `prompts/07-clinical/05-completed.md`
   - `prompts/07-clinical/06-follow-ups.md`
...
6. After each remediation, update the matching inventory file under `tabs/07-clinical/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/clinical/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/07-clinical/00-overview.md` has a completed remediation prompt under `prompts/07-clinical/` and a matching updated inventory under `tabs/07-clinical/`.
...
- Trace `frontend/lib/features/clinical/presentation/pages/clinical_workspace_page.dart`, `frontend/lib/features/clinical/presentation/clinical_access.dart`, and widgets listed in `tabs/07-clinical/00-overview.md`.
- Run feature tests under `frontend/test/features/clinical/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/07-clinical/00-overview.md`
- `prompts/07-clinical/00-shared-chrome.md`
- `prompts/07-clinical/99-convention-gaps.md`
- `prompts/07-clinical/01-pending.md`
- `prompts/07-clinical/02-assigned-to-me.md`
- `prompts/07-clinical/03-urgent.md`
- `prompts/07-clinical/04-results-ready.md`
