# Clinical Assigned To Me tab — rule compliance

## Context

Make the Assigned To Me desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/07-clinical/02-assigned-to-me.md`. Apply shared chrome fixes from `prompts/07-clinical/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `clinicalSectionAssignedToMeLabel`, deep-link `assigned-to-me` (aliases `assigned_to_me`, `assignedtome`, `mine`, `assigned`), and omit the tab when gate denies access (`ClinicalAssignedToMeAtomPermissions.tab` = `clinicalWorkspaceReadRequirement`) — never show a disabled placeholder (`tabs.mdc`). Align `ClinicalWorkspaceSection` / query helpers with `tabs/07-clinical/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.assignedToMeCount`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/07-clinical/02-assigned-to-me.md` updated to match.
