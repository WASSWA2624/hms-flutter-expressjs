...
| Field | Value |
| --- | --- |
| Symbol | `QueueActionsDialog` |
| Purpose | Queue Actions |
| Module / surface | `shared/opd_actions` |
| Inventory kind | `shared` |
| Presentation shape | `action_hub` |
| Verified definition | `frontend/lib/shared/opd_actions/opd_queue_actions_dialog.dart:45` |
...
| Paired opener(s) | `showQueueActionsDialog` |
| Primary commit | Execute selected queue action / Cancel |
| Slices to keep in sync | OPD/reception queue rows, encounter stage badges |
| Sibling reuse targets | `ReceptionQueueActionsDialog`, `FlowActionsDialog` |
...
| Mutations (region) | `mutation: prioritizeQueueEntry`, `mutation: startOpdFromQueue`, `mutation: moveQueueEntry` |
...
**Scope lock:** only `QueueActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `QueueActionsDialog` at `frontend/lib/shared/opd_actions/opd_queue_actions_dialog.dart:45` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Queue Actions** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Execute selected queue action / Cancel`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `ReceptionQueueActionsDialog`, `FlowActionsDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch OPD/reception queue rows, encounter stage badges, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `showQueueActionsDialog`. Update all call sites in the same change when signatures move.
...
1. `QueueActionsDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `ReceptionQueueActionsDialog`, `FlowActionsDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: OPD/reception queue rows, encounter stage badges.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `QueueActionsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 13 slug=opd-queue-actions-dialog symbol=QueueActionsDialog shape=action_hub -->
