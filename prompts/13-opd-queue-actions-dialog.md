...
**Scope:** only `QueueActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `opdWorkspaceControllerProvider.notifier`
...
| Purpose | Patient/encounter flow: Queue Actions (opd). |
...
1. **Read contract + source**
   ...
   - Read `QueueActionsDialog` at `frontend/lib/shared/opd_actions/opd_queue_actions_dialog.dart:45` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Queue Actions (opd).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `showQueueActionsDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `QueueActionsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 13 slug=opd-queue-actions-dialog symbol=QueueActionsDialog -->
