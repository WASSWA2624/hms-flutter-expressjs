...
| Defined in | `frontend/lib/shared/opd_actions/opd_queue_actions_dialog.dart:45` |
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `ReceptionQueueActionsDialog`, `FlowActionsDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: OPD/reception queue rows, encounter stage badges.
