...
| Defined in | `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart:2865` |
| Kind | `custom` |
| Paired opener(s) | `—` |
...
1. **Read contract + source**
   ...
   - Read `QueueActionsDialog` at `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart:2865` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Queue Actions (opd).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
   - Wire title through the shell so uppercase normalization applies.
   - Add/keep a meaningful `icon` if peer dialogs in `opd` already use icons.
...
1. `QueueActionsDialog` opens through `AppDialog` / approved helpers only.
