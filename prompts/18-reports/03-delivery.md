# Reports Delivery tab — rule compliance

## Context

Make the Delivery desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/18-reports/03-delivery.md`. Apply shared chrome fixes from `prompts/18-reports/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `reportsPanelDelivery`, deep-link `section` per inventory, and omit the tab when gate denies access (catalog read + pack includes `delivery`) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/18-reports/00-overview.md`.
...
8. Preserve row-select → detail/actions hub with **generic titles**; keep nested mutations in-desk via shared dialogs/forms; omit unauthorized nested actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Preview / run detail; Cancel confirm; Retry / Download / Print paths.
...
- [ ] `tabs/18-reports/03-delivery.md` updated to match.
...
- `tabs/18-reports/03-delivery.md`
