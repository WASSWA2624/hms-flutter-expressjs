# Reports Phi tab — rule compliance

## Context

Make the Phi desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/18-reports/08-phi.md`. Apply shared chrome fixes from `prompts/18-reports/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `reportsPanelPhi`, deep-link `section` per inventory, and omit the tab when gate denies access (`reportsComplianceReadRequirement`) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/18-reports/00-overview.md`.
...
- [ ] `tabs/18-reports/08-phi.md` updated to match.
