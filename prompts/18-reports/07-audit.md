# Reports Audit tab — rule compliance

## Context

Make the Audit desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/18-reports/07-audit.md`. Apply shared chrome fixes from `prompts/18-reports/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `reportsPanelAudit`, deep-link `section` per inventory, and omit the tab when gate denies access (`reportsComplianceReadRequirement` (∪ `compliance:read` \| `compliance:review` + admin)) — never show a disabled placeholder (`tabs.mdc`). Align `desk section` / query helpers with `tabs/18-reports/00-overview.md`.
...
8. Preserve in-desk actions for this surface via shared dialogs/forms; omit unauthorized actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Compliance detail reportsComplianceDetailTitle.
...
- [ ] `tabs/18-reports/07-audit.md` updated to match.
