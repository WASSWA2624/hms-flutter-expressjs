# Reports overview — workspace compliance program

## Context

Bring the Reports desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/18-reports/`. Per-surface remediation prompts live beside this file under `prompts/18-reports/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/18-reports/00-overview.md` as the authoritative tab index (`desk section enum` order, query values, aliases). Keep deep-link helpers aligned with that index.
...
6. After each remediation, update the matching inventory file under `tabs/18-reports/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/reports/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/18-reports/00-overview.md` has a completed remediation prompt under `prompts/18-reports/` and a matching updated inventory under `tabs/18-reports/`.
...
- Trace `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart`, `frontend/lib/features/reports/presentation/reports_access.dart`, and widgets listed in `tabs/18-reports/00-overview.md`.
- Run feature tests under `frontend/test/features/reports/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/18-reports/00-overview.md`
- `prompts/18-reports/00-shared-chrome.md`
- `prompts/18-reports/99-convention-gaps.md`
