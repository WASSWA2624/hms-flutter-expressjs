# Billing overview — workspace compliance program

## Context

Bring the Billing desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/13-billing/`. Per-surface remediation prompts live beside this file under `prompts/13-billing/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/13-billing/00-overview.md` as the authoritative tab index (`BillingQueueType` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/13-billing/01-open-work.md`
   - `prompts/13-billing/02-to-issue.md`
   - `prompts/13-billing/03-collect-due.md`
   - `prompts/13-billing/04-open-claims.md`
   - `prompts/13-billing/05-need-approval.md`
   - `prompts/13-billing/06-price-book.md`
...
6. After each remediation, update the matching inventory file under `tabs/13-billing/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/billing/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/13-billing/00-overview.md` has a completed remediation prompt under `prompts/13-billing/` and a matching updated inventory under `tabs/13-billing/`.
...
- Trace `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart`, `frontend/lib/features/billing/presentation/billing_access.dart`, and widgets listed in `tabs/13-billing/00-overview.md`.
- Run feature tests under `frontend/test/features/billing/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/13-billing/00-overview.md`
- `prompts/13-billing/00-shared-chrome.md`
- `prompts/13-billing/99-convention-gaps.md`
- `prompts/13-billing/01-open-work.md`
- `prompts/13-billing/02-to-issue.md`
- `prompts/13-billing/03-collect-due.md`
- `prompts/13-billing/04-open-claims.md`
