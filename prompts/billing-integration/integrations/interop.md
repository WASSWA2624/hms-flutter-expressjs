# Billing Integration Scan — Integrations workspace / Interop (`/integrations?…=interop`)
...
- Financial focus for this tab: Interop payloads that create orders or payments must invoke the same Billing engine/idempotency paths as UI; webhooks must not acknowledge settlement without a Billing ledger entry. Tab role: Interop probes; write to run tests.
