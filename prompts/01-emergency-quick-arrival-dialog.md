...
| Defined in | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:29` |
| Kind | `custom` |
...
1. **Read contract + source**
   ...
   - Read `QuickArrivalDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:29` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Quick Arrival (emergency).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `QuickArrivalDialog` opens through `AppDialog` / approved helpers only.
