...
| Purpose | Quick Arrival |
| Module / surface | `emergency` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:36` |
...
| Primary commit | Save / create arrival |
| Slices to keep in sync | emergency queue, encounter, patient context, badges |
| Sibling reuse targets | `DispatchDialog`, `HandoffDialog`, `AppTriageActionDialog` |
...
| Mutations (region) | `mutation: createQuickArrival` |
...
| Title snippets | `EmergencyText.quickEmergencyArrival`, `widget.title` |
...
**Scope lock:** only `QuickArrivalDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `QuickArrivalDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:36` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Quick Arrival** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Save / create arrival`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `DispatchDialog`, `HandoffDialog`, `AppTriageActionDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch emergency queue, encounter, patient context, badges, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openQuickArrivalDialog`. Update all call sites in the same change when signatures move.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `DispatchDialog`, `HandoffDialog`, `AppTriageActionDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency queue, encounter, patient context, badges.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `QuickArrivalDialog`'s path.
...
<!-- generator: encounter-dialog prompt 01 slug=emergency-quick-arrival-dialog symbol=QuickArrivalDialog shape=widget_dialog -->
