...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `DispatchDialog`, `HandoffDialog`, `AppTriageActionDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency queue, encounter, patient context, badges.
