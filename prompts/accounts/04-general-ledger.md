...
Implement the **General ledger** desk section (`?section=gl`) on `/accounts` per `accounts.md`. This is the facility GL by account — balances and activity, not a second journal queue and not patient money. Source of truth: root `accounts.md` §§2–8, 9–11.
...
9. Inside Account ledger, show **Journal** when the user can create journals (`accounts:write`); do not offer Post (Post stays on To post).
10. Deep link `?section=gl&accountId=` opens Account ledger or filters General ledger to that account after load.
11. Gate with (`accounts:read` ∪ `accounts:write`) ∩ `facility-accounts`. Omit unauthorized Next / Journal; no disabled “no access” chrome.
12. Keep count as accounts with activity. Enable realtime + light poll while active.
13. Count tone follows open-period risk only via filter chips elsewhere — this tab is not warning/danger by default.
...
- Reuse Accounts workspace page, `accounts_gl_panel`, shared Account ledger dialog, and access gates.
...
- [ ] AC6: Account ledger may show Journal when write-authorized; Post is absent from this dialog. (R9, R11)
- [ ] AC7: Unauthorized Next / Journal are absent (not disabled). (R7, R11)
- [ ] AC8: `accountId` deep link opens or filters to the account. (R10)
...
- Flow tests: row → Account ledger; Next GL; `accountId` deep link.
- Manual check: empty/loading/error, optional columns, viewports, themes.
- Confirm no patient ledger columns or journal trailing actions on this tab.
