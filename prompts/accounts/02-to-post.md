...
Implement the **To post** desk section (`?section=journals`) on `/accounts` per `accounts.md`. This queue lists draft / unposted journals only, ready to post to the books. Source of truth: root `accounts.md` §§2–8, 9–11.
...
10. Deep link `?section=journals&action=post` (and optional `id`) opens the Post modal after load when authorized.
11. Gate with (`accounts:read` ∪ `accounts:write`) ∩ `facility-accounts`. Write / Post / Post all require `accounts:write`. Omit unauthorized controls; no disabled “no access” chrome.
12. Keep count tone **warning**. Enable realtime + light poll while active.
13. After post mutations, remove or update rows that leave the draft queue and synchronize strip counts.
...
- Reuse Accounts workspace page, controller, access gates, shared table support, Detail shell, and Post modal.
...
- Flow tests: Post from Next; Post all; list membership updates; `action=post` deep link.
- Manual check: empty/loading/error, viewports, themes.
- Confirm only drafts appear on this tab.
