...
Implement the **Close books** desk section (`?section=books`) on `/accounts` per `accounts.md`. This is fiscal periods: open, review checklist, and close — not a journal reprint. Source of truth: root `accounts.md` §§2–8, 9–11.
...
9. Trailing **Open period** opens Open period modal (Label / dates · Notes). Primary: **Open**. Trailing **Close period** opens Close period modal for the selected/context period.
...
11. Deep link `?section=books&action=close&id=` opens Close period modal after load. `periodId` opens Books detail or filters Close books.
12. Gate with (`accounts:read` ∪ `accounts:write`) ∩ `facility-accounts`. Open / Close writes require `accounts:write`. Omit unauthorized controls; no disabled “no access” chrome.
13. Keep count tone **warning** for open periods. Enable realtime + light poll while active.
14. After open / close / approve mutations, snackbar (*Submitted for approval.* / *Saved.*) and refresh list + strip counts.
...
- Reuse Accounts workspace page, `accounts_books_panel`, Detail shell, Open / Close dialogs, and access gates.
...
- [ ] AC8: `action=close&id=` deep link opens Close period modal; row click opens Books detail. (R10, R11)
...
- Flow tests: Open period; Close → submit for approval; Approve from Next; deep links.
- Manual check: empty/loading/error, Open/Overdue chips, viewports, themes.
- Confirm Open/Close trailing are not duplicated on other Accounts tabs.
