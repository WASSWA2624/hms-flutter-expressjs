# Claims overview — workspace compliance program

## Context

Bring the Claims desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/15-claims/`. Per-surface remediation prompts live beside this file under `prompts/15-claims/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/15-claims/00-overview.md` as the authoritative tab index (`ClaimsDeskSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/15-claims/01-authorizations.md`
   - `prompts/15-claims/02-active-claims.md`
   - `prompts/15-claims/03-settled.md`
   - `prompts/15-claims/04-insurance-setup.md`
...
6. After each remediation, update the matching inventory file under `tabs/15-claims/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/claims/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/15-claims/00-overview.md` has a completed remediation prompt under `prompts/15-claims/` and a matching updated inventory under `tabs/15-claims/`.
...
- Trace `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart`, `frontend/lib/features/claims/presentation/claims_access.dart`, and widgets listed in `tabs/15-claims/00-overview.md`.
- Run feature tests under `frontend/test/features/claims/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/15-claims/00-overview.md`
- `prompts/15-claims/00-shared-chrome.md`
- `prompts/15-claims/99-convention-gaps.md`
- `prompts/15-claims/01-authorizations.md`
- `prompts/15-claims/02-active-claims.md`
- `prompts/15-claims/03-settled.md`
- `prompts/15-claims/04-insurance-setup.md`
