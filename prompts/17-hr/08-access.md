# HR Access tab — rule compliance

## Context

Make the Access desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/17-hr/08-access.md`. Apply shared chrome fixes from `prompts/17-hr/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `hrManageAccessAction`, deep-link `access` (aliases `roles`, `permissions`), and omit the tab when gate denies access (`HrManageUsersRolesAtomPermissions.tab` = `hrAccessReadRequirement` (∩ `hr:read` + ∪ tenant/facility/platform admin + module) **or** `policy.isElevated`) — never show a disabled placeholder (`tabs.mdc`). Align `HrDeskSection` / query helpers with `tabs/17-hr/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (**always `0`**). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
8. Preserve in-desk actions for this surface via shared dialogs/forms; omit unauthorized actions (`dialogs.mdc`, `forms.mdc`, `screens.mdc`). Inventoried dialogs to keep compliant: Create/edit role/permission; assign permissions; edit user; Open staff profile.
...
- [ ] `tabs/17-hr/08-access.md` updated to match.
