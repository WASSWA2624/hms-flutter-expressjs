# HR overview — workspace compliance program

## Context

Bring the HR desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/17-hr/`. Per-surface remediation prompts live beside this file under `prompts/17-hr/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/17-hr/00-overview.md` as the authoritative tab index (`HrDeskSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/17-hr/01-staff-directory.md`
   - `prompts/17-hr/02-positions.md`
   - `prompts/17-hr/03-shift-roster.md`
   - `prompts/17-hr/04-leave-requests.md`
   - `prompts/17-hr/05-swap-requests.md`
   - `prompts/17-hr/06-unassigned-shifts.md`
   - `prompts/17-hr/07-payroll.md`
   - `prompts/17-hr/08-access.md`
...
6. After each remediation, update the matching inventory file under `tabs/17-hr/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/hr/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/17-hr/00-overview.md` has a completed remediation prompt under `prompts/17-hr/` and a matching updated inventory under `tabs/17-hr/`.
...
- Trace `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart`, `frontend/lib/features/hr/presentation/hr_access.dart`, and widgets listed in `tabs/17-hr/00-overview.md`.
- Run feature tests under `frontend/test/features/hr/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/17-hr/00-overview.md`
- `prompts/17-hr/00-shared-chrome.md`
- `prompts/17-hr/99-convention-gaps.md`
- `prompts/17-hr/01-staff-directory.md`
- `prompts/17-hr/02-positions.md`
- `prompts/17-hr/03-shift-roster.md`
- `prompts/17-hr/04-leave-requests.md`
