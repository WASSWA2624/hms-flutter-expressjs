# Prompt 006 — Implement Coverage Rules

## Context

- **Sequence:** 006 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 129 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Tariffs & Price Lists` (`prompts/03-insurance-and-claims/005-tariffs-and-price-lists.md`).
- **Menu path:** `Insurance & Claims → Coverage Rules`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-rules.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-rules.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Exclusions & Limits` (`prompts/03-insurance-and-claims/007-exclusions-and-limits.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-rules.md`
