# Prompt 034 — Implement Overview

## Context

- **Sequence:** 034 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 157 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Disputes` (`prompts/03-insurance-and-claims/033-disputes.md`).
- **Menu path:** `Insurance & Claims → Overview`
- **Canonical route:** `/claims?section=overview`
- **Surface profile:** `overview` table/worklist opened from a flat workspace tab
- **Navigation model:** one flat tab strip under the `Insurance & Claims` menu item; no nested tabs and no sidebar submenu
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/overview.md`
...
2. **Register and scope the tab.**
   - Implement the `Overview` tab at `Insurance & Claims → Overview` with canonical section slug `overview`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/overview.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/overview.md`:
     `Queue/Metric`, `Current Count`, `Submitted Amount`, `Approved Amount`, `Rejected Amount`, `Paid Amount`, `Outstanding Amount`, `Currency`, `Average Turnaround Time`, `Oldest Item Age`, `SLA Status`, `Assigned Team`, `Trend`, `Last Refreshed`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Currency`, `Amount range`, `Owner / assigned user`.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Open queue:** gate `claims:read` ∩ `insurance-claims`; enable when metric has a destination; Opens the matching filtered tab.
   - Bulk buttons:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Insurance Reports` (`prompts/03-insurance-and-claims/035-insurance-reports.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=overview` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/overview.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/overview.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_overview_panel.dart`
