...
- **Sequence:** 001 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 124 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Billing → Billing Reports` (`prompts/02-billing/024-billing-reports.md`).
- **Menu path:** `Insurance & Claims → Insurance Providers`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/insurance-providers.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/insurance-providers.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/insurance-providers.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Corporate Payers` (`prompts/03-insurance-and-claims/002-corporate-payers.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/insurance-providers.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/insurance-providers.md`
