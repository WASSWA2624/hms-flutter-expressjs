# Prompt 004 — Implement Payer Balances

## Context

- **Sequence:** 004 of 005 in `prompts/03-insurance-and-claims/05-remittances-and-reconciliation/`
- **Overall step:** 151 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Remittance Allocations` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/003-remittance-allocations.md`).
- **Menu path:** `Insurance & Claims → Remittances & Reconciliation → Payer Balances`
- **Canonical route:** `/claims?section=payer-balances`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/payer-balances.md`
...
2. **Register and scope the tab.**
   - Implement the `Payer Balances` tab at `Insurance & Claims → Payer Balances` with canonical section slug `payer-balances`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/payer-balances.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/payer-balances.md`:
     `Payer Code`, `Payer Name`, `Payer Type`, `Open Claim Count`, `Submitted Amount`, `Approved Unpaid Amount`, `Denied Amount Under Appeal`, `Unallocated Payments`, `Current Amount`, `1–30 Days`, `31–60 Days`, `61–90 Days`, `Over 90 Days`, `Total Outstanding`, `Currency`, `Credit Limit`, `Account Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Save view:** gate `claims:read` ∩ `insurance-claims`; enable when filters changed; Stores a named personal view.
...
8. **Implement workflow, validation, permissions, and audit.**
   - Statuses represented by this tab: `Read-only`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Reconciliation` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/005-claim-reconciliation.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=payer-balances` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/payer-balances.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/payer-balances.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_payer_balances_panel.dart`
