# Prompt 002 — Implement Insurer Payments

## Context

- **Sequence:** 002 of 005 in `prompts/03-insurance-and-claims/05-remittances-and-reconciliation/`
- **Overall step:** 149 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Remittance Advice` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/001-remittance-advice.md`).
- **Menu path:** `Insurance & Claims → Remittances & Reconciliation → Insurer Payments`
- **Canonical route:** `/claims?section=insurer-payments`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/insurer-payments.md`
...
2. **Register and scope the tab.**
   - Implement the `Insurer Payments` tab at `Insurance & Claims → Insurer Payments` with canonical section slug `insurer-payments`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/insurer-payments.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/insurer-payments.md`:
     `Payment No.`, `Payment Date`, `Payer`, `Remittance No.`, `Payment Method`, `Bank Account`, `External Reference`, `Gross Amount`, `Deductions`, `Net Amount`, `Allocated Amount`, `Unallocated Amount`, `Currency`, `Value Date`, `Journal Entry No.`, `Reconciliation Status`, `Payment Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Record payment:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens payment dialog and returns the created row.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Payment Date`, `Payer`, `Remittance No.`, `Payment Method`, `Bank Account`, `External Reference`, `Gross Amount`, `Deductions`, `Net Amount`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Remittance Allocations` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/003-remittance-allocations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=insurer-payments` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/insurer-payments.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/insurer-payments.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_insurer_payments_panel.dart`
