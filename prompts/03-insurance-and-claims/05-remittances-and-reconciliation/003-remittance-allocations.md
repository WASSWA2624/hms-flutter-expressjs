# Prompt 003 — Implement Remittance Allocations

## Context

- **Sequence:** 003 of 005 in `prompts/03-insurance-and-claims/05-remittances-and-reconciliation/`
- **Overall step:** 150 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Insurer Payments` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/002-insurer-payments.md`).
- **Menu path:** `Insurance & Claims → Remittances & Reconciliation → Remittance Allocations`
- **Canonical route:** `/claims?section=remittance-allocations`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-allocations.md`
...
2. **Register and scope the tab.**
   - Implement the `Remittance Allocations` tab at `Insurance & Claims → Remittance Allocations` with canonical section slug `remittance-allocations`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-allocations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-allocations.md`:
     `Allocation No.`, `Allocation Date`, `Remittance No.`, `Payment No.`, `Claim No.`, `Invoice No.`, `Patient/Member`, `Payer`, `Approved Amount`, `Allocated Amount`, `Short/Over Amount`, `Currency`, `Adjustment Code`, `Allocation Status`, `Allocated By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Allocate:** gate `billing:write` ∩ `insurance-claims`; enable when unallocated value exists; Opens allocation workbench.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Allocation Date`, `Remittance No.`, `Payment No.`, `Claim No.`, `Invoice No.`, `Patient/Member`, `Payer`, `Short/Over Amount`, `Currency`, `Adjustment Code`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Payer Balances` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/004-payer-balances.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=remittance-allocations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-allocations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-allocations.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_remittance_allocations_panel.dart`
