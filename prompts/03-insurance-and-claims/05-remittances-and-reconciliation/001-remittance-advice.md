# Prompt 001 — Implement Remittance Advice

## Context

- **Sequence:** 001 of 005 in `prompts/03-insurance-and-claims/05-remittances-and-reconciliation/`
- **Overall step:** 148 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Resubmissions & Appeals` (`prompts/03-insurance-and-claims/04-claims-processing/007-resubmissions-and-appeals.md`).
- **Menu path:** `Insurance & Claims → Remittances & Reconciliation → Remittance Advice`
- **Canonical route:** `/claims?section=remittance-advice`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-advice.md`
...
2. **Register and scope the tab.**
   - Implement the `Remittance Advice` tab at `Insurance & Claims → Remittance Advice` with canonical section slug `remittance-advice`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-advice.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-advice.md`:
     `Remittance No.`, `Remittance Date`, `Payer`, `Payment Reference`, `Service Period`, `Claim Count`, `Gross Approved Amount`, `Deductions`, `Net Remittance Amount`, `Currency`, `File/Document Name`, `Imported At`, `Matched Claim Count`, `Unmatched Claim Count`, `Exception Amount`, `Remittance Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Import remittance:** gate `billing:write` ∩ `insurance-claims`; enable when supported payer format; Validates and stages remittance lines.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Remittance Date`, `Payer`, `Payment Reference`, `Service Period`, `Deductions`, `Net Remittance Amount`, `Currency`, `File/Document Name`, `Imported At`, `Exception Amount`.
   - Detail sections: `Summary`, `Allocations`, `Settlement`, `Documents & audit`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Insurer Payments` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/002-insurer-payments.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=remittance-advice` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-advice.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/remittance-advice.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_remittance_advice_panel.dart`
