# Prompt 005 — Implement Claim Reconciliation

## Context

- **Sequence:** 005 of 005 in `prompts/03-insurance-and-claims/05-remittances-and-reconciliation/`
- **Overall step:** 152 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Payer Balances` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/004-payer-balances.md`).
- **Menu path:** `Insurance & Claims → Remittances & Reconciliation → Claim Reconciliation`
- **Canonical route:** `/claims?section=claim-reconciliation`
- **Surface profile:** `reconciliation` table/worklist tab
- **Navigation model:** `Insurance & Claims → Remittances & Reconciliation` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/claim-reconciliation.md`
...
2. **Register and scope the tab.**
   - Implement the `Claim Reconciliation` tab at `Insurance & Claims → Claim Reconciliation` with canonical section slug `claim-reconciliation`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Remittances & Reconciliation` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/claim-reconciliation.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/claim-reconciliation.md`:
     `Reconciliation No.`, `Reconciliation Date`, `Payer`, `Period From`, `Period To`, `Submitted Claim Amount`, `Adjudicated Amount`, `Remittance Amount`, `Payment Amount`, `Allocated Amount`, `GL Receivable Balance`, `Difference`, `Currency`, `Exception Count`, `Prepared By`, `Reviewed By`, `Reconciliation Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Party`, `Currency`, `Amount range`, `Owner / assigned user`.
   ...
   - Implement these context-specific toolbar buttons:
     - **Start work:** gate `billing:write` ∩ `insurance-claims`; enable when eligible period/source selected; Creates a draft work item.
     - **Auto-match:** gate `billing:write` ∩ `insurance-claims`; enable when unmatched rows exist; Applies deterministic matching rules and shows exceptions.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Open workbench:** gate `billing:write` ∩ `insurance-claims`; enable when record not finalized; Opens matches, exceptions, evidence, and notes.
     - **Resolve exception:** gate `billing:write` ∩ `insurance-claims`; enable when exception selected; Captures resolution and supporting reference.
     - **Submit / approve:** gate `billing:write` ∩ `financial:approve` ∩ `insurance-claims`; enable when all blocking exceptions resolved; Completes maker-checker review.
     - **Finalize / reopen:** gate `billing:write` ∩ `financial:approve` ∩ `insurance-claims`; enable when transition permitted; Finalizes or creates an audited reopen request.
   - Bulk buttons:
     - **Assign selected:** gate `billing:write` ∩ `insurance-claims`; Assigns compatible exceptions to one owner.
     - **Resolve selected:** gate `billing:write` ∩ `insurance-claims`; Requires one resolution code and note.
     - **Export selected:** gate `claims:read` ∩ `evidence:export` ∩ `insurance-claims`; Includes reconciliation references and evidence counts.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Reconciliation Date`, `Payer`, `Period From`, `Period To`, `Submitted Claim Amount`, `Adjudicated Amount`, `Remittance Amount`, `Payment Amount`, `Currency`.
...
8. **Implement workflow, validation, permissions, and audit.**
   - Statuses represented by this tab: `Draft`, `In progress`, `Submitted`, `Approved`, `Finalized`, `Reopened`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Payer Statements` (`prompts/03-insurance-and-claims/06-payer-follow-up/001-payer-statements.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claim-reconciliation` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/claim-reconciliation.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/remittances-and-reconciliation/claim-reconciliation.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claim_reconciliation_panel.dart`
