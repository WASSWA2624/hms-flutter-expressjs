# Prompt 004 — Implement Contracts

## Context

- **Sequence:** 004 of 005 in `prompts/03-insurance-and-claims/01-payers-and-contracts/`
- **Overall step:** 127 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Plans & Products` (`prompts/03-insurance-and-claims/01-payers-and-contracts/003-plans-and-products.md`).
- **Menu path:** `Insurance & Claims → Payers & Contracts → Contracts`
- **Canonical route:** `/claims?section=contracts`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payers-and-contracts/contracts.md`
...
2. **Register and scope the tab.**
   - Implement the `Contracts` tab at `Insurance & Claims → Contracts` with canonical section slug `contracts`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payers-and-contracts/contracts.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payers-and-contracts/contracts.md`:
     `Contract No.`, `Payer`, `Contract Type`, `Start Date`, `End Date`, `Renewal Date`, `Payment Terms`, `Claim Submission Deadline`, `Reimbursement Method`, `Tariff/Price List`, `Currency`, `Credit Limit`, `SLA`, `Document Count`, `Account Manager`, `Approval Status`, `Contract Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Contracts create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Contract No.`, `Payer`, `Contract Type`, `Start Date`, `End Date`, `Renewal Date`, `Payment Terms`, `Claim Submission Deadline`, `Reimbursement Method`, `Tariff/Price List`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Tariffs & Price Lists` (`prompts/03-insurance-and-claims/01-payers-and-contracts/005-tariffs-and-price-lists.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=contracts` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payers-and-contracts/contracts.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payers-and-contracts/contracts.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_contracts_panel.dart`
