# Prompt 003 — Implement Plans & Products

## Context

- **Sequence:** 003 of 005 in `prompts/03-insurance-and-claims/01-payers-and-contracts/`
- **Overall step:** 126 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Corporate Payers` (`prompts/03-insurance-and-claims/01-payers-and-contracts/002-corporate-payers.md`).
- **Menu path:** `Insurance & Claims → Payers & Contracts → Plans & Products`
- **Canonical route:** `/claims?section=plans-and-products`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payers-and-contracts/plans-and-products.md`
...
2. **Register and scope the tab.**
   - Implement the `Plans & Products` tab at `Insurance & Claims → Plans & Products` with canonical section slug `plans-and-products`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payers-and-contracts/plans-and-products.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payers-and-contracts/plans-and-products.md`:
     `Plan Code`, `Plan Name`, `Provider/Corporate Payer`, `Product Type`, `Coverage Class`, `Network Type`, `Annual Limit`, `Co-pay Rule`, `Deductible Rule`, `Default Tariff`, `Currency`, `Effective From`, `Effective To`, `Active Member Count`, `Plan Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Party`, `Currency`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Plans & Products create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Plan Code`, `Plan Name`, `Provider/Corporate Payer`, `Product Type`, `Coverage Class`, `Network Type`, `Annual Limit`, `Co-pay Rule`, `Deductible Rule`, `Default Tariff`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Contracts` (`prompts/03-insurance-and-claims/01-payers-and-contracts/004-contracts.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=plans-and-products` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payers-and-contracts/plans-and-products.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payers-and-contracts/plans-and-products.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_plans_and_products_panel.dart`
