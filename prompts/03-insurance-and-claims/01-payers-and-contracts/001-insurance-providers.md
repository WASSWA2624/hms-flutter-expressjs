# Prompt 001 — Implement Insurance Providers

## Context

- **Sequence:** 001 of 005 in `prompts/03-insurance-and-claims/01-payers-and-contracts/`
- **Overall step:** 124 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Billing → Billing Reports` (`prompts/02-billing/06-billing-reports/001-billing-reports.md`).
- **Menu path:** `Insurance & Claims → Payers & Contracts → Insurance Providers`
- **Canonical route:** `/claims?section=insurance-providers`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payers-and-contracts/insurance-providers.md`
...
2. **Register and scope the tab.**
   - Implement the `Insurance Providers` tab at `Insurance & Claims → Insurance Providers` with canonical section slug `insurance-providers`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payers-and-contracts/insurance-providers.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payers-and-contracts/insurance-providers.md`:
     `Provider Code`, `Provider Name`, `Provider Type`, `Tax/Registration No.`, `Contact Person`, `Phone`, `Email`, `Address`, `Default Submission Channel`, `Settlement Terms`, `Default Currency`, `Active Plan Count`, `Open Claim Count`, `Outstanding Balance`, `Contract Expiry`, `Provider Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Currency`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Insurance Providers create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Provider Code`, `Provider Name`, `Provider Type`, `Tax/Registration No.`, `Contact Person`, `Phone`, `Email`, `Address`, `Default Submission Channel`, `Settlement Terms`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Corporate Payers` (`prompts/03-insurance-and-claims/01-payers-and-contracts/002-corporate-payers.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=insurance-providers` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payers-and-contracts/insurance-providers.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payers-and-contracts/insurance-providers.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_insurance_providers_panel.dart`
