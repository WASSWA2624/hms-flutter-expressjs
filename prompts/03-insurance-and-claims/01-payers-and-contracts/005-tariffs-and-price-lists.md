# Prompt 005 — Implement Tariffs & Price Lists

## Context

- **Sequence:** 005 of 005 in `prompts/03-insurance-and-claims/01-payers-and-contracts/`
- **Overall step:** 128 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Contracts` (`prompts/03-insurance-and-claims/01-payers-and-contracts/004-contracts.md`).
- **Menu path:** `Insurance & Claims → Payers & Contracts → Tariffs & Price Lists`
- **Canonical route:** `/claims?section=tariffs-and-price-lists`
- **Surface profile:** `master` table/worklist tab
- **Navigation model:** `Insurance & Claims → Payers & Contracts` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payers-and-contracts/tariffs-and-price-lists.md`
...
2. **Register and scope the tab.**
   - Implement the `Tariffs & Price Lists` tab at `Insurance & Claims → Tariffs & Price Lists` with canonical section slug `tariffs-and-price-lists`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Payers & Contracts` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payers-and-contracts/tariffs-and-price-lists.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payers-and-contracts/tariffs-and-price-lists.md`:
     `Tariff Code`, `Tariff Name`, `Payer`, `Plan`, `Service/Item Code`, `Service/Item Name`, `Department`, `Standard Price`, `Contract Price`, `Co-pay Amount/Rate`, `Tax Code`, `Currency`, `Effective From`, `Effective To`, `Version`, `Tariff Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Department / cost centre`, `Party`, `Currency`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Tariffs & Price Lists create dialog.
...
6. **Implement row actions, bulk actions, and CRUD.**
   - Row buttons:
     ...
     - **Edit:** gate `billing:write` ∩ `insurance-claims`; enable when record active/draft and version current; Opens prefilled edit dialog.
     - **Clone:** gate `billing:write` ∩ `insurance-claims`; enable when record clonable; Creates an unsaved copy with a new reference.
     - **Activate / deactivate:** gate `billing:write` ∩ `insurance-claims`; enable when transition allowed; Requires confirmation and refreshes dependants.
     - **Archive / restore:** gate `billing:write` ∩ `insurance-claims`; enable when not referenced by a blocking workflow; Soft archive only; never hard-delete referenced data.
   - Bulk buttons:
     - **Activate / deactivate selected:** gate `billing:write` ∩ `insurance-claims`; Only compatible, unblocked records are changed.
     - **Archive selected:** gate `billing:write` ∩ `insurance-claims`; Soft archive; partial failures return per-row reasons.
     - **Export selected:** gate `claims:read` ∩ `evidence:export` ∩ `insurance-claims`; Exports selected rows in visible-column order.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Tariff Code`, `Tariff Name`, `Payer`, `Plan`, `Service/Item Code`, `Service/Item Name`, `Department`, `Standard Price`, `Contract Price`, `Co-pay Amount/Rate`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Coverage Rules` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/001-coverage-rules.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=tariffs-and-price-lists` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payers-and-contracts/tariffs-and-price-lists.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payers-and-contracts/tariffs-and-price-lists.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_tariffs_and_price_lists_panel.dart`
