# Prompt 002 — Implement Corporate Payers

## Context

- **Sequence:** 002 of 005 in `prompts/03-insurance-and-claims/01-payers-and-contracts/`
- **Overall step:** 125 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Insurance Providers` (`prompts/03-insurance-and-claims/01-payers-and-contracts/001-insurance-providers.md`).
- **Menu path:** `Insurance & Claims → Payers & Contracts → Corporate Payers`
- **Canonical route:** `/claims?section=corporate-payers`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payers-and-contracts/corporate-payers.md`
...
2. **Register and scope the tab.**
   - Implement the `Corporate Payers` tab at `Insurance & Claims → Corporate Payers` with canonical section slug `corporate-payers`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payers-and-contracts/corporate-payers.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payers-and-contracts/corporate-payers.md`:
     `Payer Code`, `Corporate Name`, `Registration/Tax No.`, `Industry`, `Contact Person`, `Phone`, `Email`, `Billing Address`, `Contract No.`, `Credit Limit`, `Payment Terms`, `Currency`, `Employee/Member Count`, `Open Invoice/Claim Count`, `Outstanding Balance`, `Account Manager`, `Payer Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Party`, `Currency`, `Amount range`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Corporate Payers create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Payer Code`, `Corporate Name`, `Registration/Tax No.`, `Industry`, `Contact Person`, `Phone`, `Email`, `Billing Address`, `Contract No.`, `Credit Limit`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Plans & Products` (`prompts/03-insurance-and-claims/01-payers-and-contracts/003-plans-and-products.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=corporate-payers` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payers-and-contracts/corporate-payers.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payers-and-contracts/corporate-payers.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_corporate_payers_panel.dart`
