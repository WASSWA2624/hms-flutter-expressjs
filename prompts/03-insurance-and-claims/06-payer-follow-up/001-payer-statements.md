# Prompt 001 — Implement Payer Statements

## Context

- **Sequence:** 001 of 004 in `prompts/03-insurance-and-claims/06-payer-follow-up/`
- **Overall step:** 153 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Claim Reconciliation` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/005-claim-reconciliation.md`).
- **Menu path:** `Insurance & Claims → Payer Follow-up → Payer Statements`
- **Canonical route:** `/claims?section=payer-statements`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payer-follow-up/payer-statements.md`
...
2. **Register and scope the tab.**
   - Implement the `Payer Statements` tab at `Insurance & Claims → Payer Statements` with canonical section slug `payer-statements`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payer-follow-up/payer-statements.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payer-follow-up/payer-statements.md`:
     `Transaction Date`, `Payer`, `Document Type`, `Claim/Invoice/Payment Reference`, `Patient/Member`, `Description`, `Due Date`, `Debit`, `Credit`, `Running Balance`, `Currency`, `Allocation Reference`, `Posting Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claims Aging` (`prompts/03-insurance-and-claims/06-payer-follow-up/002-claims-aging.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=payer-statements` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payer-follow-up/payer-statements.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payer-follow-up/payer-statements.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_payer_statements_panel.dart`
