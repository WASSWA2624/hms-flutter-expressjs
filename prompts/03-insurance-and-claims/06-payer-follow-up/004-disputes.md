# Prompt 004 — Implement Disputes

## Context

- **Sequence:** 004 of 004 in `prompts/03-insurance-and-claims/06-payer-follow-up/`
- **Overall step:** 156 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Collection Follow-up` (`prompts/03-insurance-and-claims/06-payer-follow-up/003-collection-follow-up.md`).
- **Menu path:** `Insurance & Claims → Payer Follow-up → Disputes`
- **Canonical route:** `/claims?section=disputes`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payer-follow-up/disputes.md`
...
2. **Register and scope the tab.**
   - Implement the `Disputes` tab at `Insurance & Claims → Disputes` with canonical section slug `disputes`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payer-follow-up/disputes.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payer-follow-up/disputes.md`:
     `Dispute No.`, `Opened Date`, `Payer`, `Claim/Payment Reference`, `Dispute Type`, `Disputed Amount`, `Currency`, `Issue Summary`, `Evidence Count`, `Assigned Owner`, `Payer Contact`, `Response Due Date`, `Last Response Date`, `Resolution`, `Resolved Amount`, `Closed Date`, `Dispute Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted and source prerequisites met; Opens the Disputes mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Opened Date`, `Payer`, `Claim/Payment Reference`, `Dispute Type`, `Disputed Amount`, `Currency`, `Issue Summary`, `Assigned Owner`, `Payer Contact`, `Response Due Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Overview` (`prompts/03-insurance-and-claims/07-overview/001-overview.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=disputes` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payer-follow-up/disputes.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payer-follow-up/disputes.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_disputes_panel.dart`
