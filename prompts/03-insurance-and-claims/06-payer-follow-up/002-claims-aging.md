# Prompt 002 — Implement Claims Aging

## Context

- **Sequence:** 002 of 004 in `prompts/03-insurance-and-claims/06-payer-follow-up/`
- **Overall step:** 154 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Payer Statements` (`prompts/03-insurance-and-claims/06-payer-follow-up/001-payer-statements.md`).
- **Menu path:** `Insurance & Claims → Payer Follow-up → Claims Aging`
- **Canonical route:** `/claims?section=claims-aging`
- **Surface profile:** `analysis` table/worklist tab
- **Navigation model:** `Insurance & Claims → Payer Follow up` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payer-follow-up/claims-aging.md`
...
2. **Register and scope the tab.**
   - Implement the `Claims Aging` tab at `Insurance & Claims → Claims Aging` with canonical section slug `claims-aging`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Payer Follow up` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payer-follow-up/claims-aging.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payer-follow-up/claims-aging.md`:
     `Payer`, `Plan/Contract`, `Claim Count`, `Current Amount`, `1–30 Days`, `31–60 Days`, `61–90 Days`, `Over 90 Days`, `Total Outstanding`, `Currency`, `Oldest Claim Date`, `Maximum Age`, `Denied Amount`, `Under-appeal Amount`, `Assigned Owner`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Generate claim:** gate `billing:write` ∩ `insurance-claims`; enable when eligible unclaimed charges exist; Creates a draft claim with claim items.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Collection Follow-up` (`prompts/03-insurance-and-claims/06-payer-follow-up/003-collection-follow-up.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claims-aging` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payer-follow-up/claims-aging.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payer-follow-up/claims-aging.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claims_aging_panel.dart`
