# Prompt 003 — Implement Collection Follow-up

## Context

- **Sequence:** 003 of 004 in `prompts/03-insurance-and-claims/06-payer-follow-up/`
- **Overall step:** 155 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Claims Aging` (`prompts/03-insurance-and-claims/06-payer-follow-up/002-claims-aging.md`).
- **Menu path:** `Insurance & Claims → Payer Follow-up → Collection Follow-up`
- **Canonical route:** `/claims?section=collection-follow-up`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payer-follow-up/collection-follow-up.md`
...
2. **Register and scope the tab.**
   - Implement the `Collection Follow-up` tab at `Insurance & Claims → Collection Follow-up` with canonical section slug `collection-follow-up`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payer-follow-up/collection-follow-up.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payer-follow-up/collection-follow-up.md`:
     `Follow-up No.`, `Payer`, `Claim/Batch Reference`, `Outstanding Amount`, `Currency`, `Maximum Age`, `Priority`, `Last Contact Date`, `Contact Person`, `Contact Method`, `Promise Date`, `Promise Amount`, `Next Follow-up Date`, `Assigned Owner`, `Escalation Level`, `Follow-up Status`, `Notes`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted and source prerequisites met; Opens the Collection Follow-up mutation dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Payer`, `Claim/Batch Reference`, `Currency`, `Priority`, `Contact Person`, `Contact Method`, `Promise Date`, `Promise Amount`, `Next Follow-up Date`, `Assigned Owner`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Disputes` (`prompts/03-insurance-and-claims/06-payer-follow-up/004-disputes.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=collection-follow-up` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payer-follow-up/collection-follow-up.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payer-follow-up/collection-follow-up.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_collection_follow_up_panel.dart`
