...
- **Sequence:** 005 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 128 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Contracts` (`prompts/03-insurance-and-claims/004-contracts.md`).
- **Menu path:** `Insurance & Claims → Tariffs & Price Lists`
- **Canonical route:** `/claims?section=tariffs-and-price-lists`
- **Surface profile:** `master` table/worklist opened from a flat workspace tab
- **Navigation model:** one flat tab strip under the `Insurance & Claims` menu item; no nested tabs and no sidebar submenu
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/tariffs-and-price-lists.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/tariffs-and-price-lists.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/tariffs-and-price-lists.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Coverage Rules` (`prompts/03-insurance-and-claims/006-coverage-rules.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/tariffs-and-price-lists.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/tariffs-and-price-lists.md`
