# Prompt 015 — Implement Eligibility Checks

## Context

- **Sequence:** 015 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 138 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Members & Dependants` (`prompts/03-insurance-and-claims/014-members-and-dependants.md`).
- **Menu path:** `Insurance & Claims → Eligibility Checks`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/eligibility-checks.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/eligibility-checks.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/eligibility-checks.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Coverage Balances` (`prompts/03-insurance-and-claims/016-coverage-balances.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/eligibility-checks.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/eligibility-checks.md`
