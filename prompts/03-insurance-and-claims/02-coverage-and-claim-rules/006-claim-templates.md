# Prompt 006 — Implement Claim Templates

## Context

- **Sequence:** 006 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 134 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Submission Channels` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/005-submission-channels.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Claim Templates`
- **Canonical route:** `/claims?section=claim-templates`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-templates.md`
...
2. **Register and scope the tab.**
   - Implement the `Claim Templates` tab at `Insurance & Claims → Claim Templates` with canonical section slug `claim-templates`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-templates.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-templates.md`:
     `Template Code`, `Template Name`, `Payer`, `Plan/Contract`, `Claim Type`, `File/Message Format`, `Version`, `Required Sections`, `Item Grouping`, `Currency Rule`, `Effective From`, `Effective To`, `Last Published At`, `Published By`, `Template Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Claim Templates create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Template Code`, `Template Name`, `Payer`, `Plan/Contract`, `Claim Type`, `File/Message Format`, `Version`, `Required Sections`, `Item Grouping`, `Currency Rule`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Rules` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/007-claim-rules.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claim-templates` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-templates.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-templates.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claim_templates_panel.dart`
