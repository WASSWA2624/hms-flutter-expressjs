# Prompt 002 — Implement Exclusions & Limits

## Context

- **Sequence:** 002 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 130 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Coverage Rules` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/001-coverage-rules.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Exclusions & Limits`
- **Canonical route:** `/claims?section=exclusions-and-limits`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/exclusions-and-limits.md`
...
2. **Register and scope the tab.**
   - Implement the `Exclusions & Limits` tab at `Insurance & Claims → Exclusions & Limits` with canonical section slug `exclusions-and-limits`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/exclusions-and-limits.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/exclusions-and-limits.md`:
     `Rule Code`, `Payer`, `Plan`, `Exclusion/Limit Type`, `Service/Diagnosis/Category`, `Description`, `Limit Quantity`, `Limit Amount`, `Frequency`, `Currency`, `Waiting Period`, `Exception Conditions`, `Effective From`, `Effective To`, `Rule Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Exclusions & Limits create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Rule Code`, `Payer`, `Plan`, `Exclusion/Limit Type`, `Service/Diagnosis/Category`, `Description`, `Limit Quantity`, `Limit Amount`, `Frequency`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Required Documents` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/003-required-documents.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=exclusions-and-limits` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/exclusions-and-limits.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/exclusions-and-limits.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_exclusions_and_limits_panel.dart`
