# Prompt 007 — Implement Claim Rules

## Context

- **Sequence:** 007 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 135 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Claim Templates` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/006-claim-templates.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Claim Rules`
- **Canonical route:** `/claims?section=claim-rules`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-rules.md`
...
2. **Register and scope the tab.**
   - Implement the `Claim Rules` tab at `Insurance & Claims → Claim Rules` with canonical section slug `claim-rules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-rules.md`:
     `Rule Code`, `Rule Name`, `Payer/Plan Scope`, `Rule Category`, `Trigger Stage`, `Condition Summary`, `Validation/Action`, `Severity`, `Auto-reject`, `Override Role`, `Priority`, `Effective From`, `Effective To`, `Version`, `Last Tested At`, `Rule Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Claim Rules create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Rule Code`, `Rule Name`, `Payer/Plan Scope`, `Rule Category`, `Trigger Stage`, `Condition Summary`, `Validation/Action`, `Severity`, `Auto-reject`, `Override Role`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Patient Policies` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/001-patient-policies.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claim-rules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/claim-rules.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claim_rules_panel.dart`
