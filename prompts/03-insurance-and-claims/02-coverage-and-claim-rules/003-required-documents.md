# Prompt 003 — Implement Required Documents

## Context

- **Sequence:** 003 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 131 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Exclusions & Limits` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/002-exclusions-and-limits.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Required Documents`
- **Canonical route:** `/claims?section=required-documents`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/required-documents.md`
...
2. **Register and scope the tab.**
   - Implement the `Required Documents` tab at `Insurance & Claims → Required Documents` with canonical section slug `required-documents`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/required-documents.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/required-documents.md`:
     `Requirement Code`, `Document Type`, `Payer`, `Plan`, `Claim/Service Type`, `Required Stage`, `Mandatory`, `Minimum Count`, `Accepted Format(s)`, `Maximum Size`, `Validity Period`, `Conditional Rule`, `Effective From`, `Effective To`, `Requirement Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Required Documents create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Requirement Code`, `Document Type`, `Payer`, `Plan`, `Claim/Service Type`, `Required Stage`, `Mandatory`, `Accepted Format(s)`, `Maximum Size`, `Validity Period`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Denial Codes` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/004-denial-codes.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=required-documents` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/required-documents.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/required-documents.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_required_documents_panel.dart`
