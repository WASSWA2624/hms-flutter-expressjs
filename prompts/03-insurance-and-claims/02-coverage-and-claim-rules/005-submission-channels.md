# Prompt 005 — Implement Submission Channels

## Context

- **Sequence:** 005 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 133 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Denial Codes` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/004-denial-codes.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Submission Channels`
- **Canonical route:** `/claims?section=submission-channels`
- **Surface profile:** `master` table/worklist tab
- **Navigation model:** `Insurance & Claims → Coverage & Claim Rules` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/submission-channels.md`
...
2. **Register and scope the tab.**
   - Implement the `Submission Channels` tab at `Insurance & Claims → Submission Channels` with canonical section slug `submission-channels`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Coverage & Claim Rules` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/submission-channels.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/submission-channels.md`:
     `Channel Code`, `Channel Name`, `Payer`, `Channel Type`, `Endpoint/Portal`, `File Format`, `Authentication Profile`, `Submission Schedule`, `Acknowledgement Expected`, `Retry Policy`, `Last Successful Submission`, `Last Failure`, `Error Count`, `Channel Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Party`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Submission Channels create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Channel Code`, `Channel Name`, `Payer`, `Channel Type`, `Endpoint/Portal`, `File Format`, `Authentication Profile`, `Submission Schedule`, `Acknowledgement Expected`, `Retry Policy`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Templates` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/006-claim-templates.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=submission-channels` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/submission-channels.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/submission-channels.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_submission_channels_panel.dart`
