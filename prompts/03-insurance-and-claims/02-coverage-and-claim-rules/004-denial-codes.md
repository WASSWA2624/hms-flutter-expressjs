# Prompt 004 — Implement Denial Codes

## Context

- **Sequence:** 004 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 132 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Required Documents` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/003-required-documents.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Denial Codes`
- **Canonical route:** `/claims?section=denial-codes`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/denial-codes.md`
...
2. **Register and scope the tab.**
   - Implement the `Denial Codes` tab at `Insurance & Claims → Denial Codes` with canonical section slug `denial-codes`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/denial-codes.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/denial-codes.md`:
     `Denial Code`, `External Code`, `Denial Name`, `Category`, `Description`, `Correctable`, `Default Action`, `Appeal Allowed`, `Appeal Deadline Days`, `Responsible Team`, `GL Adjustment Rule`, `Effective From`, `Effective To`, `Code Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Denial Codes create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Denial Code`, `External Code`, `Denial Name`, `Category`, `Description`, `Correctable`, `Default Action`, `Appeal Allowed`, `Appeal Deadline Days`, `Responsible Team`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Submission Channels` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/005-submission-channels.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=denial-codes` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/denial-codes.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/denial-codes.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_denial_codes_panel.dart`
