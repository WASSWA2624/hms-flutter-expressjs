# Prompt 001 — Implement Coverage Rules

## Context

- **Sequence:** 001 of 007 in `prompts/03-insurance-and-claims/02-coverage-and-claim-rules/`
- **Overall step:** 129 of 158
- **Phase:** Insurance and claims — payer and rule setup
- **Prerequisite:** `Insurance & Claims → Tariffs & Price Lists` (`prompts/03-insurance-and-claims/01-payers-and-contracts/005-tariffs-and-price-lists.md`).
- **Menu path:** `Insurance & Claims → Coverage & Claim Rules → Coverage Rules`
- **Canonical route:** `/claims?section=coverage-rules`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/coverage-rules.md`
...
2. **Register and scope the tab.**
   - Implement the `Coverage Rules` tab at `Insurance & Claims → Coverage Rules` with canonical section slug `coverage-rules`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/coverage-rules.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/coverage-rules.md`:
     `Rule Code`, `Rule Name`, `Payer`, `Plan`, `Benefit Category`, `Service/Item Scope`, `Coverage Percentage`, `Co-pay`, `Deductible`, `Per-visit Limit`, `Annual Limit`, `Waiting Period`, `Pre-authorization Required`, `Referral Required`, `Effective From`, `Effective To`, `Rule Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Coverage Rules create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Rule Code`, `Rule Name`, `Payer`, `Plan`, `Benefit Category`, `Service/Item Scope`, `Coverage Percentage`, `Co-pay`, `Deductible`, `Per-visit Limit`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Exclusions & Limits` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/002-exclusions-and-limits.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=coverage-rules` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/coverage-rules.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-and-claim-rules/coverage-rules.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_coverage_rules_panel.dart`
