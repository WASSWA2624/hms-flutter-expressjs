# Prompt 002 — Implement Members & Dependants

## Context

- **Sequence:** 002 of 005 in `prompts/03-insurance-and-claims/03-membership-and-eligibility/`
- **Overall step:** 137 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Patient Policies` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/001-patient-policies.md`).
- **Menu path:** `Insurance & Claims → Membership & Eligibility → Members & Dependants`
- **Canonical route:** `/claims?section=members-and-dependants`
- **Surface profile:** `master` table/worklist tab
- **Navigation model:** `Insurance & Claims → Membership & Eligibility` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/membership-and-eligibility/members-and-dependants.md`
...
2. **Register and scope the tab.**
   - Implement the `Members & Dependants` tab at `Insurance & Claims → Members & Dependants` with canonical section slug `members-and-dependants`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Membership & Eligibility` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/membership-and-eligibility/members-and-dependants.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/membership-and-eligibility/members-and-dependants.md`:
     `Member No.`, `Patient No.`, `Member Name`, `Payer`, `Plan`, `Policy No.`, `Principal Member No.`, `Relationship`, `Date of Birth`, `National ID`, `Employer/Corporate`, `Enrolment Date`, `Coverage Start`, `Coverage End`, `Eligibility Status`, `Member Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Members & Dependants create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Member No.`, `Patient No.`, `Member Name`, `Payer`, `Plan`, `Policy No.`, `Principal Member No.`, `Relationship`, `Date of Birth`, `National ID`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Eligibility Checks` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/003-eligibility-checks.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=members-and-dependants` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/membership-and-eligibility/members-and-dependants.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/membership-and-eligibility/members-and-dependants.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_members_and_dependants_panel.dart`
