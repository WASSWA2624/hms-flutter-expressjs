# Prompt 004 — Implement Coverage Balances

## Context

- **Sequence:** 004 of 005 in `prompts/03-insurance-and-claims/03-membership-and-eligibility/`
- **Overall step:** 139 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Eligibility Checks` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/003-eligibility-checks.md`).
- **Menu path:** `Insurance & Claims → Membership & Eligibility → Coverage Balances`
- **Canonical route:** `/claims?section=coverage-balances`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/membership-and-eligibility/coverage-balances.md`
...
2. **Register and scope the tab.**
   - Implement the `Coverage Balances` tab at `Insurance & Claims → Coverage Balances` with canonical section slug `coverage-balances`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/membership-and-eligibility/coverage-balances.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/membership-and-eligibility/coverage-balances.md`:
     `Member No.`, `Patient Name`, `Policy No.`, `Payer`, `Plan`, `Benefit Category`, `Coverage Period`, `Annual/Period Limit`, `Used Amount`, `Reserved Amount`, `Remaining Amount`, `Currency`, `Last Claim Date`, `Last Updated At`, `Balance Status`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Pre-authorizations` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/005-pre-authorizations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=coverage-balances` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/membership-and-eligibility/coverage-balances.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/membership-and-eligibility/coverage-balances.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_coverage_balances_panel.dart`
