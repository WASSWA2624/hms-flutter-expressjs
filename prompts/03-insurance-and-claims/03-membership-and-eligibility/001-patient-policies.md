# Prompt 001 — Implement Patient Policies

## Context

- **Sequence:** 001 of 005 in `prompts/03-insurance-and-claims/03-membership-and-eligibility/`
- **Overall step:** 136 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Claim Rules` (`prompts/03-insurance-and-claims/02-coverage-and-claim-rules/007-claim-rules.md`).
- **Menu path:** `Insurance & Claims → Membership & Eligibility → Patient Policies`
- **Canonical route:** `/claims?section=patient-policies`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/membership-and-eligibility/patient-policies.md`
...
2. **Register and scope the tab.**
   - Implement the `Patient Policies` tab at `Insurance & Claims → Patient Policies` with canonical section slug `patient-policies`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/membership-and-eligibility/patient-policies.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/membership-and-eligibility/patient-policies.md`:
     `Policy No.`, `Member No.`, `Patient No.`, `Patient Name`, `Payer`, `Plan`, `Principal Member`, `Relationship`, `Start Date`, `End Date`, `Coverage Status`, `Eligibility Status`, `Remaining Limit`, `Currency`, `Employer/Corporate`, `Last Verified At`, `Policy Status`.
...
5. **Implement filters and table controls.**
   - Implement these domain filters in addition to comprehensive field filters from the specification: `Search`, `Status`, `Date/period`, `Party`, `Currency`.
   ...
   - Implement these context-specific toolbar buttons:
     - **New record:** gate `billing:write` ∩ `insurance-claims`; enable when write permitted; Opens the Patient Policies create dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Policy No.`, `Member No.`, `Patient No.`, `Patient Name`, `Payer`, `Plan`, `Principal Member`, `Relationship`, `Start Date`, `End Date`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Members & Dependants` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/002-members-and-dependants.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=patient-policies` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/membership-and-eligibility/patient-policies.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/membership-and-eligibility/patient-policies.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_patient_policies_panel.dart`
