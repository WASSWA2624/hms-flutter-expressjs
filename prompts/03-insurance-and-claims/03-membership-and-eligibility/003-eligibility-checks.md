# Prompt 003 — Implement Eligibility Checks

## Context

- **Sequence:** 003 of 005 in `prompts/03-insurance-and-claims/03-membership-and-eligibility/`
- **Overall step:** 138 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Members & Dependants` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/002-members-and-dependants.md`).
- **Menu path:** `Insurance & Claims → Membership & Eligibility → Eligibility Checks`
- **Canonical route:** `/claims?section=eligibility-checks`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/membership-and-eligibility/eligibility-checks.md`
...
2. **Register and scope the tab.**
   - Implement the `Eligibility Checks` tab at `Insurance & Claims → Eligibility Checks` with canonical section slug `eligibility-checks`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/membership-and-eligibility/eligibility-checks.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/membership-and-eligibility/eligibility-checks.md`:
     `Check No.`, `Check Date/Time`, `Patient/Member`, `Policy No.`, `Payer`, `Plan`, `Encounter No.`, `Service/Benefit`, `Coverage Status`, `Available Limit`, `Co-pay`, `Deductible`, `Currency`, `Response Code`, `Response Message`, `External Reference`, `Checked By`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Check eligibility:** gate `billing:write` ∩ `insurance-claims`; enable when member and service context supplied; Runs payer eligibility and records the response.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Check Date/Time`, `Patient/Member`, `Policy No.`, `Payer`, `Plan`, `Encounter No.`, `Service/Benefit`, `Co-pay`, `Deductible`, `Currency`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Coverage Balances` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/004-coverage-balances.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=eligibility-checks` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/membership-and-eligibility/eligibility-checks.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/membership-and-eligibility/eligibility-checks.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_eligibility_checks_panel.dart`
