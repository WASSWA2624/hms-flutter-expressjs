# Prompt 005 — Implement Pre-authorizations

## Context

- **Sequence:** 005 of 005 in `prompts/03-insurance-and-claims/03-membership-and-eligibility/`
- **Overall step:** 140 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Coverage Balances` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/004-coverage-balances.md`).
- **Menu path:** `Insurance & Claims → Membership & Eligibility → Pre-authorizations`
- **Canonical route:** `/claims?section=pre-authorizations`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/membership-and-eligibility/pre-authorizations.md`
...
2. **Register and scope the tab.**
   - Implement the `Pre-authorizations` tab at `Insurance & Claims → Pre-authorizations` with canonical section slug `pre-authorizations`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/membership-and-eligibility/pre-authorizations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/membership-and-eligibility/pre-authorizations.md`:
     `Authorization No.`, `Request Date`, `Patient/Member`, `Policy No.`, `Payer`, `Plan`, `Encounter No.`, `Requested Service/Procedure`, `Diagnosis`, `Provider/Doctor`, `Requested Amount`, `Approved Amount`, `Patient Share`, `Currency`, `Valid From`, `Valid Until`, `External Reference`, `Decision Reason`, `Authorization Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Request pre-authorization:** gate `billing:write` ∩ `insurance-claims`; enable when eligible policy selected; Opens authorization request dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Request Date`, `Patient/Member`, `Policy No.`, `Payer`, `Plan`, `Encounter No.`, `Requested Service/Procedure`, `Diagnosis`, `Provider/Doctor`, `Requested Amount`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claims` (`prompts/03-insurance-and-claims/04-claims-processing/001-claims.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=pre-authorizations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/membership-and-eligibility/pre-authorizations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/membership-and-eligibility/pre-authorizations.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_pre_authorizations_panel.dart`
