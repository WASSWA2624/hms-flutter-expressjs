# Prompt 024 — Implement Resubmissions & Appeals

## Context

- **Sequence:** 024 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 147 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Denials` (`prompts/03-insurance-and-claims/023-denials.md`).
- **Menu path:** `Insurance & Claims → Resubmissions & Appeals`
- **Canonical route:** `/claims?section=resubmissions-and-appeals`
- **Surface profile:** `transaction` table/worklist opened from a flat workspace tab
- **Navigation model:** one flat tab strip under the `Insurance & Claims` menu item; no nested tabs and no sidebar submenu
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/resubmissions-and-appeals.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/resubmissions-and-appeals.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/resubmissions-and-appeals.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Remittance Advice` (`prompts/03-insurance-and-claims/025-remittance-advice.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/resubmissions-and-appeals.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/resubmissions-and-appeals.md`
