# Prompt 001 — Implement Overview

## Context

- **Sequence:** 001 of 001 in `prompts/03-insurance-and-claims/07-overview/`
- **Overall step:** 157 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Disputes` (`prompts/03-insurance-and-claims/06-payer-follow-up/004-disputes.md`).
- **Menu path:** `Insurance & Claims → Overview → Overview`
- **Canonical route:** `/claims?section=overview`
- **Surface profile:** `overview` table/worklist tab
- **Navigation model:** `Insurance & Claims → Overview` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/overview/overview.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/overview/overview.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/overview/overview.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Insurance Reports` (`prompts/03-insurance-and-claims/08-insurance-reports/001-insurance-reports.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/overview/overview.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/overview/overview.md`
