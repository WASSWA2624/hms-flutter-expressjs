# Prompt 001 — Implement Insurance Reports

## Context

- **Sequence:** 001 of 001 in `prompts/03-insurance-and-claims/08-insurance-reports/`
- **Overall step:** 158 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Overview` (`prompts/03-insurance-and-claims/07-overview/001-overview.md`).
...
- **Navigation model:** `Insurance & Claims → Insurance Reports` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/insurance-reports/insurance-reports.md`
...
2. **Register and scope the tab.**
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Insurance Reports` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/insurance-reports/insurance-reports.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/insurance-reports/insurance-reports.md`:
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/insurance-reports/insurance-reports.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/insurance-reports/insurance-reports.md`
