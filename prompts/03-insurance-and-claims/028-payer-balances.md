# Prompt 028 — Implement Payer Balances

## Context

- **Sequence:** 028 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 151 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Remittance Allocations` (`prompts/03-insurance-and-claims/027-remittance-allocations.md`).
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/payer-balances.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/payer-balances.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/payer-balances.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Reconciliation` (`prompts/03-insurance-and-claims/029-claim-reconciliation.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/payer-balances.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/payer-balances.md`
