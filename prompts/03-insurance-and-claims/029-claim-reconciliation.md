# Prompt 029 — Implement Claim Reconciliation

## Context

- **Sequence:** 029 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 152 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Payer Balances` (`prompts/03-insurance-and-claims/028-payer-balances.md`).
- **Menu path:** `Insurance & Claims → Claim Reconciliation`
- **Canonical route:** `/claims?section=claim-reconciliation`
- **Surface profile:** `reconciliation` table/worklist opened from a flat workspace tab
- **Navigation model:** one flat tab strip under the `Insurance & Claims` menu item; no nested tabs and no sidebar submenu
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claim-reconciliation.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claim-reconciliation.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claim-reconciliation.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Payer Statements` (`prompts/03-insurance-and-claims/030-payer-statements.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claim-reconciliation.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claim-reconciliation.md`
