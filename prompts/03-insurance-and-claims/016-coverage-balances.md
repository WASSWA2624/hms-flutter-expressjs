# Prompt 016 — Implement Coverage Balances

## Context

- **Sequence:** 016 of 035 in `prompts/03-insurance-and-claims/`
- **Overall step:** 139 of 158
- **Phase:** Insurance and claims — membership and authorization
- **Prerequisite:** `Insurance & Claims → Eligibility Checks` (`prompts/03-insurance-and-claims/015-eligibility-checks.md`).
- **Menu path:** `Insurance & Claims → Coverage Balances`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/coverage-balances.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/coverage-balances.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/coverage-balances.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Pre-authorizations` (`prompts/03-insurance-and-claims/017-pre-authorizations.md`) until all acceptance criteria below pass.
...
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/coverage-balances.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/coverage-balances.md`
