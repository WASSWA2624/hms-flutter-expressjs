# Prompt 004 — Implement Claim Submissions

## Context

- **Sequence:** 004 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 144 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Claim Batches` (`prompts/03-insurance-and-claims/04-claims-processing/003-claim-batches.md`).
- **Menu path:** `Insurance & Claims → Claims Processing → Claim Submissions`
- **Canonical route:** `/claims?section=claim-submissions`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/claim-submissions.md`
...
2. **Register and scope the tab.**
   - Implement the `Claim Submissions` tab at `Insurance & Claims → Claim Submissions` with canonical section slug `claim-submissions`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/claim-submissions.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/claim-submissions.md`:
     `Submission No.`, `Submission Date/Time`, `Claim/Batch No.`, `Payer`, `Submission Channel`, `External Reference`, `Submitted Amount`, `Currency`, `Document Count`, `Payload/File Name`, `Response Code`, `Response Message`, `Acknowledged At`, `Retry Count`, `Next Retry At`, `Submitted By`, `Submission Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Submit selected:** gate `billing:write` ∩ `insurance-claims`; enable when claims validated; Sends via the configured submission channel.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Submission Date/Time`, `Claim/Batch No.`, `Payer`, `Submission Channel`, `External Reference`, `Currency`, `Payload/File Name`, `Response Code`, `Response Message`, `Acknowledged At`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Adjudications` (`prompts/03-insurance-and-claims/04-claims-processing/005-adjudications.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claim-submissions` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/claim-submissions.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/claim-submissions.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claim_submissions_panel.dart`
