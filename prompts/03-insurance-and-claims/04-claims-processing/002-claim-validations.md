# Prompt 002 — Implement Claim Validations

## Context

- **Sequence:** 002 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 142 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Claims` (`prompts/03-insurance-and-claims/04-claims-processing/001-claims.md`).
- **Menu path:** `Insurance & Claims → Claims Processing → Claim Validations`
- **Canonical route:** `/claims?section=claim-validations`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/claim-validations.md`
...
2. **Register and scope the tab.**
   - Implement the `Claim Validations` tab at `Insurance & Claims → Claim Validations` with canonical section slug `claim-validations`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/claim-validations.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/claim-validations.md`:
     `Validation No.`, `Claim No.`, `Patient/Member`, `Payer`, `Validation Date/Time`, `Rule Code`, `Validation Category`, `Severity`, `Field/Item`, `Error Code`, `Error Message`, `Suggested Resolution`, `Resolved By`, `Resolved At`, `Validation Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Run validation:** gate `billing:write` ∩ `insurance-claims`; enable when draft claims selected; Executes configured claim rules and refreshes errors.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Claim No.`, `Patient/Member`, `Payer`, `Validation Date/Time`, `Rule Code`, `Validation Category`, `Severity`, `Field/Item`, `Error Code`, `Error Message`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Batches` (`prompts/03-insurance-and-claims/04-claims-processing/003-claim-batches.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claim-validations` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/claim-validations.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/claim-validations.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claim_validations_panel.dart`
