# Prompt 006 — Implement Denials

## Context

- **Sequence:** 006 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 146 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Adjudications` (`prompts/03-insurance-and-claims/04-claims-processing/005-adjudications.md`).
- **Menu path:** `Insurance & Claims → Claims Processing → Denials`
- **Canonical route:** `/claims?section=denials`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/denials.md`
...
2. **Register and scope the tab.**
   - Implement the `Denials` tab at `Insurance & Claims → Denials` with canonical section slug `denials`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/denials.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/denials.md`:
     `Denial No.`, `Decision Date`, `Claim No.`, `Claim Item No.`, `Patient/Member`, `Payer`, `Submitted Amount`, `Denied Amount`, `Currency`, `Denial Code`, `Denial Category`, `Denial Reason`, `Correctable`, `Appeal Deadline`, `Assigned Owner`, `Follow-up Date`, `Denial Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Assign denial:** gate `billing:write` ∩ `insurance-claims`; enable when denial is open; Assigns owner and response deadline.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Decision Date`, `Claim No.`, `Claim Item No.`, `Patient/Member`, `Payer`, `Denied Amount`, `Currency`, `Denial Code`, `Denial Category`, `Denial Reason`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Resubmissions & Appeals` (`prompts/03-insurance-and-claims/04-claims-processing/007-resubmissions-and-appeals.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=denials` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/denials.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/denials.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_denials_panel.dart`
