# Prompt 007 — Implement Resubmissions & Appeals

## Context

- **Sequence:** 007 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 147 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Denials` (`prompts/03-insurance-and-claims/04-claims-processing/006-denials.md`).
- **Menu path:** `Insurance & Claims → Claims Processing → Resubmissions & Appeals`
- **Canonical route:** `/claims?section=resubmissions-and-appeals`
- **Surface profile:** `transaction` table/worklist tab
- **Navigation model:** `Insurance & Claims → Claims Processing` are the two sidebar menu levels; this surface is a tab in that category's flat `AppTabStrip`
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/resubmissions-and-appeals.md`
...
2. **Register and scope the tab.**
   - Implement the `Resubmissions & Appeals` tab at `Insurance & Claims → Resubmissions & Appeals` with canonical section slug `resubmissions-and-appeals`.
   ...
   - Add this tab to the flat `AppTabStrip` (`frontend/lib/shared/components/app_tab_strip.dart`) of the `Claims Processing` sidebar menu item. Every finance menu nests exactly one level: never add a second sidebar level, a category `AppTabStrip` above the section strip, or an `AppTabStripVariant.nested` layer. See `.cursor/finance/_shared/navigation-model.md`.

3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/resubmissions-and-appeals.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/resubmissions-and-appeals.md`:
     `Appeal/Resubmission No.`, `Type`, `Original Claim No.`, `Payer`, `Patient/Member`, `Original Denied Amount`, `Requested Amount`, `Currency`, `Reason`, `Supporting Document Count`, `Submitted Date`, `External Reference`, `Appeal Deadline`, `Decision Date`, `Approved Amount`, `Assigned Owner`, `Appeal Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Create appeal/resubmission:** gate `billing:write` ∩ `insurance-claims`; enable when denial is appealable; Opens appeal evidence dialog.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Type`, `Original Claim No.`, `Payer`, `Patient/Member`, `Original Denied Amount`, `Requested Amount`, `Currency`, `Reason`, `Submitted Date`, `External Reference`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Remittance Advice` (`prompts/03-insurance-and-claims/05-remittances-and-reconciliation/001-remittance-advice.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=resubmissions-and-appeals` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/resubmissions-and-appeals.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/resubmissions-and-appeals.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_resubmissions_and_appeals_panel.dart`
