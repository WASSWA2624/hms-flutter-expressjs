# Prompt 003 — Implement Claim Batches

## Context

- **Sequence:** 003 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 143 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Claim Validations` (`prompts/03-insurance-and-claims/04-claims-processing/002-claim-validations.md`).
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/claim-batches.md`
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/claim-batches.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/claim-batches.md`:
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Submissions` (`prompts/03-insurance-and-claims/04-claims-processing/004-claim-submissions.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claim-batches` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/claim-batches.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/claim-batches.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claim_batches_panel.dart`
