# Prompt 001 — Implement Claims

## Context

- **Sequence:** 001 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 141 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Pre-authorizations` (`prompts/03-insurance-and-claims/03-membership-and-eligibility/005-pre-authorizations.md`).
- **Menu path:** `Insurance & Claims → Claims Processing → Claims`
- **Canonical route:** `/claims?section=claims`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/claims.md`
...
2. **Register and scope the tab.**
   - Implement the `Claims` tab at `Insurance & Claims → Claims` with canonical section slug `claims`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/claims.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/claims.md`:
     `Claim No.`, `Claim Date`, `Service From`, `Service To`, `Patient/Member`, `Policy No.`, `Payer`, `Plan`, `Encounter/Admission No.`, `Invoice No.`, `Diagnosis Summary`, `Item Count`, `Submitted Amount`, `Approved Amount`, `Rejected Amount`, `Patient Share`, `Paid Amount`, `Outstanding Amount`, `Currency`, `Submission Date`, `Claim Status`, `Assigned To`.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Claim Date`, `Service From`, `Service To`, `Patient/Member`, `Policy No.`, `Payer`, `Plan`, `Encounter/Admission No.`, `Invoice No.`, `Diagnosis Summary`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Claim Validations` (`prompts/03-insurance-and-claims/04-claims-processing/002-claim-validations.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=claims` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/claims.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/claims.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_claims_panel.dart`
