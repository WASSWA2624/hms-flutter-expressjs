# Prompt 005 — Implement Adjudications

## Context

- **Sequence:** 005 of 007 in `prompts/03-insurance-and-claims/04-claims-processing/`
- **Overall step:** 145 of 158
- **Phase:** Insurance and claims — claims and settlement
- **Prerequisite:** `Insurance & Claims → Claim Submissions` (`prompts/03-insurance-and-claims/04-claims-processing/004-claim-submissions.md`).
- **Menu path:** `Insurance & Claims → Claims Processing → Adjudications`
- **Canonical route:** `/claims?section=adjudications`
...
- **Authoritative tab specification:** `.cursor/finance/insurance-and-claims/claims-processing/adjudications.md`
...
2. **Register and scope the tab.**
   - Implement the `Adjudications` tab at `Insurance & Claims → Adjudications` with canonical section slug `adjudications`.
...
3. **Implement the backend and data contract.**
   - Implement every list, detail, CRUD, workflow, report, or reconciliation operation defined under **Target API contract** in `.cursor/finance/insurance-and-claims/claims-processing/adjudications.md`.
...
4. **Build the primary table with the exact source columns.**
   ...
   - Preserve this source order in Settings/export while applying the default/optional visibility defined in `.cursor/finance/insurance-and-claims/claims-processing/adjudications.md`:
     `Adjudication No.`, `Adjudication Date`, `Claim No.`, `Claim Item No.`, `Payer`, `Service/Item`, `Submitted Amount`, `Allowed Amount`, `Approved Amount`, `Rejected Amount`, `Patient Share`, `Co-pay`, `Deductible`, `Currency`, `Decision Code`, `Decision Reason`, `Adjudicator Reference`, `Decision Status`.
...
5. **Implement filters and table controls.**
   ...
   - Implement these context-specific toolbar buttons:
     - **Record adjudication:** gate `billing:write` ∩ `insurance-claims`; enable when payer response available; Captures item-level payer decisions.
...
7. **Implement forms, detail dialogs, and nested tables.**
   - Create/edit fields: `Adjudication Date`, `Claim No.`, `Claim Item No.`, `Payer`, `Service/Item`, `Allowed Amount`, `Co-pay`, `Deductible`, `Currency`, `Decision Code`.
...
11. **Add and run verification.**
    ...
    - Do not proceed to `Insurance & Claims → Denials` (`prompts/03-insurance-and-claims/04-claims-processing/006-denials.md`) until all acceptance criteria below pass.
...
- **AC2 (R2):** `/claims?section=adjudications` opens the correct authorized tab, restores state, and omits it when access is denied.
- **AC3 (R3):** All API operations in `.cursor/finance/insurance-and-claims/claims-processing/adjudications.md` use validated, scoped, versioned contracts and pass success, denial, conflict, rollback, and audit tests.
...
- `.cursor/finance/insurance-and-claims/claims-processing/adjudications.md`
...
- `frontend/lib/features/claims/presentation/widgets/claims_adjudications_panel.dart`
