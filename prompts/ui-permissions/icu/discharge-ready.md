# UI Permission Scan — ICU workspace / Discharge ready (`/icu?…=discharge-ready`)
...
- Screen inventory (read-only; do not modify): `screens/icu.md` (reachable controls).
- Target tab: **Discharge ready** (`discharge-ready`). Step-down / discharge ready.
- Feature code: `frontend/lib/features/icu/`
- Module entitlement: `icu-critical-care`
- Route entry any-of: `clinical:read`, `emergency:read`, `operations:read`
...
| View / read UI | any-of (∪) | `clinical:read`, `emergency:read` |
...
ICU stays/alerts/transfers need clinical:write. Emergency origin cases may union emergency:read for visibility. Bed board manage follows rooms-beds admin gates.
