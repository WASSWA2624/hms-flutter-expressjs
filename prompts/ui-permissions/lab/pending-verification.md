# UI Permission Scan — Lab workspace / Pending verification (`/lab?…=pending-verification`)
...
- Screen inventory (read-only; do not modify): `screens/lab.md` (reachable controls).
- Target tab: **Pending verification** (`pending-verification`). Verify/release results need lab:write.
- Feature code: `frontend/lib/features/lab/`
- Module entitlement: `lab-workflows`
- Route entry any-of: `lab:read`, `clinical:read`, `clinical:write`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `lab:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `lab:write` |
| Update | all-of (∩) | `lab:write` |
| Delete | all-of (∩) | `lab:write` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Create order / configurations need lab:write (clinical:write may satisfy request-from-clinical any-of—reuse existing requirement). Result entry and verification need lab:write. Critical release may require intersection of lab:write + clinical:read for notify. Readers with only clinical:read must not see config/create.
