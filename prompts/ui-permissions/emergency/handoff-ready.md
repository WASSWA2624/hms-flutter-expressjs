# UI Permission Scan — Emergency workspace / Handoff ready (`/emergency?…=handoff`)
...
- Target tab: **Handoff ready** (`handoff`). Handoff to ward/OPD; clinical:write may be needed for admit.
- Feature code: `frontend/lib/features/emergency/`
- Module entitlement: `scheduling-queue (emergency)`
- Route entry any-of: `emergency:read`, `emergency:write`, `operations:read`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `emergency:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `emergency:write` |
| Update | all-of (∩) | `emergency:write` |
| Delete | all-of (∩) | `emergency:delete` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Quick arrival / triage / disposition need emergency:write. Hard delete/void needs emergency:delete. Ambulance ops may also need operations:read for vehicle context; do not expose delete to write-only staff.
