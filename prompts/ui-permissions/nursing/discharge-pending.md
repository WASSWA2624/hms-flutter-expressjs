# UI Permission Scan — Nursing workspace / Discharge pending (`/nursing?…=discharge-pending`)
...
- Screen inventory (read-only; do not modify): `screens/nursing.md` (reachable controls).
- Target tab: **Discharge pending** (`discharge-pending`). Nursing discharge checks; billing clearance needs billing:read.
- Feature code: `frontend/lib/features/nursing/`
- Module entitlement: `inpatient-bed-management`
- Route entry any-of: `clinical:read`, `patient:read`, `last_office:read`, `operations:read`
...
| Nested cross-module read | any-of (∪) | `billing:read`, `last_office:read` |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Medication-due actions need pharmacy:read to view meds and clinical:write (or pharmacy:write where dispense is involved)—use intersection when both apply. Shift context is read via roster:read when present. Handover/transfer/discharge pending writes need clinical:write; last_office:read alone must not unlock write controls.
