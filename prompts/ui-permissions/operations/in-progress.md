# UI Permission Scan — Operations workspace / In progress (`/operations?…=in-progress`)
...
- Screen inventory (read-only; do not modify): `screens/operations.md` (reachable controls).
- Target tab: **In progress** (`in-progress`). In-progress work.
...
Create/update requests and asset mutations need operations:write. Report summary dialog needs operations:read only.
