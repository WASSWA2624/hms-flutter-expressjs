# UI Permission Scan — Patients registry / Admitted (`/patients?…=admitted`)
...
- Screen inventory (read-only; do not modify): `screens/patients.md` (reachable controls).
- Target tab: **Admitted** (`admitted`). Inpatient admissions. Row/detail may surface admit/discharge actions requiring clinical:write (and billing:read for financial status) in addition to patient:read.
- Feature code: `frontend/lib/features/patients/`
- Module entitlement: `patient-registry`
- Route entry any-of: `patient:read`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `patient:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `patient:write` |
| Update | all-of (∩) | `patient:write` |
| Delete | all-of (∩) | `patient:delete` |
| Nested cross-module read | any-of (∪) | `clinical:read`, `billing:read` |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Nested Quick Actions use their module rights: OPD/admit need clinical + encounter gates; lab/radiology/theater/physio need matching write/request rights; insurance/billing chips need billing:read/write; reports need reports:read. Show a chip only when every listed right is held (intersection).
