# UI Permission Scan — Theater workspace / Follow-ups (`/theater?…=follow-ups`)
...
- Screen inventory (read-only; do not modify): `screens/theater.md` (reachable controls).
- Target tab: **Follow-ups** (`follow-ups`). No Schedule case primary.
- Feature code: `frontend/lib/features/theater/`
- Module entitlement: `theatre-anesthesia`
- Route entry any-of: `patient:read`, `clinical:read`, `billing:read`, `operations:read`
...
Schedule case / stage updates need clinical:write (theater write gate). Billing holds need billing:read. Room/asset context may need operations:read.
