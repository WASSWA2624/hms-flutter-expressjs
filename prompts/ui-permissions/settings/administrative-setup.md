# UI Permission Scan — Settings / Administrative setup workspace (`/settings?…=workspace`)
...
- Screen inventory (read-only; do not modify): `screens/settings.md` (reachable controls).
- Target tab: **Administrative setup workspace** (`workspace`). Wards/units/rooms/beds/users setup; admin or HR workspace gates.
- Feature code: `frontend/lib/features/settings/`
- Module entitlement: `settings / admin setup`
- Route entry any-of: _(n/a)_
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `profile:read` |
| View / read UI | any-of (∪) | `facility:admin`, `tenant:admin`, `system:admin`, `hr:read` |
| Create | all-of (∩) | `facility:admin` |
| Update | all-of (∩) | `profile:update` |
| Delete | all-of (∩) | `facility:admin` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Preferences/Accessibility: authenticated profile prefs. Account security: profile:update. Administration boundaries / Configuration / Administrative setup: facility:admin ∪ tenant:admin ∪ system:admin. Hide entire sections when no authorized actions remain.
