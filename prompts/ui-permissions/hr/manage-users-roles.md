# UI Permission Scan — HR workspace / Manage users and roles (`/hr?…=access`)
...
- Screen inventory: `screens/hr.md` (source of truth for reachable controls).
- Target tab: **Manage users and roles** (`access`). Embedded access admin; tenant/facility/system admin any-of.
- Feature code: `frontend/lib/features/hr/`
- Module entitlement: `hr-rosters`
- Route entry any-of: `hr:read`, `hr:write`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `hr:read` |
| View / read UI | any-of (∪) | `tenant:admin`, `facility:admin`, `system:admin` |
| Create | all-of (∩) | `tenant:admin` |
| Update | all-of (∩) | `tenant:admin` |
| Delete | all-of (∩) | `hr:write` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Add staff / leave request need hr:write. Shift templates need roster:write (and publish/approve use roster:publish / roster:approve). Access tab embeds admin-access rules (tenant/facility admin). Payroll drafts may need financial:approve for approve actions.
