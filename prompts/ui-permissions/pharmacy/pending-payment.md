# UI Permission Scan — Pharmacy workspace / Pending payment (`/pharmacy?…=pending-payment`)
...
- Screen inventory (read-only; do not modify): `screens/pharmacy.md` (reachable controls).
- Target tab: **Pending payment** (`pending-payment`). Payment gate before dispense.
- Feature code: `frontend/lib/features/pharmacy/`
- Module entitlement: `pharmacy-dispensing`
- Route entry any-of: `pharmacy:read`, `operations:read`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `pharmacy:read`, `billing:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `pharmacy:write` |
| Update | all-of (∩) | `pharmacy:write` |
| Delete | all-of (∩) | `pharmacy:write` |
...
Dispense/partial fill need pharmacy:write. Pending-payment queue may show billing:read status; collecting payment needs billing:write. Catalog/stock nested CRUD needs pharmacy:write; readers browse only. Controlled drugs may require intersection with compliance:read for audit panels.
