# UI Permission Scan — Rooms & beds / Out of service (`/rooms-beds?…=out-of-service`)
...
- Screen inventory (read-only; do not modify): `screens/rooms-beds.md` (reachable controls).
- Target tab: **Out of service** (`out-of-service`). Mark in/out of service needs manage/write.
- Feature code: `frontend/lib/features/rooms_beds/`
- Module entitlement: `inpatient-bed-management`
- Route entry any-of: `clinical:read`, `operations:read`, `tenant:admin`, `facility:admin`, `system:admin`
...
| View / read UI | any-of (∪) | `clinical:read`, `operations:read`, `facility:admin` |
| Create | all-of (∩) | `unit:manage` |
| Update | all-of (∩) | `unit:manage` |
| Delete | all-of (∩) | `unit:manage` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Bed admin create room/bed uses unit:manage or facility/tenant admin (any-of elevated admins). Assign/release/transfer occupancy needs clinical:write or operations:write per existing bed gates—prefer existing helpers; never show admin create to clinical-read-only.
