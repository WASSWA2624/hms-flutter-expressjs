# UI Permission Scan — Subscriptions workspace / Subscriptions (`/subscriptions?…=subscriptions`)
...
- Screen inventory (read-only; do not modify): `screens/subscriptions.md` (reachable controls).
- Target tab: **Subscriptions** (`subscriptions`). Assign/create tenant subscriptions.
- Feature code: `frontend/lib/features/subscriptions/`
- Module entitlement: `platform subscriptions`
- Route entry any-of: `system:admin`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `subscriptions:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `subscriptions:write` |
| Update | all-of (∩) | `subscriptions:write` |
| Delete | all-of (∩) | `subscriptions:delete` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Route is super-admin gated; still gate each atom with subscriptions:* so elevated-but-scoped sessions cannot over-grant. Overview KPIs need subscriptions:read; destructive deletes need subscriptions:delete.
