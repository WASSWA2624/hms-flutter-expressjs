# UI Permission Scan — Integrations workspace / Integrations (`/integrations?…=integrations`)
...
- Screen inventory (read-only; do not modify): `screens/integrations.md` (reachable controls).
- Target tab: **Integrations** (`integrations`). Create integration primary.
- Feature code: `frontend/lib/features/integrations/`
- Module entitlement: `integrations-core`
- Route entry any-of: `integration:read`, `integration:write`, `tenant:admin`, `facility:admin`, `system:admin`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `integration:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `integration:write` |
| Update | all-of (∩) | `integration:write` |
| Delete | all-of (∩) | `integration:delete` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Create integration/API key/webhook need integration:write (or admin any-of per existing manage requirement). Secrets reveal is write-only. Logs are read; Interop tests may need write.
