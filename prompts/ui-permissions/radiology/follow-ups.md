# UI Permission Scan — Radiology workspace / Follow-ups (`/radiology?…=follow-ups`)
...
- Screen inventory (read-only; do not modify): `screens/radiology.md` (reachable controls).
- Target tab: **Follow-ups** (`follow-ups`). No Request imaging primary.
- Feature code: `frontend/lib/features/radiology/`
- Module entitlement: `radiology-workflows`
- Route entry any-of: `radiology:read`, `radiology:write`, `clinical:read`, `clinical:write`, `billing:read`
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `radiology:read` |
| View / read UI | any-of (∪) | _(n/a)_ |
| Create | all-of (∩) | `radiology:write` |
| Update | all-of (∩) | `radiology:write` |
| Delete | all-of (∩) | `radiology:write` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Request imaging / configurations need radiology:write. Reporting and release need radiology:write. Billing holds need billing:read. Clinical:read alone may view shared results but not config.
