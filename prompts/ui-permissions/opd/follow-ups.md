# UI Permission Scan — OPD workspace / Follow-ups (`/opd?…=follow-ups`)
...
- Screen inventory (read-only; do not modify): `screens/opd.md` (reachable controls).
- Target tab: **Follow-ups** (`follow-ups`). Shared follow-up worklist; no Start OPD primary.
- Feature code: `frontend/lib/features/opd/`
- Module entitlement: `scheduling-queue`
- Route entry any-of: `patient:read`, `clinical:read`, `billing:read`, `operations:read`, `emergency:read`
...
| View / read UI | any-of (∪) | `patient:read`, `clinical:read` |
...
Start OPD encounter uses encounter permission (clinical write / patient-flow gate). Triage/queue stage actions need clinical:write. Payment-related stages need billing:read/write. Follow-ups panel is read with clinical:read; complete needs write.
