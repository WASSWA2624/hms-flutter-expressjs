# UI Permission Scan — Access admin / Registrations (`/admin/access?…=registrations`)
...
- Screen inventory (read-only; do not modify): `screens/admin-access.md` (reachable controls).
- Target tab: **Registrations** (`registrations`). Pending registrations; elevated only.
...
| --- | --- | --- |
| View / read UI | all-of (∩) | `system:admin` |
| View / read UI | any-of (∪) | `tenant:admin`, `facility:admin`, `system:admin` |
| Create | all-of (∩) | `tenant:admin` |
| Update | all-of (∩) | `tenant:admin` |
| Delete | all-of (∩) | `tenant:admin` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Reuse existing canWrite / elevated gates. Registrations tab only when isElevated. Assignable rights must stay within actor ceiling and subscription modules. Demo users follow same write gate.
