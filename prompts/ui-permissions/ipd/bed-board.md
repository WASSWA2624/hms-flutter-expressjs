# UI Permission Scan — IPD workspace / Bed board (`/ipd?…=bed-board`)
...
- Screen inventory (read-only; do not modify): `screens/ipd.md` (reachable controls).
- Target tab: **Bed board** (`bed-board`). Occupancy board; Manage beds needs bed-admin rights.
- Feature code: `frontend/lib/features/ipd/`
- Module entitlement: `inpatient-bed-management`
- Route entry any-of: `clinical:read`, `operations:read`, `billing:read`
...
| View / read UI | any-of (∪) | `clinical:read`, `operations:read` |
...
| Nested cross-module write | any-of (∪) | `unit:manage`, `facility:admin`, `tenant:admin` |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Start admission needs operational/clinical write gate. Manage beds navigates to /rooms-beds and needs bed-admin (facility/tenant admin or unit:manage). Billing panels inside detail need billing:read.
