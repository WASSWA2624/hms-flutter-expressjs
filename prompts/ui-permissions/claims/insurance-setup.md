# UI Permission Scan — Claims workspace / Insurance Setup (`/claims?…=insurance-setup`)
...
- Screen inventory: `screens/claims.md` (source of truth for reachable controls).
- Target tab: **Insurance Setup** (`insurance-setup`). Payers/plans/catalog; create/edit need billing:write (or facility admin).
- Feature code: `frontend/lib/features/claims/`
- Module entitlement: `insurance-claims`
- Route entry any-of: `billing:read`, `billing:write`, `financial:approve`
...
| View / read UI | any-of (∪) | `billing:read`, `facility:admin`, `tenant:admin` |
...
Authorizations and claim prepare use claimsWorkspaceWriteRequirement (typically billing:write). Settlement/approval steps may need financial:approve. Insurance Setup catalog edits need write; read-only users see setup without create controls.
