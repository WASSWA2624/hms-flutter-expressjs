# UI Permission Scan — Reception desk / Active visits (`/reception?…=active-visits`)
...
- Screen inventory (read-only; do not modify): `screens/reception.md` (reachable controls).
- Target tab: **Active visits** (`active-visits`). In-facility visits; continue/end may need clinical:write or OPD encounter gate.
- Feature code: `frontend/lib/features/reception/`
- Module entitlement: `patient-registry + scheduling-queue`
- Route entry any-of: `patient:read`, `last_office:read`
...
| Nested cross-module write | any-of (∪) | `clinical:write`, `patient:write` |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Desk tabs themselves may use receptionDeskSectionRequirement (hide unauthorized tabs). Register/Schedule need patient:write. Payment gate needs billing:read (view) / billing:write (collect). Follow-ups may allow last_office:read readers without patient:write.
