# UI Permission Scan — Reports workspace / Reports (all panels) (`/reports?…=reports`)
...
- Screen inventory (read-only; do not modify): `screens/reports.md` (reachable controls).
- Target tab: **Reports (all panels)** (`reports`). Catalog, schedules, compliance, timeline nested surfaces.
- Feature code: `frontend/lib/features/reports/`
- Module entitlement: `reports`
- Route entry any-of: `reports:read`, `reports:write`, `compliance:read`, `evidence:export`, `tenant:admin`, `facility:admin`, `system:admin`
...
| View / read UI | any-of (∪) | `reports:read`, `compliance:read` |
| Create | all-of (∩) | `reports:write` |
| Update | all-of (∩) | `reports:write` |
| Delete | all-of (∩) | `reports:delete` |
| Nested cross-module read | any-of (∪) | _(n/a)_ |
| Nested cross-module write | any-of (∪) | _(n/a)_ |
| Nested cross-module write | all-of (∩) | _(n/a)_ |

Compliance panels need compliance:read/review. Exports need evidence:export or reports:write per control. Hide schedule create without reports:write.
