...
**Scope:** only `_TransferRequestDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: transferOut`
...
| Purpose | Patient/encounter flow: Transfer Request (icu). |
| Defined in | `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:430` |
| Kind | `custom` |
...
- `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart (openIcuFocusPanel)`
...
| Title snippets | `l10n.icuTransferDialogTitle` |
...
1. **Read contract + source**
   ...
   - Read `_TransferRequestDialog` at `frontend/lib/features/icu/presentation/widgets/icu_action_dialogs.dart:430` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Transfer Request (icu).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `openIcuTransferDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_TransferRequestDialog`'s path.
...
<!-- generator: encounter-dialog prompt 07 slug=icu-transfer-request-dialog symbol=_TransferRequestDialog -->
