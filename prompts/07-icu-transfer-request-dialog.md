...
3. **Normalize title + icon**
   - Use a general, role-based title for **Transfer Request (icu).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_TransferRequestDialog` opens through `AppDialog` / approved helpers only.
