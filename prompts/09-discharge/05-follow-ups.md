# Discharge Follow Ups tab — rule compliance

## Context

Make the Follow Ups desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/09-discharge/05-follow-ups.md`. Apply shared chrome fixes from `prompts/09-discharge/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `opdFollowUpsTitle`, deep-link `follow-ups` (aliases `follow_ups`, `followups`), and omit the tab when gate denies access (`DischargeFollowUpsAtomPermissions.tab` = workspace read ∪) — never show a disabled placeholder (`tabs.mdc`). Align `DischargeDeskSection` / query helpers with `tabs/09-discharge/00-overview.md`.
...
- [ ] `tabs/09-discharge/05-follow-ups.md` updated to match.
