# Discharge Pending Clearance tab — rule compliance

## Context

Make the Pending Clearance desk section fully compliant with `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, `printing.mdc`, and `screens.mdc`. Inventory baseline: `tabs/09-discharge/03-pending-clearance.md`. Apply shared chrome fixes from `prompts/09-discharge/00-shared-chrome.md` first when this tab depends on them (counts, Print, Export gate, tones, shared filter labels).

## Requirements

1. Keep strip label `dischargeSectionPendingClearance`, deep-link `pending` (aliases `pending_clearance`, `pending-clearance`, `pendingclearance`), and omit the tab when gate denies access (`DischargePendingClearanceAtomPermissions.tab` = **pending clearance read ∪** (broader than workspace read)) — never show a disabled placeholder (`tabs.mdc`). Align `DischargeDeskSection` / query helpers with `tabs/09-discharge/00-overview.md`.
2. Badge count must be the authoritative total for this tab’s scope (`state.summaryPendingCount`). When Filters/search/date narrow the active tab, the **active** badge must reflect the filtered total (`tabs.mdc`). Stop using painted-row length alone when a total is available or can be derived from the same filter model.
...
- [ ] `tabs/09-discharge/03-pending-clearance.md` updated to match.
