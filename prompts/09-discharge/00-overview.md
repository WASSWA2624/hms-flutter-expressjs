# Discharge overview — workspace compliance program

## Context

Bring the Discharge desk to **100% compliance** with `prompts/.cursor` rules: `prompt.mdc`, `screens.mdc`, `tabs.mdc`, `tables.mdc`, `dialogs.mdc`, `forms.mdc`, and `printing.mdc`.

Inventory baseline (do not reinvent atoms): `tabs/09-discharge/`. Per-surface remediation prompts live beside this file under `prompts/09-discharge/`. Execute this overview first only for shared infrastructure; otherwise implement the matching per-tab / shared-chrome / gaps prompts.
...
1. Treat `tabs/09-discharge/00-overview.md` as the authoritative tab index (`DischargeDeskSection` order, query values, aliases). Keep deep-link helpers aligned with that index.
2. Implement remediation in this order so shared chrome lands once:
   ...
   3. Per-tab prompts:
   - `prompts/09-discharge/01-all.md`
   - `prompts/09-discharge/02-planned.md`
   - `prompts/09-discharge/03-pending-clearance.md`
   - `prompts/09-discharge/04-completed.md`
   - `prompts/09-discharge/05-follow-ups.md`
...
6. After each remediation, update the matching inventory file under `tabs/09-discharge/` so the catalog matches shipped behavior (still not under a restored `screens/` folder).
7. Add or extend automated tests under `frontend/test/features/discharge/` proving authorized atoms remain available and unauthorized atoms are absent.
...
- [ ] Every desk section in `tabs/09-discharge/00-overview.md` has a completed remediation prompt under `prompts/09-discharge/` and a matching updated inventory under `tabs/09-discharge/`.
...
- Trace `frontend/lib/features/discharge/presentation/pages/discharge_workspace_page.dart`, `frontend/lib/features/discharge/presentation/discharge_access.dart`, and widgets listed in `tabs/09-discharge/00-overview.md`.
- Run feature tests under `frontend/test/features/discharge/`; add cases for omit-when-unauthorized, count badges, toolbar order Filters → Settings → Export → Print → context (when Print applies), and print-preview-before-print.
...
- `tabs/09-discharge/00-overview.md`
- `prompts/09-discharge/00-shared-chrome.md`
- `prompts/09-discharge/99-convention-gaps.md`
- `prompts/09-discharge/01-all.md`
- `prompts/09-discharge/02-planned.md`
- `prompts/09-discharge/03-pending-clearance.md`
- `prompts/09-discharge/04-completed.md`
