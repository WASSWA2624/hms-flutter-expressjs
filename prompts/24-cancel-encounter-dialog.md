...
| Defined in | `frontend/lib/shared/components/opd_encounter_dialog.dart:2044` |
| Kind | `shared` |
...
- `frontend/lib/shared/components/opd_encounter_dialog.dart` (`_promptCancelEncounter`)
...
1. **Read contract + source**
   ...
   - Read `_CancelEncounterDialog` at `frontend/lib/shared/components/opd_encounter_dialog.dart:1993` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Cancel Encounter (shared/components).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_CancelEncounterDialog` opens through `AppDialog` / approved helpers only.
