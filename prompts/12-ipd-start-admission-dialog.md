...
| Field | Value |
| --- | --- |
| Symbol | `IpdStartAdmissionDialog` |
| Purpose | Ipd Start Admission |
| Module / surface | `ipd` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/ipd/presentation/widgets/ipd_start_admission_dialog.dart:19` |
...
| Paired opener(s) | `_openStartAdmissionDialog` |
| Primary commit | Start admission |
| Slices to keep in sync | IPD admissions list, bed assignment, encounter stage |
| Sibling reuse targets | `_PatientAdmissionQuickDialog`, `ClinicalAdmissionActionDialog` |
...
| Mutations (region) | `mutation: startAdmission` |
...
| Title snippets | `l10n.ipdStartAdmissionTitle`, `widget.title` |
...
**Scope lock:** only `IpdStartAdmissionDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `IpdStartAdmissionDialog` at `frontend/lib/features/ipd/presentation/widgets/ipd_start_admission_dialog.dart:19` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Ipd Start Admission** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Start admission`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_PatientAdmissionQuickDialog`, `ClinicalAdmissionActionDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch IPD admissions list, bed assignment, encounter stage, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openStartAdmissionDialog`. Update all call sites in the same change when signatures move.
...
1. `IpdStartAdmissionDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_PatientAdmissionQuickDialog`, `ClinicalAdmissionActionDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: IPD admissions list, bed assignment, encounter stage.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `IpdStartAdmissionDialog`'s path.
...
<!-- generator: encounter-dialog prompt 12 slug=ipd-start-admission-dialog symbol=IpdStartAdmissionDialog shape=widget_dialog -->
