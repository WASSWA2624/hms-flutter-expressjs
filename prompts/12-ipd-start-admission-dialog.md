...
3. **Normalize title + icon**
   - Use a general, role-based title for **Ipd Start Admission (ipd).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `IpdStartAdmissionDialog` opens through `AppDialog` / approved helpers only.
