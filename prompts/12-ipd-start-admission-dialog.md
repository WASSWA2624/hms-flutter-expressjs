...
| Defined in | `frontend/lib/features/ipd/presentation/widgets/ipd_start_admission_dialog.dart:19` |
...
| Sibling reuse targets | `_OpdAdmissionHandoffDialog`, `_PatientAdmissionQuickDialog` |
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_OpdAdmissionHandoffDialog`, `_PatientAdmissionQuickDialog`.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_OpdAdmissionHandoffDialog`, `_PatientAdmissionQuickDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: IPD admissions list, bed assignment, encounter stage.
