...
| Defined in | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:149` |
| Kind | `custom` |
| Paired opener(s) | `_openDispatchDialog`, `_openDispatchStatusDialog` |
...
1. **Read contract + source**
   ...
   - Read `DispatchDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:149` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Dispatch (emergency).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
9. **Preserve reachability**
   - Do not break ``_openDispatchDialog`, `_openDispatchStatusDialog`` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
1. `DispatchDialog` opens through `AppDialog` / approved helpers only.
