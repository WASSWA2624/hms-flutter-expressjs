...
| Defined in | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:206` |
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `QuickArrivalDialog`, `HandoffDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency dispatch state, case detail, queue row.
