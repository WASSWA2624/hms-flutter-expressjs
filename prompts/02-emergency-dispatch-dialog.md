...
| Field | Value |
| --- | --- |
| Symbol | `DispatchDialog` |
| Purpose | Dispatch |
| Module / surface | `emergency` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:206` |
...
| Paired opener(s) | `_openDispatchDialog`, `_openDispatchStatusDialog`, `showEmergencyDispatchDialog` |
| Primary commit | Dispatch / update status |
| Slices to keep in sync | emergency dispatch state, case detail, queue row |
| Sibling reuse targets | `QuickArrivalDialog`, `HandoffDialog` |
| Action helper peek | `clinicalActionDialogActions`, `AppSelectActionDialog` |
| Controllers (region) | _not detected in peek — trace widget → workspace controller → repository → backend route_ |
| Mutations (region) | `mutation: dispatchAmbulance`, `mutation: updateLatestDispatchStatus` |
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | `AppDialog`, approved `show*` helper, `AppSelectActionDialog` |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| Raw Material progress indicator | not seen |
| Title snippets | `widget.title`, `EmergencyText.updateDispatchStatus`, `title` |
...
| Cross-stack trace files found | 5 |
| Peek region size | 4279 chars |
...
**Scope lock:** only `DispatchDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `DispatchDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_dialogs.dart:206` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Dispatch** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Dispatch / update status`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `QuickArrivalDialog`, `HandoffDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch emergency dispatch state, case detail, queue row, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openDispatchDialog`, `_openDispatchStatusDialog`, `showEmergencyDispatchDialog`. Update all call sites in the same change when signatures move.
...
1. `DispatchDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `QuickArrivalDialog`, `HandoffDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency dispatch state, case detail, queue row.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `DispatchDialog`'s path.
...
<!-- generator: encounter-dialog prompt 02 slug=emergency-dispatch-dialog symbol=DispatchDialog shape=widget_dialog -->
