...
**Scope:** only `_CloseEncounterDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Title snippets | `l10n.opdCloseEncounterAction` |
...
1. **Read contract + source**
   ...
   - Read `_CloseEncounterDialog` at `frontend/lib/shared/components/opd_encounter_dialog.dart:1943` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Close Encounter (shared/components).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `_promptCloseEncounter` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_CloseEncounterDialog`'s path.
...
<!-- generator: encounter-dialog prompt 23 slug=close-encounter-dialog symbol=_CloseEncounterDialog -->
