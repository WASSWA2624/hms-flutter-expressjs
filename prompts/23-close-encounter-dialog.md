...
1. **Read contract + source**
   ...
   - Read `_CloseEncounterDialog` at `frontend/lib/shared/components/opd_encounter_dialog.dart:1935` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Close Encounter (shared/components).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_CloseEncounterDialog` opens through `AppDialog` / approved helpers only.
