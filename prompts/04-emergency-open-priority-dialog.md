...
**Scope:** only `_openPriorityDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `mutation-ish call: updatePriority`, `mutation-ish call: recordTriage`, `mutation-ish call: dispatchAmbulance`, `mutation-ish call: updateLatestDispatchStatus`, `mutation-ish call: handoff`
...
| Purpose | Patient/encounter flow: Priority (emergency). |
...
1. **Read contract + source**
   ...
   - Read `_openPriorityDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1432` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Priority (emergency).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_openPriorityDialog`'s path.
...
<!-- generator: encounter-dialog prompt 04 slug=emergency-open-priority-dialog symbol=_openPriorityDialog -->
