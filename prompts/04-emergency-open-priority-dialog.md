...
| Field | Value |
| --- | --- |
| Symbol | `_openPriorityDialog` |
| Purpose | Priority |
| Module / surface | `emergency` |
| Inventory kind | `custom` |
| Presentation shape | `inline_select` |
| Verified definition | `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1439` |
...
| Primary commit | Edit priority |
| Slices to keep in sync | emergency summary priority, queue ordering cues |
| Sibling reuse targets | `_openResponseDialog`, `AppTriageActionDialog` |
| Action helper peek | `AppSelectActionDialog` |
| Controllers (region) | _not detected in peek — trace widget → workspace controller → repository → backend route_ |
| Mutations (region) | `mutation: updatePriority` |
...
- _No delegated dialog/form/panel implementation detected; inspect the target body directly._
...
| Signal | Observation |
| --- | --- |
| Approved shell signals | approved `show*` helper, `AppSelectActionDialog` |
| Raw `showDialog` / `AlertDialog` | not seen in peek |
| Raw Material progress indicator | not seen |
| Title snippets | `EmergencyText.priority` |
...
| Delegated components scanned | 0 |
| Cross-stack trace files found | 2 |
| Peek region size | 1033 chars |
...
**Scope lock:** only `_openPriorityDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.

### Shape rules for `inline_select`
...
1. **Read contracts + source**
   ...
   - Read `_openPriorityDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1439` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Priority** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Edit priority`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `_openResponseDialog`, `AppTriageActionDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch emergency summary priority, queue ordering cues, then apply the smallest targeted reconciliation.
...
1. `_openPriorityDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_openResponseDialog`, `AppTriageActionDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency summary priority, queue ordering cues.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_openPriorityDialog`'s path.
...
<!-- generator: encounter-dialog prompt 04 slug=emergency-open-priority-dialog symbol=_openPriorityDialog shape=inline_select -->
