...
| Defined in | `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1432` |
...
1. **Read contracts + source**
   ...
   - Read `_openPriorityDialog` at `frontend/lib/features/emergency/presentation/widgets/emergency_workspace_widgets.dart:1432` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `_openResponseDialog`, `AppTriageActionDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: emergency summary priority, queue ordering cues.
