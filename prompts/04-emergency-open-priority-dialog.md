...
3. **Normalize title + icon**
   - Use a general, role-based title for **Priority (emergency).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_openPriorityDialog` opens through `AppDialog` / approved helpers only.
