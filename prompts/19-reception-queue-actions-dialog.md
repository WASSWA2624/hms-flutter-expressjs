...
| Purpose | Reception Queue Actions |
...
| Slices to keep in sync | reception queue rows, linked OPD encounter stage |
...
**Scope lock:** only `ReceptionQueueActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Reception Queue Actions** — never patient display name.
...
4. **Normalize loading + footer (Req 3)**
   ...
   - Order: secondary → **Cancel** → primary (`Execute queue action / Cancel`).
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `QueueActionsDialog`, `FlowActionsDialog`.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `ReceptionQueueActionsDialog`'s path.
