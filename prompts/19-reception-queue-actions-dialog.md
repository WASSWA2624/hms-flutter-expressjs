...
**Scope:** only `ReceptionQueueActionsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
| Field | Value |
| --- | --- |
| Symbol | `ReceptionQueueActionsDialog` |
| Purpose | Patient/encounter flow: Reception Queue Actions (reception). |
...
1. **Read contract + source**
   ...
   - Read `ReceptionQueueActionsDialog` at `frontend/lib/features/reception/presentation/widgets/reception_queue_actions_dialog.dart:29` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Reception Queue Actions (reception).** (flow/action name — never the patient display name as `AppDialog` title).
...
8. **Preserve reachability**
   - Do not break `showReceptionQueueActionsDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `ReceptionQueueActionsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 19 slug=reception-queue-actions-dialog symbol=ReceptionQueueActionsDialog -->
