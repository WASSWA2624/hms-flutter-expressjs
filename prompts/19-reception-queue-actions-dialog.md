...
| Title snippets | `widget.entry.displayTitle` |
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Reception Queue Actions (reception).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `ReceptionQueueActionsDialog` opens through `AppDialog` / approved helpers only.
