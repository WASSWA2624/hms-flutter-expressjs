...
3. **Normalize title + icon**
   - Use a general, role-based title for **Record Vitals (shared/opd_actions).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `RecordVitalsDialog` opens through `AppDialog` / approved helpers only.
