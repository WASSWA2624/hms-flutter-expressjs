...
**Scope:** only `RecordVitalsDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `opdRepositoryProvider`, `mutation-ish call: updateVitals`, `mutation-ish call: recordVitals`
...
| Purpose | Patient/encounter flow: Record Vitals (shared/opd_actions). |
| Defined in | `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1986` |
| Kind | `shared` |
...
- `frontend/lib/shared/components/app_record_vitals_dialog.dart`
...
1. **Read contract + source**
   ...
   - Read `RecordVitalsDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1986` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Record Vitals (shared/opd_actions).** (flow/action name — never the patient display name as `AppDialog` title).
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `RecordVitalsDialog`'s path.
...
<!-- generator: encounter-dialog prompt 40 slug=record-vitals-dialog symbol=RecordVitalsDialog -->
