...
| Mutations (region) | `mutation: updateVitals`, `mutation: recordVitals` |
...
1. **Read contracts + source**
   ...
   - Read `RecordVitalsDialog` at `frontend/lib/shared/opd_actions/opd_flow_actions_dialog.dart:1986` and every paired opener / *Used from* site.
...
5. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `AppRecordVitalsDialog`, `AppVitalsForm`, `FlowActionsDialog`).
...
8. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: vitals history, nursing/OPD vitals panels, encounter summary.
