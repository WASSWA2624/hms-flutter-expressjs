...
| Field | Value |
| --- | --- |
| Symbol | `_PatientTriageQuickDialog` |
| Purpose | Patient Triage Quick |
| Module / surface | `patients` |
| Inventory kind | `custom` |
| Presentation shape | `widget_dialog` |
| Verified definition | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2199` |
...
| Paired opener(s) | `_openPatientTriageQuickDialog` |
| Primary commit | Save triage |
| Slices to keep in sync | patient triage, encounter triage badges |
| Sibling reuse targets | `AppTriageActionDialog`, `_showTriageDialog` |
| Action helper peek | `AppTriageActionDialog` |
| Controllers (region) | `opdWorkspaceControllerProvider.notifier,` |
| Mutations (region) | `mutation: submitOpdEncounter`, `mutation: recordVitals`, `mutation: assignDoctor` |
...
- `AppTriageActionDialog — frontend/lib/shared/components/app_triage_components.dart`
...
| Title snippets | `l10n.patientsTriageDialogTitle`, `l10n.patientsWorkflowSectionTitle`, `widget.title`, `widget.prioritySectionTitle` |
...
**Scope lock:** only `_PatientTriageQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows. Shared extracts are allowed only when required for reuse and must stay domain-neutral under `frontend/lib/shared/`.
...
1. **Read contracts + source**
   ...
   - Read `_PatientTriageQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2199` and every paired opener / *Used from* site.
...
3. **Normalize title + icon (Req 4)**
   - General role/flow title for **Patient Triage Quick** — never patient display name.
...
5. **Reuse (Req 2)**
   ...
   - Cross-check sibling reuse targets: `AppTriageActionDialog`, `_showTriageDialog`.
...
8. **Backend + sync (Req 5 — hard)**
   ...
   - Persisted success only → patch patient triage, encounter triage badges, then apply the smallest targeted reconciliation.
   - Cancel/failure never present false success.

9. **Preserve reachability**
   - Do not break `_openPatientTriageQuickDialog`. Update all call sites in the same change when signatures move.
...
1. `_PatientTriageQuickDialog` opens only through `AppDialog` / approved helpers — no raw Material dialog APIs.
...
6. Body sections and action groups reuse canonical shared primitives; no unjustified local forks (siblings considered: `AppTriageActionDialog`, `_showTriageDialog`).
...
9. After persisted success only, Riverpod + targeted reconciliation keep dialog and parent surfaces aligned with backend truth for: patient triage, encounter triage badges.
...
- New dialog frameworks, unrelated redesigns, or drive-by refactors outside `_PatientTriageQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 15 slug=patients-patient-triage-quick-dialog symbol=_PatientTriageQuickDialog shape=widget_dialog -->
