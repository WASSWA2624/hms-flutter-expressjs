...
| Defined in | `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2169` |
| Kind | `custom` |
| Paired opener(s) | `—` |
...
1. **Read contract + source**
   ...
   - Read `_PatientTriageQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2169` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon**
   - Use a general, role-based title for **Patient Triage Quick (patients).** (e.g. flow/action name - never the patient display name as `AppDialog` title).
...
1. `_PatientTriageQuickDialog` opens through `AppDialog` / approved helpers only.
