...
**Scope:** only `_PatientTriageQuickDialog` and the minimum call-site / shared-helper edits required for compilation and compliance. Do **not** expand to unrelated inventory rows or invent a new dialog shell. Do not retain duplication merely to minimize the diff.
...
**Controller / mutation peek:** `opdWorkspaceControllerProvider.notifier,`, `mutation-ish call: submitOpdEncounter`, `mutation-ish call: recordVitals`, `mutation-ish call: assignDoctor`, `mutation-ish call: submitOpdEncounter`, `mutation-ish call: recordVitals`
...
| Purpose | Patient/encounter flow: Patient Triage Quick (patients). |
...
1. **Read contract + source**
   ...
   - Read `_PatientTriageQuickDialog` at `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart:2189` and every paired opener / *Used from* call site above.
...
3. **Normalize title + icon (Requirement 4)**
   - Use a general, role-based title for **Patient Triage Quick (patients).** (flow/action name — never the patient display name as `AppDialog` title).
   - Pass the title through the shell so uppercase normalization applies.
   - Add/keep a meaningful `icon` if peer dialogs in `patients` already use icons.
...
8. **Preserve reachability**
   - Do not break `_openPatientTriageQuickDialog` or the *Used from* sites. Update signatures only when required; fix all call sites in the same change.
...
- New dialog frameworks, redesigns unrelated to compliance, or drive-by refactors outside `_PatientTriageQuickDialog`'s path.
...
<!-- generator: encounter-dialog prompt 15 slug=patients-patient-triage-quick-dialog symbol=_PatientTriageQuickDialog -->
