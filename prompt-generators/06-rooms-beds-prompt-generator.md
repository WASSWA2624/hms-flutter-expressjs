...
# Screen Standardization Prompt Generator — Rooms & Beds

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Rooms & Beds** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
|-------|-------|
| Route | `/rooms-beds` |
| Screen name | Rooms & Beds |
| Page widget | `RoomsBedsWorkspacePage` |
| Primary page file | `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart` |
| Feature module | `rooms_beds` |
| Known tabs | `All beds`, `Available`, `Occupied`, `Turnover`, `Out of service` |
| Purpose | Facility bed board filtered by readiness and occupancy. |
...
|---|---------------------|---------------------|------------------------|
| 1 | All beds | [confirm from code] | [discover during audit] |
| 2 | Available | [confirm from code] | [discover during audit] |
| 3 | Occupied | [confirm from code] | [discover during audit] |
| 4 | Turnover | [confirm from code] | [discover during audit] |
| 5 | Out of service | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Rooms & Beds** (`/rooms-beds`):
...
**Objective encoded into every generated prompt:** Rooms & Beds must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/rooms-beds` / `RoomsBedsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/rooms_beds/`.
...
Compare `RoomsBedsWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/rooms-beds` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?section=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Rooms & Beds

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All beds`, `Available`, `Occupied`, `Turnover`, `Out of service`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/rooms-beds`]
...
- Save as `prompts/{NN}-standardize-rooms_beds-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `rooms_beds`
  - Example: `prompts/27-standardize-rooms_beds-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `RoomsBedsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Rooms & Beds business behavior.
...
This file is already bound to **Rooms & Beds** (`/rooms-beds`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart` and related `rooms_beds` files against Reception + `prompt.md`.
