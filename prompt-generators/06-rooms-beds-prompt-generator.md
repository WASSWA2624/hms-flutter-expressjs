...
# Table Standardization Prompt Generator — Rooms & Beds

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Rooms & Beds** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/06-rooms-beds-prompt.md` (same name as this generator file, without `-generator`).
...
| Known tabs | Bed board, Wards, Rooms |
| Purpose | Bed inventory, ward structure, and room management. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openBedDetailDialog` |

- Deep-link query parameter pattern: `?view=<value>`

### Known table inventory (validate and complete during audit)

| # | Table widget | File | Entity | Columns today | Detail on row select |
|---|--------------|------|--------|---------------|----------------------|
| 1 | Bed board table in _RoomsBedsWorkspaceContentState | `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart` | `BedBoardItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Rooms & Beds** (`/rooms-beds`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Rooms & Beds must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **rooms_beds_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openBedDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/rooms_beds/presentation/`
...
- `onRowSelected` → `_openBedDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/06-rooms-beds-prompt.md`
- This mirrors this generator file (`prompt-generators/06-rooms-beds-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `RoomsBedsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Rooms & Beds business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/rooms_beds/`.
...
1. Audit `frontend/lib/features/rooms_beds/presentation/pages/rooms_beds_workspace_page.dart` and related `rooms_beds` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/06-rooms-beds-prompt.md`.
