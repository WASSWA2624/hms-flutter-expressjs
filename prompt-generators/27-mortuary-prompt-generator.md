...
# Table Standardization Prompt Generator — Mortuary

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Mortuary** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/27-mortuary-prompt.md` (same name as this generator file, without `-generator`).
...
|-------|-------|
| Route | `/mortuary` |
| Screen name | Mortuary |
| Page widget | `MortuaryWorkspacePage` |
| Primary page file | `frontend/lib/features/mortuary/presentation/pages/mortuary_workspace_page.dart` |
| Feature module | `mortuary` |
| Known tabs | Cases, Storage, Releases, Audit |
| Purpose | Mortuary case registry, storage, and release workflow. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `onItemSelected → mortuary detail dialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _MortuaryWorklist | `frontend/lib/features/mortuary/presentation/pages/mortuary_workspace_page.dart` | `MortuaryWorkspaceItem` | 7 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Mortuary** (`/mortuary`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Mortuary must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/mortuary` / `MortuaryWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/mortuary/presentation/pages/mortuary_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/mortuary/`.
...
- Closest on-screen reference: **_MortuaryWorklist (chrome reference; reduce columns to ≤5)**
- `prompt.md`
...
- How `onRowSelected` opens `onItemSelected → mortuary detail dialog`.
...
- Existing detail dialogs in `frontend/lib/features/mortuary/presentation/`
...
- `onRowSelected` → `onItemSelected → mortuary detail dialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/27-mortuary-prompt.md`
- This mirrors this generator file (`prompt-generators/27-mortuary-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `MortuaryWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Mortuary business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/mortuary/`.
...
This file is already bound to **Mortuary** (`/mortuary`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/mortuary/presentation/pages/mortuary_workspace_page.dart` and related `mortuary` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/27-mortuary-prompt.md`.
