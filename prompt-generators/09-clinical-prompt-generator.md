...
# Table Standardization Prompt Generator — Clinical

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Clinical** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/09-clinical-prompt.md` (same name as this generator file, without `-generator`).
...
| Known tabs | All, Waiting review, Urgent, Results ready, My patients |
| Purpose | Clinical review worklist and results triage. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openClinicalEntryDialog` |

- Deep-link query parameter pattern: `?filter=<value>`

### Known table inventory (validate and complete during audit)

| # | Table widget | File | Entity | Columns today | Detail on row select |
|---|--------------|------|--------|---------------|----------------------|
| 1 | _ClinicalWorklistPanel | `frontend/lib/features/clinical/presentation/pages/clinical_workspace_page.dart` | `ClinicalWorklistEntry` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Clinical** (`/clinical`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Clinical must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **radiology _RadiologyWorklistPanel**
- `prompt.md`
...
- How `onRowSelected` opens `_openClinicalEntryDialog`.
...
- Existing detail dialogs in `frontend/lib/features/clinical/presentation/`
...
- `onRowSelected` → `_openClinicalEntryDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/09-clinical-prompt.md`
- This mirrors this generator file (`prompt-generators/09-clinical-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `ClinicalWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Clinical business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/clinical/`.
...
1. Audit `frontend/lib/features/clinical/presentation/pages/clinical_workspace_page.dart` and related `clinical` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/09-clinical-prompt.md`.
