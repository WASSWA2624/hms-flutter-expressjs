...
# Screen Standardization Prompt Generator — Clinical

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Clinical** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
|-------|-------|
| Route | `/clinical` |
| Screen name | Clinical |
| Page widget | `ClinicalWorkspacePage` |
| Primary page file | `frontend/lib/features/clinical/presentation/pages/clinical_workspace_page.dart` |
| Feature module | `clinical` |
| Known tabs | `All`, `Waiting review`, `Urgent`, `Results ready`, `In consultation`, `Completed` |
| Purpose | Clinical encounter worklist by review and consultation stage. |
...
| 2 | Waiting review | [confirm from code] | [discover during audit] |
| 3 | Urgent | [confirm from code] | [discover during audit] |
| 4 | Results ready | [confirm from code] | [discover during audit] |
| 5 | In consultation | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Clinical** (`/clinical`):
...
**Objective encoded into every generated prompt:** Clinical must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/clinical` / `ClinicalWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/clinical/presentation/pages/clinical_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/clinical/`.
...
Compare `ClinicalWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/clinical` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Clinical

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All`, `Waiting review`, `Urgent`, `Results ready`, `In consultation`, `Completed`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/clinical`]
...
- Save as `prompts/{NN}-standardize-clinical-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `clinical`
  - Example: `prompts/27-standardize-clinical-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `ClinicalWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Clinical business behavior.
...
This file is already bound to **Clinical** (`/clinical`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/clinical/presentation/pages/clinical_workspace_page.dart` and related `clinical` files against Reception + `prompt.md`.
