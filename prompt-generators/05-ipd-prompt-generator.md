...
# Screen Standardization Prompt Generator — IPD
...
| Purpose | Inpatient admissions, transfers, discharge queue, and bed board. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Admission Queue | [confirm from code] | [discover during audit] |
| 2 | Active Patients | [confirm from code] | [discover during audit] |
| 3 | Transfers | [confirm from code] | [discover during audit] |
| 4 | Discharge | [confirm from code] | [discover during audit] |
| 5 | Bed board | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **IPD** (`/ipd`):
...
**Objective encoded into every generated prompt:** IPD must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `IpdWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/ipd` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?section=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for IPD

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Admission Queue`, `Active Patients`, `Transfers`, `Discharge`, `Bed board`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/ipd`]
...
- Save as `prompts/{NN}-standardize-ipd-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `ipd`
  - Example: `prompts/27-standardize-ipd-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `IpdWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep IPD business behavior.
...
1. Audit `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart` and related `ipd` files against Reception + `prompt.md`.
