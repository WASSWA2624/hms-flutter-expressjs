...
# Table Standardization Prompt Generator — IPD

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **IPD** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/05-ipd-prompt.md` (same name as this generator file, without `-generator`).
...
| Primary page file | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart` |
| Feature module | `ipd` |
| Known tabs | Admission queue, Active patients, Transfers, Discharge, Bed board |
| Purpose | Inpatient admissions, transfers, discharge, and bed board. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openIpdDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _IpdBoardPanel | `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart` | `IpdAdmissionSummary` | 7 (audit exact) | [discover during audit] |
| 2 | _IpdBedBoardPanel | `frontend/lib/features/ipd/presentation/widgets/ipd_bed_board_panel.dart` | `IpdBedBoardEntry` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **IPD** (`/ipd`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on IPD must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/ipd` / `IpdWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/ipd/`.
...
- Closest on-screen reference: **_IpdBedBoardPanel (5 columns)**
- `prompt.md`
...
- How `onRowSelected` opens `_openIpdDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/ipd/presentation/`
...
- `onRowSelected` → `_openIpdDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/05-ipd-prompt.md`
- This mirrors this generator file (`prompt-generators/05-ipd-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `IpdWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep IPD business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/ipd/`.
...
This file is already bound to **IPD** (`/ipd`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/ipd/presentation/pages/ipd_workspace_page.dart` and related `ipd` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/05-ipd-prompt.md`.
