...
# Screen Standardization Prompt Generator — Access Admin

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Access Admin** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
|-------|-------|
| Route | `/admin/access` |
| Screen name | Access Admin |
| Page widget | `AccessAdminWorkspacePage` |
| Primary page file | `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart` |
| Feature module | `access_admin` |
| Known tabs | `Overview`, `User directory`, `Roles`, `Permissions`, `Module entitlements`, `Demo accounts`, `Pending registrations` |
| Purpose | Platform access administration for users, roles, and entitlements. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Overview | [confirm from code] | [discover during audit] |
| 2 | User directory | [confirm from code] | [discover during audit] |
| 3 | Roles | [confirm from code] | [discover during audit] |
| 4 | Permissions | [confirm from code] | [discover during audit] |
| 5 | Module entitlements | [confirm from code] | [discover during audit] |
| 6 | Demo accounts | [confirm from code] | [discover during audit] |
| 7 | Pending registrations | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Access Admin** (`/admin/access`):
...
**Objective encoded into every generated prompt:** Access Admin must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/admin/access` / `AccessAdminWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/access_admin/`.
...
Compare `AccessAdminWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/admin/access` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Access Admin

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Overview`, `User directory`, `Roles`, `Permissions`, `Module entitlements`, `Demo accounts`, `Pending registrations`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/admin/access`]
...
- Save as `prompts/{NN}-standardize-access_admin-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `access_admin`
  - Example: `prompts/27-standardize-access_admin-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `AccessAdminWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Access Admin business behavior.
...
This file is already bound to **Access Admin** (`/admin/access`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart` and related `access_admin` files against Reception + `prompt.md`.
