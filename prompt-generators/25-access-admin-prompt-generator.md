...
# Table Standardization Prompt Generator — Access Admin

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Access Admin** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/25-access-admin-prompt.md` (same name as this generator file, without `-generator`).
...
| Known tabs | Users, Roles, Permissions, Registrations |
| Purpose | User access, roles, permissions, and registrations. |
| Has workflow status columns | No (data-only tables) |
| Row detail handler (starting point) | `audit per-tab management dialogs on row select` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _WorklistPanel | `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart` | `AccessAdminItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Access Admin** (`/admin/access`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Access Admin must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- How `onRowSelected` opens `audit per-tab management dialogs on row select`.
...
- Existing detail dialogs in `frontend/lib/features/access_admin/presentation/`
...
- `onRowSelected` → `audit per-tab management dialogs on row select` (or audited replacement)
...
- **Output path (fixed):** `prompts/25-access-admin-prompt.md`
- This mirrors this generator file (`prompt-generators/25-access-admin-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `AccessAdminWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Access Admin business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/access_admin/`.
...
1. Audit `frontend/lib/features/access_admin/presentation/pages/access_admin_workspace_page.dart` and related `access_admin` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/25-access-admin-prompt.md`.
