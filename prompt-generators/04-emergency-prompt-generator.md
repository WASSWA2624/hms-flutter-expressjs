...
# Table Standardization Prompt Generator — Emergency

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Emergency** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/04-emergency-prompt.md` (same name as this generator file, without `-generator`).
...
| Row detail handler (starting point) | `openEmergencyDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | Emergency worklist table in _EmergencyWorkspaceContentState | `frontend/lib/features/emergency/presentation/pages/emergency_workspace_page.dart` | `EmergencyCaseSummary` | 6 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Emergency** (`/emergency`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Emergency must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **emergency_workspace_widgets.dart (emergencyNextActionColumn)**
- `prompt.md`
...
- How `onRowSelected` opens `openEmergencyDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/emergency/presentation/`
...
- `onRowSelected` → `openEmergencyDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/04-emergency-prompt.md`
- This mirrors this generator file (`prompt-generators/04-emergency-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `EmergencyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Emergency business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/emergency/`.
...
1. Audit `frontend/lib/features/emergency/presentation/pages/emergency_workspace_page.dart` and related `emergency` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/04-emergency-prompt.md`.
