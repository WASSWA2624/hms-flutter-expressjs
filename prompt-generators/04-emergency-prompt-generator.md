...
# Screen Standardization Prompt Generator — Emergency

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Emergency** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Primary page file | `frontend/lib/features/emergency/presentation/pages/emergency_workspace_page.dart` |
| Feature module | `emergency` |
| Known tabs | `Active cases`, `Critical`, `Ambulance`, `Handoff ready`, `Closed`, `All` |
| Purpose | Emergency department board by acuity, ambulance, handoff, and closed cases. |

- Deep-link query parameter pattern: `?scope=<value>`
...
|---|---------------------|---------------------|------------------------|
| 1 | Active cases | [confirm from code] | [discover during audit] |
| 2 | Critical | [confirm from code] | [discover during audit] |
| 3 | Ambulance | [confirm from code] | [discover during audit] |
| 4 | Handoff ready | [confirm from code] | [discover during audit] |
| 5 | Closed | [confirm from code] | [discover during audit] |
| 6 | All | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Emergency** (`/emergency`):
...
**Objective encoded into every generated prompt:** Emergency must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/emergency` / `EmergencyWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/emergency/presentation/pages/emergency_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/emergency/`.
...
Compare `EmergencyWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/emergency` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?scope=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Emergency

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Active cases`, `Critical`, `Ambulance`, `Handoff ready`, `Closed`, `All`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/emergency`]
...
- Save as `prompts/{NN}-standardize-emergency-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `emergency`
  - Example: `prompts/27-standardize-emergency-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `EmergencyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Emergency business behavior.
...
This file is already bound to **Emergency** (`/emergency`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/emergency/presentation/pages/emergency_workspace_page.dart` and related `emergency` files against Reception + `prompt.md`.
