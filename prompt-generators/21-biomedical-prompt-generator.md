...
# Screen Standardization Prompt Generator — Biomedical

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Biomedical** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Purpose | Biomedical equipment lifecycle panels and work orders. |
...
| 2 | Registry | [confirm from code] | [discover during audit] |
| 3 | Preventive | [confirm from code] | [discover during audit] |
| 4 | Work orders | [confirm from code] | [discover during audit] |
| 5 | Compliance | [confirm from code] | [discover during audit] |
| 6 | Support | [confirm from code] | [discover during audit] |
| 7 | Analytics | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Biomedical** (`/biomedical`):
...
**Objective encoded into every generated prompt:** Biomedical must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `BiomedicalWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/biomedical` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?panel=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Biomedical

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Overview`, `Registry`, `Preventive`, `Work orders`, `Compliance`, `Support`, `Analytics`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/biomedical`]
...
- Save as `prompts/{NN}-standardize-biomedical-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `biomedical`
  - Example: `prompts/27-standardize-biomedical-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `BiomedicalWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Biomedical business behavior.
...
1. Audit `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart` and related `biomedical` files against Reception + `prompt.md`.
