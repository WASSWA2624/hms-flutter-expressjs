...
# Table Standardization Prompt Generator — Biomedical

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Biomedical** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/21-biomedical-prompt.md` (same name as this generator file, without `-generator`).
...
|-------|-------|
| Route | `/biomedical` |
| Screen name | Biomedical |
| Page widget | `BiomedicalWorkspacePage` |
| Primary page file | `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart` |
| Feature module | `biomedical` |
| Known tabs | Overview, Registry, Preventive, Work orders, Compliance, Support, Analytics |
| Purpose | Biomedical asset registry and maintenance work orders. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openAssetDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | Asset worklist in _BiomedicalWorkspaceContentState | `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart` | `BiomedicalAsset` | 6 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Biomedical** (`/biomedical`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Biomedical must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/biomedical` / `BiomedicalWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/biomedical/`.
...
- Closest on-screen reference: **biomedical_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openAssetDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/biomedical/presentation/`
...
Refactor every `AppListTable` on the Biomedical workspace (`/biomedical`, `BiomedicalWorkspacePage`) so each table fully complies with `prompt.md` (search chrome, ≤5 columns, status/action columns when applicable, row detail dialog, responsiveness, realtime).
...
- `onRowSelected` → `_openAssetDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/21-biomedical-prompt.md`
- This mirrors this generator file (`prompt-generators/21-biomedical-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `BiomedicalWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Biomedical business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/biomedical/`.
...
This file is already bound to **Biomedical** (`/biomedical`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/biomedical/presentation/pages/biomedical_workspace_page.dart` and related `biomedical` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/21-biomedical-prompt.md`.
