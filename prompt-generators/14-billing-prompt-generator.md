...
# Screen Standardization Prompt Generator — Billing

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Billing** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Primary page file | `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart` |
| Feature module | `billing` |
| Known tabs | `All billing work items`, `Needs issue`, `Awaiting payment`, `Claims pending`, `Approval required`, `Overdue` |
| Purpose | Billing work-item queues by issue, payment, claims, and approval. |
...
|---|---------------------|---------------------|------------------------|
| 1 | All billing work items | [confirm from code] | [discover during audit] |
| 2 | Needs issue | [confirm from code] | [discover during audit] |
| 3 | Awaiting payment | [confirm from code] | [discover during audit] |
| 4 | Claims pending | [confirm from code] | [discover during audit] |
| 5 | Approval required | [confirm from code] | [discover during audit] |
| 6 | Overdue | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Billing** (`/billing`):
...
**Objective encoded into every generated prompt:** Billing must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/billing` / `BillingWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/billing/`.
...
Compare `BillingWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/billing` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Billing

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All billing work items`, `Needs issue`, `Awaiting payment`, `Claims pending`, `Approval required`, `Overdue`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/billing`]
...
- Save as `prompts/{NN}-standardize-billing-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `billing`
  - Example: `prompts/27-standardize-billing-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `BillingWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Billing business behavior.
...
This file is already bound to **Billing** (`/billing`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart` and related `billing` files against Reception + `prompt.md`.
