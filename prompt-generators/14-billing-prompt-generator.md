...
# Table Standardization Prompt Generator — Billing

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Billing** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/14-billing-prompt.md` (same name as this generator file, without `-generator`).
...
| Known tabs | All, Needs issue, Pending payment, Claims pending, Approval required, Overdue |
| Purpose | Billing queue, invoices, and payment follow-up. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_showBillingDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _BillingQueuePanel | `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart` | `BillingWorkItem` | 7 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Billing** (`/billing`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Billing must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- How `onRowSelected` opens `_showBillingDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/billing/presentation/`
...
- `onRowSelected` → `_showBillingDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/14-billing-prompt.md`
- This mirrors this generator file (`prompt-generators/14-billing-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `BillingWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Billing business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/billing/`.
...
1. Audit `frontend/lib/features/billing/presentation/pages/billing_workspace_page.dart` and related `billing` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/14-billing-prompt.md`.
