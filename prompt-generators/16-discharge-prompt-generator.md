...
# Screen Standardization Prompt Generator — Discharge
...
| Known tabs | `All patients`, `Planned`, `Pending clearance`, `Completed` |
| Purpose | Discharge planning and clearance queues. |
...
|---|---------------------|---------------------|------------------------|
| 1 | All patients | [confirm from code] | [discover during audit] |
| 2 | Planned | [confirm from code] | [discover during audit] |
| 3 | Pending clearance | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Discharge** (`/discharge`):
...
**Objective encoded into every generated prompt:** Discharge must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `DischargeWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/discharge` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Discharge

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All patients`, `Planned`, `Pending clearance`, `Completed`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/discharge`]
...
- Save as `prompts/{NN}-standardize-discharge-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `discharge`
  - Example: `prompts/27-standardize-discharge-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `DischargeWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Discharge business behavior.
...
1. Audit `frontend/lib/features/discharge/presentation/pages/discharge_workspace_page.dart` and related `discharge` files against Reception + `prompt.md`.
