...
# Table Standardization Prompt Generator — Discharge

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Discharge** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/16-discharge-prompt.md` (same name as this generator file, without `-generator`).
...
|-------|-------|
| Route | `/discharge` |
| Screen name | Discharge |
| Page widget | `DischargeWorkspacePage` |
| Primary page file | `frontend/lib/features/discharge/presentation/pages/discharge_workspace_page.dart` |
| Feature module | `discharge` |
| Known tabs | All, Planned, Pending clearance, Completed |
| Purpose | Discharge planning and clearance workflow. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openDischargeDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | Discharge worklist in _DischargeWorkspaceContentState | `frontend/lib/features/discharge/presentation/pages/discharge_workspace_page.dart` | `IpdAdmissionSummary` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Discharge** (`/discharge`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Discharge must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/discharge` / `DischargeWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/discharge/presentation/pages/discharge_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/discharge/`.
...
- Closest on-screen reference: **discharge_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openDischargeDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/discharge/presentation/`
...
- `onRowSelected` → `_openDischargeDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/16-discharge-prompt.md`
- This mirrors this generator file (`prompt-generators/16-discharge-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `DischargeWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Discharge business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/discharge/`.
...
This file is already bound to **Discharge** (`/discharge`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/discharge/presentation/pages/discharge_workspace_page.dart` and related `discharge` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/16-discharge-prompt.md`.
