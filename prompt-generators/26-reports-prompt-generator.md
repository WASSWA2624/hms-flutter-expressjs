...
# Table Standardization Prompt Generator — Reports

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Reports** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/26-reports-prompt.md` (same name as this generator file, without `-generator`).
...
| Route | `/reports` |
| Screen name | Reports |
| Page widget | `ReportsWorkspacePage` |
| Primary page file | `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart` |
| Feature module | `reports` |
| Known tabs | Overview, Catalog, Delivery, Dashboards, Monitor, Activity, Audit, PHI, Processing |
| Purpose | Report catalog, schedules, compliance logs, and monitoring. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `audit per tab for detail dialog on row tap` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _ReportItemsPanel | `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart` | `ReportsWorkspaceItem` | 5 (audit exact) | [discover during audit] |
| 2 | _ComplianceLogPanel | `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart` | `ComplianceLogItem` | 5 (audit exact) | [discover during audit] |
| 3 | _ReportSchedulesPanel | `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart` | `ReportsWorkspaceItem` | 4 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Reports** (`/reports`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Reports must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/reports` / `ReportsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/reports/`.
...
- How `onRowSelected` opens `audit per tab for detail dialog on row tap`.
...
- Existing detail dialogs in `frontend/lib/features/reports/presentation/`
...
- `onRowSelected` → `audit per tab for detail dialog on row tap` (or audited replacement)
...
- **Output path (fixed):** `prompts/26-reports-prompt.md`
- This mirrors this generator file (`prompt-generators/26-reports-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `ReportsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Reports business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/reports/`.
...
This file is already bound to **Reports** (`/reports`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/reports/presentation/pages/reports_workspace_page.dart` and related `reports` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/26-reports-prompt.md`.
