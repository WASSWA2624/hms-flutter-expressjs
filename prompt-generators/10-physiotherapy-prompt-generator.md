...
# Table Standardization Prompt Generator — Physiotherapy

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Physiotherapy** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/10-physiotherapy-prompt.md` (same name as this generator file, without `-generator`).
...
| Known tabs | All, Scheduled, In progress, Completed |
| Purpose | Therapy session scheduling and progress tracking. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openTherapyDetailDialog` |

- Deep-link query parameter pattern: `?status=<value>`

### Known table inventory (validate and complete during audit)

| # | Table widget | File | Entity | Columns today | Detail on row select |
|---|--------------|------|--------|---------------|----------------------|
| 1 | _PhysiotherapyWorkspace table | `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart` | `TherapyWorkItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Physiotherapy** (`/physiotherapy`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Physiotherapy must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **physiotherapy_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openTherapyDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/physiotherapy/presentation/`
...
- `onRowSelected` → `_openTherapyDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/10-physiotherapy-prompt.md`
- This mirrors this generator file (`prompt-generators/10-physiotherapy-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `PhysiotherapyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Physiotherapy business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/physiotherapy/`.
...
1. Audit `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart` and related `physiotherapy` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/10-physiotherapy-prompt.md`.
