...
# Screen Standardization Prompt Generator — Physiotherapy

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Physiotherapy** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
|-------|-------|
| Route | `/physiotherapy` |
| Screen name | Physiotherapy |
| Page widget | `PhysiotherapyWorkspacePage` |
| Primary page file | `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart` |
| Feature module | `physiotherapy` |
| Known tabs | `Referrals`, `Today`, `Active plans`, `Follow-up due`, `Missed`, `Completed`, `All work` |
| Purpose | Physiotherapy referral and session queues. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Referrals | [confirm from code] | [discover during audit] |
| 2 | Today | [confirm from code] | [discover during audit] |
| 3 | Active plans | [confirm from code] | [discover during audit] |
| 4 | Follow-up due | [confirm from code] | [discover during audit] |
| 5 | Missed | [confirm from code] | [discover during audit] |
| 6 | Completed | [confirm from code] | [discover during audit] |
| 7 | All work | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Physiotherapy** (`/physiotherapy`):
...
**Objective encoded into every generated prompt:** Physiotherapy must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/physiotherapy` / `PhysiotherapyWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/physiotherapy/`.
...
Compare `PhysiotherapyWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/physiotherapy` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Physiotherapy

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Referrals`, `Today`, `Active plans`, `Follow-up due`, `Missed`, `Completed`, `All work`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/physiotherapy`]
...
- Save as `prompts/{NN}-standardize-physiotherapy-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `physiotherapy`
  - Example: `prompts/27-standardize-physiotherapy-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `PhysiotherapyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Physiotherapy business behavior.
...
This file is already bound to **Physiotherapy** (`/physiotherapy`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/physiotherapy/presentation/pages/physiotherapy_workspace_page.dart` and related `physiotherapy` files against Reception + `prompt.md`.
