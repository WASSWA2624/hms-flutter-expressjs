...
# Screen Standardization Prompt Generator — Patients
...
| 2 | Active | [confirm from code] | [discover during audit] |
| 3 | Admitted | [confirm from code] | [discover during audit] |
| 4 | Balance due | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Patients** (`/patients`):
...
**Objective encoded into every generated prompt:** Patients must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `PatientRegistryPage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/patients` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?section=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Patients

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All patients`, `Active`, `Admitted`, `Balance due`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/patients`]
...
- Save as `prompts/{NN}-standardize-patients-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `patients`
  - Example: `prompts/27-standardize-patients-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `PatientRegistryPage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Patients business behavior.
...
1. Audit `frontend/lib/features/patients/presentation/pages/patient_registry_page.dart` and related `patients` files against Reception + `prompt.md`.
