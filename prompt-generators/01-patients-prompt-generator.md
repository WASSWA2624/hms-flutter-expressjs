...
You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Patients** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/01-patients-prompt.md` (same name as this generator file, without `-generator`).
...
- **Output path (fixed):** `prompts/01-patients-prompt.md`
- This mirrors this generator file (`prompt-generators/01-patients-prompt-generator.md`) with `-generator` removed from the filename.
...
3. Save it to `prompts/01-patients-prompt.md`.
