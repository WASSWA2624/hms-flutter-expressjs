...
# Screen Standardization Prompt Generator — Laboratory

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Laboratory** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
|-------|-------|
| Route | `/lab` |
| Screen name | Laboratory |
| Page widget | `LabWorkspacePage` |
| Primary page file | `frontend/lib/features/lab/presentation/pages/lab_workspace_page.dart` |
| Feature module | `lab` |
| Known tabs | `All`, `Awaiting results`, `Processing`, `Pending verification`, `Critical`, `Verified` |
| Purpose | Lab order desk by processing and verification stage. |
...
| 2 | Awaiting results | [confirm from code] | [discover during audit] |
| 3 | Processing | [confirm from code] | [discover during audit] |
| 4 | Pending verification | [confirm from code] | [discover during audit] |
| 5 | Critical | [confirm from code] | [discover during audit] |
| 6 | Verified | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Laboratory** (`/lab`):
...
**Objective encoded into every generated prompt:** Laboratory must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/lab` / `LabWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/lab/presentation/pages/lab_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/lab/`.
...
Compare `LabWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/lab` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Laboratory

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All`, `Awaiting results`, `Processing`, `Pending verification`, `Critical`, `Verified`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/lab`]
...
- Save as `prompts/{NN}-standardize-lab-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `lab`
  - Example: `prompts/27-standardize-lab-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `LabWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Laboratory business behavior.
...
This file is already bound to **Laboratory** (`/lab`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/lab/presentation/pages/lab_workspace_page.dart` and related `lab` files against Reception + `prompt.md`.
