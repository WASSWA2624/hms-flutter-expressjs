...
# Table Standardization Prompt Generator — Laboratory

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Laboratory** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/11-lab-prompt.md` (same name as this generator file, without `-generator`).
...
| Feature module | `lab` |
| Known tabs | Worklist, Collection, Processing, Verification, Critical, Completed |
| Purpose | Lab order worklist across collection through verification. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openLabDetailDialog` |

- Deep-link query parameter pattern: `?stage=<value>`

### Known table inventory (validate and complete during audit)

| # | Table widget | File | Entity | Columns today | Detail on row select |
|---|--------------|------|--------|---------------|----------------------|
| 1 | _LabWorklistPanel | `frontend/lib/features/lab/presentation/pages/lab_workspace_page.dart` | `LabOrderSummary` | 9 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Laboratory** (`/lab`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Laboratory must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- How `onRowSelected` opens `_openLabDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/lab/presentation/`
...
- `onRowSelected` → `_openLabDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/11-lab-prompt.md`
- This mirrors this generator file (`prompt-generators/11-lab-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `LabWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Laboratory business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/lab/`.
...
1. Audit `frontend/lib/features/lab/presentation/pages/lab_workspace_page.dart` and related `lab` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/11-lab-prompt.md`.
