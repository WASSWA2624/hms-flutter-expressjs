...
# Screen Standardization Prompt Generator — Communications
...
| Known tabs | `Messages`, `Notifications`, `Deliveries`, `Templates` |
| Purpose | Messaging, notifications, delivery logs, and templates. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Messages | [confirm from code] | [discover during audit] |
| 2 | Notifications | [confirm from code] | [discover during audit] |
| 3 | Deliveries | [confirm from code] | [discover during audit] |
| 4 | Templates | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Communications** (`/communications`):
...
**Objective encoded into every generated prompt:** Communications must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `CommunicationsWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/communications` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Communications

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Messages`, `Notifications`, `Deliveries`, `Templates`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/communications`]
...
- Save as `prompts/{NN}-standardize-communications-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `communications`
  - Example: `prompts/27-standardize-communications-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `CommunicationsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Communications business behavior.
...
1. Audit `frontend/lib/features/communications/presentation/pages/communications_workspace_page.dart` and related `communications` files against Reception + `prompt.md`.
