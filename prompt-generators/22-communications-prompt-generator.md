...
# Table Standardization Prompt Generator — Communications

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Communications** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/22-communications-prompt.md` (same name as this generator file, without `-generator`).
...
| Feature module | `communications` |
| Known tabs | Notifications, Deliveries, Templates |
| Purpose | Notification inbox, delivery log, and templates. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `audit inline selection vs modal detail on row tap` |

- Deep-link query parameter pattern: `?panel=<value>`

### Known table inventory (validate and complete during audit)

| # | Table widget | File | Entity | Columns today | Detail on row select |
|---|--------------|------|--------|---------------|----------------------|
| 1 | _NotificationsTable | `frontend/lib/features/communications/presentation/pages/communications_workspace_page.dart` | `NotificationItem` | 5 (audit exact) | [discover during audit] |
| 2 | _DeliveriesTable | `frontend/lib/features/communications/presentation/pages/communications_workspace_page.dart` | `NotificationDelivery` | 5 (audit exact) | [discover during audit] |
| 3 | _TemplatesTable | `frontend/lib/features/communications/presentation/pages/communications_workspace_page.dart` | `CommunicationTemplate` | 4 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Communications** (`/communications`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Communications must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/communications` / `CommunicationsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/communications/presentation/pages/communications_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/communications/`.
...
- How `onRowSelected` opens `audit inline selection vs modal detail on row tap`.
...
- Existing detail dialogs in `frontend/lib/features/communications/presentation/`
...
- `onRowSelected` → `audit inline selection vs modal detail on row tap` (or audited replacement)
...
- **Output path (fixed):** `prompts/22-communications-prompt.md`
- This mirrors this generator file (`prompt-generators/22-communications-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `CommunicationsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Communications business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/communications/`.
...
This file is already bound to **Communications** (`/communications`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/communications/presentation/pages/communications_workspace_page.dart` and related `communications` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/22-communications-prompt.md`.
