...
# Screen Standardization Prompt Generator — Theater

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Theater** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Known tabs | `Scheduled`, `In theater`, `Recovery`, `All cases` |
| Purpose | Operating theater case board by schedule and recovery stage. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Scheduled | [confirm from code] | [discover during audit] |
| 2 | In theater | [confirm from code] | [discover during audit] |
| 3 | Recovery | [confirm from code] | [discover during audit] |
| 4 | All cases | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Theater** (`/theater`):
...
**Objective encoded into every generated prompt:** Theater must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `TheaterWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/theater` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Theater

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Scheduled`, `In theater`, `Recovery`, `All cases`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/theater`]
...
- Save as `prompts/{NN}-standardize-theater-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `theater`
  - Example: `prompts/27-standardize-theater-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `TheaterWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Theater business behavior.
...
1. Audit `frontend/lib/features/theater/presentation/pages/theater_workspace_page.dart` and related `theater` files against Reception + `prompt.md`.
