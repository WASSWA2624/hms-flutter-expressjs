...
# Table Standardization Prompt Generator — Theater

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Theater** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/17-theater-prompt.md` (same name as this generator file, without `-generator`).
...
| Primary page file | `frontend/lib/features/theater/presentation/pages/theater_workspace_page.dart` |
| Feature module | `theater` |
| Known tabs | Scheduled, In theater, Recovery, All |
| Purpose | Operating theater case scheduling and progression. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openTheaterCaseDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _TheaterCaseBoard | `frontend/lib/features/theater/presentation/pages/theater_workspace_page.dart` | `TheaterCase` | 9 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Theater** (`/theater`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Theater must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/theater` / `TheaterWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/theater/presentation/pages/theater_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/theater/`.
...
- Closest on-screen reference: **emergency (WorkflowActionButton)**
- `prompt.md`
...
- How `onRowSelected` opens `_openTheaterCaseDialog`.
...
- Existing detail dialogs in `frontend/lib/features/theater/presentation/`
...
- `onRowSelected` → `_openTheaterCaseDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/17-theater-prompt.md`
- This mirrors this generator file (`prompt-generators/17-theater-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `TheaterWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Theater business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/theater/`.
...
This file is already bound to **Theater** (`/theater`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/theater/presentation/pages/theater_workspace_page.dart` and related `theater` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/17-theater-prompt.md`.
