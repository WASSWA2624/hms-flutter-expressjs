...
# Screen Standardization Prompt Generator — Settings

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Settings** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Page widget | `SettingsPage` |
| Primary page file | `frontend/lib/features/settings/presentation/pages/settings_page.dart` |
| Feature module | `settings` |
| Known tabs | `Preferences`, `Accessibility`, `Account and security`, `Administration boundaries`, `Configuration`, `Administrative setup workspace` |
| Purpose | Application settings sections as top-level tabs (some permission-gated). |

- Deep-link query parameter pattern: `?tab=<value>`

- Related navigation / section files:
...
|---|---------------------|---------------------|------------------------|
| 1 | Preferences | [confirm from code] | [discover during audit] |
| 2 | Accessibility | [confirm from code] | [discover during audit] |
| 3 | Account and security | [confirm from code] | [discover during audit] |
| 4 | Administration boundaries | [confirm from code] | [discover during audit] |
| 5 | Configuration | [confirm from code] | [discover during audit] |
| 6 | Administrative setup workspace | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Settings** (`/settings`):
...
**Objective encoded into every generated prompt:** Settings must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/settings` / `SettingsPage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/settings/presentation/pages/settings_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/settings/`.
...
Compare `SettingsPage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/settings` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?tab=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Settings

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Preferences`, `Accessibility`, `Account and security`, `Administration boundaries`, `Configuration`, `Administrative setup workspace`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/settings`]
...
- Save as `prompts/{NN}-standardize-settings-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `settings`
  - Example: `prompts/27-standardize-settings-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `SettingsPage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Settings business behavior.
...
This file is already bound to **Settings** (`/settings`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/settings/presentation/pages/settings_page.dart` and related `settings` files against Reception + `prompt.md`.
