...
# Table Standardization Prompt Generator — Subscriptions

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Subscriptions** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/24-subscriptions-prompt.md` (same name as this generator file, without `-generator`).
...
| Page widget | `SubscriptionsWorkspacePage` |
| Primary page file | `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart` |
| Feature module | `subscriptions` |
| Known tabs | Overview, Tenants, Plans, Subscriptions, Modules, Notifications |
| Purpose | SaaS tenant subscriptions and plan administration. |
| Has workflow status columns | No (data-only tables) |
| Row detail handler (starting point) | `_openSubscriptionDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _SubscriptionsWorklistPanel | `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart` | `SubscriptionItem` | 4 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Subscriptions** (`/subscriptions`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Subscriptions must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/subscriptions` / `SubscriptionsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/subscriptions/`.
...
- How `onRowSelected` opens `_openSubscriptionDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/subscriptions/presentation/`
...
- `onRowSelected` → `_openSubscriptionDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/24-subscriptions-prompt.md`
- This mirrors this generator file (`prompt-generators/24-subscriptions-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `SubscriptionsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Subscriptions business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/subscriptions/`.
...
This file is already bound to **Subscriptions** (`/subscriptions`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart` and related `subscriptions` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/24-subscriptions-prompt.md`.
