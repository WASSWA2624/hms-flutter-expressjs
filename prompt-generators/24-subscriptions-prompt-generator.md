...
# Screen Standardization Prompt Generator — Subscriptions

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Subscriptions** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Known tabs | `Overview`, `Plans`, `Subscriptions`, `Invoices`, `Licenses`, `Notifications` |
| Purpose | Tenant subscription admin: plans, invoices, licenses, and notices. |
...
| 2 | Plans | [confirm from code] | [discover during audit] |
| 3 | Subscriptions | [confirm from code] | [discover during audit] |
| 4 | Invoices | [confirm from code] | [discover during audit] |
| 5 | Licenses | [confirm from code] | [discover during audit] |
| 6 | Notifications | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Subscriptions** (`/subscriptions`):
...
**Objective encoded into every generated prompt:** Subscriptions must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `SubscriptionsWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/subscriptions` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Subscriptions

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Overview`, `Plans`, `Subscriptions`, `Invoices`, `Licenses`, `Notifications`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/subscriptions`]
...
- Save as `prompts/{NN}-standardize-subscriptions-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `subscriptions`
  - Example: `prompts/27-standardize-subscriptions-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `SubscriptionsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Subscriptions business behavior.
...
1. Audit `frontend/lib/features/subscriptions/presentation/pages/subscriptions_workspace_page.dart` and related `subscriptions` files against Reception + `prompt.md`.
