...
# Screen Standardization Prompt Generator — Nursing

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Nursing** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Primary page file | `frontend/lib/features/nursing/presentation/pages/nursing_workspace_page.dart` |
| Feature module | `nursing` |
| Known tabs | `All`, `Assigned ward`, `Urgent`, `Medication due`, `Handover pending`, `Transfer pending`, `Discharge pending` |
| Purpose | Nursing care queues by ward assignment, urgency, meds, and handovers. |

- Deep-link query: confirm whether tab state is URL-backed; add if missing.

- Related navigation / section files:
  - `frontend/lib/features/nursing/presentation/nursing_scope_navigation.dart`
...
|---|---------------------|---------------------|------------------------|
| 1 | All | [confirm from code] | [discover during audit] |
| 2 | Assigned ward | [confirm from code] | [discover during audit] |
| 3 | Urgent | [confirm from code] | [discover during audit] |
| 4 | Medication due | [confirm from code] | [discover during audit] |
| 5 | Handover pending | [confirm from code] | [discover during audit] |
| 6 | Transfer pending | [confirm from code] | [discover during audit] |
| 7 | Discharge pending | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Nursing** (`/nursing`):
...
**Objective encoded into every generated prompt:** Nursing must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/nursing` / `NursingWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/nursing/presentation/pages/nursing_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/nursing/`.
...
Compare `NursingWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/nursing` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Nursing

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All`, `Assigned ward`, `Urgent`, `Medication due`, `Handover pending`, `Transfer pending`, `Discharge pending`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/nursing`]
...
- Save as `prompts/{NN}-standardize-nursing-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `nursing`
  - Example: `prompts/27-standardize-nursing-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `NursingWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Nursing business behavior.
...
This file is already bound to **Nursing** (`/nursing`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/nursing/presentation/pages/nursing_workspace_page.dart` and related `nursing` files against Reception + `prompt.md`.
