...
# Table Standardization Prompt Generator — Nursing

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Nursing** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/08-nursing-prompt.md` (same name as this generator file, without `-generator`).
...
| Row detail handler (starting point) | `openNursingPatientDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | NursingWorklistPanel | `frontend/lib/features/nursing/presentation/widgets/nursing_worklist_panel.dart` | `NursingWorkItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Nursing** (`/nursing`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Nursing must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **nursing_worklist_panel.dart**
- `prompt.md`
...
- How `onRowSelected` opens `openNursingPatientDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/nursing/presentation/`
...
- `onRowSelected` → `openNursingPatientDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/08-nursing-prompt.md`
- This mirrors this generator file (`prompt-generators/08-nursing-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `NursingWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Nursing business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/nursing/`.
...
1. Audit `frontend/lib/features/nursing/presentation/pages/nursing_workspace_page.dart` and related `nursing` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/08-nursing-prompt.md`.
