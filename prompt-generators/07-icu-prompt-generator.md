...
# Table Standardization Prompt Generator — ICU

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **ICU** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/07-icu-prompt.md` (same name as this generator file, without `-generator`).
...
| Feature module | `icu` |
| Known tabs | Active, Critical, Transfer, Discharge, Ended, All |
| Purpose | ICU patient board and critical-care workflow. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openIcuDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _IcuBoardPanel | `frontend/lib/features/icu/presentation/pages/icu_workspace_page.dart` | `IcuPatientSummary` | 7 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **ICU** (`/icu`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on ICU must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- How `onRowSelected` opens `_openIcuDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/icu/presentation/`
...
- `onRowSelected` → `_openIcuDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/07-icu-prompt.md`
- This mirrors this generator file (`prompt-generators/07-icu-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `IcuWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep ICU business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/icu/`.
...
1. Audit `frontend/lib/features/icu/presentation/pages/icu_workspace_page.dart` and related `icu` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/07-icu-prompt.md`.
