...
# Screen Standardization Prompt Generator — Radiology
...
| Purpose | Imaging worklist, reporting, and released orders. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Worklist | [confirm from code] | [discover during audit] |
| 2 | Reporting | [confirm from code] | [discover during audit] |
| 3 | Released | [confirm from code] | [discover during audit] |
| 4 | All orders | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Radiology** (`/radiology`):
...
**Objective encoded into every generated prompt:** Radiology must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `RadiologyWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/radiology` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Radiology

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Worklist`, `Reporting`, `Released`, `All orders`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/radiology`]
...
- Save as `prompts/{NN}-standardize-radiology-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `radiology`
  - Example: `prompts/27-standardize-radiology-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `RadiologyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Radiology business behavior.
...
1. Audit `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart` and related `radiology` files against Reception + `prompt.md`.
