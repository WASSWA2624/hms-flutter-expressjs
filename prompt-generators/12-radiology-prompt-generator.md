...
# Table Standardization Prompt Generator — Radiology

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Radiology** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/12-radiology-prompt.md` (same name as this generator file, without `-generator`).
...
|-------|-------|
| Route | `/radiology` |
| Screen name | Radiology |
| Page widget | `RadiologyWorkspacePage` |
| Primary page file | `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart` |
| Feature module | `radiology` |
| Known tabs | Worklist, Reporting, Released, All orders |
| Purpose | Radiology order worklist and reporting queue. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openRadiologyDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _RadiologyWorklistPanel | `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart` | `RadiologyOrder` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Radiology** (`/radiology`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Radiology must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/radiology` / `RadiologyWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/radiology/`.
...
- Closest on-screen reference: **radiology_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openRadiologyDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/radiology/presentation/`
...
- `onRowSelected` → `_openRadiologyDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/12-radiology-prompt.md`
- This mirrors this generator file (`prompt-generators/12-radiology-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `RadiologyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Radiology business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/radiology/`.
...
This file is already bound to **Radiology** (`/radiology`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/radiology/presentation/pages/radiology_workspace_page.dart` and related `radiology` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/12-radiology-prompt.md`.
