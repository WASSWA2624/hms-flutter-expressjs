...
# Screen Standardization Prompt Generator — Operations
...
| Purpose | Facilities operations requests and asset management. |
...
|---|---------------------|---------------------|------------------------|
| 1 | All requests | [confirm from code] | [discover during audit] |
| 2 | Open | [confirm from code] | [discover during audit] |
| 3 | In progress | [confirm from code] | [discover during audit] |
| 4 | Completed | [confirm from code] | [discover during audit] |
| 5 | Assets | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Operations** (`/operations`):
...
**Objective encoded into every generated prompt:** Operations must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `OperationsWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/operations` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Operations

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All requests`, `Open`, `In progress`, `Completed`, `Assets`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/operations`]
...
- Save as `prompts/{NN}-standardize-operations-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `operations`
  - Example: `prompts/27-standardize-operations-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `OperationsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Operations business behavior.
...
1. Audit `frontend/lib/features/operations/presentation/pages/operations_workspace_page.dart` and related `operations` files against Reception + `prompt.md`.
