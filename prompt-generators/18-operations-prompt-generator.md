...
# Table Standardization Prompt Generator — Operations

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Operations** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/18-operations-prompt.md` (same name as this generator file, without `-generator`).
...
| Page widget | `OperationsWorkspacePage` |
| Primary page file | `frontend/lib/features/operations/presentation/pages/operations_workspace_page.dart` |
| Feature module | `operations` |
| Known tabs | All requests, Open, In progress, Completed, Assets |
| Purpose | Facilities operations requests and asset registry. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openRequestDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _OperationsQueuePanel | `frontend/lib/features/operations/presentation/pages/operations_workspace_page.dart` | `OperationsWorkItem` | 5 (audit exact) | [discover during audit] |
| 2 | _OperationsAssetsPanel | `frontend/lib/features/operations/presentation/pages/operations_workspace_page.dart` | `OperationsAsset` | 4 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Operations** (`/operations`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Operations must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/operations` / `OperationsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/operations/presentation/pages/operations_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/operations/`.
...
- Closest on-screen reference: **operations_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openRequestDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/operations/presentation/`
...
- `onRowSelected` → `_openRequestDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/18-operations-prompt.md`
- This mirrors this generator file (`prompt-generators/18-operations-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `OperationsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Operations business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/operations/`.
...
This file is already bound to **Operations** (`/operations`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/operations/presentation/pages/operations_workspace_page.dart` and related `operations` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/18-operations-prompt.md`.
