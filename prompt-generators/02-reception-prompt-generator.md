...
# Screen Standardization Prompt Generator — Reception

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Reception** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Known tabs | `Appointments`, `Desk queue`, `Active visits`, `Payment gate` |
...
> **Note:** Reception is the canonical reference layout. The generated prompt should still audit Reception against `prompt.md` and close any remaining gaps (header removal, contextual toolbar, Filters/Settings-only in tables), not treat it as already perfect.
...
|---|---------------------|---------------------|------------------------|
| 1 | Appointments | [confirm from code] | [discover during audit] |
| 2 | Desk queue | [confirm from code] | [discover during audit] |
| 3 | Active visits | [confirm from code] | [discover during audit] |
| 4 | Payment gate | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Reception** (`/reception`):
...
**Objective encoded into every generated prompt:** Reception must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `ReceptionWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/reception` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Reception

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Appointments`, `Desk queue`, `Active visits`, `Payment gate`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/reception`]
...
- Save as `prompts/{NN}-standardize-reception-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `reception`
  - Example: `prompts/27-standardize-reception-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `ReceptionWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Reception business behavior.
...
1. Audit `frontend/lib/features/reception/presentation/pages/reception_workspace_page.dart` and related `reception` files against Reception + `prompt.md`.
