...
# Table Standardization Prompt Generator — Claims

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Claims** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/15-claims-prompt.md` (same name as this generator file, without `-generator`).
...
| Primary page file | `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart` |
| Feature module | `claims` |
| Known tabs | Authorizations, Active claims, Settled, Insurance setup |
| Purpose | Insurance authorizations, claims, and payer setup. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openClaimsDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _ClaimsQueuePanel | `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart` | `ClaimsQueueItem` | 6 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Claims** (`/claims`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Claims must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/claims` / `ClaimsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/claims/`.
...
- How `onRowSelected` opens `_openClaimsDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/claims/presentation/`
...
- `onRowSelected` → `_openClaimsDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/15-claims-prompt.md`
- This mirrors this generator file (`prompt-generators/15-claims-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `ClaimsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Claims business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/claims/`.
...
This file is already bound to **Claims** (`/claims`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart` and related `claims` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/15-claims-prompt.md`.
