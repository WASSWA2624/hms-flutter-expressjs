...
# Screen Standardization Prompt Generator — Claims

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Claims** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Purpose | Insurance authorizations, active claims, settled claims, and setup. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Authorizations | [confirm from code] | [discover during audit] |
| 2 | Active Claims | [confirm from code] | [discover during audit] |
| 3 | Settled | [confirm from code] | [discover during audit] |
| 4 | Insurance Setup | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Claims** (`/claims`):
...
**Objective encoded into every generated prompt:** Claims must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `ClaimsWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/claims` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Claims

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Authorizations`, `Active Claims`, `Settled`, `Insurance Setup`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/claims`]
...
- Save as `prompts/{NN}-standardize-claims-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `claims`
  - Example: `prompts/27-standardize-claims-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `ClaimsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Claims business behavior.
...
1. Audit `frontend/lib/features/claims/presentation/pages/claims_workspace_page.dart` and related `claims` files against Reception + `prompt.md`.
