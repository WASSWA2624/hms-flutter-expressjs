...
# Screen Standardization Prompt Generator — Pharmacy
...
| Known tabs | `Ready`, `Partial`, `Pending payment`, `Completed`, `All orders` |
| Purpose | Pharmacy dispense queue by fulfillment and payment status. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Ready | [confirm from code] | [discover during audit] |
| 2 | Partial | [confirm from code] | [discover during audit] |
| 3 | Pending payment | [confirm from code] | [discover during audit] |
| 4 | Completed | [confirm from code] | [discover during audit] |
| 5 | All orders | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Pharmacy** (`/pharmacy`):
...
**Objective encoded into every generated prompt:** Pharmacy must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `PharmacyWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/pharmacy` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Pharmacy

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Ready`, `Partial`, `Pending payment`, `Completed`, `All orders`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/pharmacy`]
...
- Save as `prompts/{NN}-standardize-pharmacy-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `pharmacy`
  - Example: `prompts/27-standardize-pharmacy-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `PharmacyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Pharmacy business behavior.
...
1. Audit `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` and related `pharmacy` files against Reception + `prompt.md`.
