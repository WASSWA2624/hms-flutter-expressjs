...
# Table Standardization Prompt Generator — Pharmacy

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Pharmacy** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/13-pharmacy-prompt.md` (same name as this generator file, without `-generator`).
...
|-------|-------|
| Route | `/pharmacy` |
| Screen name | Pharmacy |
| Page widget | `PharmacyWorkspacePage` |
| Primary page file | `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` |
| Feature module | `pharmacy` |
| Known tabs | Ready queue, In progress, Pending payment, Completed, All orders |
| Purpose | Pharmacy dispensing queue, stock, and returns. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openPharmacyDetailDialog` |

- Deep-link query parameter pattern: `?queue=<value>`

### Known table inventory (validate and complete during audit)

| # | Table widget | File | Entity | Columns today | Detail on row select |
|---|--------------|------|--------|---------------|----------------------|
| 1 | _PharmacyQueuePanel | `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` | `PharmacyOrder` | 7 (audit exact) | [discover during audit] |
| 2 | _MedicationItemsPanel | `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` | `PharmacyOrderItem` | 4 (audit exact) | [discover during audit] |
| 3 | _DrugStockPanel | `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` | `PharmacyDrug` | 5 (audit exact) | [discover during audit] |
| 4 | _ReturnMedicationsTable | `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` | `_LineEditState` | 4 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Pharmacy** (`/pharmacy`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Pharmacy must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/pharmacy` / `PharmacyWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/pharmacy/`.
...
- Closest on-screen reference: **mortuary (_MortuaryWorklist)**
- `prompt.md`
...
- How `onRowSelected` opens `_openPharmacyDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/pharmacy/presentation/`
...
- `onRowSelected` → `_openPharmacyDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/13-pharmacy-prompt.md`
- This mirrors this generator file (`prompt-generators/13-pharmacy-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `PharmacyWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Pharmacy business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/pharmacy/`.
...
This file is already bound to **Pharmacy** (`/pharmacy`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/pharmacy/presentation/pages/pharmacy_workspace_page.dart` and related `pharmacy` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/13-pharmacy-prompt.md`.
