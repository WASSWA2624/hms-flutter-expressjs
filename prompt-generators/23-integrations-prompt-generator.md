...
# Table Standardization Prompt Generator — Integrations

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Integrations** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/23-integrations-prompt.md` (same name as this generator file, without `-generator`).
...
| Primary page file | `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart` |
| Feature module | `integrations` |
| Known tabs | Integrations, API keys, Webhooks, Logs, Interop |
| Purpose | Integration connectors, keys, webhooks, and logs. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openIntegrationDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _IntegrationWorklistPanel | `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart` | `IntegrationWorkItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Integrations** (`/integrations`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Integrations must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/integrations` / `IntegrationsWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/integrations/`.
...
- How `onRowSelected` opens `_openIntegrationDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/integrations/presentation/`
...
- `onRowSelected` → `_openIntegrationDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/23-integrations-prompt.md`
- This mirrors this generator file (`prompt-generators/23-integrations-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `IntegrationsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Integrations business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/integrations/`.
...
This file is already bound to **Integrations** (`/integrations`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart` and related `integrations` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/23-integrations-prompt.md`.
