...
# Screen Standardization Prompt Generator — Integrations

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **Integrations** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Purpose | External integrations, API keys, webhooks, logs, and interop. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Integrations | [confirm from code] | [discover during audit] |
| 2 | API keys | [confirm from code] | [discover during audit] |
| 3 | Webhooks | [confirm from code] | [discover during audit] |
| 4 | Logs | [confirm from code] | [discover during audit] |
| 5 | Interop | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Integrations** (`/integrations`):
...
**Objective encoded into every generated prompt:** Integrations must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.
...
Compare `IntegrationsWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/integrations` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring.
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Integrations

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Integrations`, `API keys`, `Webhooks`, `Logs`, `Interop`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/integrations`]
...
- Save as `prompts/{NN}-standardize-integrations-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `integrations`
  - Example: `prompts/27-standardize-integrations-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `IntegrationsWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Integrations business behavior.
...
1. Audit `frontend/lib/features/integrations/presentation/pages/integrations_workspace_page.dart` and related `integrations` files against Reception + `prompt.md`.
