...
# Table Standardization Prompt Generator — HR

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **HR** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/20-hr-prompt.md` (same name as this generator file, without `-generator`).
...
| Feature module | `hr` |
| Known tabs | Staff directory, Work queue, Rosters, Leave, Compensation, Activity |
| Purpose | HR staff directory, work queue, and workforce admin. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `showHrStaffDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _HrStaffDirectory | `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart` | `HrStaffProfile` | 5 (audit exact) | [discover during audit] |
| 2 | _HrWorkQueueTable | `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart` | `HrWorkItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **HR** (`/hr`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on HR must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **_HrStaffDirectory**
- `prompt.md`
...
- How `onRowSelected` opens `showHrStaffDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/hr/presentation/`
...
- `onRowSelected` → `showHrStaffDetailDialog` (or audited replacement)
...
- **Output path (fixed):** `prompts/20-hr-prompt.md`
- This mirrors this generator file (`prompt-generators/20-hr-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `HrWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep HR business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/hr/`.
...
1. Audit `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart` and related `hr` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/20-hr-prompt.md`.
