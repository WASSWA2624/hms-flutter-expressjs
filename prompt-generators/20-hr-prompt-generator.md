...
# Screen Standardization Prompt Generator — HR

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **HR** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Primary page file | `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart` |
| Feature module | `hr` |
| Known tabs | `Human resources`, `Leave requests`, `Shifts`, `Payroll drafts`, `Manage users and roles` |
| Purpose | Staff directory, leave, roster, payroll drafts, and access management. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Human resources | [confirm from code] | [discover during audit] |
| 2 | Leave requests | [confirm from code] | [discover during audit] |
| 3 | Shifts | [confirm from code] | [discover during audit] |
| 4 | Payroll drafts | [confirm from code] | [discover during audit] |
| 5 | Manage users and roles | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **HR** (`/hr`):
...
**Objective encoded into every generated prompt:** HR must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/hr` / `HrWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/hr/`.
...
Compare `HrWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/hr` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?section=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for HR

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Human resources`, `Leave requests`, `Shifts`, `Payroll drafts`, `Manage users and roles`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/hr`]
...
- Save as `prompts/{NN}-standardize-hr-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `hr`
  - Example: `prompts/27-standardize-hr-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `HrWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep HR business behavior.
...
This file is already bound to **HR** (`/hr`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/hr/presentation/pages/hr_workspace_page.dart` and related `hr` files against Reception + `prompt.md`.
