...
# Table Standardization Prompt Generator — OPD

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **OPD** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to `prompts/03-opd-prompt.md` (same name as this generator file, without `-generator`).
...
| Feature module | `opd` |
| Known tabs | All, Arrivals, Queue, Triage, Active |
| Purpose | Outpatient department encounter worklist and triage queue. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openTableItemActions` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _OpdMainTable | `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart` | `_OpdTableItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **OPD** (`/opd`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on OPD must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **emergency (next-action column)**
- `prompt.md`
...
- How `onRowSelected` opens `_openTableItemActions`.
...
- Existing detail dialogs in `frontend/lib/features/opd/presentation/`
...
- `onRowSelected` → `_openTableItemActions` (or audited replacement)
...
- **Output path (fixed):** `prompts/03-opd-prompt.md`
- This mirrors this generator file (`prompt-generators/03-opd-prompt-generator.md`) with `-generator` removed from the filename.
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `OpdWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep OPD business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/opd/`.
...
1. Audit `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart` and related `opd` files against Mortuary/Emergency references + `prompt.md`.
2. Produce a self-contained, agent-executable table-standardization prompt.
3. Save it to `prompts/03-opd-prompt.md`.
