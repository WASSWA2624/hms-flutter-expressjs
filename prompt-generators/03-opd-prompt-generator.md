...
# Screen Standardization Prompt Generator — OPD

You are a coding AI agent acting as a **prompt generator**. Your job is to audit the codebase for the **OPD** screen, then produce a comprehensive, context-aware refactoring prompt that another coding AI agent can execute autonomously — and save that prompt to the `prompts/` folder.
...
| Primary page file | `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart` |
| Feature module | `opd` |
| Known tabs | `All worklist`, `Arrivals`, `Queue`, `Triage`, `Active` |
| Purpose | Outpatient department worklist by arrival, queue, triage, and active stages. |
...
|---|---------------------|---------------------|------------------------|
| 1 | All worklist | [confirm from code] | [discover during audit] |
| 2 | Arrivals | [confirm from code] | [discover during audit] |
| 3 | Queue | [confirm from code] | [discover during audit] |
| 4 | Triage | [confirm from code] | [discover during audit] |
| 5 | Active | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **OPD** (`/opd`):
...
**Objective encoded into every generated prompt:** OPD must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/opd` / `OpdWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/opd/`.
...
Compare `OpdWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/opd` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?section=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for OPD

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`All worklist`, `Arrivals`, `Queue`, `Triage`, `Active`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/opd`]
...
- Save as `prompts/{NN}-standardize-opd-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `opd`
  - Example: `prompts/27-standardize-opd-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `OpdWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep OPD business behavior.
...
This file is already bound to **OPD** (`/opd`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/opd/presentation/pages/opd_workspace_page.dart` and related `opd` files against Reception + `prompt.md`.
