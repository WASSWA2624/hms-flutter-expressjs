...
# Screen Standardization Prompt Generator — Housekeeping
...
|-------|-------|
| Route | `/housekeeping` |
| Screen name | Housekeeping |
| Page widget | `HousekeepingWorkspacePage` |
| Primary page file | `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart` |
| Feature module | `housekeeping` |
| Known tabs | `Tasks`, `Schedules`, `Maintenance requests` |
| Purpose | Housekeeping tasks, schedules, and maintenance requests. |
...
|---|---------------------|---------------------|------------------------|
| 1 | Tasks | [confirm from code] | [discover during audit] |
| 2 | Schedules | [confirm from code] | [discover during audit] |
| 3 | Maintenance requests | [confirm from code] | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Housekeeping** (`/housekeeping`):
...
**Objective encoded into every generated prompt:** Housekeeping must present a consistent, clean layout. Tabs and toolbars behave uniformly with clearly contextual actions.

---

## Step 1: Codebase Discovery (mandatory — do this first)

Perform a full audit focused on `/housekeeping` / `HousekeepingWorkspacePage`.

### 1.1 Locate the target screen

- Start at `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart` and walk all imports: widgets, controllers, providers, repositories, models, routes, dialogs, and tests under `frontend/lib/features/housekeeping/`.
...
Compare `HousekeepingWorkspacePage` against the rules. Produce a concrete gap list, for example:
...
- Confirm `/housekeeping` in `frontend/lib/app/router/app_routes.dart` and `app_router.dart`.
- Document current tab query wiring (`?section=`).
- Specify exact changes needed for deep-linkable tabs.

### 1.6 Domain-specific requirements for Housekeeping

- Entities, providers, and API surfaces this screen uses.
- Validate the known tabs (`Tasks`, `Schedules`, `Maintenance requests`) against enums / section models in code; correct labels if l10n differs.
...
[Exact router / query-model changes for `/housekeeping`]
...
- Save as `prompts/{NN}-standardize-housekeeping-tabs-toolbar.md` using the next number.
  - Feature slug for this screen: `housekeeping`
  - Example: `prompts/27-standardize-housekeeping-tabs-toolbar.md`
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, and per-tab actions discovered in the audit of `HousekeepingWorkspacePage`.
...
3. **Preserve domain logic.** Restructure chrome/layout only; keep Housekeeping business behavior.
...
This file is already bound to **Housekeeping** (`/housekeeping`). Run it as a prompt. The generator agent will:

1. Audit `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart` and related `housekeeping` files against Reception + `prompt.md`.
