...
| Known tabs | Tasks, Schedules, Maintenance |
| Purpose | Housekeeping tasks, schedules, and maintenance. |
| Has workflow status columns | Yes |
| Row detail handler (starting point) | `_openTaskDetailDialog` |
...
|---|--------------|------|--------|---------------|----------------------|
| 1 | _HousekeepingWorklistPanel | `frontend/lib/features/housekeeping/presentation/pages/housekeeping_workspace_page.dart` | `HousekeepingWorkItem` | 5 (audit exact) | [discover during audit] |
...
Before generating anything, read `prompt.md` at the repository root. The generated prompt **must enforce all of the following** for **Housekeeping** (`/housekeeping`) — **for every `AppListTable` on this screen**:
...
**Objective:** Every worklist table on Housekeeping must match the same chrome, column budget, interaction model, and freshness behavior defined in `prompt.md`.
...
- Closest on-screen reference: **housekeeping_workspace_page.dart**
- `prompt.md`
...
- How `onRowSelected` opens `_openTaskDetailDialog`.
...
- Existing detail dialogs in `frontend/lib/features/housekeeping/presentation/`
...
- `onRowSelected` → `_openTaskDetailDialog` (or audited replacement)
...
1. **Never generate a generic prompt.** Every section must cite real paths, classes, providers, column ids, and per-tab table bindings discovered in the audit of `HousekeepingWorkspacePage`.
...
3. **Preserve domain logic.** Restructure table chrome/columns/row interactions only; keep Housekeeping business behavior.
...
8. **Include verification.** Concrete shell commands and tests under `test/features/housekeeping/`.
