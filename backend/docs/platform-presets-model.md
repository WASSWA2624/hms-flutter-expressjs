...
| **Adoption** | The facility offering table (`facility_lab_test_offering`, `facility_lab_panel_offering`, `facility_radiology_procedure_offering`, `facility_pharmacy_offering`) | Always the tenant, keyed `@@unique([facility_id, <definition>_id])` |
...
`name` and `code` are never overridable — they are the identity a shared preset
is recognised by, and a local rename would make the catalog unsafe to share.
...
| Unit tests (38) | ✅ `src/tests/lib/catalog/preset-ownership.test.js` |
...
| Item | Requirement |
| :--- | :--- |
| Enforcement: `drug` / pharmacy, `clinical_term_catalog` | R4 |
| Adoption API — browse, adopt, customise, facility-specific entries | R3 |
| New tables: `facility_clinical_term_offering` | R1, R2 |
