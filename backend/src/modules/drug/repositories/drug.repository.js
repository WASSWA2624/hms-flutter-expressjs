 ...
 * Drug repository
 *
 * @module modules/drug/repositories
 * @description Data access layer for drug operations.
...
/**
 * Find many drugs with pagination
 *
...
/**
 * Count drugs with filters
 *
 * @param {Object} filters - Filter criteria
 * @returns {Promise<number>} Count of drugs
 */
const count = async (filters = {}) => {
  try {
    ...
    return await prisma.drug.count({ where });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};
 ...
 *
 * @param {string} id - Drug ID
 * @returns {Promise<Object>} Deleted drug
 */
