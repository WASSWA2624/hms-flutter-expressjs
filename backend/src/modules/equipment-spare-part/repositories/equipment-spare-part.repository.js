...
const count = async (filters = {}) => {
  try {
    return await prisma.equipment_spare_part.count({ where: { deleted_at: null, ...filters } });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};
...
const create = async (data) => {
  try {
    return await prisma.equipment_spare_part.create({ data });
  } catch (error) {
...
const update = async (id, data) => {
  try {
    return await prisma.equipment_spare_part.update({ where: { id }, data });
  } catch (error) {
    if (error.code === 'P2025') throw new HttpError('errors.equipment_spare_part.not_found', 404);
    if (error.code === 'P2002') throw new HttpError('errors.database.unique_field', 409);
    if (error.code === 'P2003') throw new HttpError('errors.database.foreign_key_field', 400);
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
...
const softDelete = async (id) => {
  try {
    return await prisma.equipment_spare_part.update({ where: { id }, data: { deleted_at: new Date() } });
  } catch (error) {
    if (error.code === 'P2025') throw new HttpError('errors.equipment_spare_part.not_found', 404);
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
