  ...
  try {
    const metricsEntries = await Promise.all([
      countAndLatest({
        ...
        where: {
          deleted_at: null,
          ...(scope.tenant_id ? { id: scope.tenant_id } : {})}}).then((value) => ['tenant', value]),
      countAndLatest({
        model: 'facility',
        where: {
          ...tenantScopedWhere(scope),
          ...(scope.facility_id ? { id: scope.facility_id } : {})}}).then((value) => ['facility', value]),
      countAndLatest({
        where: {
          ...tenantScopedWhere(scope),
          ...(scope.facility_id ? { facility_id: scope.facility_id } : {})}}).then((value) => ['branch', value]),
      countAndLatest({
        model: 'department',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['department', value]),
      countAndLatest({
        model: 'unit',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['unit', value]),
      countAndLatest({
        model: 'room',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['room', value]),
      countAndLatest({
        model: 'ward',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['ward', value]),
      countAndLatest({
        model: 'bed',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['bed', value]),
      countAndLatest({
        model: 'address',
        where: tenantScopedWhere(scope)}).then((value) => ['address', value]),
      countAndLatest({
        model: 'contact',
        where: tenantScopedWhere(scope)}).then((value) => ['contact', value]),
      countAndLatest({
        model: 'user',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['user', value]),
      countAndLatest({
        model: 'user_profile',
        where: {
          deleted_at: null,
          user: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}}).then((value) => ['user-profile', value]),
      countAndLatest({
        model: 'role',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['role', value]),
      countAndLatest({
        model: 'permission',
        where: tenantScopedWhere(scope)}).then((value) => ['permission', value]),
      countAndLatest({
        model: 'role_permission',
        where: {
          deleted_at: null,
          role: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}}).then((value) => ['role-permission', value]),
      countAndLatest({
        model: 'user_role',
        where: tenantScopedWhere(scope, { includeFacility: Boolean(scope.facility_id) })}).then((value) => ['user-role', value]),
      countAndLatest({
        model: 'user_session',
        where: relationScopedWhere(scope, 'user')}).then((value) => ['user-session', value]),
      countAndLatest({
        model: 'api_key',
        where: tenantScopedWhere(scope)}).then((value) => ['api-key', value]),
      countAndLatest({
        model: 'api_key_permission',
        where: {
          deleted_at: null,
          api_key: tenantScopedWhere(scope)}}).then((value) => ['api-key-permission', value]),
      countAndLatest({
        model: 'user_mfa',
        where: relationScopedWhere(scope, 'user')}).then((value) => ['user-mfa', value]),
      countAndLatest({
        model: 'oauth_account',
        where: relationScopedWhere(scope, 'user')}).then((value) => ['oauth-account', value])]);
