 ...
 *
 * @module modules/radiology-procedure/services
...
const resolveOrCreateStandardRadiologyTest = async ({
  ...
  const existing = await radiologyProcedureRepository.findMany(
    {
      tenant_id: tenantId,
      code: definition.code},
    0,
  ...
  const radiologyProcedure = await radiologyProcedureRepository.create({
    tenant_id: tenantId,
    name: definition.name,
    code: definition.code,
    modality: definition.modality,
  ...
  createAuditLog({
    ...
    diff: {
      after: {
        ...definition,
        id: radiologyProcedure.id,
        source: 'STANDARD_RADIOLOGY_CATALOG'}},
    ip_address: ipAddress}).catch(() => {});

  return { id: radiologyProcedure.id };
};
...
const listRadiologyProcedures = async (filters, page, limit, sortBy, order, userId, ipAddress) => {
  try {
    ...
    return {
      radiologyProcedures: merged.records,
...
const assertRadiologyTestUniqueness = async ({
  ...
  const existingDbTests = await radiologyProcedureRepository.findMany(
    { tenant_id: tenantId },
    0,
    7500,
    { name: 'asc' }
...
const createRadiologyProcedure = async (data, userId, ipAddress) => {
  try {
    ...
    const radiologyProcedure = await radiologyProcedureRepository.create(normalizedData);
...
const updateRadiologyProcedure = async (id, data, userId, ipAddress) => {
  try {
    ...
    const radiologyProcedure = await radiologyProcedureRepository.update(resolvedId, payload);
    ...
    createAuditLog({
      ...
      entity: 'radiology_procedure',
      entity_id: radiologyProcedure.id,
      diff: { before, after: radiologyProcedure },
      ip_address: ipAddress
    }).catch(() => {});
