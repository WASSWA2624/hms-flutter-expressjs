/**
 * License repository
 *
 * @module modules/license/repositories
 * @description Data access layer for license operations.
 * Per module-creation.mdc: Only standard CRUD operations allowed in repositories.
 * Per prisma.mdc: All queries use soft delete filtering (deleted_at: null).
 */

const prisma = require('@prisma/client');
const { HttpError } = require('@lib/errors');

/**
 * Find license by ID
 *
 * @param {string} id - License ID
 * @returns {Promise<Object|null>} License object or null
 */
const findById = async (id) => {
  try {
    return await prisma.license.findFirst({
      where: {
        id,
        deleted_at: null
      },
      include: {
        tenant: true
      }
    });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};

/**
 * Find many licenses with pagination
 *
 * @param {Object} filters - Filter criteria
 * @param {number} skip - Number of records to skip
 * @param {number} take - Number of records to take
 * @param {Object} orderBy - Sort order
 * @returns {Promise<Array>} Array of licenses
 */
const findMany = async (filters = {}, skip = 0, take = 20, orderBy = { created_at: 'desc' }) => {
  try {
    // Build where clause
    const where = {
      deleted_at: null,
      ...filters
    };

    return await prisma.license.findMany({
      where,
      skip,
      take,
      orderBy,
      include: {
        tenant: true
      }
    });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};

/**
 * Count licenses with filters
 *
 * @param {Object} filters - Filter criteria
 * @returns {Promise<number>} Count of licenses
 */
const count = async (filters = {}) => {
  try {
    const where = {
      deleted_at: null,
      ...filters
    };

    return await prisma.license.count({ where });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};

/**
 * Create new license
 *
 * @param {Object} data - License data
 * @returns {Promise<Object>} Created license
 */
const create = async (data) => {
  try {
    return await prisma.license.create({
      data,
      include: {
        tenant: true
      }
    });
  } catch (error) {
    if (error.code === 'P2002') {
      // Unique constraint violation
      const target = error.meta?.target?.[0] || 'field';
      throw new HttpError('errors.database.unique_field', 409, [{ field: target }]);
    }
    if (error.code === 'P2003') {
      // Foreign key violation
      throw new HttpError('errors.database.foreign_key', 400);
    }
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};

/**
 * Update license
 *
 * @param {string} id - License ID
 * @param {Object} data - Update data
 * @returns {Promise<Object>} Updated license
 */
const update = async (id, data) => {
  try {
    return await prisma.license.update({
      where: { id },
      data,
      include: {
        tenant: true
      }
    });
  } catch (error) {
    if (error.code === 'P2025') {
      throw new HttpError('errors.license.not_found', 404);
    }
    if (error.code === 'P2002') {
      // Unique constraint violation
      const target = error.meta?.target?.[0] || 'field';
      throw new HttpError('errors.database.unique_field', 409, [{ field: target }]);
    }
    if (error.code === 'P2003') {
      // Foreign key violation
      throw new HttpError('errors.database.foreign_key', 400);
    }
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};

/**
 * Soft delete license
 * Per prisma.mdc: Only soft deletes allowed
 *
 * @param {string} id - License ID
 * @returns {Promise<Object>} Deleted license
 */
const softDelete = async (id) => {
  try {
    return await prisma.license.update({
      where: { id },
      data: {
        deleted_at: new Date()
      }
    });
  } catch (error) {
    if (error.code === 'P2025') {
      throw new HttpError('errors.license.not_found', 404);
    }
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};

module.exports = {
  findById,
  findMany,
  count,
  create,
  update,
  softDelete
};
