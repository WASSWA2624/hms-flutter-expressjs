...
const findMany = async (where = {}, skip = 0, take = 20, orderBy = { created_at: 'desc' }, include = DEFAULT_INCLUDE) => {
  try {
    return await prisma.day_close.findMany({
...
const count = async (where = {}) => {
  try {
    return await prisma.day_close.count({ where: { deleted_at: null, ...where } });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};
...
const create = async (data) => {
  try {
    return await prisma.day_close.create({ data, include: DEFAULT_INCLUDE });
  } catch (error) {
...
const update = async (id, data) => {
  try {
    return await prisma.day_close.update({ where: { id }, data, include: DEFAULT_INCLUDE });
  } catch (error) {
    if (error.code === 'P2025') throw new HttpError('errors.day_close.not_found', 404);
    if (error.code === 'P2002') throw new HttpError('errors.database.unique_field', 409);
    if (error.code === 'P2003') throw new HttpError('errors.database.foreign_key_field', 400);
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
