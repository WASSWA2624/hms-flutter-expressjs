 ...
 *
 * @module modules/patient-document/repositories
 * @description Data access layer for patient document operations.
...
/**
 * Find patient document by ID
 *
 * @param {string} id - Patient document ID
 * @param {Object} include - Relations to include
 * @returns {Promise<Object|null>} Patient document object or null
 */
const findById = async (id, include = {}) => {
  try {
    return await prisma.patient_document.findFirst({
      where: {
        id,
        deleted_at: null
      },
...
/**
 * Find many patient documents with pagination
 *
...
/**
 * Count patient documents with filters
 *
 * @param {Object} filters - Filter criteria
 * @returns {Promise<number>} Count of patient documents
 */
const count = async (filters = {}) => {
  try {
    ...
    return await prisma.patient_document.count({ where });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};
...
const update = async (id, data) => {
  try {
    return await prisma.patient_document.update({
      where: { id },
      data
    });
  } catch (error) {
