 ...
 * Consent repository
 *
 * @module modules/consent/repositories
 * @description Data access layer for consent operations.
...
/**
 * Find many consents with pagination
 *
 ...
 * @param {Object} include - Relations to include
 * @returns {Promise<Array>} Array of consents
 */
...
/**
 * Count consents with filters
 *
 * @param {Object} filters - Filter criteria
 * @returns {Promise<number>} Count of consents
 */
const count = async (filters = {}) => {
  try {
    ...
    return await prisma.consent.count({ where });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};
 ...
 *
 * @param {string} id - Consent ID
 * @returns {Promise<Object>} Deleted consent
 */
