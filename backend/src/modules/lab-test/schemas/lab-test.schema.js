...
const labReferenceRangeSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    const criticalMin = value.critical_min_value == null ? null : Number(value.critical_min_value);
    const criticalMax = value.critical_max_value == null ? null : Number(value.critical_max_value);
...
const withUniqueUnitOptions = (schema) =>
  schema.superRefine((value, ctx) => {
    ...
    value.unit_options.forEach((entry, index) => {
      const identifier = String(entry?.unit || '')
        .trim()
        .toUpperCase();
...
const withUniqueResultOptions = (schema) =>
  schema.superRefine((value, ctx) => {
    ...
    value.result_options.forEach((entry, index) => {
      const identifier = String(entry?.value || '')
        .trim()
        .toUpperCase();
      if (!identifier) return;
      if (seen.has(identifier)) {
 ...
 */
const createLabTestSchema = withUniqueResultOptions(
  withUniqueUnitOptions(
    z.object({
 ...
 */
const updateLabTestSchema = withUniqueResultOptions(
  withUniqueUnitOptions(
    z.object({
