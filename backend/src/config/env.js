...
const {
  ...
  AI_DEFAULT_CLINICAL_NOTE_FORMAT_TIMEOUT_MS,
  AI_DEFAULT_DRUG_PACK_EXTRACT_TIMEOUT_MS,
...
const buildEnv = () => {
  ...
  const AUTH_SESSION_IDLE_TIMEOUT_MINUTES = parseBoundedInteger(
    process.env.AUTH_SESSION_IDLE_TIMEOUT_MINUTES,
    10080,
    'AUTH_SESSION_IDLE_TIMEOUT_MINUTES',
    { min: 5, max: 129600 }
  );
  const AUTH_SESSION_ABSOLUTE_TIMEOUT_HOURS = parseBoundedInteger(
    process.env.AUTH_SESSION_ABSOLUTE_TIMEOUT_HOURS,
    720,
    'AUTH_SESSION_ABSOLUTE_TIMEOUT_HOURS',
    { min: 1, max: 8760 }
  );
  const REGISTRATION_IDEMPOTENCY_TTL_HOURS = parseBoundedInteger(
    process.env.REGISTRATION_IDEMPOTENCY_TTL_HOURS,
    24,
    'REGISTRATION_IDEMPOTENCY_TTL_HOURS',
    { min: 1, max: 168 }
  );
  const REGISTRATION_EMAIL_WAIT_MS = parseBoundedInteger(
    process.env.REGISTRATION_EMAIL_WAIT_MS,
    3000,
    'REGISTRATION_EMAIL_WAIT_MS',
    { min: 0, max: 15000 }
  );
  const PASSWORD_RESET_TTL_MINUTES = parseBoundedInteger(
  ...
  const PLATFORM_WHATSAPP = process.env.PLATFORM_WHATSAPP || PLATFORM_ADMIN_PHONE || null;
  ...
  const PLATFORM_MOBILE_MONEY_MTN = process.env.PLATFORM_MOBILE_MONEY_MTN || null;
  const PLATFORM_MOBILE_MONEY_AIRTEL = process.env.PLATFORM_MOBILE_MONEY_AIRTEL || null;
  const PLATFORM_MOBILE_MONEY_NAME = process.env.PLATFORM_MOBILE_MONEY_NAME || null;
  ...
  const AI_CLINICAL_NOTE_FORMAT_TIMEOUT_MS = parseBoundedInteger(
    process.env.AI_CLINICAL_NOTE_FORMAT_TIMEOUT_MS,
    AI_DEFAULT_CLINICAL_NOTE_FORMAT_TIMEOUT_MS,
    'AI_CLINICAL_NOTE_FORMAT_TIMEOUT_MS',
    { min: 1000 }
  );
  const AI_DRUG_PACK_EXTRACT_TIMEOUT_MS = parseBoundedInteger(
    process.env.AI_DRUG_PACK_EXTRACT_TIMEOUT_MS,
    AI_DEFAULT_DRUG_PACK_EXTRACT_TIMEOUT_MS,
    'AI_DRUG_PACK_EXTRACT_TIMEOUT_MS',
    { min: 1000 }
  );
  const AI_VISION_MODEL =
    String(process.env.AI_VISION_MODEL || '').trim() || AI_MODEL;
  ...
  const envConfig = {
    ...
    PLATFORM_BANK_IBAN,
    PLATFORM_MOBILE_MONEY_MTN,
    PLATFORM_MOBILE_MONEY_AIRTEL,
    PLATFORM_MOBILE_MONEY_NAME,
    SMTP_HOST,
    SMTP_PORT,
    SMTP_USER,
    SMTP_PASS,
