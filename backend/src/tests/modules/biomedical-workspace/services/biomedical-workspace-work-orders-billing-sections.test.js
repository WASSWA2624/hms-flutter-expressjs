...
describe('biomedical Work orders billing-sections scan', () => {
  const scopedUser = {
    id: 'user-123',
  ...
  it('Work orders panel workspace read does not touch patient billing ledger', async () => {
    ...
    expect(data.filters.panel).toBe('work-orders');
  ...
  it('Work orders workspace GET is idempotent on replay (no double billing post)', async () => {
    const query = { panel: 'work-orders' };
  ...
  it('serializes Work orders items without local paid flags or balances', async () => {
    const data = await biomedicalWorkspaceService.getWorkspace(
      { panel: 'work-orders' },
      1,
      20,
  ...
  it('Create work order primary stays NOT_BILLED (no patient ledger post)', async () => {
    ...
    const result = await createEquipmentWorkOrder(
      {
        ...
        title: 'Service infusion pump',
        description: 'Internal maintenance from Work orders primary',
        priority: 'NORMAL',
        status: 'OPEN',
      },
      {
  ...
  it('Update work order stays NOT_BILLED', async () => {
    ...
    equipmentWorkOrderRepository.update.mockResolvedValue({
      ...workOrderItem,
      title: 'Pump repair — updated',
      status: 'OPEN',
    });

    const result = await updateEquipmentWorkOrder(
      'WO-100',
      { title: 'Pump repair — updated', notes: 'Internal update' },
      {
        user: scopedUser,
        user_id: 'user-123',
        tenant_id: 'tenant-123',
    ...
    expect(result.title).toBe('Pump repair — updated');
    expect(result).not.toHaveProperty('payment_status');
    expect(result).not.toHaveProperty('balance');
    expect(clinicalRequestBilling.upsertClinicalRequestBilling).not.toHaveBeenCalled();
    expect(clinicalRequestBilling.receiveClinicalRequestPayment).not.toHaveBeenCalled();
  ...
  it('Start work order follow-up stays NOT_BILLED', async () => {
    equipmentWorkOrderRepository.findById.mockResolvedValue({
      ...workOrderItem,
    });
    equipmentWorkOrderRepository.update.mockResolvedValue({
    ...
    const result = await startEquipmentWorkOrder(
      'WO-100',
      { notes: 'Started from Work orders next-action' },
      {
        user: scopedUser,
        user_id: 'user-123',
        tenant_id: 'tenant-123',
  ...
  it('Return to service stays NOT_BILLED', async () => {
    equipmentWorkOrderRepository.findById.mockResolvedValue({
      ...workOrderItem,
      status: 'IN_REPAIR',
      started_at: new Date('2026-07-01T09:00:00.000Z'),
    ...
    const result = await returnToServiceEquipmentWorkOrder(
      'WO-100',
      {
        notes: 'Verified and returned',
        verification_evidence_manifest: [
          { type: 'CHECKLIST', reference: 'RTS-001' },
        ],
      },
      {
        user: scopedUser,
    ...
    expect(result.status).toBe('RETURNED_TO_SERVICE');
    expect(result).not.toHaveProperty('payment_status');
    expect(result).not.toHaveProperty('balance');
    expect(clinicalRequestBilling.upsertClinicalRequestBilling).not.toHaveBeenCalled();
    expect(clinicalRequestBilling.receiveClinicalRequestPayment).not.toHaveBeenCalled();
  ...
  it('unauthorized actor without write scopes still cannot settle via Work orders handlers', async () => {
