...
const stripClinicalNoteCompletion = (raw) => {
  ...
  text = text.replace(/^(?:here(?:'s| is)|formatted(?: clinical)? note)\s*:\s*/i, '').trim();
  return text;
};
...
const clinicalNoteFormatTask = {
  ...
  systemPrompt: [
    ...
    'Preserve every clinical fact, symptom, finding, medication, dose, time, and identifier exactly as stated.',
    'Do not invent diagnoses, assessments, plans, medications, vitals, identifiers, or missing details.',
    'Improve grammar, punctuation, spelling, and clinical phrasing only.',
