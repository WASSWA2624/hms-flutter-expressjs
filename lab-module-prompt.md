...
- [`.cursor/flows/icu-flow.mdc`](.cursor/flows/icu-flow.mdc) — ICU step-by-step workflow, entry paths, queue contract, stay lifecycle, bed board, monitoring, orders, transfers, discharge handoff, cross-flow rules
...
1. Re-read [`.cursor/flows/icu-flow.mdc`](.cursor/flows/icu-flow.mdc) — §3 workflow, §4 entry paths, §5 queue filters, §6–§12 stay/bed/monitoring/orders/transfer/discharge, §17 cross-flow integration.
2. Re-read [`.cursor/flows/ipd-flow.mdc`](.cursor/flows/ipd-flow.mdc) — especially §8 care loop, §9 transfers, §10–§12 discharge, §11 `In ICU`, §14 boards, §15 status-based actions, §16 encounter hub rule.
3. Re-read [`.cursor/flows/opd-flow.mdc`](.cursor/flows/opd-flow.mdc) — §3 stage contract (`ADMITTED`), §7 OPD-to-IPD handoff, §6 UI rules (summary cards filter worklist).
4. Re-read [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU vs Inpatient vs Nursing vs Clinical vs Discharge vs Billing boundaries.
...
| Emergency ICU path | ipd-flow §2.1, icu-flow §4.1 | Patients from `startEmergencyIcuFlow` appear on active ICU board with `Billing Deferred` when applicable |
| IPD bed manager | ipd-flow §3, §6, §14.2; icu-flow §7 | ICU bed board is ICU-ward scoped view of same bed catalog |
...
| Lab / Radiology results | ipd-flow §7, §12; opd-flow §3 | Pending/completed results surface on ICU detail; critical results drive alerts |
| Theater post-op | app-write-up Theater row; icu-flow §4.4 | Post-op handoff summary on ICU detail when theatre reference exists |
...
|------|----------------|-------|
| ICU flow spec | `.cursor/flows/icu-flow.mdc` | Full workflow, entry paths, queue contract, actionable implementation section |
...
| Repository | `icu_repository.dart` / `icu_repository_impl.dart` | Board via `GET /ipd-flows` with ICU filters; detail via `include_icu=true`; observation, vitals, alerts, ward round, transfer request, plan discharge, end ICU stay (`transferOut`) |
...
Implement or finish the following, in priority order. Each item maps to icu-flow, IPD flow sections, OPD handoff rules, and app-write-up ICU responsibilities.
...
**Goal:** Continuous critical-care documentation per app-write-up and icu-flow §8.
...
**Goal:** Intensivists document ICU rounds and update care plan (icu-flow §9, ipd-flow §6–§8 scoped to ICU).
...
**Goal:** Doctors can order services without leaving ICU workspace (ipd-flow §7, icu-flow §10).
...
- Each order uses **`ClinicalRequestBillingPanel`** for pay-now vs bill-later (ipd-flow §4, OPD §5 billing visibility).
- Show pending order/result summary on ICU detail; route to Lab/Radiology/Pharmacy queues automatically.
- When Lab returns critical results, ensure ICU detail and critical-alert queue reflect them (do not duplicate Lab workspace workflows).
...
- Surface pending lab/radiology orders from ipd-flow §10 discharge checklist as read-only blockers on ICU detail when discharge is planned.

### 9. Cross-module integration (OPD, IPD, Emergency, Theater, Nursing, Billing, Lab)
...
- **Lab / Radiology:** show pending and critical results on ICU detail; link to order in Lab workspace when staff need full workflow view.
...
- **Notifications:** critical alerts, transfer requests, discharge readiness, critical lab results update board badges and row alerts.

### 10. Billing and insurance (read-mostly in ICU UI)

**Goal:** ICU respects configurable billing gates without duplicating cashier workflows (icu-flow §13, ipd-flow §4).
...
| Lab sample collection, processing, verification | Lab workspace execution (Laboratory module) |
...
Follow `frontend/.cursor/ui-patterns.mdc`, `design-system.mdc`, `components.mdc`, and `layouts.mdc`. Mirror proven workspace patterns from **Nursing**, **IPD**, **Lab**, and **Radiology**.
...
- **Role-appropriate actions:** ICU nurse (vitals, observations, alerts, MAR), intensivist/doctor (rounds, orders, discharge plan), bed manager (ICU bed assign/transfer), billing (clearance links) — per icu-flow §14 and ipd-flow §13 adapted for ICU.
...
- Before creating UI, check `frontend/lib/shared/*` for reusable components (opd-flow §6).
...
1. **Repository gaps + start ICU stay** — `startIcuStay`; error mapping for duplicate stay.
2. **Localization pass** — migrate `_IcuText` to `app_en.arb`.
3. **Widget extraction + deep links** — split `icu_workspace_page.dart`; parse `/icu?id=&panel=` query params in `app_router.dart`.
4. **Patient board polish** — source encounter, consultant, payer, LOS columns; critical queue badge alignment.
5. **ICU bed board** — ICU ward filter, bed status, assign/transfer complete.
6. **Transfer lifecycle** — approve/start/complete/cancel; end stay on step-down.
7. **Clinical orders + billing panel** — lab/radiology/prescription from ICU detail.
8. **IPD / OPD / Emergency bridge** — cross-links, context headers, notification deep links.
9. **Discharge readiness handoff** — clearance links to IPD/Billing; pending lab results as blockers.
10. **Theater post-op context** — handover summary when API provides theatre reference.
...
- [ ] Pending/critical lab results visible on ICU detail with link to Lab workspace.
...
Manually smoke-test cross-flow paths:

1. OPD consult → disposition `ADMITTED` → IPD bed assign (ICU ward) → start ICU stay → ICU monitoring → step-down → IPD ward.
2. Emergency stabilize → ICU disposition → ICU board with emergency context.
3. ICU doctor orders lab → Lab workspace fulfills → critical result appears on ICU detail.
...
```
.cursor/flows/icu-flow.mdc                          # Primary ICU workflow source of truth
...
frontend/lib/features/lab/                        # Order fulfillment — link from ICU, do not duplicate
...
| app-write-up | Module boundaries | vs IPD/Nursing/Billing/Lab | No duplicate admission, cashier, or lab execution logic |
