...
| Awaiting payment atom map | `BillingAwaitingPaymentAtomPermissions` (tab/list/detail/receivePayment/refund/adjust/void/send/close/approve/claims/document) |
...
- Widget tests in `frontend/test/features/billing/presentation/billing_workspace_page_test.dart`, `billing_access_test.dart`, `billing_all_permissions_test.dart`, `billing_needs_issue_permissions_test.dart`, and `billing_approval_required_permissions_test.dart` prove:
  - `BillingAllAtomPermissions` reuses read/write/approve/claims helpers; widget tests in `billing_all_permissions_test.dart` (∩ denial for read-only and financial-approve-without-write, ∪ route entry, subscription strip, nested Claims pending absent/restored, nestedWrite denial without insurance, Issue + receive-payment sync paths, document Print/Download / View ledger for readers, Print/Download absent on approval detail, approve ∩ with `financial:approve`, secondary detail mutations, list search chrome, `action=pay` write gate, empty/error/retry, light/dark, mobile/desktop).
  ...
  - `BillingApprovalRequiredAtomPermissions` reuses approve/write helpers; widget tests in `billing_approval_required_permissions_test.dart` (∩ denial for write-without-approve and financial-approve-without-write, ∪ route entry, subscription strip, nested Claims pending, View ledger for readers, Print/Download absent on approval detail, Approve sync + Reject reason-dialog validation chrome, light/dark, mobile/desktop).
  - Claims pending tab and claim mutations absent without `insurance-claims`.
  - `BillingClaimsPendingAtomPermissions` reuses tab/claim-write/submit/reconcile/preAuth/close/route-entry helpers; widget tests in `billing_claims_pending_permissions_test.dart` (∩ denial for read-only and missing `insurance-claims`, ∪ route entry, subscription strip, View ledger + Print/Download absent on claim rows, Record insurer response for SUBMITTED + reconcile sync + detail validation chrome, pre-auth Approve/Deny + Approve sync + Deny validation chrome, submit-claim sync path, nestedWrite denial without insurance, list search chrome, empty/error/retry, light/dark, mobile/desktop). Unit tests note create/update keep source claims write ∩ (`billing:write` + `insurance-claims`), not matrix `billing:write` alone.
  - `BillingNeedsIssueAtomPermissions` reuses read/write/issue helpers; widget tests in `billing_needs_issue_permissions_test.dart` (∩ denial for read-only, ∪ route entry, subscription strip, nested Claims pending absent/restored, nestedWrite denial without insurance, Issue mutation sync path, Close shift form chrome, document Print/Download / View ledger for readers, list search chrome, empty/error/retry, light/dark, mobile/desktop).
  - `BillingAwaitingPaymentAtomPermissions` reuses read/write/approve/claims helpers; widget tests in `billing_awaiting_payment_permissions_test.dart` (∩ denial for read-only and financial-approve-without-write, ∪ route entry, subscription strip, nested Claims pending absent/restored, nestedWrite denial without insurance, receive-payment sync + validation chrome + success snackbar, document Print/Download / View ledger for readers, list search chrome, `action=pay` write gate, empty/error/retry, light/dark, mobile/desktop).
  - `BillingOverdueAtomPermissions` reuses read/write/approve/claims helpers; widget tests in `billing_overdue_permissions_test.dart` (∩ denial for read-only, ∪ route entry, subscription strip, nested Claims pending absent/restored, nestedWrite denial without insurance, receive-payment sync path, document Print/Download / View ledger for readers, empty/error/retry, `action=pay` write gate, light/dark, mobile/desktop).
