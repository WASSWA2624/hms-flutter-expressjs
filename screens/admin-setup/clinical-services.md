...
| Similarity review | Modal after Save (tests) | Create and edit always open the similarity dialog with score (including 0%), excluding the test id being edited. Actions: Cancel / Use this test / Continue save or Create/Save anyway (including exact clashes). Proceed after matches sends `confirm_similar: true`. Use existing pops `LabCatalogItem` so the parent can open edit (tenant) or details (standard). |
