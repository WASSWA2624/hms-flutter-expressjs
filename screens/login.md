...
- Success: navigation away (no success shell on `/login`). Password-reset and email-verify completion show a one-shot SnackBar when arriving from `/reset-password` or `/verify-email` (see those screen inventories).
