...
- **Create request** (primary)
  ...
  - Condition: `operationsSectionCreateRequirement` / write ∩; omitted when unauthorized.
...
- **Report** (secondary)
  ...
  - Condition: `operationsSectionReportRequirement` (read ∩; Assets matrix narrows source “Always”).
...
- **Search**, **Clear**, **Filters** (advanced), **Settings** (columns), pagination
  ...
  - Condition: Always when worklist / assets panel is loaded (read ∩).
...
- **Row select** (desktop row / mobile item)
  ...
  - Condition: Always when rows exist (read ∩).
...
- [x] Assets tab: read ∩ shows list/detail/Report; write ∩ shows Create request; write-only ∪ entry without read omits Assets; module/ABAC strip covered in `operations_assets_permissions_test.dart`.
