...
**Source of truth:** implement ICU workflow in alignment with:

- [`.cursor/flows/ipd-flow.mdc`](.cursor/flows/ipd-flow.mdc) — steps 8–9 (ward/ICU handover, nursing admission), §6–§8 doctor care and care loop, §9 transfers (ICU escalation/step-down), §11 `In ICU` status, §14 bed board, §15 ICU actions, §16 IPD encounter as hub
- [`.cursor/flows/opd-flow.mdc`](.cursor/flows/opd-flow.mdc) — §3 stage contract (`ADMITTED`), §6 UI rules (summary cards filter worklist), §7 OPD-to-IPD handoff when admission routes to ICU
- [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU module boundaries vs IPD, Nursing, Clinical, Billing, Theater, Emergency

**Central encounter rule:** every ICU stay, observation, alert, round, order, and transfer attaches to the **IPD admission / encounter**. ICU is a clinical overlay and workspace on top of inpatient flow — not a parallel admission system. Emergency `ICU` disposition and OPD `ADMITTED` handoff must land in ICU workspace with source context preserved.
...
1. Re-read [`.cursor/flows/ipd-flow.mdc`](.cursor/flows/ipd-flow.mdc) — especially §8 care loop, §9 transfers, §10–§12 discharge, §11 `In ICU`, §14 boards, §15 status-based actions, §16 encounter hub rule.
2. Re-read [`.cursor/flows/opd-flow.mdc`](.cursor/flows/opd-flow.mdc) — §3 stage contract (`ADMITTED`), §7 OPD-to-IPD handoff, §6 UI rules (summary cards filter worklist).
3. Re-read [`.cursor/app-write-up.mdc`](.cursor/app-write-up.mdc) — ICU vs Inpatient vs Nursing vs Clinical vs Discharge vs Billing boundaries.

---

## Cross-Flow Integration Map

ICU sits **after** OPD/Emergency admission intake and **on top of** the IPD encounter.
...
| -------- | -------------- | ------------------ |
| OPD doctor admits | opd-flow §3 `ADMITTED`, §7 | Show linked OPD encounter on ICU detail; do not recreate admission |
| Emergency ICU path | ipd-flow §2.1 | Patients from `startEmergencyIcuFlow` appear on active ICU board with `Billing Deferred` when applicable |
| IPD bed manager | ipd-flow §3, §6, §14.2 | ICU bed board is ICU-ward scoped view of same bed catalog |
| IPD transfer | ipd-flow §9 | Ward→ICU: complete transfer then `start-icu-stay`; step-down ends stay |
| Nursing | app-write-up Nursing row | Shared vitals/MAR visibility; ICU owns stay-scoped observations/alerts |
| Clinical / doctor orders | app-write-up Clinical row, ipd-flow §7 | Doctors place lab/radiology/medication orders from ICU via shared clinical actions |
| Theater post-op | app-write-up Theater row | Post-op handoff summary on ICU detail when theatre reference exists |
| Billing | ipd-flow §4 | Orders use `ClinicalRequestBillingPanel`; clearance stays in Billing/IPD |
| Discharge | ipd-flow §10–§12 | ICU marks readiness only; IPD/Discharge own multi-step clearance |
...
| Frontend scaffold | `frontend/lib/features/icu/` | `data/`, `domain/`, `presentation/` layers (6 files) |
| Workspace UI | `icu_workspace_page.dart` | ICU board, scope filters, summary cards, detail panels, observations/vitals/alerts/round/transfer/discharge dialogs (~1.9k lines) |
| Controller | `icu_workspace_controller.dart` | Realtime + 8s periodic sync, pagination, scope/search, patient selection, mutations |
...
| Standalone ICU APIs | `icu-stay/`, `icu-observation/` | CRUD at `/api/v1/icu-stays`, `/icu-observations`; frontend should use IPD-flow actions as canonical |
...
| Nursing workspace | `frontend/lib/features/nursing/` | Ward vitals, handover, MAR — shared encounter context; ICU status read-only |
| Clinical workspace | `frontend/lib/features/clinical/` | Encounter-centric doctor worklist; ICU only affects source-queue tone — no cross-nav yet |
| Clinical shared UI | `lib/shared/clinical_actions/` | Lab, radiology, prescription, billing panel, free-text note (`ClinicalFreeTextActionDialog` used for ICU rounds) |
| Permissions / module gate | `icu-critical-care` entitlement | Route: `clinicalRead`/`emergencyRead`/`operationsRead`; write: `clinicalWrite` + module |
| Realtime | `RealtimeEventGroups.icu` | Controller subscribes to admissions, critical alerts, diagnostics, pharmacy |
| Shell integration | `app_router.dart`, `app_routes.dart` | `/icu` route with nav badge from critical-alert count |
| Backend tests | `backend/src/tests/modules/ipd-flow/`, `icu-stay/`, `icu-observation/` | Route, service, RBAC coverage |

### Known gaps to close

- **`startIcuStay` missing from repository** — backend and Emergency path support `POST /ipd-flows/:id/start-icu-stay`; neither `IcuRepository` nor `IpdRepository` exposes it; no UI action to start stay.
- **Module entitlement mismatch** — ICU UI requires `icu-critical-care`; IPD-flow APIs gate on `inpatient-bed-management`. Align subscription/entitlement or document dual requirement; tenants with ICU-only may fail API calls.
- **No ICU bed board** — flow §14.2 expects bed occupancy visibility; UI shows ward/bed on patient rows only.
...
- **No inpatient clinical orders from ICU** — meds/orders shown read-only; no `ClinicalActionsPanel`, lab, radiology, or prescription dialogs scoped to ICU encounter.
- **Ward round reused for ICU round** — `add-ward-round` works via free-text dialog; not labeled or structured as ICU-specific round documentation.
- **Alert UX partial** — only `acknowledgeLatestAlert`; cannot resolve a specific alert from the list.
- **Timeline unused** — `IcuPatientDetail.timeline` parsed but not rendered on detail.
- **Localization gap** — most ICU copy is private `_IcuText` constants; only `icuStayDialogTitle` and `navigationIcuLabel` exist in `app_en.arb`.
- **No deep links** — `/icu` is flat; router does not parse `?id=…&panel=…` to pre-select patient or panel.
...
- **Theater handoff** — post-op ICU patients lack explicit cross-navigation and handover summary in ICU detail.
...
- **Clinical module bridge** — no navigation from Clinical worklist ICU-tagged encounters to `/icu`; no shared order placement pattern.
- **Standalone `icu-stays` API unused** — confirm whether admin/reporting needs direct CRUD or IPD-flow actions remain canonical.
- **No frontend tests** — `test/features/icu/` does not exist (Clinical/Nursing have DTO or page tests as reference).
- **Large page file** — `icu_workspace_page.dart` mixes board, detail, dialogs; needs extraction per project conventions.

---

## Scope — Core Capabilities

Implement or finish the following, in priority order. Each item maps to IPD flow sections, OPD handoff rules, and app-write-up ICU responsibilities.
...
- Preserve and align board scopes with backend filters:
...
**Goal:** Authorized staff can open, track, and close ICU stays on the IPD admission per flow §9 and §11 `In ICU`.

**Actions:**

- Add **`startIcuStay`** to `IcuRepository` → `POST /ipd-flows/:id/start-icu-stay` with optional `started_at`.
- Optionally expose **`startIcuStay`** on `IpdRepository` for IPD workspace “Start ICU stay” action.
- Expose **Start ICU stay** when admission has no active stay but is eligible (from IPD transfer complete to ICU bed, Emergency disposition, or manual escalation with reason).
...
**Goal:** Bed managers and ICU charge nurses see ICU bed occupancy and can assign or transfer to suitable ICU beds (flow §3, §14.2 bed board pattern scoped to ICU wards).
...
**Goal:** Continuous critical-care documentation per app-write-up and IPD care loop §8.
...
- **Critical alerts** — raise with severity + message; **acknowledge/resolve** via `resolve-critical-alert` for **each** open alert (not only latest); open alerts drive nav badge and critical queue.
- Surface alert history, observation timeline, and parsed `timeline` events on detail; sync to IPD timeline (`ICU_OBSERVATION` events).
- Nursing overlap: reuse Nursing vitals patterns where identical; ICU module remains primary owner for ICU-stay-scoped observations and alerts.

### 5. ICU rounds and clinical assessment

**Goal:** Intensivists document ICU rounds and update care plan (flow §6–§8 inpatient doctor care, scoped to ICU).
...
- Link to **Clinical workspace** or doctor notes / care plan on encounter when clinical module APIs provide them.

### 6. Inpatient orders from ICU

**Goal:** Doctors can order services without leaving ICU workspace (flow §7 department routing).

**Actions:**

- Add clinical order actions scoped to IPD encounter, mirroring IPD/Nursing/Clinical patterns:
...
- Consider wiring **`ClinicalActionsPanel`** on ICU detail for consistent doctor UX.
- Each order uses **`ClinicalRequestBillingPanel`** for pay-now vs bill-later (flow §4, OPD §5 billing visibility).
...
**Goal:** Internal ICU escalation and step-down without closing IPD encounter (flow §9).
...
**Goal:** ICU signals discharge readiness; IPD/Discharge modules own full clearance (flow §10, §12).
...
### 9. Cross-module integration (OPD, IPD, Emergency, Clinical, Theater, Nursing, Billing)

**Goal:** ICU connects cleanly across the patient journey defined in OPD and IPD flows.
...
- **OPD → IPD → ICU:**
  ...
  - Show OPD encounter summary (visit type, disposition note, consulting doctor) on ICU detail — read-only source context.
...
- **Clinical workspace:** when encounter source is ICU/IPD with active ICU stay, show **Open in ICU** action; do not duplicate ICU monitoring mutations in Clinical.
...
- **Deep links:** handle `/icu?id={admissionDisplayId}&panel={vitals|alerts|observations|orders|transfer|discharge}` — select patient and focus panel.
- **Notifications:** critical alerts, transfer requests, discharge readiness update board badges and row alerts.

### 10. Billing, insurance, and entitlement alignment

**Goal:** ICU respects configurable billing gates without duplicating cashier workflows (flow §4, app-write-up Billing boundary).
...
- Link to billing workspace for deposits and interim bills.
- **Resolve entitlement mismatch:** ensure tenants with ICU module can call IPD-flow ICU actions (coordinate `icu-critical-care` + `inpatient-bed-management` in middleware or document required entitlements).
...
| ICU-specific monitoring and escalation | Outpatient consultation queues (OPD / Clinical) |

---

## UI / UX Requirements

Follow `frontend/.cursor/ui-patterns.mdc`, `design-system.mdc`, `components.mdc`, and `layouts.mdc`. Mirror proven workspace patterns from **Nursing**, **IPD**, **Clinical**, **Lab**, and **Radiology**.
...
- **Role-appropriate actions:** ICU nurse (vitals, observations, alerts, MAR), intensivist/doctor (rounds, orders, discharge plan), bed manager (ICU bed assign/transfer), billing (clearance links) — per flow §13 adapted for ICU.
...
**Reuse existing services** — analyze Emergency ICU start, IPD ICU overlay, Nursing vitals, Clinical order dialogs, and clinical-order billing before adding endpoints.

---

## Suggested Implementation Order

1. **Entitlement alignment** — verify ICU tenants can call IPD-flow APIs; fix middleware or document dual module requirement.
2. **Repository gaps + start ICU stay** — `startIcuStay` on `IcuRepository` (+ optional `IpdRepository`); error mapping for duplicate stay.
3. **Localization pass** — migrate `_IcuText` to `app_en.arb`.
4. **Widget extraction + deep links** — split `icu_workspace_page.dart`; parse `/icu?id=&panel=` query params in `app_router.dart`.
5. **Patient board polish** — source encounter, consultant, payer, LOS columns; critical queue badge alignment.
6. **ICU bed board** — ICU ward filter, bed status, assign/transfer complete.
7. **Transfer lifecycle** — approve/start/complete/cancel; end stay on step-down.
8. **Clinical orders + billing panel** — lab/radiology/prescription from ICU detail; optional `ClinicalActionsPanel`.
9. **IPD / OPD / Emergency / Clinical bridge** — cross-links, context headers, notification deep links.
10. **Alert UX + timeline** — per-alert resolve; render timeline on detail.
11. **Discharge readiness handoff** — clearance links to IPD/Billing; no fake “discharged” state.
12. **Theater post-op context** — handover summary when API provides theatre reference.
13. **Tests + quality gate** — see below.
...
- [ ] Tenants with ICU module can perform ICU mutations without entitlement failures on IPD-flow APIs.
- [ ] **ICU bed board** shows ICU ward bed status and supports assign/transfer complete.
- [ ] Observations, vitals, critical alerts (raise + per-alert resolve), and round notes persist and appear on timeline.
...
- [ ] IPD and Clinical workspaces link to ICU when patient has active ICU stay.
...
Enable `icu-critical-care` and `inpatient-bed-management` module entitlements and `icuManager` / clinical roles for integration testing. Add focused tests during development; run the full gate before PR or merge.
...
frontend/lib/features/clinical/                   # Doctor orders, encounter worklist
...
frontend/lib/app/router/app_routes.dart            # route access + module entitlements
...
backend/src/middlewares/module-entitlement.middleware.js                 # Entitlement gates
```
...
|--------|---------|-------|-------------------------------|
| ipd-flow | §2.1 | Emergency ICU admission | Board intake + billing deferred display |
| ipd-flow | §6–§8 | Doctor care + care loop | ICU orders, rounds, pending results loop |
...
| app-write-up | Clinical module row | Doctor orders | Shared clinical action dialogs from ICU |
