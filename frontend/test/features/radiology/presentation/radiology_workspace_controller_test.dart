...
void main() {
  ...
  void stubInitialLoad(_MockRadiologyRepository repository) {
    ...
    when(() => repository.getWorkbench(any())).thenAnswer(
      (_) async => Result<RadiologyWorkbench>.success(_workbench()),
    );
    when(
      ...
      (_) async =>
          const Result<List<RadiologyCatalogTest>>.success(
    ...
    when(() => repository.getWorkflow(any())).thenAnswer(
      (_) async => Result<RadiologyWorkflow>.success(_workflow()),
    );
  }
  ...
  test('build loads the workbench and selects the first order workflow', () async {
    ...
    final RadiologyWorkspaceState state = (result as ResultSuccess<RadiologyWorkspaceState>).value;
    ...
    verify(() => repository.getWorkflow('RAD-001')).called(greaterThanOrEqualTo(1));
  });
  ...
  test('completeOrder mutates the selected order through the repository', () async {
    final _MockRadiologyRepository repository = _MockRadiologyRepository();
    stubInitialLoad(repository);
  ...
  test('createRadiologyTest submits payload and refreshes the catalog', () async {
    final _MockRadiologyRepository repository = _MockRadiologyRepository();
    stubInitialLoad(repository);
