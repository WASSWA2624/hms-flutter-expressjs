...
final PharmacyInventoryStock _nearExpiryStock = PharmacyInventoryStock(
  id: 'stock-near-1',
  displayId: 'STK-NEAR-1',
  inventoryItem: const PharmacyInventoryItem(
    id: 'item-1',
    name: 'Ceftriaxone vial',
    sku: 'CEF1G',
  ),
  quantity: 12,
  reorderLevel: 5,
  stockStatus: 'IN_STOCK',
  nextExpiry: DateTime.utc(2026, 8, 20),
...
void _stubPharmacyRepository(_MockPharmacyRepository repository) {
  ...
  when(() => repository.getInventoryStock(any())).thenAnswer((
    ...
    if (query.expiringWithinDays == null && !query.expiredOnly) {
      // Unfiltered alert-summary refresh still returns the row.
      items = <PharmacyInventoryStock>[_nearExpiryStock];
    }
...
Future<void> _pumpNearExpiryTab(
  WidgetTester tester, {
...
void main() {
  ...
  group('PharmacyNearExpiryAtomPermissions helpers', () {
    test('reuses catalog browse/write/export helpers', () {
      expect(
        identical(
          PharmacyNearExpiryAtomPermissions.tab,
      ...
      expect(
        identical(
          PharmacyNearExpiryAtomPermissions.write,
      ...
      expect(
        identical(
          PharmacyNearExpiryAtomPermissions.export,
      ...
      expect(
        identical(
          PharmacyNearExpiryAtomPermissions.print,
      ...
      expect(
        identical(
          pharmacySectionTabRequirement(PharmacyDeskSection.nearExpiry),
          PharmacyNearExpiryAtomPermissions.tab,
        ),
        isTrue,
      );
    ...
    test('∩ denial: missing pharmacy:read hides Near expiry tab gate', () {
      final AppAccessPolicy writeOnly = _policy(
        permissions: <AppPermission>{AppPermissions.pharmacyWrite},
      );
      expect(
        PharmacyNearExpiryAtomPermissions.tab.isAllowed(writeOnly),
        isFalse,
      );
      expect(
        pharmacyAllowedSections(writeOnly),
        isNot(contains(PharmacyDeskSection.nearExpiry)),
      );
    });
  });

  group('pharmacy Near expiry UI permission enforcement', () {
    ...
    testWidgets(
      '∩ denial: read-only Near expiry keeps browse; write actions absent',
      (WidgetTester tester) async {
        await _pumpNearExpiryTab(
          tester,
        ...
        expect(_tab('Near expiry'), findsOneWidget);
        expect(find.byType(PharmacyCatalogPanel), findsOneWidget);
        expect(find.textContaining('Ceftriaxone'), findsAtLeastNWidgets(1));
        ...
        final AppTabItem nearExpiry = strip.tabs.firstWhere(
          (AppTabItem tab) => tab.label == 'Near expiry',
        );
        expect(nearExpiry.countTone, AppTabCountTone.warning);
        expect(nearExpiry.count, isNotNull);
    ...
    testWidgets(
      'Near expiry toolbar: Filters/Settings/Close, ≤5 columns, Export/Print gate',
      (WidgetTester tester) async {
        await _pumpNearExpiryTab(
          tester,
          repository: repository,
    ...
    testWidgets(
      'Near expiry Export/Print present when evidence:export granted',
      (WidgetTester tester) async {
        await _pumpNearExpiryTab(
          tester,
          repository: repository,
    ...
    testWidgets('Near expiry desktop light theme keeps authorized chrome', (
      WidgetTester tester,
    ) async {
      await _pumpNearExpiryTab(
        tester,
    ...
    testWidgets('Near expiry mobile dark theme keeps authorized chrome', (
      WidgetTester tester,
    ) async {
      await _pumpNearExpiryTab(
        tester,
