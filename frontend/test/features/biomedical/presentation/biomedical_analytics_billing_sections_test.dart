...
import 'package:hosspi_hms/features/biomedical/presentation/biomedical_analytics_billing_inventory.dart';
...
void main() {
  ...
  group('Biomedical Analytics financial inventory (AC1)', () {
    test('every mounted atom is explicitly not billable with audit code', () {
      expect(
        BiomedicalAnalyticsBillingInventory.analyticsTabHasNoBillableActions,
        isTrue,
      );
      expect(
        BiomedicalAnalyticsBillingInventory.allMountedAtomsExplicitlyNotBillable,
        isTrue,
      );
      expect(BiomedicalAnalyticsBillingInventory.atoms, isNotEmpty);
      expect(
        BiomedicalAnalyticsBillingInventory.billableClasses.every(
          (BiomedicalAnalyticsFinancialAtom atom) => !atom.mounted,
        ),
        isTrue,
      );
      expect(biomedicalAnalyticsBillingScopeNote, contains('NOT_BILLED'));

      for (final BiomedicalAnalyticsFinancialAtom atom
          in BiomedicalAnalyticsBillingInventory.mountedAtoms) {
        expect(
          atom.financialClass,
          isIn(<BiomedicalAnalyticsFinancialClass>[
            BiomedicalAnalyticsFinancialClass.notRequired,
            BiomedicalAnalyticsFinancialClass.notBilled,
            BiomedicalAnalyticsFinancialClass.noCharge,
          ]),
          reason: atom.id,
        );
        expect(
    ...
    test('utilization metrics are NOT_BILLED analytics, not ledger balances', () {
      final BiomedicalAnalyticsFinancialAtom metric =
          BiomedicalAnalyticsBillingInventory.atoms.singleWhere(
            (BiomedicalAnalyticsFinancialAtom atom) =>
                atom.id == 'utilization_metric_display',
          );
      expect(
        metric.financialClass,
        BiomedicalAnalyticsFinancialClass.notBilled,
      );
      expect(metric.auditCode, 'NOT_BILLED');
    });

    test('internal maintenance writes stay NOT_BILLED', () {
      final BiomedicalAnalyticsFinancialAtom writes =
          BiomedicalAnalyticsBillingInventory.atoms.singleWhere(
            (BiomedicalAnalyticsFinancialAtom atom) =>
    ...
    test('unmounted billable atoms document Billing system of record', () {
      expect(
        BiomedicalAnalyticsBillingInventory.atoms
            .singleWhere(
              (BiomedicalAnalyticsFinancialAtom atom) =>
      ...
      expect(biomedicalAnalyticsBillingScopeNote, contains('Billing'));
    });
  });

  group('Biomedical Analytics billing bypass (AC2–AC4)', () {
  ...
  group('Biomedical Analytics section layout (AC5)', () {
    testWidgets('desktop Analytics: flat sections on list + detail', (
      WidgetTester tester,
    ) async {
      await _pumpAnalytics(
        tester,
    ...
    testWidgets('mobile Analytics: flat sections', (WidgetTester tester) async {
      await _pumpAnalytics(
        tester,
        repository: repository,
        accessPolicy: _policy(
  ...
  group('Biomedical Analytics sync / UI states (AC3–AC4, AC6)', () {
    ...
    test('inventory reuses shared financial class vocabulary', () {
      expect(
        BiomedicalAnalyticsBillingInventory.atoms.any(
          (BiomedicalAnalyticsFinancialAtom atom) =>
              atom.financialClass ==
