...
void main() {
  ...
  testWidgets(
    'authorized empty / loading / retry states remain observable',
    (WidgetTester tester) async {
      final AppAccessPolicy reader = _policy(
        permissions: <AppPermission>{AppPermissions.emergencyRead},
      );
  ...
  testWidgets(
    'post-mutation sync: triage updates board after next-action',
    (WidgetTester tester) async {
      final AppAccessPolicy writer = _policy(
        permissions: <AppPermission>{
          AppPermissions.emergencyRead,
      ...
      // Close dialog without submitting — proves authorized write path mounts.
      await tester.tap(find.byTooltip('Close'));
      await tester.pumpAndSettle();
      expect(find.text('Record triage'), findsNothing);
      verifyNever(
        () => repository.recordTriage(
          detail: any(named: 'detail'),
          triageLevel: any(named: 'triageLevel'),
