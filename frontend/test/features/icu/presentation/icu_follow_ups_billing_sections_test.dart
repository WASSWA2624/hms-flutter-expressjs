...
import 'package:hosspi_hms/features/icu/presentation/icu_follow_ups_billing_inventory.dart';
...
void main() {
  ...
  group('ICU Follow-ups financial inventory (AC1)', () {
    test('every mounted atom is explicitly not billable with audit code', () {
      expect(
        IcuFollowUpsBillingInventory.followUpsTabHasNoBillableActions,
        isTrue,
      );
      expect(
        IcuFollowUpsBillingInventory.allMountedAtomsExplicitlyNotBillable,
        isTrue,
      );
      expect(IcuFollowUpsBillingInventory.atoms, isNotEmpty);
      expect(
        IcuFollowUpsBillingInventory.billableAtoms.every(
          (IcuFollowUpsFinancialAtom atom) => !atom.mounted,
        ),
        isTrue,
      );
      expect(icuFollowUpsBillingScopeNote, contains('NOT_BILLED'));
      expect(icuFollowUpsBillingScopeNote, contains('Billing'));
      expect(IcuFollowUpsBillingInventory.summary(), contains('NOT_BILLED'));

      for (final IcuFollowUpsFinancialAtom atom
          in IcuFollowUpsBillingInventory.mountedAtoms) {
        expect(
          atom.financialClass,
          isIn(<IcuFollowUpsFinancialClass>[
            IcuFollowUpsFinancialClass.notRequired,
            IcuFollowUpsFinancialClass.notBilled,
            IcuFollowUpsFinancialClass.noCharge,
          ]),
          reason: atom.id,
        );
        expect(
    ...
    test('Mark completed and reschedule stay NOT_BILLED', () {
      expect(
        IcuFollowUpsBillingInventory.markCompleted.financialClass,
        IcuFollowUpsFinancialClass.notBilled,
      );
      expect(
        IcuFollowUpsBillingInventory.markCompleted.auditCode,
        'NOT_BILLED',
      );
      expect(
        IcuFollowUpsBillingInventory.reschedule.financialClass,
        IcuFollowUpsFinancialClass.notBilled,
      );
      expect(
        IcuFollowUpsBillingInventory.saveFollowUp.financialClass,
        IcuFollowUpsFinancialClass.notBilled,
      );
    });

    test('unmounted billable atoms document Billing system of record', () {
      expect(IcuFollowUpsBillingInventory.openBilling.mounted, isFalse);
      expect(
        IcuFollowUpsBillingInventory.openBilling.billingPath,
        contains('Billing'),
      );
      expect(
        IcuFollowUpsBillingInventory.followUpVisitCharge.mounted,
        isFalse,
      );
      expect(IcuFollowUpsBillingInventory.icuBedDayPackage.mounted, isFalse);
      expect(
        IcuFollowUpsBillingInventory.icuBedDayPackage.billingPath,
        contains('persistIcuStayBilling'),
      );
      expect(IcuFollowUpsBillingInventory.transferCharges.mounted, isFalse);
      expect(
        IcuFollowUpsBillingInventory.dischargeFinancialGate.mounted,
        isFalse,
      );
      expect(IcuFollowUpsBillingInventory.collectPayment.mounted, isFalse);
      expect(
        IcuFollowUpsBillingInventory.issueInvoiceAdjustRefund.mounted,
        isFalse,
      );
      expect(
        IcuFollowUpsBillingInventory.isInlineCollectionForbidden(
          IcuFollowUpsFinancialClass.settle,
        ),
        isTrue,
      );
    });

    test('inventory reuses ICU Follow-ups access gates', () {
      expect(
        IcuFollowUpsBillingInventory.tab.requirement,
        same(IcuFollowUpsAtomPermissions.tab),
      );
      expect(
        IcuFollowUpsBillingInventory.markCompleted.requirement,
        same(IcuFollowUpsAtomPermissions.markCompleted),
      );
      expect(
        IcuFollowUpsBillingInventory.reschedule.requirement,
        same(IcuFollowUpsAtomPermissions.reschedule),
      );
    });
  });

  group('ICU Follow-ups billing bypass (AC2–AC4)', () {
  ...
  group('ICU Follow-ups section layout (AC5)', () {
  ...
  group('ICU Follow-ups sync / UI states (AC3–AC4, AC6)', () {
