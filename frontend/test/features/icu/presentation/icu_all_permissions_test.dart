...
void main() {
  ...
  group('IcuAllAtomPermissions helpers', () {
    ...
    test('∪ allowance: clinical:read alone shows All tab; write alone fails', () {
      ...
      // Intersection-style denial for write: clinical:write without module
      // still needs source ∪ clinical|emergency write + icu module.
    ...
    test('∪ allowance: route entry accepts operations:read; All tab still ∪ read', () {
      ...
      expect(IcuAllAtomPermissions.routeEntryUnion.isAllowed(opsReadOnly), isTrue);
  ...
  testWidgets('dark theme: All read chrome mounts without mutation atoms', (
    ...
    expect(find.text('All Tab Patient'), findsOneWidget);
    expect(find.text('Assign ICU bed'), findsNothing);
    expect(Theme.of(tester.element(find.text('All Tab Patient'))).brightness, Brightness.dark);
  });
