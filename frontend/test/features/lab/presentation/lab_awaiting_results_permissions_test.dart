...
const LabOrderSummary _awaitingOrder = LabOrderSummary(
  id: 'LAB-ORDER-AWAIT-1',
  displayId: 'LO-AWAIT-1',
  status: 'ORDERED',
  patientDisplayName: 'Ann Awaiting',
  patientId: 'PAT-AWAIT-1',
  paymentStatus: 'PAID',
);
...
void _stubWorkspace(
  ...
  List<LabOrderSummary> items = const <LabOrderSummary>[_awaitingOrder],
...
Future<GoRouter> _pumpAwaitingResultsTab(
  WidgetTester tester, {
  required _MockLabRepository repository,
  AppAccessPolicy? policy,
  Size viewport = const Size(1440, 900),
  ThemeMode themeMode = ThemeMode.light,
  String initialLocation = '/lab?section=awaiting-results',
  List<LabOrderSummary> items = const <LabOrderSummary>[_awaitingOrder],
  Result<LabWorkbenchBundle>? workbenchOverride,
}) async {
...
void main() {
  ...
  group('LabAwaitingResultsAtomPermissions inventory (AC1)', () {
    test('maps atoms to matrix ∩ / ∪ helpers', () {
      ...
      expect(
        identical(
          LabAwaitingResultsAtomPermissions.delete,
          labWorkspaceWriteRequirement,
        ),
        isTrue,
      );
  ...
  group('Awaiting results tab UI authorization (AC2-AC5)', () {
    testWidgets('deep link section=awaiting-results selects collection scope', (
      ...
      final GoRouter router = await _pumpAwaitingResultsTab(
        tester,
      ...
      expect(
        queries.any(
          (LabWorkbenchQuery q) => q.scope == LabQueueScope.collection,
        ),
        isTrue,
      );
    ...
    testWidgets(
      'union route entry: clinical:read sees awaiting chrome without create',
      (WidgetTester tester) async {
        await _pumpAwaitingResultsTab(
          tester,
          repository: repository,
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        expect(
          LabAwaitingResultsAtomPermissions.previewReport.isAllowed(
            _labReadPolicy(),
          ),
          isTrue,
        );
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          LabAwaitingResultsAtomPermissions.requestFromClinical.isAllowed(
            clinicalWriter,
          ),
          isTrue,
        );
        expect(
          LabAwaitingResultsAtomPermissions.create.isAllowed(clinicalWriter),
          isFalse,
        );
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        expect(
          LabAwaitingResultsAtomPermissions.criticalNotify.isAllowed(
            _labWritePolicy(),
          ),
          isFalse,
        );
    ...
    testWidgets('post-mutation sync: collect updates selected workflow in place', (
      WidgetTester tester,
    ) async {
      await _pumpAwaitingResultsTab(tester, repository: repository);
    ...
    testWidgets('mobile viewport: awaiting-results chrome remains', (
      WidgetTester tester,
    ) async {
      await _pumpAwaitingResultsTab(
        tester,
    ...
    testWidgets('legacy section=collection alias still opens awaiting tab', (
      ...
      await _pumpAwaitingResultsTab(
        tester,
        repository: repository,
        initialLocation: '/lab?section=collection',
      );
