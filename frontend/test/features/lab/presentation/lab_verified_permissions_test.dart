...
void main() {
  ...
  group('Verified tab UI authorization (AC2-AC5)', () {
    ...
    testWidgets(
      'Completed toolbar: Filters/Settings labels, ≤5 columns, info tone, Close',
      (WidgetTester tester) async {
        ...
        final AppTabItem completed = strip.tabs.firstWhere(
          (AppTabItem tab) => tab.label.toLowerCase().contains('completed'),
        );
    ...
    testWidgets(
      'Completed Export/Print omit without evidence:export; present when granted',
      (WidgetTester tester) async {
        await _pumpVerifiedTab(tester, repository: repository);
    ...
    testWidgets('Completed tab omitted when workspace entry denied', (
      WidgetTester tester,
    ) async {
      await _pumpVerifiedTab(
        tester,
