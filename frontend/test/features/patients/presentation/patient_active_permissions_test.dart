...
void main() {
  ...
  group('Active tab UI authorization (AC2-AC5)', () {
    ...
    testWidgets(
      'defaults five data columns; Active count tone is info; Settings lists choices',
      (WidgetTester tester) async {
        await _pumpActiveTab(
          tester,
          patientRepository: patientRepository,
        ...
        expect(active.countTone, AppTabCountTone.info);
        ...
        // Alerts is optional on Active — not in the default five.
    ...
    testWidgets(
      'active Active badge uses filtered total; All sibling stays overview',
      (WidgetTester tester) async {
        await _pumpActiveTab(
          tester,
          patientRepository: patientRepository,
        ...
        when(() => patientRepository.listPatients(any())).thenAnswer((
          ...
          return Result<AppPage<Patient>>.success(
            AppPage<Patient>(
              items: narrowed
                  ? const <Patient>[_idlePatient]
                  : const <Patient>[_idlePatient, _activePatient],
              request: query.pageRequest,
              totalItemCount: narrowed ? 1 : 5,
            ),
          );
        });
