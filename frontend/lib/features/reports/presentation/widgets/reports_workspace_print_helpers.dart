...
/// Column choice for Reports workspace list print (aligned with exportable fields).
final class ReportsWorkspacePrintColumn {
  const ReportsWorkspacePrintColumn({
    required this.id,
    required this.label,
  });
...
final class ReportsWorkspacePrintOptionsController extends ChangeNotifier {
  ReportsWorkspacePrintOptionsController({
    required List<ReportsWorkspacePrintColumn> columns,
  }) : _columns = List<ReportsWorkspacePrintColumn>.unmodifiable(columns) {
    _selectedColumnIds =
        columns.map((ReportsWorkspacePrintColumn c) => c.id).toSet();
    _includeSummary = true;
    _includeRows = true;
  }

  final List<ReportsWorkspacePrintColumn> _columns;
  late Set<String> _selectedColumnIds;
  late bool _includeSummary;
  late bool _includeRows;

  List<ReportsWorkspacePrintColumn> get columns => _columns;
  ...
  void setSelectedColumns(Set<String> selected) {
    final Set<String> next = selected
        .where(
          (String id) =>
              _columns.any((ReportsWorkspacePrintColumn c) => c.id == id),
        )
        .toSet();
...
/// Preview-first worklist print for Reports queue tables.
Future<void> printReportsWorkspaceList({
  ...
  final ReportsWorkspacePrintOptionsController options =
      ReportsWorkspacePrintOptionsController(columns: columns);

  String buildBodyHtml() {
    return reportsWorkspaceListHtml(
      l10n: l10n,
      rows: rows,
      options: options,
      emptyText: emptyText,
  ...
  try {
    await PrintDocumentTemplates.registry(
      ...
      previewSectionsExtra: ReportsWorkspacePrintOptionsSection(
        controller: options,
      ),
...
/// Maps [AppListTable] exportable columns + rows into Reports list print.
Future<void> printReportsListTable<T>({
  required WidgetRef ref,
  required BuildContext context,
  required String title,
  ...
  final List<ReportsWorkspacePrintColumn> printColumns =
      <ReportsWorkspacePrintColumn>[
        for (final AppListTableColumn<T> column in exportColumns)
          ReportsWorkspacePrintColumn(id: column.key, label: column.label),
      ];
  ...
  await printReportsWorkspaceList(
    ref: ref,
    context: context,
    title: title,
...
String reportsWorkspaceListHtml({
  required AppLocalizations l10n,
  required List<Map<String, String>> rows,
  required ReportsWorkspacePrintOptionsController options,
  required String emptyText,
}) {
  ...
  if (options.includeRows) {
    final List<ReportsWorkspacePrintColumn> selected = options.columns
        .where(
          (ReportsWorkspacePrintColumn column) =>
    ...
    final List<List<String>> tableRows = <List<String>>[
      for (final Map<String, String> row in rows)
        <String>[
          for (final ReportsWorkspacePrintColumn column in selected)
            row[column.id] ?? '',
        ],
    ];
    buffer.write(
...
class ReportsWorkspacePrintOptionsSection extends StatelessWidget {
  const ReportsWorkspacePrintOptionsSection({
    required this.controller,
    super.key,
  });

  final ReportsWorkspacePrintOptionsController controller;
          ...
          children: <Widget>[
            ...
            if (controller.includeRows)
              AppReportSectionPicker(
                compact: true,
                sections: <AppReportSectionData>[
                  for (final ReportsWorkspacePrintColumn column
