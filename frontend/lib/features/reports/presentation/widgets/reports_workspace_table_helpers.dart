...
class ReportMobileTile extends ConsumerWidget {
  const ReportMobileTile({
    required this.item,
    required this.state,
  ...
  final ReportRowActionHandler onNextAction;
  final ReportDetailDialogOpener openDetailDialog;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ReportsMobileTile(
      title: item.title,
      subtitle: reportsJoinDisplay(<String?>[
        item.subtitle,
        item.reference,
        reportsDateTime(context, item.occurredAt),
      ]),
      ...
      nextActionEnabled: !state.isSaving,
      onNextAction: nextAction == null
          ? null
          : () => onNextAction(context, ref, item),
      onOpenDetail: () => openDetailDialog(context, ref, state, item, policy),
    );
  }
}

class ComplianceMobileTile extends ConsumerWidget {
  const ComplianceMobileTile({
    required this.item,
    required this.state,
    required this.policy,
    required this.onNextAction,
  ...
  final ReportsWorkspaceState state;
  final AppAccessPolicy policy;
  final ComplianceRowActionHandler onNextAction;
  final ComplianceDetailDialogOpener openDetailDialog;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final AppLocalizations l10n = context.l10n;
    final bool canExport = canExportEvidence(policy);
    final String? nextAction = complianceNextActionLabel(
      l10n,
      canExport: canExport,
    );
